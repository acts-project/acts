...
struct random_track_generator_config {
  ...
  darray<value_t, 2> m_phi_range{-constant<value_t>::pi, constant<value_t>::pi};
  darray<value_t, 2> m_theta_range{0.f, constant<value_t>::pi};

  /// Momentum range
  darray<value_t, 2> m_mom_range{1.f * unit<value_t>::GeV,
                                 1.f * unit<value_t>::GeV};
  /// Whether to interpret the momentum @c m_mom_range as p_T
  bool m_is_pT{false};

  /// Track origin
  darray<value_t, 3> m_origin{0.f, 0.f, 0.f};
  darray<value_t, 3> m_origin_stddev{0.f, 0.f, 0.f};
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& phi_range(
      const value_t low, const value_t high) {
    auto min_phi{
        std::clamp(low, -constant<value_t>::pi, constant<value_t>::pi)};
    auto max_phi{
        std::clamp(high, -constant<value_t>::pi, constant<value_t>::pi)};
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& theta_range(value_t low,
                                                                value_t high) {
    auto min_theta{std::clamp(low, value_t{0.f}, constant<value_t>::pi)};
    auto max_theta{std::clamp(high, value_t{0.f}, constant<value_t>::pi)};
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& eta_range(value_t low,
                                                              value_t high) {
    ...
    auto get_theta = [](const value_t eta) {
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& mom_range(value_t low,
                                                              value_t high) {
    m_is_pT = false;
    assert(low >= 0.f);
    assert(low <= high);
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& mom_range(
      darray<o_value_t, 2> r) {
    mom_range(static_cast<o_value_t>(r[0]), static_cast<o_value_t>(r[1]));
    return *this;
  }
  DETRAY_HOST_DEVICE random_track_generator_config& pT_range(value_t low,
                                                             value_t high) {
    m_is_pT = true;
    assert(low >= 0.f);
    assert(low <= high);
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& pT_range(
      darray<o_value_t, 2> r) {
    pT_range(static_cast<o_value_t>(r[0]), static_cast<o_value_t>(r[1]));
    return *this;
  }
  DETRAY_HOST_DEVICE random_track_generator_config& p_tot(value_t p) {
    mom_range(p, p);
    return *this;
  }
  DETRAY_HOST_DEVICE random_track_generator_config& p_T(value_t p) {
    pT_range(p, p);
    return *this;
  }
  DETRAY_HOST_DEVICE random_track_generator_config& origin(const value_t x,
                                                           const value_t y,
                                                           const value_t z) {
    m_origin = {x, y, z};
    return *this;
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& origin_stddev(
      const value_t x, const value_t y, const value_t z) {
  ...
  DETRAY_HOST_DEVICE random_track_generator_config& time(value_t t) {
    assert(t >= 0.f);
    m_time = t;
    return *this;
  }
  DETRAY_HOST_DEVICE random_track_generator_config& charge(value_t q) {
    m_charge = q;
    return *this;
  }
  /// @}
  ...
  DETRAY_HOST_DEVICE constexpr const darray<value_t, 2>& phi_range() const {
    return m_phi_range;
  }
  DETRAY_HOST_DEVICE constexpr const darray<value_t, 2>& theta_range() const {
    return m_theta_range;
  }
  DETRAY_HOST_DEVICE constexpr const darray<value_t, 2>& mom_range() const {
    return m_mom_range;
  }
  ...
  friend std::ostream& operator<<(std::ostream& out,
    ...
    out << mom_range[0] / detray::unit<value_t>::GeV << ", "
        << mom_range[1] / detray::unit<value_t>::GeV << ") [GeV]\n"
        << "  Phi range             : ["
        ...
        << "  Origin                : [" << ori[0] / detray::unit<value_t>::mm
    ...
    if (cfg.do_vertex_smearing()) {
      ...
      out << "  Origin stddev         : ["
          << ori_stddev[0] / detray::unit<value_t>::mm << ", "
          << ori_stddev[1] / detray::unit<value_t>::mm << ", "
          << ori_stddev[2] / detray::unit<value_t>::mm << "] [mm]\n";
    }
