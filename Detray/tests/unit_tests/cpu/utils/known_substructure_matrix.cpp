...
template <typename A, typename B>
concept congruable = detail::checks::is_symmetric<typename B::canonical_type> &&
                     (A::columns == B::rows);

template <typename T>
struct index_var_id {
  static constexpr long long value = -1;  // not an index variable
};
template <std::size_t N>
struct index_var_id<index_variable<N>> {
  static constexpr long long value = static_cast<long long>(N);
};

template <typename Sub, std::size_t... Ks>
constexpr std::size_t expected_stored_count(std::index_sequence<Ks...>) {
  constexpr std::size_t C = Sub::columns;
  constexpr std::size_t N = sizeof...(Ks);
  const std::array<bool, N> is_var{
      std::is_same_v<typename Sub::template value_at<Ks / C, Ks % C>,
                     variable>...};
  const std::array<long long, N> ids{
      index_var_id<typename Sub::template value_at<Ks / C, Ks % C>>::value...};
  std::size_t count = 0;
  for (std::size_t k = 0; k < N; ++k) {
    if (is_var[k]) {
      ++count;  // every anonymous variable gets its own slot
    } else if (ids[k] >= 0) {
      bool seen = false;
      for (std::size_t j = 0; j < k; ++j)
        seen = seen || (ids[j] == ids[k]);
      if (!seen)
        ++count;  // first occurrence of this index_variable index
    }
  }
  return count;
}
...
constexpr bool verify_stored_count() {
  constexpr std::size_t expected = expected_stored_count<Sub>(
      std::make_index_sequence<Sub::rows * Sub::columns>{});
  static_assert(Sub::num_variables == expected,
                "substructure::num_variables does not match the number of "
                "values the matrix needs to store");
  return true;
}
...
void set_cell(matrix<Sub, S> &m, S v) {
  ...
  } else if constexpr (detail::checks::is_index_variable<value_t>) {
...
void test_pair(const std::string &an, const std::string &bn) {
  ...
  if constexpr (addable<A, B>) {
    ...
    static_assert(verify_stored_count<typename ADD::canonical_type>());
    static_assert(verify_stored_count<typename SUB::canonical_type>());
  ...
  if constexpr (multipliable<A, B>) {
    ...
    static_assert(verify_stored_count<typename MUL::canonical_type>());
  ...
  if constexpr (congruable<A, B>) {
    ...
    static_assert(verify_stored_count<typename CONG::canonical_type>());
    ...
    check(detail::checks::is_symmetric<typename CONG::canonical_type>,
...
void test_single(const std::string &an) {
  ...
  static_assert(verify_stored_count<typename NEG::canonical_type>());
  ...
  if constexpr (detail::checks::is_symmetric<typename A::canonical_type>) {
  ...
  static_assert(verify_stored_count<typename TR::canonical_type>());
  ...
  if constexpr (detail::checks::is_symmetric<typename A::canonical_type>) {
    check(detail::checks::is_symmetric<typename TR::canonical_type>,
  ...
  if constexpr ((A::rows == A::columns) && (A::rows <= 2) &&
                detail::checks::is_invertible<typename A::canonical_type>) {
    ...
    static_assert(verify_stored_count<typename INV::canonical_type>());
    ...
    if constexpr (detail::checks::is_symmetric<CA>) {
      check(detail::checks::is_symmetric<typename INV::canonical_type>,
