...
// Project include(s)
#include "stepper_cuda_kernel.hpp"
...
TEST(stepper_cuda, stepper) {
  using track_parameters_aos = free_track_parameters<algebra_s>;
  using track_parameters_soa = free_track_parameters<algebra_v>;

  detray::types::print<detray::types::list<algebra_s>>();
  detray::types::print<detray::types::list<algebra_v>>();

  constexpr std::size_t simd_size{detray::detail::size<dscalar<algebra_v>>()};
  ...
  // Create detector
  auto [det, names] = build_toy_detector<algebra_s>(mng_mr);
  ...
  // Create the vector of initial track parameters (SoA)
  vecmem::vector<track_parameters_soa> tracks_host(&mng_mr);
  vecmem::vector<track_parameters_soa> tracks_device(&mng_mr);

  // Magnitude of total momentum of tracks
  const scalar p_mag{10.f * unit<scalar>::GeV};

  // Iterate through uniformly distributed momentum directions
  for (auto track : uniform_track_generator<track_parameters_aos>(
  ...
  // Iterate through uniformly distributed momentum directions
  for (auto track : uniform_track_generator<track_parameters_soa>(
           phi_steps, theta_steps, p_mag)) {
    std::cout << track << std::endl;
    std::cout << std::endl;
