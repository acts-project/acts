...
using algebra_s = test::algebra;
using algebra_v = detray::array_soa<test::value>;

using metadata_t = detray::toy_metadata<algebra_s>;
using test_algebra = metadata_t::algebra_type;
using scalar = dscalar<algebra_s>;
using point3 = dpoint3D<algebra_s>;
using host_detector_t = host::detector<metadata_t>;
using device_detector_t = device::detector<metadata_t>;

using intersection_t =
    intersection2D<typename device_detector_t::surface_type, algebra_s>;

using navigator_host_t = caching_navigator<host_detector_t>;
using navigator_device_t = caching_navigator<device_detector_t>;
using stepper_t = line_stepper<algebra_s>;
...
constexpr unsigned int theta_steps{5u};
constexpr unsigned int phi_steps{9u};
