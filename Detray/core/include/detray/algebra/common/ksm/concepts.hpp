...
template <typename Sub>
struct is_diagonal_helper<Sub, true> {
  template <std::size_t... Ks>
...
///
/// @{
template <typename Sub, std::size_t I, std::size_t J,
          bool Stored = concepts::is_index_variable<
              typename Sub::template value_at<I, J>>>
struct identity_constant_ok {
  static constexpr bool value =
      Sub::template value_at<I, J>::value == ((I == J) ? 1 : 0);
};

template <typename Sub, std::size_t I, std::size_t J>
struct identity_constant_ok<Sub, I, J, true> {
  static constexpr bool value = true;
};
/// @}
...
/// zero in the second.
template <typename Sub, std::size_t A, std::size_t B>
struct identity_sharing_ok {
 private:
  static constexpr std::size_t C = Sub::columns;
  using a_value = typename Sub::template value_at<A / C, A % C>;
  using b_value = typename Sub::template value_at<B / C, B % C>;
...
template <typename Sub>
concept is_diagonal =
    is_canonical<Sub> && detail::is_diagonal_helper<Sub>::value;
...
concept can_represent_identity =
    is_canonical<Sub> && detail::can_represent_identity_helper<Sub>::value;
...
concept is_closed_under_multiplication =
    is_canonical<Sub> && (Sub::rows == Sub::columns) &&
    std::same_as<typename Sub::template multiplication_type<Sub>, Sub>;
}  // namespace concepts
}  // namespace detray::ksm
