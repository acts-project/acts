...
/// @brief Geometrical shape of a line surface with a circular cross section.
///
/// The boundary bounds[0] refers to the radius of the cross section. The
/// second boundary bounds[1] is the half length in z.
class line_circular {
 public:
  /// The name for this shape
  static constexpr std::string_view name = "line_circular";
  ...
  DETRAY_HOST_DEVICE inline scalar_t min_dist_to_boundary(
    ...
    return math::min(math::fabs(math::fabs(loc_p[0]) - bounds[e_cross_section]),
  ...
  /// the cylinder 2D frame (sgn * r, z, phi) or (sgn * r, z).
  ///
  /// @param bounds the boundary values for this shape
  /// @param loc_p the point to be checked in the local coordinate system
  /// @param tol dynamic tolerance determined by caller
