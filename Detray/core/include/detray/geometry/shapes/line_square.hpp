...
/// @brief Geometrical shape of a line surface with a square cross section.
///
/// The boundary bounds[0] refers to the half length of the square cross
/// section. The second boundary bounds[1] is the half length in z.
class line_square {
 public:
  /// The name for this shape
  static constexpr std::string_view name = "line_square";
  ...
  DETRAY_HOST_DEVICE inline scalar_t min_dist_to_boundary(
    ...
    const scalar_t dist_z = math::fabs(math::fabs(loc_p[1]) - bounds[e_half_z]);
  ...
  /// the cylinder 2D frame (sgn * r, z, phi).
  ///
  /// @param bounds the boundary values for this shape
  /// @param loc_p the point to be checked in the local coordinate system
  /// @param tol dynamic tolerance determined by caller
  ...
  DETRAY_HOST constexpr bool check_consistency(
    ...
    if (bounds[e_cross_section] < tol) {
      os << "DETRAY ERROR (HOST): Sides must be in the range (0, "
            "numeric_max)"
         << std::endl;
      return false;
    }
