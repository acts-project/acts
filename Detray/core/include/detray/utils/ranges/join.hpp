...
// deduction guides
DETRAY_HOST_DEVICE_DEDUCTION_GUIDE join()
    -> join<detray::ranges::views::empty<dvector<int>>>;

template <detray::ranges::range R>
DETRAY_HOST_DEVICE_DEDUCTION_GUIDE join(R &&ranges)
    -> join<std::remove_reference_t<R>>;
