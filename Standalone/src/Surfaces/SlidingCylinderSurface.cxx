///////////////////////////////////////////////////////////////////
// SlidingCylinderSurface.cxx, ACTS project
///////////////////////////////////////////////////////////////////
...
// copy constructor
Acts::SlidingCylinderSurface::SlidingCylinderSurface(const SlidingCylinderSurface& dsf) :
...
Acts::SlidingCylinderSurface::SlidingCylinderSurface(const SlidingCylinderSurface& dsf, const Acts::Transform3D& transf ) :
...
// constructor
Acts::SlidingCylinderSurface::SlidingCylinderSurface(const Acts::CylinderSurface& dsf, Acts::BinUtility* bu, 
					    const std::vector<float>* offset, Acts::Transform3D* align) :
  Acts::CylinderSurface(dsf),
  m_depth(offset),
  m_etaBin(bu),
  m_align(align)
...
Acts::SlidingCylinderSurface& Acts::SlidingCylinderSurface::operator=(const SlidingCylinderSurface& dsf)
{
  if (this!=&dsf){
...
bool Acts::SlidingCylinderSurface::operator==(const Acts::Surface& sf) const
{
  // first check the type not to compare apples with oranges
  const Acts::SlidingCylinderSurface* dsf = dynamic_cast<const Acts::SlidingCylinderSurface*>(&sf);
  if (!dsf) return false;
...
void Acts::SlidingCylinderSurface::localToGlobal(const Acts::Vector2D& locpos, const Acts::Vector3D&, Acts::Vector3D& glopos) const
{
...
bool Acts::SlidingCylinderSurface::globalToLocal(const Acts::Vector3D& glopos, const Acts::Vector3D&, Acts::Vector2D& locpos) const
{
    ...
    if (Acts::Surface::m_transform) {
      ...
      locpos = Acts::Vector2D((bounds().r()+offset)*loc3Dframe.phi(), loc3Dframe.z());
...
bool Acts::SlidingCylinderSurface::isOnSurface(const Acts::Vector3D& glopo, const Acts::BoundaryCheck& bchk) const
{
  Acts::Vector3D loc3D0 = m_align ? m_align->inverse()*glopo : glopo;
  Acts::Vector3D loc3Dframe = m_transform ? (transform().inverse())*glopo : glopo;
  float offset = (*m_depth)[m_etaBin->bin(loc3D0)];
