///////////////////////////////////////////////////////////////////
// CylinderSurface.cxx, ACTS project
///////////////////////////////////////////////////////////////////
...
Acts::CylinderSurface::CylinderSurface(const CylinderSurface& csf, const Acts::Transform3D& transf) :
...
Acts::CylinderSurface::CylinderSurface(std::shared_ptr<Acts::Transform3D> htrans, const Acts::CylinderBounds* cbounds):
  Acts::Surface(htrans),
  m_bounds(cbounds),
  m_rotSymmetryAxis(nullptr)
{
...
// constructor by CylinderBounds
Acts::CylinderSurface::CylinderSurface(std::shared_ptr<Acts::Transform3D> htrans, std::shared_ptr<const Acts::CylinderBounds> cbounds):
  Acts::Surface(htrans),
  m_bounds(cbounds),
  m_rotSymmetryAxis(nullptr)
{
...
Acts::CylinderSurface::CylinderSurface(std::unique_ptr<Acts::Transform3D> htrans):
...
// constructor by radius, halflenght and phisector
Acts::CylinderSurface::CylinderSurface(double radius, double hphi, double hlength) :
  Acts::Surface(),
  m_bounds(new Acts::CylinderBounds(radius, hphi, hlength)),
  m_rotSymmetryAxis(nullptr)
{}
...
Acts::CylinderSurface& Acts::CylinderSurface::operator=(const CylinderSurface& csf)
{
  if (this!=&csf){
...
bool Acts::CylinderSurface::operator==(const Acts::Surface& sf) const
{
  // first check the type not to compare apples with oranges
  const Acts::CylinderSurface* csf = dynamic_cast<const Acts::CylinderSurface*>(&sf);
  if (!csf) return false;
...
Acts::Vector3D Acts::CylinderSurface::binningPosition(Acts::BinningValue bValue) const {
    // special binning type for R-type methods
    if (bValue == Acts::binR || bValue == Acts::binRPhi ){
        double R   = bounds().r();
        double phi = m_bounds ? m_bounds->averagePhi() : 0.;
        return Acts::Vector3D(center().x()+R*cos(phi),center().y()+R*sin(phi),center().z());
    }
...
const Acts::RotationMatrix3D Acts::CylinderSurface::measurementFrame(const Acts::Vector3D& pos, const Acts::Vector3D&) const
{
...
const Acts::Vector3D& Acts::CylinderSurface::rotSymmetryAxis() const
{
...
void Acts::CylinderSurface::localToGlobal(const Acts::Vector2D& locpos, const Acts::Vector3D&, Acts::Vector3D& glopos) const
{
...
bool Acts::CylinderSurface::isOnSurface(const Acts::Vector3D& glopo, const BoundaryCheck& bchk) const
{
	Acts::Vector3D loc3Dframe = Acts::Surface::m_transform ? (transform().inverse())*glopo : glopo;
    return ( bchk ? bounds().inside3D(loc3Dframe,bchk) : true );
}


Acts::Intersection Acts::CylinderSurface::intersectionEstimate(const Acts::Vector3D& pos,
