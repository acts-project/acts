///////////////////////////////////////////////////////////////////
// DetachedTrackingVolume.cxx, ACTS project
///////////////////////////////////////////////////////////////////
...
void Acts::DetachedTrackingVolume::move(Acts::Transform3D&) const 
{
  //!< @TODO implement
  //// move the volume 
...
std::shared_ptr<const Acts::DetachedTrackingVolume> Acts::DetachedTrackingVolume::clone( std::string name, Acts::Transform3D& shift) const
{
    // create the new base tracking volume
    std::shared_ptr<const Acts::TrackingVolume> shiftedTrackingVolume = m_trkVolume->cloneWithShift(shift);    
...
void Acts::DetachedTrackingVolume::sign(GeometrySignature signat, GeometryType geotype) const 
{ m_trkVolume->sign(signat, geotype); }

Acts::GeometrySignature Acts::DetachedTrackingVolume::geometrySignature() const 
{ return m_trkVolume->geometrySignature(); }

Acts::GeometryType Acts::DetachedTrackingVolume::geometryType() const 
{ return m_trkVolume->geometryType(); }

void Acts::DetachedTrackingVolume::setBaseTransform( Acts::Transform3D* transf ) const 
{
  if ( transf ) m_baseTransform = transf ;
  else {
    delete m_baseTransform;
    m_baseTransform = new Acts::Transform3D(this->trackingVolume()->transform());
  }
}

void Acts::DetachedTrackingVolume::realign( Acts::Transform3D* transf ) const 
{
  if ( transf ) {
    Acts::Transform3D shift = (*transf)*this->trackingVolume()->transform().inverse();
    this->move(shift);
  } else if (m_baseTransform) {
    Acts::Transform3D shift = *m_baseTransform*this->trackingVolume()->transform().inverse();
    this->move(shift);
  }
}
