...
__global__ void cuTransformCoordinate(
    const int nSpM, const float* spMmat, const int* McompIndex, const int nSpB,
    const float* spBmat, const int nSpBcompPerSpM_Max, const int* BcompIndex,
    const int nSpT, const float* spTmat, const int nSpTcompPerSpM_Max,
    const int* TcompIndex, float* spMcompMat, float* spBcompMatPerSpM,
...
__global__ void cuSearchTriplet(
    ...
    const float pT2perRadius, const float impactMax, const int nTrplPerSpMLimit,
    const int nTrplPerSpBLimit, const float deltaInvHelixDiameter,
    const float impactWeightFactor, const float deltaRMin,
    const float compatSeedWeight, const size_t compatSeedLimit,
    int* nTrplPerSpM, Triplet* TripletsPerSpM);

void searchDoublet(
    int grid, int block, int nSpM, const DeviceMatrix<float>& spMmat, int nSpB,
    const DeviceMatrix<float>& spBmat, int nSpT,
    const DeviceMatrix<float>& spTmat, float deltaRMin, float deltaRMax,
    float cotThetaMax, float collisionRegionMin, float collisionRegionMax,
    ResultScalar<int>& nSpMcomp, ResultScalar<int>& nSpBcompPerSpM_Max,
    ResultScalar<int>& nSpTcompPerSpM_Max, DeviceVector<int>& nSpBcompPerSpM,
    DeviceVector<int>& nSpTcompPerSpM, DeviceVector<int>& McompIndex,
    DeviceMatrix<int>& BcompIndex, DeviceMatrix<int>& tmpBcompIndex,
    DeviceMatrix<int>& TcompIndex, DeviceMatrix<int>& tmpTcompIndex) {
  ...
  cuSearchDoublet<<<grid, block, sharedMemSize>>>(
      nSpM, spMmat.getPtr(), nSpB, spBmat.getPtr(), nSpT, spTmat.getPtr(),
      deltaRMin, deltaRMax, cotThetaMax, collisionRegionMin, collisionRegionMax,
      nSpMcomp.getPtr(), nSpBcompPerSpM_Max.getPtr(),
      nSpTcompPerSpM_Max.getPtr(), nSpBcompPerSpM.getPtr(),
      nSpTcompPerSpM.getPtr(), McompIndex.getPtr(), BcompIndex.getPtr(),
      tmpBcompIndex.getPtr(), TcompIndex.getPtr(), tmpTcompIndex.getPtr());
  ACTS_CUDA_ERROR_CHECK(cudaGetLastError());
}

void transformCoordinate(
    int grid, int block, int nSpM, const DeviceMatrix<float>& spMmat,
    const DeviceVector<int>& McompIndex, int nSpB,
    const DeviceMatrix<float>& spBmat, int nSpBcompPerSpM_Max,
    const DeviceMatrix<int>& BcompIndex, int nSpT,
    const DeviceMatrix<float>& spTmat, int nSpTcompPerSpM_Max,
    const DeviceMatrix<int>& TcompIndex, DeviceMatrix<float>& spMcompMat,
  ...
  cuTransformCoordinate<<<grid, block, sharedMemSize>>>(
      nSpM, spMmat.getPtr(), McompIndex.getPtr(), nSpB, spBmat.getPtr(),
      nSpBcompPerSpM_Max, BcompIndex.getPtr(), nSpT, spTmat.getPtr(),
      nSpTcompPerSpM_Max, TcompIndex.getPtr(), spMcompMat.getPtr(),
      spBcompMatPerSpM.getPtr(), circBcompMatPerSpM.getPtr(),
      spTcompMatPerSpM.getPtr(), circTcompMatPerSpM.getPtr());
  ACTS_CUDA_ERROR_CHECK(cudaGetLastError());
}

void searchTriplet(int grid, int block, const int* nSpTcompPerSpM, int nSpMcomp,
                   const float* spMcompMat, int nSpBcompPerSpM_Max,
                   const int* BcompIndex, const float* circBcompMatPerSpM,
                   int nSpTcompPerSpM_Max, const int* TcompIndex,
                   const float* spTcompMatPerSpM,
                   const float* circTcompMatPerSpM, float maxScatteringAngle2,
                   float sigmaScattering, float minHelixDiameter2,
                   float pT2perRadius, float impactMax, int nTrplPerSpMLimit,
                   int nTrplPerSpBLimit, float deltaInvHelixDiameter,
                   float impactWeightFactor, float deltaRMin,
                   float compatSeedWeight, int compatSeedLimit,
                   int* nTrplPerSpM, Triplet* TripletsPerSpM,
  ...
  cuSearchTriplet<<<grid, block, sharedMemSize, getStreamFrom(streamWrapper)>>>(
      nSpTcompPerSpM, nSpMcomp, spMcompMat, nSpBcompPerSpM_Max, BcompIndex,
      ...
      nTrplPerSpBLimit, deltaInvHelixDiameter, impactWeightFactor, deltaRMin,
      compatSeedWeight, compatSeedLimit, nTrplPerSpM, TripletsPerSpM);
  ACTS_CUDA_ERROR_CHECK(cudaGetLastError());
}
...
__global__ void cuTransformCoordinate(
  ...
  while (offset < max(nSpBcompPerSpM_Max, nSpTcompPerSpM_Max)) {
    if (threadIdx.x + offset < nSpBcompPerSpM_Max) {
      ...
      // matrix reduction
      for (int i = 0; i < 6; i++) {
        spB[i] = spBcompMatPerSpM[threadIdx.x + offset +
    ...
    if (threadIdx.x + offset < nSpTcompPerSpM_Max) {
      ...
      // matrix reduction
      for (int i = 0; i < 6; i++) {
        spT[i] = spTcompMatPerSpM[threadIdx.x + offset +
      ...
      if (isPassed == 1) {
        ...
        if (tPos < nTrplPerSpBLimit) {
          ...
          triplets[tPos].topRadius =
              spTcompMatPerSpM[threadIdx.x + offset + nSpTcompPerSpM_Max * 3];
