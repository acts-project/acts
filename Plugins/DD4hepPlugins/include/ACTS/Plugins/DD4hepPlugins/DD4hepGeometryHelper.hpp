...
/** @ class DD4hepGeometryHelper
 ...
 Provides helper function to translate the DD4hep geometry into the ACTS
 Tracking Geometry.
...
public:
   ...
   * DetElement*/
  static std::shared_ptr<Acts::Transform3D>
  extractTransform(DD4hep::Geometry::DetElement& detElement);
  /**helper method to extract the volume boundaries of a cylindrical volume*/
  static std::shared_ptr<const Acts::VolumeBounds>
  extractVolumeBounds(DD4hep::Geometry::DetElement& detElement);
...
private:
   ...
   * modules*/
  static std::unique_ptr<Acts::SurfaceArray>
  ...
  createBinValues(std::vector<std::pair<float, float>> old);
