...
/// as the world DD4hep::DetElement. It walks through the DD4hep detector hierarchy
/// and translates it to the ACTS tracking geometry. It walks through the possible subdetectors,
/// which can contain layers. The layers themselves can contain modules and/or are support structure.
...
public:
  ...
  trackingGeometry() const override;
   ...
   /// DetElement
   /// @param detElement DD4hep detector element of which the transform should be returned
    std::shared_ptr<Acts::Transform3D>
    extractTransform(DD4hep::Geometry::DetElement& detElement) const;
    /// helper method to extract the ACTS volume boundaries of a cylindrical volume
    /// @note Only cylindrical volume boundary implemented
    /// @param detElement DD4hep detector element of which the volume boundaries should be returned
    std::shared_ptr<const Acts::VolumeBounds>
    extractVolumeBounds(DD4hep::Geometry::DetElement& detElement) const;

private:
    
    /// Creates a triple of volumes of a possible barrel-endcap configuration and of
    ...
    /// @param motherDetElement The DD4hep detector element containing the sub volumes
    ...
    /// @param motherTransform the global transformation matrix of the motherDetElement
    void
    ...
    /// @note the BinnedArray is two dimensional and one binning value will alway be phi
    /// @param modules Vector of DD4hep detector modules which should get translated into surfaces
    /// @param lValue The second binning value which can be either r or z
    /// @param motherTransform The global transformation matrix of the mother detector element containing the modules/surfaces(the layer)
    ...
    createSurfaceArray(std::vector<DD4hep::Geometry::DetElement>& modules,
                       ...
                       = nullptr) const;
    /// Creates a two dimensional surface array binned in phi and a longitudinal direction which
    ...
    /// Helper method needed to get the bin values for a binned array out of overlapping modules in r
    std::vector<float>
