...
auto newBuilder(const magnetic_field_t& magneticField,
                ...
                const std::array<std::size_t, 3>& nPoints, const Vector3& min,
                const Vector3& max) {
  ...
  for (std::size_t x = 0; x < nPoints[0]; x++) {
    for (std::size_t y = 0; y < nPoints[1]; y++) {
      for (std::size_t z = 0; z < nPoints[2]; z++) {
        Vector3 position{std::min(x * sampleSpacing[0] + min[0], maxima[0]),
...
/// @param max (max_x, max_y, max_z)
/// @return A clamp affine linear strided covfie field.
InterpolatedField covfieField(const MagneticFieldProvider& magneticField,
...
/// @return A clamp affine linear strided covfie field.
InterpolatedField covfieField(const InterpolatedMagneticField& magneticField) {
  ...
  Vector3 min{old_min.at(0), old_min.at(1), old_min.at(2)};
  Vector3 max{old_max.at(0), old_max.at(1), old_max.at(2)};
...
ConstantField covfieField(const ConstantBField& magneticField) {
