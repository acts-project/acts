...
nlohmann::json Acts::MultiLayerNavigationJsonConverter::toJson(
    const Experimental::MultiLayerNavigationPolicy& policy) {
  nlohmann::json jPolicy;
  jPolicy["type"] = "MultiLayerNavigationPolicy";
...
Acts::MultiLayerNavigationJsonConverter::fromJson(
  ...
  std::vector<AxisDirection> castsVec =
      j.at("casts").get<std::vector<AxisDirection>>();
  ...
  config.binExpansion =
      j.at("binExpansion").get<std::vector<std::size_t>>();

  return Experimental::MultiLayerNavigationPolicy(gctx, volume, logger, config,
