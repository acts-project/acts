...
nlohmann::json Acts::DetectorJsonConverter::toJsonDetray(
  ...
  /**
    // Keep track of the portal splits for the volumes
    using VolumePortalSplits = std::map<const Experimental::Portal*,
                                        std::vector<std::shared_ptr<Surface>>>;
    using DetectorPoralSplits =
        std::map<const Experimental::DetectorVolume*, VolumePortalSplits>;

    DetectorPoralSplits detectorPortalSplits;

    // Convert the volumes
    nlohmann::json jVolumes;
    for (const auto& volume : volumes) {
      auto [jVolume, portalSplits] = DetectorVolumeJsonConverter::toJsonDetray(
          gctx, *volume, volumes, options.volumeOptions);
      detectorPortalSplits[volume] = portalSplits;
      jVolumes.push_back(jVolume);
    ...
    for (const auto [iv, volume] : enumerate(volumes)) {
      ...
      // Surface counter: start with surface offset
      auto volumePortalSplits = detectorPortalSplits.find(volume);
      std::size_t surfaceCounter = volume->surfaces().size();

      // Now write the material for the portals
      //
      // - this is tricky because portals are split for detray
      // - we need to utilize the split map to get the correct surfaces
      for (const auto* portal : volume->portals()) {
        const ISurfaceMaterial* surfaceMaterial =
            portal->surface().surfaceMaterial();

        if (surfaceMaterial != nullptr) {
          // If a portal split had occured
          if (volumePortalSplits != detectorPortalSplits.end()) {
            auto portalSplit = volumePortalSplits->second.find(portal);
            if (portalSplit != volumePortalSplits->second.end()) {
              // Write the material for the split surfaces
              for (const auto& splitSurface : portalSplit->second) {
                nlohmann::json jSurfaceMaterial =
                    MaterialJsonConverter::toJsonDetray(
                        *surfaceMaterial, *splitSurface, ++surfaceCounter,
                        gridLinks);
                if (!jSurfaceMaterial.empty()) {
                  ++nGrids;
                  jMaterialVolumeGridsData.push_back(jSurfaceMaterial);
          ...
          nlohmann::json jSurfaceMaterial = MaterialJsonConverter::toJsonDetray(
              *surfaceMaterial, portal->surface(), ++surfaceCounter, gridLinks);
          if (!jSurfaceMaterial.empty()) {
            ++nGrids;
            jMaterialVolumeGridsData.push_back(jSurfaceMaterial);
          }
