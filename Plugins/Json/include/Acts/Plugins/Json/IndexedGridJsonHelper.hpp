...
updator_type generateFromJson(const nlohmann::json& jUpdater,
  ...
  auto eqExtractor = [](const nlohmann::json& jAxis)
      -> std::tuple<std::array<long double, 2u>, std::size_t> {
    std::array<long double, 2u> range = jAxis["range"];
  ...
  auto vExtractor =
      [](const nlohmann::json& jAxis) -> std::vector<long double> {
    std::vector<long double> vEx(jAxis["boundaries"]);
    return vEx;
  };
