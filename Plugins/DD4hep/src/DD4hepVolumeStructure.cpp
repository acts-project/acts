...
bool Acts::Experimental::DD4hepVolumeStructure::recursiveParse(
  ...
  bool protoVolume = getParamOr<bool>("proto_volume", dd4hepElement, false);
  if (protoVolume) {
    ACTS_DEBUG("ProtoVolume description detected.");
    auto bValueInt =
        getParamOr<int>("proto_volume_type", dd4hepElement,
    ...
    if (bValueType < VolumeBounds::BoundsType::eOther) {
      auto bValues = extractSeries<ActsScalar>(
          dd4hepElement, "proto_volume_bvalues", unitLength);
