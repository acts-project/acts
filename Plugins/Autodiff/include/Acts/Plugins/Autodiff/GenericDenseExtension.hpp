...
/// @warning This file is just copied from the Core/Propagation directory,
/// and then templated. It was last updated in July 2020.
...
/// @note This is the templated generic type. The default one with 'double'
/// is a typedef below.
template <typename float_t>
struct GenericDenseEnvironmentExtension {
  /// Some typedefs for templated types
  template <typename T>
  using Vector3D = Eigen::Matrix<T, 3, 1>;

  template <typename T>
  using FreeMatrix = Eigen::Matrix<T, 8, 8>;
  ...
  bool k(const propagator_state_t& state, const stepper_t& stepper,
    ...
    if (i == 0) {
      ...
      float_t qopNew = qop[0] + h * Lambdappi[i - 1];
      Lambdappi[i] =
  ...
  template <typename propagator_state_t, typename stepper_t>
  bool finalize(propagator_state_t& state, const stepper_t& stepper,
                const double h, FreeMatrix<double>& D) const {
  ...
  void initializeEnergyLoss(const propagator_state_t& state) {
    ...
    if (state.options.meanEnergyLoss) {
      g = -computeEnergyLossMean(
          ...
          static_cast<float>(static_cast<double>(qop[0])));
    } else {
