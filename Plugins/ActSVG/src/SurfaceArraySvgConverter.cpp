...
Acts::Svg::SurfaceArrayConverter::convert(
  ...
  if (!binning.empty() && binning.size() == 2 && axes.size() == 2) {
    ...
    if (binning[0] == BinningValue::binPhi && binning[1] == BinningValue::binZ) {
      vType = cylinder;
    ...
    } else if (binning[0] == BinningValue::binPhi && binning[1] == BinningValue::binR) {
      vType = polar;
      //  flip to fit with actsvg convention
      edges1 = axes[0]->getBinEdges();
      edges0 = axes[1]->getBinEdges();
      pGrid._type = actsvg::proto::grid::e_r_phi;
    } else if (binning[0] == BinningValue::binZ && binning[1] == BinningValue::binPhi) {
      // good
      vType = cylinder;
      edges0 = axes[0]->getBinEdges();
      edges1 = axes[1]->getBinEdges();
      pGrid._type = actsvg::proto::grid::e_z_phi;
    } else if (binning[0] == BinningValue::binR && binning[1] == BinningValue::binPhi) {
      // good
      vType = polar;
      edges0 = axes[0]->getBinEdges();
      edges1 = axes[1]->getBinEdges();
