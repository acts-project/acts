...
Acts::TGeoLayerBuilder::buildLayers(LayerVector& layers, int type) const
{
  ...
  switch (type) {
    case -1 : { layerConfigs = m_cfg.negativeLayerConfigs; layerType = "Negative"; } break;
    case  0 : { layerConfigs = m_cfg.centralLayerConfigs;  layerType = "Central"; } break;
    case  1 : { layerConfigs = m_cfg.positiveLayerConfigs; layerType = "Positive"; } break;
  }
  // screen output
  ...
  for (auto layerCfg :layerConfigs) {
    ...
    ACTS_DEBUG("- layer configuration found for layer " << layerCfg.layerName
                                                        << " with sensor "
                                                        << layerCfg.sensorName);
    // we have to step down from the top volume each time to collect the logical tree
    ...
    if (tvolume){
      ...
      collectSensitive(layerSurfaces,
                       ...
                       type);
      // screen output
      ACTS_DEBUG("- number of senstive sensors found : " << layerSurfaces.size()); 
...
void
Acts::TGeoLayerBuilder::collectSensitive(std::vector<const Acts::Surface*>& layerSurfaces,
                                         TGeoVolume*         tgVolume,
                                         TGeoNode*           tgNode,
...
{    
  ...
  if (tgVolume){
    ...
    if (correctVolume == false && volumeName.find(layerConfig.layerName) != std::string::npos) {
        correctVolume = true;
    ...
    // screen output
    ACTS_VERBOSE(offset << "has " << tgVolume->GetNdaughters() << " daughters.");
    // a daughter iterator 
    ...
    while (TObject* obj = iObj()) {
      ...
      if (node) collectSensitive(layerSurfaces,
                                 nullptr,
                                 node,
                                 tgTransform,
                                 layerConfig,
  ...
  if (tgNode){
    ...
    if (correctBranch && tNodeName.find(layerConfig.sensorName) != std::string::npos) {
