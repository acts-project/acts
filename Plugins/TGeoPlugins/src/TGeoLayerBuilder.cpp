...
Acts::TGeoLayerBuilder::TGeoLayerBuilder(const Acts::TGeoLayerBuilder::Config& config):
    m_config()
{
...
void Acts::TGeoLayerBuilder::setConfiguration(const Acts::TGeoLayerBuilder::Config& config)
{
    m_config = config;
}
...
const Acts::LayerVector Acts::TGeoLayerBuilder::negativeLayers() const
{
    Acts::LayerVector nVector;
    return std::move(nVector);
}

const Acts::LayerVector Acts::TGeoLayerBuilder::centralLayers() const
{
    // the return vector
    ...
    MSG_INFO("Central Layers : found " << m_config.centralLayerConfigs.size() << " configurations.");
    ...
    for (auto layerCfg : m_config.centralLayerConfigs ){
        ...
        if (volume){
            ...
            for (auto& sNode : sensitiveNodes){
                ...
                auto tgElement = std::make_shared<Acts::TGeoDetectorElement>(Identifier(),sNode);
                m_elementStore.push_back(tgElement);
                // create a layer out of the surfaces
                detSurfaces.push_back(&(tgElement->surface()));
                // create the layer
                cVector.push_back(m_config.layerCreator->cylinderLayer(detSurfaces, 1., 5., layerCfg.binsLoc0, layerCfg.binsLoc1));
            }
...
const Acts::LayerVector Acts::TGeoLayerBuilder::positiveLayers() const
{
...
void Acts::TGeoLayerBuilder::collectSensitive(TGeoVolume* tgVolume, TGeoNode* tNode, const std::string& sensitiveName, std::vector<TGeoNode*>& sensitiveNodes)const
{
