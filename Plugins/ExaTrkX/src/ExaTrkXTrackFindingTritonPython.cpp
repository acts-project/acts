...
ExaTrkXTrackFindingTritonPython::ExaTrkXTrackFindingTritonPython(
    ...
    : ExaTrkXTrackFindingBase("ExaTrkXTrackFindingTritonPython",
                              config.verbose),
      m_cfg(config) {
...
void ExaTrkXTrackFindingTritonPython::getTracks(
  ...
  auto edgeList =
      torch::from_blob(edgeListData.data(), edgeListShape, edges_opts);
  ...
  // << std::endl; std::cout << edgeList.slice(1, 0, 5) << std::endl;
  ...
  // The following two hard copy are needed in order to produce sensible
  // results!
