...
ExaTrkXTrackFindingTritonOne::ExaTrkXTrackFindingTritonOne(
    ...
    : ExaTrkXTrackFindingBase("ExaTrkXTrackFindingTritonOne", config.verbose),
      m_cfg(config) {
  bool verbose = false;
  uint32_t client_timeout = 0;
  std::string model_version = "";

  e_client_ = std::make_unique<ExaTrkXTriton>(
      m_cfg.embedModelName, m_cfg.url, model_version, client_timeout, verbose);
  b_client_ =
      std::make_unique<ExaTrkXTriton>(m_cfg.buildingModelName, m_cfg.url,
                                      model_version, client_timeout, verbose);
  f_client_ = std::make_unique<ExaTrkXTriton>(
      m_cfg.filterModelName, m_cfg.url, model_version, client_timeout, verbose);
  g_client_ = std::make_unique<ExaTrkXTriton>(
      m_cfg.gnnModelName, m_cfg.url, model_version, client_timeout, verbose);
  l_client_ =
      std::make_unique<ExaTrkXTriton>(m_cfg.labelingModelName, m_cfg.url,
                                      model_version, client_timeout, verbose);
}
...
void ExaTrkXTrackFindingTritonOne::getTracks(
  ...
  // std::cout << "After filtering: " << fOutput.size(0) << " " <<
  // fOutput.size(1) << std::endl; std::cout << fOutput.slice(/*dim=*/0,
  // /*start=*/0, /*end=*/9) << std::endl;
  ...
  // std::cout << "After filtering: " << numEdgesAfterF << " edges." <<
  // std::endl;
  ...
  // std::cout << "GNN scores for " << gOutput.size(0) << " edges." <<
  // std::endl; std::cout << gOutput.slice(0, 0, 5) << std::endl;
  // ************
