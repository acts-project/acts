...
class ExaTrkXTriton {
 public:
  ExaTrkXTriton(const std::string& modelName, const std::string& url,
  ...
  template <typename T>
  bool AddInput(const std::string& inputName,
    ...
    FAIL_IF_ERR(input0_ptr->AppendRaw(
                    ...
                    inputValues.size() * sizeof(T)),
                "unable to set data" + inputName);
  ...
  template <typename T>
  bool AddInputFromTorch(const std::string& inputName,
                         const at::Tensor& inputTensor) {
    std::vector<int64_t> inputShape{inputTensor.sizes().vec()};
    std::vector<T> inputValues;
    std::copy(inputTensor.data_ptr<T>(),
  ...
  template <typename T>
  bool GetOutput(const std::string& outputName, std::vector<T>& outputData,
    ...
    // std::cout << "prepare to run inference with " << inputs_.size() << "
    // input(s)." << std::endl;
    std::vector<tc::InferInput*> inputs;
    for (auto& input : inputs_) {
      inputs.push_back(input.get());
    }
    FAIL_IF_ERR(m_Client_->Infer(&results, *options_, inputs, outputs,
    ...
    T* output_data;
    size_t output_size;
    results_ptr->RawData(outputName, (const uint8_t**)&output_data,
                         &output_size);

    // std::cout << "before transfer data: " << output_size << " " <<
    // sizeof(output_data) << std::endl;
    outputData.clear();
    int64_t output_entries = output_size / sizeof(output_data);
    // std::cout << "output_size: " << output_entries << " " << output_data[0]
    ...
    // std::cout << "output_shape: " << num_rows << " " << num_cols <<
    // std::endl;
