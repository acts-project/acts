...
Acts::detail::GeoSubtractionConverter::operator()(
    const PVConstLink& geoPV, const GeoShapeSubtraction& sub,
    const Transform3& absTransform, bool sensitive) const {
  const GeoShapeSubtraction* a = dynamic_cast<const GeoShapeSubtraction*>(sub.getOpA());
  const GeoShapeShift* b = dynamic_cast<const GeoShapeShift*>(sub.getOpB());
  //const GeoShape* b = sub.getOpB();//Shift

  std::cout << a->type() <<" " << b->getOp()->type() << std::endl;
  const GeoShape* aa = a->getOpA();
  //const GeoShape* ab = a->getOpB();
  const GeoShapeShift* ab = dynamic_cast<const GeoShapeShift*>(a->getOpB());
  std::cout << aa->type() <<" " << ab->getOp()->type() << std::endl;
  //TODO while we have a subtraction as shape
