...
void GeoModelDetectorObjectFactory::convertFpv(const std::string &name,
  ...
  if (convertBox(name)) {
    ...
    cache.volumeBoxFPVs.emplace_back(
        std::make_tuple(volume, volumeGen2, physVol));
  }
}
