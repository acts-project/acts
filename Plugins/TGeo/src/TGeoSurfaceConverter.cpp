    ...
    if (interNode != nullptr) {
      ...
      if (baseTube != nullptr) {
        ...
        if (maskShape != nullptr) {
          ...
          // the transformation ?
          //
          // Vertices in TGeo are [0,3] - face at -z  [4,7] - face at +z
          // Vertices are clock-wise
          ...
          // Use the helper to create the bound lines
          auto [annulusBounds, annulusTransform] =
              detail::AnnulusBoundsHelper::create(transform, rMin, rMax,
                                                  vertices);
          transform = annulusTransform;
