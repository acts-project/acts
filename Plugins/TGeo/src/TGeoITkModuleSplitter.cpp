...
Acts::TGeoITkModuleSplitter::split(
  ...
  if (sensorName.find("BRL") != std::string::npos) {
    if (sensorName.find("MS") != std::string::npos) {
      return Acts::TGeoITkModuleSplitter::splitBarrelModule(
  ...
  if (sensorName.find("EC") != std::string::npos) {
    if (sensorName.find("Sensor0") != std::string::npos) {
      return Acts::TGeoITkModuleSplitter::splitDiscModule(
...
/// If applicable, returns a split detector element
inline std::vector<std::shared_ptr<const Acts::TGeoDetectorElement>>
Acts::TGeoITkModuleSplitter::splitBarrelModule(
    const GeometryContext& gctx,
  ...
  auto rectBounds =
      std::make_shared<RectangleBounds>(0.5 * lengthX, 0.5 * lengthY);
  ...
  for (size_t i = 0; i < nSegments; i++) {
    ...
    auto element = std::make_shared<const TGeoDetectorElement>(
...
/// If applicable, returns a split detector element
inline std::vector<std::shared_ptr<const Acts::TGeoDetectorElement>>
Acts::TGeoITkModuleSplitter::splitDiscModule(
    const GeometryContext& gctx,
    std::shared_ptr<const Acts::TGeoDetectorElement> detElement,
    const std::vector<Acts::TGeoITkModuleSplitter::SplitRange>& splitRanges)
    const {
  // Retrive the surface
  auto identifier = detElement->identifier();
  const Surface& surface = detElement->surface();
