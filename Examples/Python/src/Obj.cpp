...
void addObj(Context& ctx) {
...

  {
    ...
    mex.def(
        ...
        [](const std::vector<std::shared_ptr<Acts::Surface>>& surfaces,
           ...
           const Acts::ViewConfig& viewConfig, const std::string& fileName) {
    ...
    mex.def(
        "writeVolumesObj",
        [](const std::vector<
               std::shared_ptr<Acts::Experimental::DetectorVolume>>& Volumes,
           const Acts::GeometryContext& viewContext,
           const Acts::ViewConfig& viewConfig, const std::string& fileName) {
          Acts::GeometryView3D view3D;
          Acts::ObjVisualization3D obj;
  ...
  py::class_<Acts::ObjVisualization3D, Acts::IVisualization3D>(
      m, "ObjVisualization3D")
      .def(py::init<>());
}
}  // namespace ActsPython
