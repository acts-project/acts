...
def addSeeding(
    ...
    seedFinderConfigArg : SeedFinderConfigArg(maxSeedsPerSpM, cotThetaMax, sigmaScattering, radLengthPerSeed, minPt, impactMax, deltaPhiMax, interactionPointCut, arithmeticAverageCotTheta, deltaZMax, maxPtScattering, zBinEdges, skipPreviousTopSP, zBinsCustomLooping, rRangeMiddleSP, useVariableMiddleSPRange, binSizeR, seedConfirmation, centralSeedConfirmationRange, forwardSeedConfirmationRange, deltaR, deltaRBottomSP, deltaRTopSP, deltaRMiddleSPRange, collisionRegion, r, z, zLastLayer)
        SeedFinderConfig settings. deltaR, deltaRBottomSP, deltaRTopSP, deltaRMiddleSPRange, collisionRegion, r, z, zLastLayer are ranges specified as a tuple of (min,max). beamPos is specified as (x,y).
...
def addStandardSeeding(
    ...
    seedFinderConfig = acts.SeedFinderConfig(
        **acts.examples.defaultKWArgs(
            ...
            zMinLastLayer=(seedFinderConfigArg.zLastLayer[0] if seedFinderConfigArg.zLastLayer[0] not None else seedFinderConfigArg.z[0]),
            zMaxLastLayer=(seedFinderConfigArg.zLastLayer[1] if seedFinderConfigArg.zLastLayer[1] not None else seedFinderConfigArg.z[1]),
