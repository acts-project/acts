...
class CylindricalDetectorVolume:
    ...
    def __init__(self, name, extent,  provider = None, binnings = None, supports = [], loglevel = logging.INFO) :
    ...
    def builder(self):
        ...
        shapeConfig.boundValues = [ rRange[0], rRange[1], 0.5 * (zRange[1]-zRange[0]), math.pi, 0]    
        ...
        volConfig.externalsBuilder = VolumeStructureBuilder(shapeConfig, shapeConfig.auxiliary, self._loglevel) 
        if self._provider is not None :
            ...
            volConfig.internalsBuilder = LayerStructureBuilder(layerConfig, layerConfig.auxiliary, self._loglevel)      
        # Return the builder 
...
class CylindricalDetectorContainer :
    ...
    def __init__(self, name, extent, volumes, layers = None, binning = [], loglevel = logging.INFO) :
        ...
        # Otherwise, build the container from the layers
            ...
            for layer in self._layers :
                ...
                if bReference < bRange[0] :
                    gExtent =  Extent([[ self._binning, [bReference, bRange[0]]], [ orthogonal, oRange]])
                    builders += [ CylindricalDetectorVolume(self._name+'_gap_'+str(il), gExtent).builder() ]
            ...
            if bReference < self._extent.range(self._binning)[1] :
                gExtent =  Extent([[ self._binning, [bReference, self._extent.range(self._binning)[1]]], [ orthogonal, oRange]])
                builders += [ CylindricalDetectorVolume(self._name+'_gap_'+str(il), gExtent).builder() ]
