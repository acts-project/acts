...
class DetectorVolume:
    def __init__(self, name, shape, bounds, transform, surfaces = None, binnings = None, supports = None):
    ...
    def getBuilder(self):
        ...
        if self.__surfaces is not None:
            ...
            layerStructureBuilder = acts.LayerStructureBuilder(
                layerStructureConfig, self.__name + '_LayerBuilder', acts.logging.VERBOSE)
...
def createCylindricalContainer(name, dimensions, layer_volumes, binning) :
    ...
    for layer in layer_volumes:
        ...
        if layer_r_min < volume_r_min or layer_r_max > volume_r_max:
            raise Exception("Layer volume r dimensions are bigger than container dimensions")
        if layer_z_min < volume_z_min or layer_z_max > volume_z_max:
            raise Exception("Layer volume z dimensions are bigger than container dimensions")
        ...
        if binning is acts.Binning.r:
            ...
            volume_transform = acts.Transform3([0.,0.,volume_z])
            ...
            if layer_r_min > last_r_max:
                ...
                gap_volume = DetectorVolume(
                    name + '_gap_'+str(igap), acts.VolumeBoundsType.Cylinder, gap_bounds, volume_transform)
            ...
            if layer_r_max < volume_r_max:
                ...
                last_gap = DetectorVolume(
                    name + '_gap_'+str(igap+1), acts.VolumeBoundsType.Cylinder, gap_bounds, volume_transform)
            else :
                last_gap = None
             # Remember the last   
        ...
        # The container is z-ordered    
            ...
            # Force detector container dimention onto layer 
            layer.setBounds([volume_r_min, volume_r_max, 0.5*(layer_z_max - layer_z_min)])
            if layer_z_min > last_z_max:
                gap_bounds = [volume_r_min, volume_r_max, 0.5*(layer_z_min-last_z_max)]
                gap_transform = acts.Transform3([0., 0. ,0.5*(last_z_max + layer_z_min)])
                gap_volume = DetectorVolume(
                    name + '_gap_'+str(igap), acts.VolumeBoundsType.Cylinder, gap_bounds, gap_transform)
                complete_volumes += [gap_volume, layer]
            else:
                complete_volumes += [layer]
            # potential last gap
            if layer_z_max < volume_z_max:
                gap_bounds = [volume_r_min, volume_r_max, 0.5*(volume_z_max-layer_z_max)]
                gap_transform = acts.Transform3([0., 0. ,0.5*(volume_z_max + layer_z_max)])
                last_gap = DetectorVolume(
                    name + '_gap_'+str(igap+1), acts.VolumeBoundsType.Cylinder, gap_bounds, gap_transform)
            else :
                last_gap = None
            # Remember the last
...
def createBarrel(name, dimensions, layerSetups, surfaceSetups, binningSetups):
    ...
    volume_r_min, volume_r_max, volume_z_min, volume_z_max = dimensions
    volume_half_z = 0.5*(volume_z_max - volume_z_min)
    volume_transform = acts.Transform3([0.,0.,0.5*(volume_z_min + volume_z_max)])
    ...
    for i, layer_setup in enumerate(layerSetups):
        ...
        layer = DetectorVolume(name+'_layer_'+str(i), acts.VolumeBoundsType.Cylinder,
