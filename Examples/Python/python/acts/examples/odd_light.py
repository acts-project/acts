...
def necBarrelPec(name, rRange, zDivisions, sensitivesKdt, 
				 necPositionsZ, necHalfThickness, necBinning,
				 barrelPositionsR, barrelHalfThickness, barrelBinnings, llevel = logging.INFO) :
	...
	for zPos in necPositionsZ :
		ilay = len(necLayers)
		lExtent = Extent([[Binning.r, rRange], [Binning.z, [zPos - necHalfThickness, zPos + necHalfThickness]]])
		provider = KdtSurfacesProvider2D(sensitivesKdt, lExtent)
		necLayers += [ detector.CylindricalDetectorVolume('lay_'+str(ilay), lExtent, provider, necBinning, [], llevel) ]
	...
	for layerR in barrelPositionsR :
		...
		lExtent = Extent(
			[[Binning.r, [layerR - barrelHalfThickness, layerR + barrelHalfThickness ]], 
	         [Binning.z, zDivisions[1:3]]])
		provider = KdtSurfacesProvider2D(sensitivesKdt, lExtent)
		binning = barrelBinnings[ilay]
		barrelLayers += [ detector.CylindricalDetectorVolume('lay_'+str(ilay), lExtent, provider, binning, [], llevel) ]
	...
	for zPos in necPositionsZ:
		ilay = len(pecLayers)
		lExtent = Extent([[Binning.r, rRange], 
					      [Binning.z, [-1*zPos - necHalfThickness, -1*zPos + necHalfThickness]]])
		provider = KdtSurfacesProvider2D(sensitivesKdt, lExtent)
		pecLayers += [ detector.CylindricalDetectorVolume('lay_'+str(ilay), lExtent, provider, necBinning, [], llevel) ]
...
def get_odd_light(geoContext, gdmlFile, sMatches, pMatches, llevel = logging.VERBOSE) :
    ...
    [ elements, ssurfaces, psurfaces ] = acts_g4.convertSurfaces(gdmlFile, sMatches, pMatches)
    ...
    bp = detector.CylindricalDetectorVolume('ODD_beampipe', bpExtent, bpProvider, bpBinning, [], llevel)
    ...
    pixNecBinning = [ ProtoBinning(Binning.r, Binning.bound, 40, 175, 2, 1), detector.phiBinning(56, 1) ]
    ...
    pixBarrelBinning = [  [pixBarrelZbinning, 
    				   detector.phiBinning(phiBinning[0], phiBinning[1]) ] for phiBinning in pixBarrelPhiBinnings ]
    
    pix = necBarrelPec('ODD_pixel', pixRrange, pixZdivisions, sensitivesKdt, 
    			 pixNecPositionsZ, pixNecHalfThickness, pixNecBinning,
    			 pixBarrelPositionsR, pixBarrelHalfThickness, pixBarrelBinning, llevel)
    ...
    pst = detector.CylindricalDetectorVolume('ODD_pst', pstExtent, pstProvider, pstBinning, [], llevel)
    ...
    sstripNecBinning = [ ProtoBinning(Binning.r, Binning.bound, 230, 710, 3, 1), detector.phiBinning(40, 1) ]
    ...
    sstripBarrelBinning = [  [sstripBarrelZbinning, 
    				   detector.phiBinning(phiBinning[0], phiBinning[1]) ] for phiBinning in sstripBarrelPhiBinnings ]
    
    sstrip = necBarrelPec('ODD_sstrip', sstripRrange, sstripZdivisions, sensitivesKdt,
                       sstripNecPositionsZ, sstripNecHalfThickness, sstripNecBinning,
                       sstripBarrelPositionsR, sstripBarrelHalfThickness, sstripBarrelBinning, llevel)
    ...
    lstripNecBinning = [ ProtoBinning(Binning.r, Binning.bound, 720, 1020, 2, 1), detector.phiBinning(40, 1) ]
    ...
    lstripBarrelBinning = [  [lstripBarrelZbinning, 
    				   detector.phiBinning(phiBinning[0], phiBinning[1]) ] for phiBinning in lstripBarrelPhiBinnings ]
    
    lstrip = necBarrelPec('ODD_lstrip', lstripRrange, lstripZdivisions, sensitivesKdt,
                       lstripNecPositionsZ, lstripNecHalfThickness, lstripNecBinning,
                       lstripBarrelPositionsR, lstripBarrelHalfThickness, lstripBarrelBinning, llevel)
...
def main():
	...
	p.add_argument(
        "-i", "--input", type=str, default="odd-light.gdml", help="GDML input file (optional)"
    )
	p.add_argument(
        "-s", "--sensitives", type=str, default="phys_vol", help="Match string for sensitive surfaces"
    )
	p.add_argument(
        "-p", "--passives", type=str, default="pass_vol", help="Match string for passive surfaces"
    )	
	args = p.parse_args()
	geoContext = GeometryContext()
	odd_light = get_odd_light(geoContext, args.input, [ args.sensitives ], [ args.passives ], logging.DEBUG)
