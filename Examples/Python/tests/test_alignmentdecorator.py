...
def test_alignmentdecorator_io_mode() :
    ...
    sequencer = Sequencer( events=100,
        numThreads=4,
...
def test_alignmentdecorator_gen_mode() :
    ...
    trf0 = acts.Transform3(acts.Vector3(0.,0.,10.))    
    geoId1 = acts.GeometryIdentifier(volume = 1, layer = 4)
    trf1 = acts.Transform3(acts.Vector3(0.,0.,20.))    
    geoIdMap = {}
    ...
    gShift.shift = acts.Vector3(0.,0.,100.)
    
    gRot = acts.examples.AlignmentGeneratorGlobalRotation()
    gRot.axis = acts.Vector3(1.,0.,0.)
    gRot.angle = 0.15
