...
@pytest.mark.skipif(not ATLAS_TOROIDAL_FIELD_AVAILABLE, reason="ATLASToroidalField not available")
def test_atlas_toroidal_field_basic():
    ...
    assert abs(field_value[0]) > 0.0 or abs(field_value[1]) > 0.0 or abs(field_value[2]) > 0.0
...
def test_atlas_toroidal_field_regions():
    ...
    mag_barrel = (field_barrel[0]**2 + field_barrel[1]**2 + field_barrel[2]**2)**0.5
    
    # Test in endcap region
    pos_endcap = acts.Vector3(2000.0, 0.0, 15000.0)
    field_endcap = field.getField(pos_endcap, cache)
    mag_endcap = (field_endcap[0]**2 + field_endcap[1]**2 + field_endcap[2]**2)**0.5
