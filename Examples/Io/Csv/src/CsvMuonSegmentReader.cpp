...
CsvMuonSegmentReader::CsvMuonSegmentReader(
    const CsvMuonSegmentReader::Config& config,
...
std::string CsvMuonSegmentReader::CsvMuonSegmentReader::name()
    const {
  return "CsvMuonSegmentReader";
}

std::pair<std::size_t, std::size_t>
CsvMuonSegmentReader::availableEvents() const {
  return m_eventsRange;
}

ProcessCode CsvMuonSegmentReader::read(
  ...
  while (reader.read(data)) {
    ...
    newSeg.setLocalCoords(Acts::Vector3{data.localPositionX, data.localPositionY, data.localPositionZ}, 
