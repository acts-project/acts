...
std::string ActsExamples::CsvMuonSimHitReader::CsvMuonSimHitReader::name() const {
...
ActsExamples::ProcessCode ActsExamples::CsvMuonSimHitReader::read(
  ...
  while (reader.read(data)) {
    ...
    ActsFatras::Hit::Vector4 mom{
        ...
        std::sqrt(data.LocalDirectionx*data.LocalDirectionx+data.LocalDirectiony*data.LocalDirectiony+data.LocalDirectionz*data.LocalDirectionz) * Acts::UnitConstants::GeV
    };
    ...
    unordered.push_back(std::move(SimHit(compressId(f),data.pdgId, pos, mom,mom,-1)));
  }
