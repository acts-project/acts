...
ActsExamples::ORSteppingAction::UserSteppingAction(const G4Step* step)
{
	...
	constexpr double convertEnergy = HepMC3::Units::MomentumUnit::GEV / CLHEP::GeV;
	constexpr double convertTime = 1. / CLHEP::s; // HepMC3 has no concept of time
		...
		if(event->vertices().empty())
		{
			...
			for(const auto& vertex : event->vertices())
			{
				if(vertex->position() == prePos)
				{
					...
					vertex->add_particle_out(postParticle);
					vertex->add_attribute("NextProcessOf-" + trackId, process);
					auto preStepMomentum = step->GetPreStepPoint()->GetMomentum() * convertEnergy;
					auto preStepEnergy = step->GetPreStepPoint()->GetTotalEnergy() * convertEnergy;
    ...
    const double L0 = track->GetMaterial()->GetNuclearInterLength() * convertLength;
    const double stepLength = track->GetStepLength();
	postParticle->add_attribute("NextX0", std::make_shared<HepMC3::DoubleAttribute>(X0));
	postParticle->add_attribute("NextL0", std::make_shared<HepMC3::DoubleAttribute>(L0));
	postParticle->add_attribute("StepLength", std::make_shared<HepMC3::DoubleAttribute>(stepLength));
	if(track->GetCreatorProcess())
		postParticle->add_attribute("CreatorProcessOf-" + trackId, std::make_shared<HepMC3::StringAttribute>(track->GetCreatorProcess()->GetProcessName()));
