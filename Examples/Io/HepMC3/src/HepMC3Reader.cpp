...
ProcessCode HepMC3Reader::read(const AlgorithmContext& ctx) {
  ...
  if (m_cfg.perEvent) {
    ...
    if (!m_events.empty() && ctx.eventNumber < m_nextEvent) {
      ...
      auto it = std::ranges::find_if(m_events, [&ctx](auto& elem) {
        return elem.first == ctx.eventNumber;
      });
      ...
      // std::size_t maxEvent = m_events.empty() ? 0 : m_events.back().first;
      ...
      throw_assert(!m_events.empty(),
                   "Event should not be empty, this is a bug.");
