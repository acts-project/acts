...
PYBIND11_MODULE(acts_atlas_toroidal_field, m) {
  ...
  py::class_<ATLASToroidalField::LayoutConfig>(m, "LayoutConfig")
    .def(py::init<>())
    .def_readwrite("theta0_deg", &ATLASToroidalField::LayoutConfig::theta0_deg)
    .def_readwrite("thetaStep_deg", &ATLASToroidalField::LayoutConfig::thetaStep_deg)
  ...
  py::class_<ATLASToroidalField, std::shared_ptr<ATLASToroidalField>, Acts::MagneticFieldProvider>
    (m, "ATLASToroidalField")
    .def(py::init<const ATLASToroidalField::Config&>(), py::arg("config"))
    .def("getField", &getField)
    .def("makeCache", &ATLASToroidalField::makeCache)
    .def("config", &ATLASToroidalField::config, py::return_value_policy::reference_internal);
}
