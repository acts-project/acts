...
toroidalFieldMapCyl(const std::pair<double,double>& rLim,
  ...
  if (nBinsR < 2 || nBinsP < 2 || nBinsZ < 2) {
    throw std::invalid_argument("toroidalFieldMapCyl: each dimension needs at least 2 bins");
  }

  // Follow solenoid style: extend upper edge by one step to accommodate overflow bin edges
  ...
  Grid grid(Type<Vector3>, std::move(rAxis), std::move(pAxis), std::move(zAxis));
  ...
  auto transformPos = [](const Vector3& pos) {
    ...
    return Vector3(r, phi, pos.z());  // note: we reuse Vector3 to carry (r,phi,z)
  };
  ...
  for (std::size_t ir = 0; ir <= nBinsR + 1; ++ir) {
    for (std::size_t ip = 0; ip <= nBinsP + 1; ++ip) {
      for (std::size_t iz = 0; iz <= nBinsZ + 1; ++iz) {
        ...
        if (ir == 0 || ip == 0 || iz == 0 ||
            ir == nBinsR + 1 || ip == nBinsP + 1 || iz == nBinsZ + 1) {
          ...
          double cosPhi = (r > std::numeric_limits<double>::min()) ? std::cos(phi) : 1.0;
          double sinPhi = (r > std::numeric_limits<double>::min()) ? std::sin(phi) : 0.0;
  ...
  // Mapper
  InterpolatedBFieldMap<Grid_t> map({transformPos, transformBField, std::move(grid)});
  return map;
}
...
toroidalFieldMapXYZ(const std::pair<double,double>& xLim,
  ...
  if (nBinsX < 2 || nBinsY < 2 || nBinsZ < 2) {
    throw std::invalid_argument("toroidalFieldMapXYZ: each dimension needs at least 2 bins");
  }
  ...
  Grid grid(Type<Vector3>, std::move(xAxis), std::move(yAxis), std::move(zAxis));
  using Grid_t = decltype(grid);
  ...
  for (std::size_t ix = 0; ix <= nBinsX + 1; ++ix) {
    for (std::size_t iy = 0; iy <= nBinsY + 1; ++iy) {
      for (std::size_t iz = 0; iz <= nBinsZ + 1; ++iz) {
        ...
        if (ix == 0 || iy == 0 || iz == 0 ||
            ix == nBinsX + 1 || iy == nBinsY + 1 || iz == nBinsZ + 1) {
          grid.atLocalBins(index) = Grid_t::value_type(0.0, 0.0, 0.0);
        } else {
