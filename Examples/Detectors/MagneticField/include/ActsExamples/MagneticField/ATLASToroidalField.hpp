...
// Full ATLAS toroidal field (barrel + two endcaps) with per-coil current senses.
...
class ATLASToroidalField final : public MagneticFieldProvider {
 public:
  ...
  struct Cache { explicit Cache(const MagneticFieldContext& /*ctx*/) {} };
  ...
  explicit ATLASToroidalField(Config cfg) : m_cfg(std::move(cfg)) {
    ...
    if (m_cfg.barrelSigns.empty()) {
      ...
      for (int k = 0; k < nC; ++k) m_cfg.barrelSigns[k] = (k % 2 == 0) ? +1 : -1;
    }
    if (m_cfg.ectSigns.empty()) {
      // Updated default to match Python:
      // coil_signs_ect_front = coil_signs_ect_back = [-1, +1, -1, +1, -1, +1, -1, +1]
    ...
    if (static_cast<int>(m_cfg.barrelSigns.size()) != nC)
      throw std::invalid_argument("ATLASToroidalField: barrelSigns size must equal nCoils");
    if (static_cast<int>(m_cfg.ectSigns.size()) != 2 * nC)
      throw std::invalid_argument("ATLASToroidalField: ectSigns size must equal 2*nCoils");
  ...
  Result<Vector3> getField(const Vector3& position,
    ...
    for (std::size_t i = 0; i < m_mids_barrel.size(); ++i) {
      ...
      const double r2 = static_cast<double>(rx)*rx + static_cast<double>(ry)*ry + static_cast<double>(rz)*rz + static_cast<double>(eps);
 ...
 private:
  ...
  ectRacetrackRadial(float Lrho, float Lz, int nArc, int nStraight, bool close) {
  ...
  static void buildSegsMidsRZ(const std::vector<std::array<float,2>>& rz,
    ...
    for (std::size_t i=0;i+1<rz.size();++i) {
      ...
      m_rz.push_back({0.5f*(rz[i+1][0]+rz[i][0]), 0.5f*(rz[i+1][1]+rz[i][1])});
    }
  }
  ...
  void buildGeometry() {
    ...
    const auto rz_barrel = racetrackRZ(aB, m_cfg.barrel.b, m_cfg.barrel.c,
    ...
    const auto  rz_ect = ectRacetrackRadial(/*Lrho=*/LrhoE,
                                            /*Lz=*/m_cfg.ect.c,
                                            m_cfg.layout.nArc,
                                            m_cfg.layout.nStraight,
    ...
    for (int k=0;k<nC;++k) {
      ...
      mapRingToXYZ(lB, m_rzB, d_rzB, phi, sign, /*zShift=*/0.0f,
                   m_mids_barrel, m_segs_barrel);
    }

    // Endcap centers: z = ±(c/2 + gap + c_ECT/2)
    const float zECT = 0.5f * m_cfg.barrel.c + m_cfg.ect.gap + 0.5f * m_cfg.ect.c;

    // +z endcap (indices 0..nC-1 in ectSigns)
    for (int k=0;k<nC;++k) {
      const float phi = th0 + k*dth;
      const int sign  = m_cfg.ectSigns[k];
      mapRingToXYZ(lE, m_rzE, d_rzE, phi, sign, /*zShift=*/+zECT,
                   m_mids_ect, m_segs_ect);
    }
