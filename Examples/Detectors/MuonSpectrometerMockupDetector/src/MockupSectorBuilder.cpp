...
ActsExamples::MockupSectorBuilder::buildChamber(
  ...
  for (auto& detectorElement : detectorElements) {
    ...
    min_max[1].first =
        std::min(min_max[1].first, static_cast<float>(g4ConvSurf->center(context).y()));
    min_max[1].second =
        std::max(min_max[1].second, static_cast<float>(g4ConvSurf->center(context).y()));

    min_max[2].first =
        std::min(min_max[2].first, static_cast<float>(g4ConvSurf->center(context).z()));
    min_max[2].second =
        std::max(min_max[2].second, static_cast<float>(g4ConvSurf->center(context).z()));
  }
