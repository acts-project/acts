    ...
    class GeoMuonMockupExperiment: public GeoDeDuplicator {
        public:
            ...
            struct Config{
                ...
                std::array<double, MuonLayer::nLayers> barrelRadii{6.*GeoModelKernelUnits::m, 
                                                                   9.*GeoModelKernelUnits::m, 
        ...
        private:
            ...
            constexpr static double s_rpcGasSingletSeparation = 2.*GeoModelKernelUnits::cm;

            /// @brief Total height of a rpc station
            double m_rpcChamberHeight = m_cfg.nRpcGasGaps * (s_rpcGasHeight + s_rpcGasSingletSeparation);
            ...
            double m_muonStationHeight = 2.*m_multiLayerHeight + m_cfg.multiLayerSeparation + 
             ...
             *         virtual tube layer volumes each containing a serial transformer placing the
             *         tubes per layer
             ...
             *  @param width: 
             */
            PVLink buildAbsorber(const double thickness, 
                                 const double width, 
                                 const double length);


            PVLink assembleBarrelStation(const MuonLayer layer,
                                         const unsigned int sector,
                                         const int etaIdx);

            /// @brief Assemble a new multilayer volume to fit inside the barrel station
            FPVLink assembleMultilayerBarrel(const unsigned ml,
