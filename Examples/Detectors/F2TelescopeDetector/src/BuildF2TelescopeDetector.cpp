...
#include "ActsExamples/F2TelescopeDetector/BuildF2TelescopeDetector.hpp"
...
ActsExamples::F2Telescope::buildDetector(
    const typename ActsExamples::F2Telescope::F2TelescopeDetectorElement::
        ContextType& gctx,
    std::vector<
        std::shared_ptr<ActsExamples::F2Telescope::F2TelescopeDetectorElement>>&
        detectorStore,
    ...
    ActsExamples::F2Telescope::F2TelescopeSurfaceType surfaceType,
  ...
  if(boundsX.size() != positions.size() || boundsY.size() != positions.size()) {
      throw std::invalid_argument("Inconsistent bounds and positions sizes");
  }
  ...
  for (unsigned int i = 0; i < nLayers; ++i) {
    ...
    const auto pBounds =
        std::make_shared<const Acts::RectangleBounds>(boundsX[i], boundsY[i]);
    // The radial bounds for disc surface
    const auto rBounds =
        std::make_shared<const Acts::RadialBounds>(boundsX[i], boundsY[i]);
    
    F2TelescopeSurfaceType current_surfaceType = static_cast<ActsExamples::F2Telescope::F2TelescopeSurfaceType>(Type_surf[i]);

    // Create the detector element
    std::shared_ptr<F2TelescopeDetectorElement> detElement = nullptr;
    if (current_surfaceType == F2TelescopeSurfaceType::Plane) {
      detElement = std::make_shared<F2TelescopeDetectorElement>(
  ...
  if (surfaceType == F2TelescopeSurfaceType::Plane) {
  ...
  auto trackVolume =
      Acts::TrackingVolume::create(trafoVol, boundsVol, nullptr,
                                   std::move(layArr), nullptr, {}, "F2Telescope");
