...
//a function for sorting the detector volumes from the innermost to the outermost

bool sortByRadialDistance(std::shared_ptr<Acts::Experimental::DetectorVolume>& det_vol1, 
...
std::shared_ptr<Acts::Experimental::DetectorVolume> ActsExamples::MockupSectorBuilder::BuildChamber(
	...
	const ActsExamples::MockupSectorBuilder::ChamberConfig& chamber_config){
      ...
      auto g4SurfaceOptions = Acts::Geant4DetectorSurfaceFactory::Options();
  		...
  		auto [surfaces, detectorElements] = g4detector.convertGeant4Volumes(g4World_config);
  		...
  		for (auto& detectorElement : detectorElements) {
    		...
    		auto g4conv_surf = g4conv.Geant4PhysicalVolumeConverter::surface(
        		detectorElement->g4PhysicalVolume(), detectorElement->transform(context));
  		...
  		auto detectorVolumeBounds = std::make_unique<Acts::CuboidVolumeBounds>(hx, hy, hz);

  		Acts::Vector3 chamber_position = {(maxX+minX)/2, (maxY+minY)/2, (maxZ+minZ)/2};
      ...
      std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>>{},  Acts::Experimental::allPortalsAndSurfaces());  	
    ...
    std::shared_ptr<Acts::Experimental::DetectorVolume> ActsExamples::MockupSectorBuilder::BuildSector(
    	std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>> det_volumes,
    	const Acts::GeometryContext& gctx){
      ...
      //sort the dtector volumes by their radial distance (from innermost---->outermost)
      std::sort(det_volumes.begin(), det_volumes.end(), sortByRadialDistance);
    	...
    	auto rmin = det_volumes.front()->center().y()-2*det_volumes.front()->volumeBounds().values()[1];
    	auto rmax = det_volumes.back()->center().y()+ 3*det_volumes.back()->volumeBounds().values()[1];
      auto halfZ = det_volumes.back()->volumeBounds().values()[2];

      //radius of the inner cylinder for the inner chambers
      auto rmin_inner = rmin;
      auto rmax_inner = det_volumes.front()->center().y()+ 3*det_volumes.front()->volumeBounds().values()[1];

      //radius of the middle cylinder for the middle chambers
      auto rmin_middle = det_volumes[1]->center().y()-2*det_volumes[1]->volumeBounds().values()[1];
      auto rmax_middle = det_volumes[1]->center().y() + 3*det_volumes[1]->volumeBounds().values()[1];

      //radius of the outer cylinder 
      auto rmin_outer = det_volumes.back()->center().y() - 2*det_volumes.back()->volumeBounds().values()[1];
      auto rmax_outer = det_volumes.back()->center().y() + 3*det_volumes.back()->volumeBounds().values()[1];


    	//create two vectors in order to define the opening angle of each sector
      auto xA = det_volumes.back()->center().x()+det_volumes.back()->volumeBounds().values()[0];
      auto yA = det_volumes.back()->center().y()-det_volumes.back()->volumeBounds().values()[1];
      auto zA = det_volumes.back()->center().z();

      auto xB = det_volumes.back()->center().x()-det_volumes.back()->volumeBounds().values()[0];
      auto yB = det_volumes.back()->center().y()-det_volumes.back()->volumeBounds().values()[1];
      ...
      if(m_cfg.NumberOfSectors == 1) {
    	...
    	auto detectorVolumeBounds = std::make_unique<Acts::CylinderVolumeBounds>(rmin,rmax,halfZ,sector_angle);
      auto inner_detectorVolumeBounds = std::make_unique<Acts::CylinderVolumeBounds>(rmin_inner,rmax_inner,halfZ,sector_angle);
      auto middle_detectorVolumeBounds = std::make_unique<Acts::CylinderVolumeBounds>(rmin_middle,rmax_middle,halfZ,sector_angle);      
      auto outer_detectorVolumeBounds= std::make_unique<Acts::CylinderVolumeBounds>(rmin_outer,rmax_outer,halfZ,sector_angle);               	  	
      ...
      //keep the inner, middle and outer chambers in order to constraint them in cylinder bounds
      std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>> detectorVolumesOfSector = {}; 
      std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>> inner_detectorVolumesOfSector = {};
      std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>> middle_detectorVolumesOfSector = {};
      std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>> outer_detectorVolumesOfSector = {};
      ...
      for(int i=0; i<m_cfg.NumberOfSectors; i++){
        ...
        Acts::AngleAxis3 rotation(2*i*hopening_angle, Acts::Vector3(0.,0., 1.));  

        itr=0;       

        for(auto& det_vol: det_volumes){
         
          auto shift_vol = rotation*Acts::Transform3(Acts::Translation3(det_vol->center()));
          ...
          for(auto& det_surf: det_vol->surfaces()){
           ...
           //create the shifted surfaces by creating copied surface objects
            ...
            auto copiedTransformStrawSurface = Acts::Surface::makeShared<Acts::StrawSurface>(Acts::GeometryContext(), *strawSurfaceObject, shift_surf);
            shifted_surfaces.push_back(copiedTransformStrawSurface);
          }

            //create the bounds of the volumes of each chamber         
            auto bounds = std::make_unique<Acts::CuboidVolumeBounds>(det_vol->volumeBounds().values()[0], 
                                                                     det_vol->volumeBounds().values()[1],
                                                                     det_vol->volumeBounds().values()[2]);
           //create the shifted chamber 
            auto detectorVolume_sec = Acts::Experimental::DetectorVolumeFactory::construct(
             Acts::Experimental::defaultPortalGenerator(),gctx, "test-cylinder",
             shift_vol, 
             std::move(bounds),
             shifted_surfaces, 
            ...
            if(itr == 0) inner_detectorVolumesOfSector.push_back(detectorVolume_sec);
            if(itr == 1) middle_detectorVolumesOfSector.push_back(detectorVolume_sec);
            if(itr == 2) outer_detectorVolumesOfSector.push_back(detectorVolume_sec);
      ...
      auto detectorVolume_inner = Acts::Experimental::DetectorVolumeFactory::construct(
      Acts::Experimental::defaultPortalGenerator(), gctx, "inner_cylinder",
      transform,
      std::move(inner_detectorVolumeBounds), std::vector<std::shared_ptr<Acts::Surface>>{},
      inner_detectorVolumesOfSector, Acts::Experimental::allPortalsAndSurfaces());

       auto detectorVolume_middle = Acts::Experimental::DetectorVolumeFactory::construct(
      Acts::Experimental::defaultPortalGenerator(), gctx, "middle_cylinder",
      transform,
      std::move(middle_detectorVolumeBounds), std::vector<std::shared_ptr<Acts::Surface>>{},
      middle_detectorVolumesOfSector, Acts::Experimental::allPortalsAndSurfaces());

        auto detectorVolume_outer = Acts::Experimental::DetectorVolumeFactory::construct(
      Acts::Experimental::defaultPortalGenerator(), gctx, "outer_cylinder",
      transform,
      std::move(outer_detectorVolumeBounds), std::vector<std::shared_ptr<Acts::Surface>>{},
      outer_detectorVolumesOfSector, Acts::Experimental::allPortalsAndSurfaces());
      ...
      detectorVolumesOfSector.push_back(detectorVolume_outer);
       ...
       std::cout<<"volumes in sector"<<detectorVolumesOfSector.size()<<std::endl;


      //creation of the mother volume 
    	auto detectorVolume = Acts::Experimental::DetectorVolumeFactory::construct(
      Acts::Experimental::defaultPortalGenerator(), gctx, "sector",
      transform,
      std::move(detectorVolumeBounds), std::vector<std::shared_ptr<Acts::Surface>>{},
      detectorVolumesOfSector, Acts::Experimental::allPortalsAndSurfaces());  
