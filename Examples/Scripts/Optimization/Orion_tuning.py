...
srcDir = Path(__file__).resolve().parent.parent/"Python/"

def run_ckf(params, names, outFile):

    if len(params) != len(names):
        raise Exception("Length of Params must equal names")

    ckf_script = srcDir/"ckf_tracks.py"
    nevts = "--nEvents=1"
    indir = "--indir="+str(srcDir)
    outdir = "--output="+str(srcDir)
    outfile = "--outputfile="+str(outFile)
...
class Objective:
    ...
    def __call__(self, maxSeedsPerSpM, cotThetaMax, sigmaScattering, radLengthPerSeed, impactMax, maxPtScattering, deltaRMin, deltaRMax):

        params = [maxSeedsPerSpM, cotThetaMax, sigmaScattering, radLengthPerSeed, impactMax, maxPtScattering, deltaRMin, deltaRMax]
        keys = ["maxSeedsPerSpM", "cotThetaMax", "sigmaScattering", "radLengthPerSeed", "impactMax", "maxPtScattering", "deltaRMin", "deltaRMax"]

        outputfile = srcDir/"performance_ckf_tuned.root"
        run_ckf(params, keys, outputfile)
        ...
        self.res["duplicaterate"].append(rootFile["duplicaterate_tracks"].member("fElements")[0])

        timingfile = srcDir/"timing.tsv"
        ...
        penalty = self.res["fakerate"][-1] + self.res["duplicaterate"][-1]/self.k_dup + self.res["runtime"][-1]/self.k_time

        return [{"name": "objective", "type": "objective", "value": -(efficiency-penalty)}]
...
def main():
    ...
    space = {"maxSeedsPerSpM": "uniform(0,10,discrete=True)", 
    ...
    df_imp = df[["objective", "maxSeedsPerSpM", "cotThetaMax", "sigmaScattering", "radLengthPerSeed", "impactMax", "maxPtScattering", "deltaRMin", "deltaRMax"]]
    df_obj = df["objective"]
    min_obj = df_obj.min()
