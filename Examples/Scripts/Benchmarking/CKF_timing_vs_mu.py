...
ax = plt.subplot(111)
plt.plot(muDict.keys(),[i[0][0] for i in np.array(list(muDict.values()))],'-+', label='$\chi^{2}$<15, $n_{source link}	<=10$')
plt.plot(muDict.keys(),[i[1][0] for i in np.array(list(muDict.values()))],'-*', label='$\chi^{2}$<10, $n_{source link}=1$')
ax.set_xlabel('$<\mu>$')
