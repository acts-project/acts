...
def runSimulation(
    ...
    logger.info("particle_measurements_map will be generated after simulation completes")
...
def setupMisalignment(
    ...
    print(f"  Magnitudes:  ±{shift_mag_mm} mm (translation), ±{rotation_mag_rad} rad (rotation)")
    ...
    for element in detector_elements:
        ...
        if match_filter(geoid_parts, target_volume, target_layer, target_sensitive, target_extra):
            ...
            if layer_config == -1:
                ...
                for lay_id, sensor_config in sorted(layer_config.items()):
                    if sensor_config == -1:
                        ...
                        sensor_str = f"{sensor_list}{'...' if len(sensor_config) > 10 else ''}"
                        volume_str += f"      Layer {lay_id}: sensors {sensor_str}\n"
                    else:
                        volume_str += f"      Layer {lay_id}: sensor {sensor_config}\n"
            elif isinstance(layer_config, list):
                volume_str += f"    Volume {vol_id}: layers {layer_config}, ALL sensors\n"
            else:
                volume_str += f"    Volume {vol_id}: layer {layer_config}, ALL sensors\n"
        volume_str = volume_str.rstrip()
    elif target_volume == -1:
        volume_str = 'ALL'
    elif isinstance(target_volume, list):
        volume_str = f'{target_volume}' if len(target_volume) <= 5 else f'{len(target_volume)} IDs: {target_volume[:5]}...'
    else:
    ...
    if isinstance(target_volume, dict):
        ...
        layer_str = f'{target_layer}' if len(target_layer) <= 5 else f'{len(target_layer)} IDs: {target_layer[:5]}...'
    else:
    ...
    for element in selected_elements:
        ...
        trf = acts.Transform3(
            acts.Vector3(nom_trans[0], nom_trans[1], nom_trans[2]),
            nom_rotation
        )
        ...
        rotation_matrix = [
            [float(misaligned_rot[0, 0]), float(misaligned_rot[0, 1]), float(misaligned_rot[0, 2])],
            [float(misaligned_rot[1, 0]), float(misaligned_rot[1, 1]), float(misaligned_rot[1, 2])],
            [float(misaligned_rot[2, 0]), float(misaligned_rot[2, 1]), float(misaligned_rot[2, 2])]
        ]
        
        # Simplified record: only ID, Translation, and RotationMatrix
        record = {
            "ID": element['ID'],
            "Translation": [misaligned_trans[0], misaligned_trans[1], misaligned_trans[2]],
...
def runAlignment(
    ...
    alignment_dof = 0
    if tx: alignment_dof |= Center0
    if ty: alignment_dof |= Center1
    if tz: alignment_dof |= Center2
    if rx: alignment_dof |= Rotation0
    if ry: alignment_dof |= Rotation1
    if rz: alignment_dof |= Rotation2
    
    print(f"Alignment DOF enabled (matches misalignment):")
    print(f"  dx={bool(alignment_dof & Center0)}, dy={bool(alignment_dof & Center1)}, dz={bool(alignment_dof & Center2)}")
    print(f"  rx={bool(alignment_dof & Rotation0)}, ry={bool(alignment_dof & Rotation1)}, rz={bool(alignment_dof & Rotation2)}")
    ...
    def visit(surface: acts.Surface) -> bool:
        ...
        if de is not None:
            ...
            if match_surface_to_filter(gid, target_volume, target_layer, target_sensitive, target_extra):
    ...
    if len(det_elements) != len(geoIdMap):
        print(f"WARNING: Mismatch between misaligned ({len(geoIdMap)}) and alignment ({len(det_elements)}) element counts!")
    else:
    ...
    aal_cfg.alignedTransformUpdater = acts.examples.makeAlignedTransformUpdater(mutableStore)
    
    aal_cfg.alignedDetElements = det_elements
    aal_cfg.align = acts.examples.makeAlignmentFunction(trackingGeometry, field, acts.logging.INFO)
    aal_cfg.maxNumIterations = 100  # Reasonable number of iterations
    aal_cfg.maxNumTracks = 100000
    aal_cfg.iterationState = {i: int(alignment_dof) for i in range(aal_cfg.maxNumIterations)}
    ...
    for surface in aligned_surfaces:
        ...
        if aligned_trf is not None:
            ...
            aligned_rotation_matrix = [
                [float(aligned_rot[0, 0]), float(aligned_rot[0, 1]), float(aligned_rot[0, 2])],
                [float(aligned_rot[1, 0]), float(aligned_rot[1, 1]), float(aligned_rot[1, 2])],
                [float(aligned_rot[2, 0]), float(aligned_rot[2, 1]), float(aligned_rot[2, 2])]
            ]
...
def runReconstruction(
    ...
    if mode == "nominal":
        ...
        raise ValueError(f"Unknown mode: {mode}. Must be 'nominal', 'misaligned', or 'aligned'")
...
if "__main__" == __name__:
    ...
    full_parser = subparsers.add_parser('full', help='Run complete workflow: simulation -> alignment -> reconstruction (all geometries)')
    full_parser.add_argument('--num-events', type=int, default=100000, help='Number of events')
    full_parser.add_argument('--simulation-dir', type=str, default=None, help='Simulation output directory (default: simulation_output)')
    full_parser.add_argument('--alignment-dir', type=str, default=None, help='Alignment output directory (default: alignment_output)')
    full_parser.add_argument('--reconstruction-dir', type=str, default=None, help='Reconstruction output directory (default: reconstruction_output)')
    
    # Simulation command
    sim_parser = subparsers.add_parser('simulation', help='Run simulation')
    sim_parser.add_argument('--num-events', type=int, default=100000, help='Number of events')
    sim_parser.add_argument('--output-dir', type=str, default=None, help='Output directory (default: simulation_output)')
    
    # Alignment command
    align_parser = subparsers.add_parser('alignment', help='Run alignment algorithm')
    align_parser.add_argument('--num-events', type=int, default=100000, help='Number of events')
    align_parser.add_argument('--input-dir', type=str, default=None, help='Input directory (default: simulation_output)')
    align_parser.add_argument('--output-dir', type=str, default=None, help='Output directory (default: alignment_output)')
    
    # Reconstruction command
    recon_parser = subparsers.add_parser('reconstruction', help='Run reconstruction')
    recon_parser.add_argument('--mode', type=str, choices=['nominal', 'misaligned', 'aligned'], default='nominal', help='Reconstruction mode')
    recon_parser.add_argument('--num-events', type=int, default=100000, help='Number of events')
    recon_parser.add_argument('--input-dir', type=str, default=None, help='Input directory (default: simulation_output)')
    recon_parser.add_argument('--output-dir', type=str, default=None, help='Output directory (default: reconstruction_output)')
    ...
    if args.command == 'full':
        ...
        simulationDir = Path(args.simulation_dir) if args.simulation_dir else Path.cwd() / "simulation_output"
        alignmentDir = Path(args.alignment_dir) if args.alignment_dir else Path.cwd() / "alignment_output"
        reconstructionDir = Path(args.reconstruction_dir) if args.reconstruction_dir else Path.cwd() / "reconstruction_output"
        ...
        outputDir = Path(args.output_dir) if args.output_dir else Path.cwd() / "simulation_output"
        ...
        inputDir = Path(args.input_dir) if args.input_dir else Path.cwd() / "simulation_output"
        outputDir = Path(args.output_dir) if args.output_dir else Path.cwd() / "alignment_output"
        outputDir.mkdir(exist_ok=True)
        ...
        inputDir = Path(args.input_dir) if args.input_dir else Path.cwd() / "simulation_output"
        outputDir = Path(args.output_dir) if args.output_dir else Path.cwd() / "reconstruction_output"
        outputDir.mkdir(exist_ok=True)
