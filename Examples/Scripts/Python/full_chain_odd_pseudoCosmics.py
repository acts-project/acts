...
print(trackingGeometry.highestTrackingVolume)
# trackingGeometry.highestTrackingVolume.Print() 
...
addParticleGun(
    ...
    PhiConfig(269.0* u.degree, 271.0 * u.degree),
    # PhiConfig(0, 360.0 * u.degree),
    ParticleConfig(
        args.gun_particles, acts.PdgParticle.eMuon, randomizeCharge=True
    ),
    vtxGen=acts.examples.CartesianVertexPositionGenerator(
        xMean=0.0,
        xStdDev=200.0 * u.mm,
        yMean=1000.0 * u.mm,
        yStdDev=0 * u.mm,
...
if args.reco:
    ...
    addSeeding(
        ...
        trackingGeometry,
        field,
        seedingAlgorithm = SeedingAlgorithm.CosmicsGridTriplet,
        initialSigmas=[
            1 * u.mm,
            1 * u.mm,
            1 * u.degree,
