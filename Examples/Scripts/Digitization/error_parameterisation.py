...
def run_error_parametriation(rfile, 
                             digi_cfg,
                             volumes, 
                             output_dir = Path.cwd(),
    ...
    for iv, v_id_n in enumerate(volumes) :
        ...
        for l in local_values :
            ...
            lplots = []
            # Overview plot
            plt.hist(vol['clus_size_loc'+str(l)], 
                    bins = range(1, max(vol['clus_size_loc'+str(l)])+3),
                    histtype='step', fill=True, color=view_colors[l])
            ...
            for c_size in range(1, max_clus_size) :
                ...
                if not next_sel.empty :
                    ...
                    next_res = next_sel['rec_loc'+str(l)]-next_sel['true_loc'+str(l)]
                    if len(next_sel) < break_min_stat or abs(rms-np.std(next_res))/rms < break_rms_change:
                        # Recaluate with rest
                        vol_sel = vol[vol['clus_size_loc'+str(l)] >= c_size]
                        res = vol_sel['rec_loc'+str(l)]-vol_sel['true_loc'+str(l)]
                ...
                plt.hist(res, bins = 100, 
                         ...
                         histtype='step', fill=True, color=view_colors[l])
                plt.text(0.05, 0.95, 'rms = '+str(round(rms, 3)), 
                         transform=plt.gca().transAxes, fontsize=14, verticalalignment='top')
                plt.xlabel('Resolution - local '+str(l)+', cluster size '+c_size_flag)
                # Save the figure
                svg_path = output_fig_dir / f"{v_id_str}_res_loc{l}_clus_size{c_size_flag}.svg"
                plt.savefig(svg_path)
                lplots.append(svg_path)
                plt.clf()
                if break_condition :
...
<div class="wrapper">
    ...
    """.format(vid=v_id, previous=str(previous_file), next=str(next_file),
            content=plot_content, date=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
    )
...
{volume_content}
        ...
        """.format(date=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), volume_content=volume_links))
...
if "__main__" == __name__:
    ...
    volumes = [ (16, 'Pixel NEC'), (17, 'Pixel Barrel'), (18, 'Pixel PEC'), 
                (23, 'SStrips NEC'), (24, 'SStrips Barrel'), (25, 'SStrips PEC'), 
                (28, 'LStrips NEC'), (29, 'LStrips Barrel'), (30, 'LStrips PEC') ]

    # Open the json to be updated
    digi_cfg = None
    if args.json_in is not None and os.path.isfile(args.json_in) and os.access(args.json_in, os.R_OK):
    ...
    run_error_parametriation(rfile, digi_cfg, volumes,  Path.cwd() / 'output', args.json_out)
