...
} // this is not FTF config type because it is a meber of the algs config, which
  // is of type FTF cofig

// exectute:
ActsExamples::ProcessCode
ActsExamples::SeedingFTFAlgorithm::execute(const AlgorithmContext &ctx) const {
...
ActsExamples::SeedingFTFAlgorithm::Make_ACTS_FTF_Map() const {
  ...
  for (auto i : parsedCsv) {
    ...
    // here think should have pair of new number, vol*100+layer, mod which is 0
    // or number
  }
...
ActsExamples::SeedingFTFAlgorithm::Make_FTF_spacePoints(
  ...
  FTF_spacePoints.reserve(
      m_inputSpacePoints
          .size()); // not sure if this is enough, each one has several sp
  ...
  for (const auto &isp : m_inputSpacePoints) {
    for (const auto &spacePoint : (*isp)(ctx)) {
      ...
      auto key = std::make_pair(
          ACTS_joint_id, 0); // here the key needs to be pair of(vol*100+lay, 0)
...
ActsExamples::SeedingFTFAlgorithm::LayerNumbering() const {
  ...
  for (Acts::GeometryIdentifier geoId :
       m_cfg.geometrySelection) { // think this is the volume loop
    ...
    m_cfg.trackingGeometry->visitSurfaces([this, &input_vector, &count_vector](
      ...
      short barrel_ec = 0; // a variable that says if barrrel, 0 = barrel, need
                           // to find what the n values are
      int eta_mod = Find->second.second;
      ...
      if (m_cfg.fill_module_csv) {
        ...
        // print to csv for each module, no repeates so dont need to make map
        // for averaging
