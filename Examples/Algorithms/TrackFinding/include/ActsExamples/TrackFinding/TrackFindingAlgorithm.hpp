...
class TrackFindingAlgorithm final : public BareAlgorithm {
 ...
 private:
  template<typename source_link_accessor_container_t,
    ...
    void computeSharedHits(const source_link_accessor_container_t& sourcelinks,
			   ...
			   Acts::CombinatorialKalmanFilterResult<source_link_accessor_value_t>>>&) const;
 ...
 private:
  ...
  template<typename source_link_accessor_container_t,
	   typename source_link_accessor_value_t>
  void TrackFindingAlgorithm::computeSharedHits(const source_link_accessor_container_t& sourceLinks,
						std::vector<Acts::Result<
						Acts::CombinatorialKalmanFilterResult<source_link_accessor_value_t>>>& results) const
  {
