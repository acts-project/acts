...
ProcessCode TruthJetAlgorithm::execute(const AlgorithmContext& ctx) const {
  ...
  if (m_cfg.doJetLabeling) {
    ...
    std::ranges::copy(hadronView, std::back_inserter(labelCandidates));

    // Deduplicate label candidates based on their pdg id
    std::ranges::sort(labelCandidates, [](const auto& a, const auto& b) {
      return a.second->pdg() < b.second->pdg();
    });

    auto unique = std::ranges::unique(labelCandidates);
    labelCandidates.erase(unique.begin(), unique.end());
  }
  ...
  auto classifyJet = [&](const fastjet::PseudoJet& jet) {
    ...
    if (maxHadronIt == labelCandidatesInJet.end()) {
      // Now hadronic "jet"
      return ActsExamples::JetLabel::OtherJet;
    }
