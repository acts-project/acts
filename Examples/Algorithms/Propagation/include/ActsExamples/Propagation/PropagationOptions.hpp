...
inline void addPropagationOptions(
  ...
  // clang-format off
  opt.add_options()
  ("prop-debug", po::value<bool>()->default_value(false), "Run in debug mode, will create propagation screen output.")
  ("prop-step-collection", po::value<std::string>()->default_value("propagation-steps"), "Propgation step collection.")
  ("prop-stepper", po::value<int>()->default_value(1), "Propgation type: 0 (StraightLine), 1 (Eigen), 2 (Atlas).")
  ...
  ("prop-energyloss", po::value<bool>()->default_value(true), "Apply energy loss correction - in extrapolation mode only.")
  ("prop-scattering", po::value<bool>()->default_value(true), "Apply scattering correction - in extrapolation mode only.")
  ("prop-record-material", po::value<bool>()->default_value(true), "Record the material interaction and - in extrapolation mode only.")
  ("prop-material-collection", po::value<std::string>()->default_value("propagation-material"), "Propagation material collection.")
  ("prop-ntests", po::value<size_t>()->default_value(1000), "Number of tests performed.")
  ...
  ("prop-d0-sigma", po::value<double>()->default_value(15_um), "Sigma of the transverse impact parameter [in mm].")
  (
  ...
  (
      "prop-qp-sigma", po::value<double>()->default_value(0.0001),
      "Sigma of the signed inverse momentum [in GeV^{-1}]."
  )
  (
      "prop-t-sigma", po::value<double>()->default_value(1),
  ...
  (
      "prop-pt-range",
      po::value<Reals<2>>()->default_value({{0.100, 100}}),
      "Transverse momentum range for proprapolated tracks [in GeV]."
  )
  (
      "prop-max-stepsize", po::value<double>()->default_value(3000),
      "Maximum step size for the propagation [in mm]."
  )
  (
      "prop-pt-loopers", po::value<double>()->default_value(0.500),
