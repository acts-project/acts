...
class TracccChainAlgorithm : public IAlgorithm {
 ...
 private:
  ...
  const traccc::geometry m_surfaceTransforms;
  const std::map<std::uint64_t, detray::geometry::barcode> m_barcodeMap;
 ...
 protected:
  ...
  runReconstruction(const vecmem::vector<traccc::measurement>& measurements,
