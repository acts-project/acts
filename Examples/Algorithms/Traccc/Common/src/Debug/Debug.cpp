...
auto createTable(const MeasurementContainer& measurements1,
                 const MeasurementContainer& measurements2,
  ...
  for (std::size_t idx1 = 0; idx1 < measurements1.size(); ++idx1) {
    ...
    MeasurementContainer::ConstVariableProxy measurement1 =
        measurements1.getMeasurement(idx1);
    MeasurementContainer::ConstVariableProxy measurement2 =
        measurements2.getMeasurement(idx2);

    Acts::ActsVector<2> local1 = Conversion::getLocal(measurement1);
    Acts::ActsVector<2> local2 = Conversion::getLocal(measurement2);

    table.push_back({idx1, local1, Conversion::getVariance(measurement1), idx2,
                     local2, Conversion::getVariance(measurement2),
                     (local1 - local2).norm()});
  }
  return table;
}
...
std::string pairingStatistics(
    const MeasurementContainer& measurements1,
    const MeasurementContainer& measurements2,
    const std::map<std::size_t, std::size_t>& indexMap) {
