...
std::string pairingStatistics(
    ...
    const MeasurementContainer& measurements, const detector_t& detector) {
  MeasurementContainer convertedMeasurements;
  Conversion::convertMeasurements(detector, tracccMeasurements.cbegin(),
                                  tracccMeasurements.cend(),
                                  convertedMeasurements);

  std::map<std::size_t, std::size_t> indexMap = Util::matchMap(
      convertedMeasurements.cbegin(), convertedMeasurements.cend(),
      measurements.cbegin(), measurements.cend(),
      Conversion::MeasurementGeoIDHash{}, Conversion::MeasurementAproxEquals{});

  return pairingStatistics(convertedMeasurements, measurements, indexMap);
}
