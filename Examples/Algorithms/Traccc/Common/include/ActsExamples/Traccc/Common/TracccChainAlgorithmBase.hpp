 ...
 protected:
  ...
  ActsExamples::ProcessCode execute(const ActsExamples::AlgorithmContext& ctx) const override{
    ...
    if (!m_cfg.externalDigitization){
      ...
      ACTS_INFO("Flag 'external digitization' set to true - discarding traccc digitization data and using external measurements, spacepoints, and seeds");
