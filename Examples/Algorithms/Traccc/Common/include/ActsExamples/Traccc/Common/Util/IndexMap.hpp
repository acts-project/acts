...
/// Each item in the vector 'from̈́' must be equal to at least 1 item in the vector 'to'.
/// Furthermore, if the flag 'bijection' is enabled each element in ̈́'to' and 'from' can only be paired with exactly 1 element.
/// @note When 'bijection' is corresponds to the assumption that the two vectors contain exactly the same elements. 
/// The ordering in the vectors may however be different (determining the difference in the order is where this function can be helpful).
...
inline auto matchMap(const std::vector<T1, A1>& from, const std::vector<T1, A2>& to, const std::function<T2(const T1&)>& lshFn, const std::function<bool(const T1&, const T1&)>& eqFn, const bool bijection = true){
    // By composition of functions, we can combine the maps "index ('from' vector) -> hash value" and "hash value -> index ('to' vector)"
    // to obtain "index ('from' vector) -> index ('to' vector)".
