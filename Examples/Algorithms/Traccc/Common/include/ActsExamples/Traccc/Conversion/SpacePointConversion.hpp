...
template <typename T>
SimSpacePoint convertSpacePoint(traccc::spacepoint& spacePoint,
                                T& measurementConv) {
  using Scalar = Acts::ActsScalar;
  const Acts::Vector3 globalPos(spacePoint.x(), spacePoint.y(), spacePoint.z());
  const std::optional<Scalar> t = std::nullopt;
  const Scalar varRho = 0;
  const Scalar varZ = 0;
  const std::optional<Scalar> varT = std::nullopt;
  const Acts::SourceLink sourceLink =
      measurementConv.valueToValue(spacePoint.meas).sourceLink();

  boost::container::static_vector<Acts::SourceLink, 2> sourceLinks = {
      sourceLink};

  return SimSpacePoint(globalPos, t, varRho, varZ, varT,
...
auto convertSpacePoints(
    std::vector<traccc::spacepoint, allocator_t>& spacePoints,
    T& measurementConv, output_container_t& outputContainer) {
  auto fn = [&measurementConv](traccc::spacepoint& spacePoint) {
    return convertSpacePoint(spacePoint, measurementConv);
  };
  return Util::convert<TracccSpacePointHash, std::equal_to<traccc::spacepoint>>(
      spacePoints, fn, outputContainer);
}
...
template <typename T>
traccc::spacepoint convertSpacePoint(const SimSpacePoint& spacePoint,
                                     T& measurementConv) {
  auto idx =
      spacePoint.sourceLinks()[0].get<ActsExamples::IndexSourceLink>().index();

  traccc::point3 global{spacePoint.x(), spacePoint.y(), spacePoint.z()};
  return traccc::spacepoint{global, measurementConv.indexToValue(idx)};
}
...
auto convertSpacePoints(const SimSpacePointContainer& spacePoints,
                        T& measurementConv,
                        output_container_t& outputContainer) {
  auto fn = [&measurementConv](const SimSpacePoint& spacePoint) {
    return convertSpacePoint(spacePoint, measurementConv);
  };
  return Util::convert<ActsSpacePointHash, std::equal_to<SimSpacePoint>>(
      spacePoints, fn, outputContainer);
}
