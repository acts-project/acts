...
struct TracccSeedEquals {
  bool operator()(const traccc::seed& lhs, const traccc::seed& rhs) const {
    return lhs.spB_link == rhs.spB_link && lhs.spM_link == rhs.spM_link &&
           lhs.spT_link == rhs.spT_link && lhs.weight == rhs.weight &&
...
struct ActsSeedHash {
  std::size_t operator()(const SimSeed& s) const noexcept {
    ...
    std::ranges::transform(s.sp(), std::back_inserter(indices), getIdx);
...
struct ActsSeedEquals {
  bool operator()(const SimSeed& lhs, const SimSeed& rhs) const {
    ...
    std::ranges::transform(lhs.sp(), std::back_inserter(lhsIndices), getIdx);
    std::vector<std::size_t> rhsIndices;
    std::ranges::transform(rhs.sp(), std::back_inserter(rhsIndices), getIdx);
    return lhsIndices[0] == rhsIndices[0] && lhsIndices[1] == rhsIndices[1] &&
           lhsIndices[2] == rhsIndices[2];
  }
};
...
SimSeed convertSeed(const traccc::seed& seed, T& spacePointConv) {
  return SimSeed(spacePointConv.indexToValue(seed.spB_link),
...
auto convertSeeds(std::vector<traccc::seed, allocator_t>& seeds,
                  T& spacePointConv, output_container_t& outputContainer) {
  auto fn = [&spacePointConv](auto& seed) {
    return convertSeed(seed, spacePointConv);
  };
  return Util::convert<TracccSeedHash, TracccSeedEquals>(seeds, fn,
                                                         outputContainer);
}
...
traccc::seed convertSeed(const SimSeed& seed, T& spacePointConv) {
  ...
  return traccc::seed{spacePointConv.valueToIndex(*seed.sp()[0]),
                      spacePointConv.valueToIndex(*seed.sp()[1]),
                      spacePointConv.valueToIndex(*seed.sp()[2]),
                      static_cast<Scalar>(seed.z()), seed.seedQuality()};
}
...
auto convertSeeds(const SimSeedContainer& seeds, T& spacePointConv,
                  output_container_t& outputContainer) {
  auto fn = [&spacePointConv](const SimSeed& seed) {
    return convertSeed(seed, spacePointConv);
  };
  return Util::convert<ActsSeedHash, ActsSeedEquals>(seeds, fn,
                                                     outputContainer);
}
