...
struct TracccChain::Impl {
  ...
  struct HostMemory : public BaseMemory {
    ...
    HostMemory() : host_mr(), host_copy() { copy = &host_copy; mr = &host_mr; }
  };
#if defined(ACTS_ENABLE_CUDA)
  struct CudaMemory : public BaseMemory {
    ...
    CudaMemory() : cuda_host_mr(), device_mr(), stream(), cuda_copy(stream.cudaStream()) {
      copy = &cuda_copy;
  ...
  std::unique_ptr<traccc::detector_design_description::buffer>     m_device_det_descr;
  std::unique_ptr<traccc::detector_conditions_description::buffer> m_device_det_cond;
#endif

  // ---- magnetic field ----
  traccc::magnetic_field m_host_field;
  traccc::vector3        m_field_vec;  // uniform approximation for CPU params estimation
#if defined(ACTS_ENABLE_CUDA)
  ...
  std::unique_ptr<traccc::host::silicon_pixel_spacepoint_formation_algorithm> m_host_spacepoint_formation;
  ...
  std::unique_ptr<traccc::host::combinatorial_kalman_filter_algorithm>        m_host_finding;
  std::unique_ptr<traccc::host::greedy_ambiguity_resolution_algorithm>        m_host_ambiguity_resolution;
  ...
  // ---- algorithm objects (device / CUDA) ----
    ...
    std::unique_ptr<traccc::device::triplet_seeding_algorithm>
        m_device_seeding;
//   std::unique_ptr<traccc::algorithm<traccc::edm::measurement_collection::host(
//       const traccc::edm::silicon_cell_collection::const_view&,
//       const traccc::detector_design_description::const_view&,
//       const traccc::detector_conditions_description::const_view&)>>
//       m_device_clusterization;
//   std::unique_ptr<traccc::algorithm<traccc::edm::spacepoint_collection::host(
//       const traccc::host_detector&,
//       const traccc::edm::measurement_collection::const_view&)>>
//       m_device_spacepoint_formation;
//   std::unique_ptr<traccc::algorithm<traccc::edm::seed_collection::buffer(
//       const traccc::edm::spacepoint_collection::const_view&,
//       const traccc::edm::measurement_collection::const_view&)>>
//       m_device_seeding;
  std::unique_ptr<traccc::algorithm<traccc::bound_track_parameters_collection_types::buffer(
  ...
  explicit Impl(const TracccChainConfig& cfg, Acts::Logging::Level logLevel)
      : m_logger(Acts::getDefaultLogger("TracccChain", logLevel)),
        m_cfg(cfg)
  {
    if (cfg.backend == TracccChainConfig::Backend::CUDA) {
#if !defined(ACTS_ENABLE_CUDA)
      throw std::runtime_error(
          "TracccChainConfig requests CUDA backend but ACTS_ENABLE_CUDA is not set");
#else
      ...
      m_det_descr = std::make_unique<traccc::detector_design_description::host>(cuda_mem->cuda_host_mr);
      m_det_cond  = std::make_unique<traccc::detector_conditions_description::host>(cuda_mem->cuda_host_mr);

      m_device_clusterization = std::make_unique<traccc::cuda::clusterization_algorithm>(
          cuda_mem->mr_device, cuda_mem->cuda_copy, cuda_mem->stream, cfg.clusteringConfig,
          Acts::getDefaultLogger("TracccClusterization", logLevel));
      m_device_spacepoint_formation = std::make_unique<traccc::cuda::silicon_pixel_spacepoint_formation_algorithm>(
          cuda_mem->mr_device, cuda_mem->cuda_copy, cuda_mem->stream,
          Acts::getDefaultLogger("TracccSpFormation", logLevel));
      m_device_seeding = std::make_unique<traccc::cuda::triplet_seeding_algorithm>(
      ...
      m_device_params_est = std::make_unique<traccc::cuda::seed_parameter_estimation_algorithm>(
          cfg.trackParamsEstConfig, cuda_mem->mr_device, cuda_mem->cuda_copy, cuda_mem->stream,
          Acts::getDefaultLogger("TracccParamsEst", logLevel));
      m_device_finding = std::make_unique<traccc::cuda::combinatorial_kalman_filter_algorithm>(
          cfg.findingConfig, cuda_mem->mr_device, cuda_mem->cuda_copy, cuda_mem->stream,
          Acts::getDefaultLogger("TracccFinding", logLevel));
      m_device_fitting = std::make_unique<traccc::cuda::kalman_fitting_algorithm>(
          cfg.fittingConfig, cuda_mem->mr_device, cuda_mem->cuda_copy, cuda_mem->stream,
      ...
      m_det_descr = std::make_unique<traccc::detector_design_description::host>(host_mem->host_mr);
      m_det_cond  = std::make_unique<traccc::detector_conditions_description::host>(host_mem->host_mr);

      m_host_clusterization = std::make_unique<traccc::host::clusterization_algorithm>(
          host_mem->host_mr, Acts::getDefaultLogger("TracccClusterization", logLevel));
      m_host_spacepoint_formation = std::make_unique<traccc::host::silicon_pixel_spacepoint_formation_algorithm>(
          host_mem->host_mr, Acts::getDefaultLogger("TracccSpFormation", logLevel));
      m_host_seeding = std::make_unique<traccc::host::seeding_algorithm>(
          ...
          host_mem->host_mr,
          Acts::getDefaultLogger("TracccSeeding", logLevel));
      m_host_params_est = std::make_unique<traccc::host::track_params_estimation>(
          cfg.trackParamsEstConfig, host_mem->host_mr,
          Acts::getDefaultLogger("TracccParamsEst", logLevel));
      m_host_finding = std::make_unique<traccc::host::combinatorial_kalman_filter_algorithm>(
          cfg.findingConfig, host_mem->host_mr,
          Acts::getDefaultLogger("TracccFinding", logLevel));
      m_host_ambiguity_resolution = std::make_unique<traccc::host::greedy_ambiguity_resolution_algorithm>(
    ...
    if (!cfg.digitizationFile.empty()) {
      traccc::io::read_detector_description(
          *m_det_descr, *m_det_cond,
          cfg.detectorFile, cfg.digitizationFile, cfg.conditionsFile,
          traccc::data_format::json);
    }

    // Load detector geometry — requires a host memory resource
    traccc::io::read_detector(m_host_detector, *m_memory->mr,
                              cfg.detectorFile, cfg.materialFile,
                              cfg.gridFile);
    ...
    if (cfg.backend == TracccChainConfig::Backend::CUDA) {
      ...
      for (std::size_t i = 0; i < m_det_descr->size(); ++i) {
        ...
        descr_sizes[i] = std::max(
            static_cast<unsigned int>(design.bin_edges_x().size()),
            static_cast<unsigned int>(design.bin_edges_y().size()));
      }
      m_device_det_descr = std::make_unique<traccc::detector_design_description::buffer>(
          descr_sizes, cuda_mem->device_mr, &cuda_mem->cuda_host_mr,
          vecmem::data::buffer_type::resizable);

      cuda_mem->copy->setup(*m_device_det_descr)->wait();
      cuda_mem->copy->operator()(vecmem::get_data(*m_det_descr), *m_device_det_descr)->wait();

      m_device_det_cond = std::make_unique<traccc::detector_conditions_description::buffer>(
          static_cast<traccc::detector_conditions_description::buffer::size_type>(
              m_det_cond->size()),
          cuda_mem->device_mr);
      cuda_mem->copy->setup(*m_device_det_cond)->wait();
      cuda_mem->copy->operator()(vecmem::get_data(*m_det_cond), *m_device_det_cond)->wait();
  ...
  EventResult run(traccc::edm::measurement_collection::host& meas_host,
    ...
    ACTS_DEBUG("Running traccc chain with " << meas_host.size()
               << " measurements and " << sp_host.size() << " spacepoints.");
    ...
    if (m_cfg.backend != TracccChainConfig::Backend::CPU) {
      ...
      auto params_buf = (*m_device_params_est)(m_device_field,meas_buf, sp_buf,seeds_buf);

      auto track_candidates_buf = (*m_device_finding)(
          *m_device_detector, m_device_field,meas_buf, params_buf);
      result.n_track_candidates = cuda_mem->copy->get_size(track_candidates_buf.tracks);

      // No device ambiguity resolution yet — pass candidates directly to fitter if needed
      // Fitting happened through MBF track finding
    ...
    //   traccc::edm::track_container<traccc::default_algebra>::host found_tracks_host;
    //   cuda_mem->copy->operator()(track_candidates_buf.tracks, found_tracks_host.tracks,
    //                              vecmem::copy::type::device_to_host)->wait();
    //   cuda_mem->copy->operator()(track_candidates_buf.states, found_tracks_host.states,
    //                              vecmem::copy::type::device_to_host)->wait();
    //   cuda_mem->stream.synchronize();

      result.n_fitted_tracks = cuda_mem->copy->get_size(track_candidates_buf.tracks);
#endif
    } else {
      // CPU execution
      ...
      auto params_host = (*m_host_params_est)(
          vecmem::get_data(meas_host),
          vecmem::get_data(sp_host),
          vecmem::get_data(seeds_host),
          m_field_vec);

      auto track_candidates_host = (*m_host_finding)(
          m_host_detector, m_host_field,
          vecmem::get_data(meas_host), vecmem::get_data(params_host));
