 ...
 *         external global position. It serves  the `SensitiveSurfaceMapper` to map the Acts::Surfaces 
 *         to the G4PhysicalVolumes. Based on the global position of the G4PhyicalVolume or on the position of 
 *         the volume bound vertices, the tracking geometry is queried to provide the candidate sensitive surfaces. 
 *         Due to the concurrent TrackingGeometry schemes currently in use, concrete implementations need to follow */
 ...
 *         The Geant 4 geometry tree is traversed and logical volumes where either its name or the name
 *         of the volume's material satisfies the user-configured tokens, the name of the associated
 *         physical volume is prepended with the `mappingPrefix` to mark the volume as sensitive for 
 *         the `SensitiveSteppingAction`. In the next step, the global position of the volume or of the
 *         boundary vertices is used to query the tracking geometry for candidate Acts::Surfaces. In case 
 *         of multiple candidates, the closest one is choosen. The surface's geometry identifier is later used
 *         to associate the recorded G4 hits to the surfaces. 
 * 
 *         The constructed pair of G4PhysicalVolume* and Surface* is then inserted into the connection map. */
class SensitiveSurfaceMapper {
 public:
  ...
  struct State {
    ...
    using VolumeToSurfAssocMap_t = std::multimap<const G4VPhysicalVolume*, const Acts::Surface*>;
