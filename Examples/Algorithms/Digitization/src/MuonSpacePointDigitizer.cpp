...
void MuonSpacePointDigitizer::visualizeBucket(
  ...
  RangeXD<2, double> canvasBound{
      filledArray<double, 2>(std::numeric_limits<double>::max()),
      filledArray<double, 2>(-std::numeric_limits<double>::max())};

  constexpr double extra = 2._cm;
  for (const auto& sp : bucket) {
    canvasBound.expand(0, sp.localPosition().z() - extra,
                       sp.localPosition().z() + extra);
    canvasBound.expand(1, sp.localPosition().y() - extra,
                       sp.localPosition().y() + extra);
  }
  ...
  chambVolume->apply(overloaded{
      [this, &canvasBound, &gctx, &primitives](const Surface& surface) {
        ...
        const auto toSpTrf = toSpacePointFrame(gctx, surface.geometryId());
        const double y = toSpTrf.translation().y();
        const double z = toSpTrf.translation().z();
        const auto& bounds = surface.bounds();
        switch (surface.type()) {
          using enum Surface::SurfaceType;
          case Plane: {
            if (bounds.type() == SurfaceBounds::BoundsType::eRectangle) {
              const double hL = static_cast<const RectangleBounds&>(bounds).get(
                  RectangleBounds::eMaxY);
              const double minZ = std::max(z - hL, canvasBound.min(0) + extra);
              const double maxZ = std::min(z + hL, canvasBound.max(0) - extra);
              /// The maximum is below the left side of the strip plane
              /// or the minimum is above the right side of the plane
              if (maxZ < z - hL || minZ > z + hL) {
                return;
              }
              if (y < canvasBound.min(1) + extra ||
                  y > canvasBound.max(1) - extra) {
                return;
              }
              primitives.push_back(drawBox(0.5 * (minZ + maxZ), y, maxZ - minZ,
                                           0.3_cm, kBlack, 0));
            } else {
              return;
            }
          ...
          case Straw: {
            ...
            /// check whether the circle is fully on the pad
            for (const auto sY : {-1, 1}) {
              for (const auto sZ : {-1, 1}) {
                if (!canvasBound.contains(std::array{z + sZ * r, y + sY * r})) {
                  return;
                }
              }
            }
            primitives.push_back(drawCircle(z, y, r, kBlack, 0));
            break;
          }
          default:
            return;
  ...
  for (auto& sp : bucket) {
    ...
    if (sp.isStraw()) {
      primitives.push_back(drawCircle(z, y, r, kRed, 0));
    } else {
      primitives.push_back(drawBox(z, y, 3._cm, 0.5_cm, kRed, 1001));
    }
  }
  ...
  for (const auto& simHit : gotSimHits) {
    ...
    if ((*simPartItr).hypothesis() != ParticleHypothesis::muon()) {
      continue;
    }
