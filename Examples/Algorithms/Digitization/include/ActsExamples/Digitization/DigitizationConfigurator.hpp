...
struct DigitizationConfigurator {
  ...
  std::vector<
      std::pair<Acts::GeometryIdentifier, std::vector<DigiComponentsConfig>>>
  ...
  void operator()(const Acts::Surface* surface) {
    if (surface->associatedDetectorElement() != nullptr) {
      ...
      auto dInputConfigVector = inputDigiComponents.find((geoId));
      if (dInputConfigVector != inputDigiComponents.end()) {
        for (const auto& dInputConfig : *dInputConfigVector) {
          // Check if the constraint applies
          if (not dInputConfig.constraint(*surface, Acts::GeometryContext{})) {
            continue;
          }

          // The output config, copy over the smearing part and the constraint
          // part
            ...
            switch (sBounds.type()) {
              ...
              case Acts::SurfaceBounds::eRectangle: {
                if (inputSegmentation.binningData()[0].binvalue == Acts::binX) {
                  ...
                  outputSegmentation += Acts::BinUtility(
                      nBins, minX, maxX, Acts::open, Acts::binX);
                }
                if (inputSegmentation.binningData()[0].binvalue == Acts::binY or
                  ...
                  outputSegmentation += Acts::BinUtility(
                      nBins, minY, maxY, Acts::open, Acts::binY);
                }
              } break;
              ...
              case Acts::SurfaceBounds::eTrapezoid: {
                if (inputSegmentation.binningData()[0].binvalue == Acts::binX) {
                  ...
                  outputSegmentation += Acts::BinUtility(
                      nBins, -maxX, maxX, Acts::open, Acts::binX);
                }
                if (inputSegmentation.binningData()[0].binvalue == Acts::binY or
                    inputSegmentation.dimensions() == 2) {
                  unsigned int accessBin =
                  ...
                  unsigned int nBins = std::round(
                      ...
                      inputSegmentation.binningData()[accessBin].step);
                  outputSegmentation += Acts::BinUtility(
                      nBins, -maxY, maxY, Acts::open, Acts::binY);
                }
              } break;
              ...
              case Acts::SurfaceBounds::eAnnulus: {
                if (inputSegmentation.binningData()[0].binvalue == Acts::binR) {
                  Acts::ActsScalar minR =
                      boundValues[Acts::AnnulusBounds::eMinR];
                  Acts::ActsScalar maxR =
                      boundValues[Acts::AnnulusBounds::eMaxR];
                  unsigned int nBins = std::round(
                      (maxR - minR) / inputSegmentation.binningData()[0].step);
                  outputSegmentation += Acts::BinUtility(
                      nBins, minR, maxR, Acts::open, Acts::binR);
                }
              ...
              case Acts::SurfaceBounds::eDisc: {
                if (inputSegmentation.binningData()[0].binvalue == Acts::binR) {
                  Acts::ActsScalar minR =
                      boundValues[Acts::RadialBounds::eMinR];
                  Acts::ActsScalar maxR =
                      boundValues[Acts::RadialBounds::eMaxR];
                  unsigned int nBins = std::round(
                      (maxR - minR) / inputSegmentation.binningData()[0].step);
                  outputSegmentation += Acts::BinUtility(
                      nBins, minR, maxR, Acts::open, Acts::binR);
          ...
          if (compactify) {
            ...
            // Check for a representing layer configuration, insert if not
            // present
          ...
          // If one constraint was valid, break the for-loop to prevent
          // duplicates in the case of misconfiguration
          break;
        }
      }
    }
