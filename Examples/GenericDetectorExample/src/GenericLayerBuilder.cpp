  ...
  m_pLayers()
{
    ...
    for (size_t idisc = 0; idisc < m_config.posnegLayerPositionsZ.size(); ++idisc){
        ...
        discPhiPositions.reserve(m_config.posnegModuleRadii.at(idisc).size());
        // that's for the module material
        std::vector < std::vector< double > > discModuleMaterial;
        discModuleMaterial.reserve(m_config.posnegModuleRadii.at(idisc).size());
        size_t iphistream = 0;
        size_t imstream   = 0;
        for (size_t iring = 0; iring < m_config.posnegModuleRadii.at(idisc).size(); ++iring){
            // resolve the phi positions
            std::vector< double > ringPhiPositions;
            size_t nphiModules = size_t(m_config.posnegModuleInPhi.at(idisc).at(iring));
            for (size_t iphi = 0; iphi < nphiModules; ++iphi, ++iphistream){
                //ringPhiPositions.push_back(m_config.posnegModulePositionPhiStream.at(idisc).at(iphistream));
                ringPhiPositions.push_back(m_config.posnegModulePositionPhi.at(idisc).at(iring).at(iphi));
            }
            ...
            if (m_config.posnegModuleMaterialStream.size()){
                ...
                for (size_t im = 0; im < 5; ++im, ++imstream)
                    ringModuleMaterial.push_back(m_config.posnegModuleMaterialStream.at(idisc).at(imstream));
...
void Acts::GenericLayerBuilder::constructLayers()
{
   ...
   if (numcLayers){
       ...
       for (size_t icl = 0; icl < numcLayers; ++icl){
           // layer R/Z
           double layerR      = m_config.centralLayerRadii.at(icl);
           // some screen output
           //MSG_VERBOSE("Build layer " << icl << " with target radius = " << layerR);
           // surface vector 
           std::vector<const Surface*> sVector;
           // z/phi values for this layer
           std::vector<double> zValues   = m_config.centralModulePositionZ.at(icl);
           std::vector<double> phiValues = m_config.centralModulePositionPhi.at(icl);
           std::sort(phiValues.begin(),phiValues.end());
           std::sort(zValues.begin(),zValues.end());
           // envelope stagger & cover
           double layerModuleStaggerZ = m_config.centralModuleStaggerZ.size() ? m_config.centralModuleStaggerZ.at(icl) : 0.;
           double layerEnvelopeCoverZ = m_config.centralLayerEnvelopeZ.size() ? m_config.centralLayerEnvelopeZ.at(icl) : 0.;
           // module size & tilt
           double modulePhiTilt   = m_config.centralModuleTiltPhi.at(icl);
           double moduleHalfX     = m_config.centralModuleHalfX.at(icl);
           double moduleHalfY     = m_config.centralModuleHalfY.at(icl);
           double moduleThickness = m_config.centralModuleThickness.at(icl);
           ...
           if (m_config.centralModuleMaterial.size()){
               ...
               double x0  = m_config.centralModuleMaterial.at(icl).at(0);
               double l0  = m_config.centralModuleMaterial.at(icl).at(1);
               double a   = m_config.centralModuleMaterial.at(icl).at(2);
               double z   = m_config.centralModuleMaterial.at(icl).at(3);
               double rho = m_config.centralModuleMaterial.at(icl).at(4);
           ...
           for (auto& moduleZ : zValues){
               ...
               for (auto& modulePhi : phiValues){
                   ...
                   if (m_config.centralModuleFrontsideStereo.size() && m_config.centralModuleFrontsideStereo.at(icl) != 0.){
                       // twist by the stereo angle
                       double stereo = m_config.centralModuleFrontsideStereo.at(icl);
                   ...
                   if (m_config.centralModuleBacksideGap.size()){
                       ...
                       moduleCenter = moduleCenter + m_config.centralModuleBacksideGap.at(icl)*moduleLocalZ;
                       ...
                       if (m_config.centralModuleBacksideStereo.size()){
                           // twist by the stereo angle
                           double stereoBackSide = m_config.centralModuleBacksideStereo.at(icl);
           ...
           if (m_config.centralLayerMaterialProperties.size()){
               ...
               double lMaterialThickness = m_config.centralLayerMaterialProperties.at(icl).at(0);
               double lMaterialX0        = m_config.centralLayerMaterialProperties.at(icl).at(1);
               double lMaterialL0        = m_config.centralLayerMaterialProperties.at(icl).at(2);
               double lMaterialA         = m_config.centralLayerMaterialProperties.at(icl).at(3);
               double lMaterialZ         = m_config.centralLayerMaterialProperties.at(icl).at(4);
               double lMaterialRho       = m_config.centralLayerMaterialProperties.at(icl).at(5);
               ...
               if (m_config.centralLayerMaterialConcentration.at(icl) == 0.){
                   ...
                   if (m_config.centralLayerMaterialConcentration.at(icl) > 0){
                       ...
                       //MSG_VERBOSE("- and material at outer approach surfaceat radius =  " << approachSurfaces.at(1)->bounds().r() );
                   }
                   else {
                       approachSurfaces.at(0)->setSurfaceMaterial(layerMaterialPtr);
                       //MSG_VERBOSE("- and material at inner approach surface at radius =  " << approachSurfaces.at(0)->bounds().r() );
                   }
               }
           }
    ...
    if (numpnLayers){
      ...
      for (size_t ipnl = 0; ipnl < numpnLayers; ++ipnl){
         // some screen output
         //MSG_VERBOSE("- build layers " << (2*ipnl) << " and "<<  (2*ipnl)+1 << " at +/- z = " << m_posnegLayerPositionsZ.at(ipnl));
         // layer position update
         double layerPosZ                          = m_config.posnegLayerPositionsZ.at(ipnl);
         double layerEnvelopeR                     = m_config.posnegLayerEnvelopeR.at(ipnl);
         double layerStaggerR                      = m_config.posnegModuleStaggerR.at(ipnl);
         size_t layerBinsR                         = 0;
         ...
         auto layerModuleHalfY                     = m_config.posnegModuleHalfY.at(ipnl);
         auto layerModuleThickness                 = m_config.posnegModuleThickness.at(ipnl);
         ...
         for (size_t ipnR = 0; ipnR < layerModuleRadii.size(); ++ipnR){
             ...
             // and the bounds
             double moduleMinHalfX  = layerModuleMinHalfX.size() ? layerModuleMinHalfX.at(ipnR) : 0.;
             ...
             if (m_config.posnegModuleMaterial.size()){
                 // get the sensor material - it has to be vectors of 5
                 double x0  = m_config.posnegModuleMaterial.at(ipnl).at(ipnR).at(0);
                 double l0  = m_config.posnegModuleMaterial.at(ipnl).at(ipnR).at(1);
                 double a   = m_config.posnegModuleMaterial.at(ipnl).at(ipnR).at(2);
                 double z   = m_config.posnegModuleMaterial.at(ipnl).at(ipnR).at(3);
                 double rho = m_config.posnegModuleMaterial.at(ipnl).at(ipnR).at(4);
                 // the moduel moaterial from input 
                 Material moduleMaterial(x0,l0,a,z,rho);
                 MaterialProperties moduleMaterialProperties(moduleMaterial,moduleThickness);
             ...
             for (auto& modulePhi : ringModulePositionsPhi){
                 ...
                 if (m_config.posnegModuleBacksideGap.size()){
                     ...
                     // the new centers
                     nModuleCenter = nModuleCenter + m_config.posnegModuleBacksideGap.at(ipnl).at(ipnR)*nModuleLocalZ;
                     pModuleCenter = pModuleCenter + m_config.posnegModuleBacksideGap.at(ipnl).at(ipnR)*pModuleLocalZ;
                     // the new transforms
                     ...
                     if (m_config.posnegModuleBacksideStereo.size()){
                         // twist by the stereo angle
                         double stereoBackSide = m_config.posnegModuleBacksideStereo.at(ipnl).at(ipnR);
         ...
         if (m_config.posnegLayerMaterialProperties.size()){
             // get the material from configuration
             double lMaterialThickness = m_config.posnegLayerMaterialProperties.at(ipnl).at(0);
             double lMaterialX0        = m_config.posnegLayerMaterialProperties.at(ipnl).at(1);
             double lMaterialL0        = m_config.posnegLayerMaterialProperties.at(ipnl).at(2);
             double lMaterialA         = m_config.posnegLayerMaterialProperties.at(ipnl).at(3);
             double lMaterialZ         = m_config.posnegLayerMaterialProperties.at(ipnl).at(4);
             double lMaterialRho       = m_config.posnegLayerMaterialProperties.at(ipnl).at(5);
             MaterialProperties layerMaterialProperties(lMaterialThickness,lMaterialX0,lMaterialL0,lMaterialA,lMaterialZ,lMaterialRho);
             std::shared_ptr<const SurfaceMaterial> layerMaterialPtr(new HomogeneousSurfaceMaterial(layerMaterialProperties));
             // central material
