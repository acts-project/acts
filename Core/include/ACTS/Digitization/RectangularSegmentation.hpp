...
namespace Acts {
  ...
  class RectangularSegmentation : public Segmentation {
        public:
            ...
            void createSegmenationSurfaces(SurfacePtrVector& boundarySurfaces,
            ...
            const Vector2D cellPosition(const DigitizationCell& cId) const override;
    ...
    RectangularSegmentation::moduleBounds() const { return (*(m_activeBounds.get())); }
    
    inline size_t 
    RectangularSegmentation::numCellsX() const { return m_binsX; }

    inline size_t 
    RectangularSegmentation::numCellsY() const { return m_binsY; }
    
    template <class T> const DigitizationCell 
    ...
    RectangularSegmentation::cell(const Vector3D& position) const { return cellT<Vector3D>(position); }

    inline const DigitizationCell 
    RectangularSegmentation::cell(const Vector2D& position) const { return cellT<Vector2D>(position); }
