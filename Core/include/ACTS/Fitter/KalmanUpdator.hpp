...
namespace Acts
{
  ...
  public:
    ...
    return_type operator()(const Meas_t& m,const BoundParameters& pars) const
    {
  ...
  private:
    ...
    public:
      ...
      void setParameters(const BoundParameters& pars)
      {
        m_pParameters = &pars;
      }

      template<typename Meas_t>
      return_type operator()(const Meas_t& m) const
      {
        static const ActsSymMatrixD<Acts::NGlobalPars> unit = ActsSymMatrixD<Acts::NGlobalPars>::Identity();
        ...
        ActsMatrixD<Acts::NGlobalPars,Meas_t::size()> K = (*pCov_trk) * H.transpose() * (H * (*pCov_trk) * H.transpose() + m.covariance()).inverse();

        BoundParameters::ParVector_t newParValues = m_pParameters->parameters() + K * m.residual(*m_pParameters);
        BoundParameters::CovMatrix_t newCov = (unit - K * H) * (*pCov_trk);

        return std::make_unique<BoundParameters>(std::make_unique<BoundParameters::CovMatrix_t>(std::move(newCov)),newParValues,m_pParameters->associatedSurface());
      }
