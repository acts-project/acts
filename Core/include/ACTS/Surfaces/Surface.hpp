...
class Surface : public virtual GeometryObject {
 public:
   ...
   * memory allocation */
  virtual void localToGlobal(const Vector2D& locp, const Vector3D& mom,
  ...
  virtual bool globalToLocal(const Vector3D& glob, const Vector3D& mom,
  ...
  /** Return the measurement frame - this is needed for alignment, in particular
     ...
     */
  virtual const Acts::RotationMatrix3D measurementFrame(
      const Vector3D& glopos, const Vector3D& glomom) const;

  /** fst straight line intersection schema - templated for cvharged and neutral
   * parameters */
  template <class T>
  Intersection intersectionEstimate(const T& pars, bool forceDir = false,
                                    Acts::BoundaryCheck bchk = false) const {
    return intersectionEstimate(pars.position(), pars.momentum().unit(),
                                forceDir, bchk);
  }
  ...
  virtual Intersection intersectionEstimate(
      const Vector3D& pos, const Vector3D& dir, bool forceDir = false,
...
inline double Surface::pathCorrection(const Vector3D&,
...
template <class T>
bool Surface::onSurface(const T& pars, const BoundaryCheck& bcheck) const {
...
inline void Surface::setSurfaceMaterial(
    std::shared_ptr<const SurfaceMaterial> material) const {
