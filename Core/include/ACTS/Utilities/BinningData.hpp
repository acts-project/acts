  ...
  searchEquidistantWithBoundary(float value, const BinningData& bData)
  {
    ...
    */
    ...
    if (bData.option == open) {
      ...
      double w = bData.max - bData.min;
      value = bData.min + std::fmod(w + std::fmod(value - bData.min, w), w);
      //value = min + wrapMax(x - min, max - min);
    }
    int bin = ((value - bData.min) / bData.step);
    */
    ...
    if (bData.option == open) {
      ...
      if (bData.binvalue == binPhi) {
        ...
        // this assumed that we cannot ever have phi lookup values
        // that are more than 2pi away from boundary. should be fine
        if (value > bData.max) value -= twopi;
        else if (value < bData.min) value += twopi;

        //if (value > bData.max) {
          //if (value - bData.max < twopi) {
            //value -= twopi;
          //} else {
            //value -= std::ceil((value - bData.max) * itwopi) * twopi;
          //}
        //} else if (value < bData.min) {
          //if (bData.max - value < twopi) {
            //value += twopi;
          //} else {
            //value += std::ceil((bData.min - value) * itwopi) * twopi;
          //}
        //}
      } else {
        // generic but slower
