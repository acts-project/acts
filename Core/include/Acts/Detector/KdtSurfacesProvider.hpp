...
class KdtSurfaces {
 public:
  ...
  using KDTS =
      KDTree<kDIM, std::shared_ptr<Surface>, long double, std::array, bSize>;

  /// Broadcast the query definition
  using Query = std::array<long double, kDIM>;
  ...
  std::vector<std::shared_ptr<Surface>> surfaces(
      const RangeXD<kDIM, long double>& range) const {
    // Strip the surfaces
  ...
  std::vector<std::shared_ptr<Surface>> surfaces(const Extent& extent) const {
    RangeXD<kDIM, long double> qRange;
