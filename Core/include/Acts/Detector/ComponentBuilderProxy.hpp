...
class ComponentBuilderProxy {
 protected:
  /// Creatinga ComponentBuilderProxy is not allowed
 ...
 public:
  ...
  /// The proxy name - this needs to be uniuqueå
  std::string name = "";

  /// The Parent proxy, optionally set
  const ComponentBuilderProxy* parent = nullptr;
  ...
  ///
  /// @return The created Root proxy as reference
  static std::unique_ptr<ComponentBuilderProxy> createRootProxy(
    ...
    auto proxy = std::make_unique<ComponentBuilderProxy>(n);
    proxy->transform = t;
    proxy->boundsType = bt;
  ...
  void addChildVolumeProxy(
    ...
    if (std::holds_alternative<ContainerProxy>(this->holder)) {
      std::get<ContainerProxy>(this->holder)
          .children.push_back(std::move(proxy));
    } else {
      throw std::runtime_error(
