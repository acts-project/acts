...
class GenericBoundTrackParameters {
 public:
  ...
  /// Factory to construct from four-position, direction, and qOverP
  ///
  /// @param surface Reference surface the parameters are defined on
  ...
  static Result<GenericBoundTrackParameters<ParticleHypothesis>> create(
  ...
  static GenericBoundTrackParameters create(
    ...
    return GenericBoundTrackParameters(
        ...
        other.particleHypothesis(), other.covariance());
  }
  ...
  /// Parameters vector.
  constexpr ParametersVector& parameters() { return m_params; }
  /// Parameters vector.
  constexpr const ParametersVector& parameters() const { return m_params; }
  /// Optional covariance matrix.
  constexpr std::optional<CovarianceMatrix>& covariance() { return m_cov; }
  /// Optional covariance matrix.
  constexpr const std::optional<CovarianceMatrix>& covariance() const {
    return m_cov;
  }
  ...
  /// Time coordinate.
  constexpr Scalar time() const { return m_params[eBoundTime]; }

  /// Charge over momentum
  constexpr Scalar phi() const { return m_params[eBoundPhi]; }
  /// Charge over momentum
  constexpr Scalar theta() const { return m_params[eBoundTheta]; }
  /// Charge over momentum
  constexpr Scalar qOverP() const { return m_params[eBoundQOverP]; }
 ...
 private:
  ...
  constexpr void normalizePhiTheta() {
    auto [phi, theta] =
  ...
  friend bool operator==(
      const GenericBoundTrackParameters<particle_hypothesis_t>& lhs,
