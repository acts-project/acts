...
/// Interface concept to define the straw measurement calibrator used by the
/// FastStrawLineFitter. The basic assumption is that the straw radii are
/// reconstructed from a time measurement which is converted into a drift-radius
/// using a differentiable r-t relation. Due to e.g. late arrival of the
/// particle, the drift time may be subject to corrections which is summarized
/// by a general time shift t0. This shift can be estimated during a straight
/// line fit. The calibrator returns the updated drift radius & the first and
/// second derivative of the r-t relation called driftVelocity and
/// driftAcceleration, respectively.
...
concept CompositeSpacePointFastCalibrator =
    ...
    requires(const Calibrator_t calibrator, const Acts::CalibrationContext& ctx,
      ...
      /// Returns the drift velocity of the straw measurement's radius - time
      /// relation which is defined as the first derivative of the relation.
      ...
      /// Returns the drift acceleration of the straw measurement's radius -
      /// time relation which is defined as the second derivative of the
      /// relation
      /// @param ctx: Calibration context to access the calibration constants (Experiment specific)
      /// @param spacePoint: Reference to the calibrated space point
      {
...
/// Interface concept for a CompositeSpacePointCalibrator. The space point
/// container is parsed to the interface together with a reference track
/// position, direction and the time offset, all expressed at the reference
/// plane z=0.
...
concept CompositeSpacePointCalibrator =
    ...
    requires(const Calibrator_t calibrator, const UnCalibCont_t& uncalibCont,
             ...
             const Acts::RemovePointer_t<typename CalibCont_t::value_type>&
                 measurement) {
      /// Calibrate the entire input space point container using the external
      /// track parameters
      /// @param ctx: Calibration context to access the calibration constants (Experiment specific)
      ...
      /// Update the signs of each straw measurement according to whether the
      /// measurement is on the left or on the right-hand side of the given line
      /// @param trackPos: Position of the track / segment
      /// @param trackDir: Direction of the track / segment
