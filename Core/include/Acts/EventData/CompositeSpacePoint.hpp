...
/// Concept definition of the station space points. They're primarily used in
/// composite detectors, like the Muon stations in side the ATLAS experiment.
/// The stations usually consist of few layers of straw tubes which maybe
/// sandwiched by strip detector layers. The straws are used to measure the
/// passage of the particle in the bending plane, while the strips may
/// supplement the track measurement by providing coordinates along the straw.
/// The CompositeSpacePoint assumes an orthogonal coordinate system, where
...
concept CompositeSpacePoint = requires(const SpacePointType sp) {
  /// Local position of the space point measurement. It's either the position of
  /// the wire or the position of the fired strip in the station
  ...
  /// Normal vector on the strip-plane spanned by `sensorDirection()` &
  /// `toNextSensor()`.
  { sp.planeNormal() } -> std::same_as<const Vector3&>;
  /// Radius of the straw-tube measurement. The returned value is zero for strip
  /// measurements
  { sp.driftRadius() } -> std::same_as<double>;
  /// Recorded time of the measurement, if provided by the technology
  { sp.time() } -> std::same_as<double>;
  /// Measurement covariance array. It's composed of the individual strip
  /// covariances, making up the composite space point and the covariance on
  /// time, if provided
  { sp.covariance() } -> std::same_as<const std::array<double, 3>&>;
  /// Return whether the space point represents a straw measurement
  { sp.isStraw() } -> std::same_as<bool>;
  /// Return whether the space point provides a direct time constraint
  { sp.hasTime() } -> std::same_as<bool>;
  /// Returns whether the station space point measures the 0-th coordinate and
  /// hence constrains the track parameters in the non-bending direction
  { sp.measuresLoc0() } -> std::same_as<bool>;
  /// Returns whether the station space point measures the 1-st coordinate and
  /// hence constrains the track parameters in the bending direction
  { sp.measuresLoc1() } -> std::same_as<bool>;
};

/// Define the space point pointer concept as an ordinary / smart pointer over
/// space points
...
/// Define the CompositeSpacePointContainer concept as any std::container over
/// a collection of `CompositeSpacePoint` pointers
