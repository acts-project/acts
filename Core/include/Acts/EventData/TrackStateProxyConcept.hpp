...
concept TrackStateProxyConcept =
  ...
  { cv.getMask() }
  ->std::same_as<TrackStatePropMask>;

  { cv.referenceSurface() }
  ->std::same_as<const Surface&>;

  { cv.hasReferenceSurface() }
  ->std::same_as<bool>;

  { cv.template has<hashString("blubb")>() }
  ->std::same_as<bool>;
  ...
  { cv.effectiveProjector() }
  ->std::same_as<detail::EffectiveProjector>;
  { v.effectiveProjector() }
  ->std::same_as<detail::EffectiveProjector>;

  { cv.projectorBitset() }
  ->std::same_as<ProjectorBitset>;
  { v.projectorBitset() }
  ->std::same_as<ProjectorBitset>;

  { cv.getUncalibratedSourceLink() }
  ->std::same_as<SourceLink>;
  { v.getUncalibratedSourceLink() }
  ->std::same_as<SourceLink>;
  ...
  { cv.template calibrated<2>() }
  ->std::same_as<detail::ConstMeasurement>;
  ...
  { cv.typeFlags() }
  ->std::same_as<ConstTrackStateType>;
};
...
concept ConstTrackStateProxyConcept =
  ...
  { v.template component<int>(key) }
  ->std::same_as<const int&>;

  { v.predicted() }
  ->std::same_as<detail::ConstParameters>;
  { v.predictedCovariance() }
  ->std::same_as<detail::ConstCovariance>;

  { v.filtered() }
  ->std::same_as<detail::ConstParameters>;
  { v.filteredCovariance() }
  ->std::same_as<detail::ConstCovariance>;

  { v.smoothed() }
  ->std::same_as<detail::ConstParameters>;
  { v.smoothedCovariance() }
  ->std::same_as<detail::ConstCovariance>;

  { v.jacobian() }
  ->std::same_as<detail::ConstCovariance>;

  { v.template calibrated<2>() }
  ->std::same_as<detail::ConstMeasurement>;
...
template <typename T>
concept MutableTrackStateProxyConcept = TrackStateProxyConcept<T>&& requires(
    T v, HashedString key, TrackStatePropMask mask, TrackIndexType index,
    std::shared_ptr<const Surface> surface,
    Eigen::Matrix<double, 3, 6> projector, ProjectorBitset projectorBitset,
    SourceLink sl, Acts::Measurement<BoundIndices, 2> meas,
    std::size_t measdim) {
  ...
  { v.template calibrated<2>() }
  ->std::same_as<detail::Measurement>;
  { v.template calibratedCovariance<2>() }
  ->std::same_as<detail::MeasurementCovariance>;

  { v.effectiveCalibrated() }
  ->std::same_as<detail::DynamicMeasurement>;
  { v.effectiveCalibratedCovariance() }
