...
template <typename L, typename track_state_t>
auto visit_measurement(track_state_t ts, size_t dim, L&& lambda) {
  return template_switch<detail::visit_measurement_callable_ts, 1, eBoundSize>(
      dim, std::move(ts), lambda);
}
