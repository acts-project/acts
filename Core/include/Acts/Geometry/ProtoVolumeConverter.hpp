...
#include <tuple>
  ...
  struct ConcentricCylinderConverter {
    ...
    DetectorVolumeExternals create(const GeometryContext& /*ignored*/) {
      ...
      DetectorVolumeExternals rTuple = {std::move(transform),
                                        std::move(bounds)};
      return rTuple;
    }
  };
  ...
  struct SingleBlockBuilder {
    ...
    void operator()(DetectorBlock& dBlock, const GeometryContext& gctx,
      ...
      auto dVolume = DetectorVolumeFactory::construct(
          portals, gctx, protoVolume.name, transform, std::move(bounds),
          surfaces, volumes, std::move(updator));
  ...
  struct ContainerBlockBuilder {
    ...
    void operator()(DetectorBlock& dBlock, const GeometryContext& gctx,
      ...
      if (protoVolume.container.has_value() and protoVolume.container.value().constituentBinning.size() == 1u) {
        // Get the container 
        ...
        ACTS_VERBOSE(" - this container has "
                     << container.constituentVolumes.size()
                     << " constituents");
