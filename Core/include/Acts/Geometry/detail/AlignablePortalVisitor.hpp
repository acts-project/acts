...
/// @brief TrackingGeometry visitor that connects the portal surfaces of the
...
class AlignablePortalVisitor final : public TrackingGeometryMutableVisitor {
 public:
  /// @brief Standard constructor taking the geometry context with the current alignment
  ...
  /// @brief Enables the visitor's inspection mode. The transforms of the portals
 ...
 private:
  ...
  /// @brief Pointer to the held logger object
  const Logger& m_logger;
  /// @brief Inspect the portal transforms
  bool m_doInspect{false};
  /// @brief Return the reference to the logger object
