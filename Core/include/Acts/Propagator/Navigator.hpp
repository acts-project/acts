...
class Navigator {
 public:
  ...
  SurfaceIntersection estimateNextTarget(State& state, const Vector3& position,
    ...
    if (state.navigationStage <= Stage::surfaceTarget &&
        targetSurfaces(state, position, direction)) {
 ...
 private:
  ...
  bool targetSurfaces(State& state, const Vector3& position,
                      const Vector3& direction) const {
    ...
    auto layerID = state.navSurface().object()->geometryId().layer();
    ...
    while (state.navSurfaceIndex != state.navSurfaces.size()) {
      // Screen output how much is left to try
      ACTS_VERBOSE(volInfo(state)
                   << (state.navSurfaces.size() - state.navSurfaceIndex)
                   << " out of " << state.navSurfaces.size()
                   << " surfaces remain to try.");
      auto& intersection = state.navSurface();
      // Take the surface
      ...
      intersection = surface->intersect(
          state.options.geoContext, position, direction, boundaryTolerance,
          s_onSurfaceTolerance)[intersection.index()];
  ...
  bool targetLayers(State& state, const Vector3& position,
    ...
    while (state.navLayerIndex != state.navLayers.size()) {
      auto& intersection = state.navLayer().first;
      // The layer surface
      const auto* layerSurface = intersection.object();
      // We are on the layer
      if (state.currentSurface == layerSurface) {
      ...
      intersection =
          layerSurface->intersect(state.options.geoContext, position, direction,
                                  BoundaryTolerance::None(),
                                  s_onSurfaceTolerance)[intersection.index()];
      if (intersection.status() == Intersection3D::Status::reachable) {
        ACTS_VERBOSE(volInfo(state) << "Layer reachable at distance "
  ...
  bool targetBoundaries(State& state, const Vector3& position,
    ...
    while (state.navBoundaryIndex != state.navBoundaries.size()) {
      auto& intersection = state.navBoundary().first;
      // That is the current boundary surface
      const auto* boundarySurface = intersection.object();
      // Step towards the boundary surfrace
      intersection = boundarySurface->intersect(
          state.options.geoContext, position, direction,
          BoundaryTolerance::None(),
          s_onSurfaceTolerance)[intersection.index()];
      if (intersection.status() >= Intersection3D::Status::reachable) {
        ACTS_VERBOSE(volInfo(state) << "Boundary reachable at distance "
                                    << intersection.pathLength());
        return true;
      }
      ACTS_VERBOSE("Boundary "
                   << (state.navBoundaries.size() - state.navBoundaryIndex)
                   << " out of " << state.navBoundaries.size()
                   << " not reachable anymore, switching to next.");
      ACTS_VERBOSE("Targeted boundary surface was: \n"
                   << boundarySurface->toStream(state.options.geoContext));
