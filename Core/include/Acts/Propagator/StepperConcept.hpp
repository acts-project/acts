...
template <typename Stepper, typename State = typename Stepper::State>
concept SingleStepper = CommonStepper<Stepper, State> &&
    requires(const Stepper& s, State& t) {
...
concept Stepper =
    SingleStepper<_Stepper, State> || MultiStepper<_Stepper, State>;

/// @brief Concept that is satisfied by stepper states.
template <typename State>
concept StepperState = requires(const State& t) {
  { t.covTransport } -> decayed_same_as<const bool&>;
  { t.pathAccumulated } -> decayed_same_as<const double&>;
};
