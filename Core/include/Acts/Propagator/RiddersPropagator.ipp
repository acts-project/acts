...
// file, You can obtain one at http://mozilla.org/MPL/2.0/.
  ...
  auto
  Acts::RiddersPropagator<propagator_t>::propagate(const parameters_t& start,
  ...
  template <typename parameters_t, typename surface_t, typename action_list_t,
            ...
            template <typename, typename> class propagator_options_t>
  auto
  Acts::RiddersPropagator<propagator_t>::propagate(const parameters_t& start, const surface_t& target,
            const propagator_options_t<action_list_t, aborter_list_t>& options)
  ...
  Acts::RiddersPropagator<propagator_t>::wiggleDimension(
      ...
      const Acts::BoundVector& nominal, const std::vector<double>& deviations) const {
  ...
  auto  Acts::RiddersPropagator<propagator_t>::calculateCovariance(
      const std::array<std::vector<Acts::BoundVector>, Acts::BoundParsDim>& derivatives,
      const Acts::BoundSymMatrix& startCov, const std::vector<double>& deviations) const -> const Covariance{
    Jacobian jacobian;
	...
	template <typename propagator_t>
  Acts::BoundVector 
  Acts::RiddersPropagator<propagator_t>::fitLinear(const std::vector<Acts::BoundVector>& values,
