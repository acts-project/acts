...
Acts::Result<double> Acts::EigenStepper<E, A>::step(
  ...
  while (true) {
    ...
    stepSizeScaling =
        std::min(std::max(0.25f, std::sqrt(std::sqrt(static_cast<float>(
                 ...
                 4.0f);
    std::cout << "EigenStepper: pre step " << state.stepping.stepSize.toString()
              << " scale " << stepSizeScaling << std::endl;
    state.stepping.stepSize.scale(stepSizeScaling);
    std::cout << "EigenStepper: post step "
  ...
  double h = state.stepping.stepSize.value();
  ...
  h = state.stepping.stepSize.value();
  return h;
}
