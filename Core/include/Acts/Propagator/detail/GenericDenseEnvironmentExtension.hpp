...
struct GenericDenseEnvironmentExtension {
 ...
 private:
  ...
  bool transportMatrix(propagator_state_t& state, const stepper_t& stepper,
    ...
    //~ double dtpp2dl = -mass * mass * qopNew * qopNew *
    //~ (3. * g * (1. + half_h * jdL[0]) +
    //~ qopNew * dgdqop(energy[1], mass, absPdg,
    //~ state.options.meanEnergyLoss));

    double dtp2dl = qopNew * mass * mass / hypot(1, qopNew * mass);
    qopNew = qop[0] + half_h * Lambdappi[1];

    //~ double dtpp3dl = -mass * mass * qopNew * qopNew *
    //~ (3. * g * (1. + half_h * jdL[1]) +
    //~ qopNew * dgdqop(energy[2], mass,
    //~ absPdg,
    //~ state.options.meanEnergyLoss));
  ...
  void initializeEnergyLoss(const propagator_state_t& state,
    ...
    if (state.stepping.covTransport) {
      ...
      if (state.options.includeGgradient) {
        if (state.options.meanEnergyLoss) {
          dgdqopValue = deriveEnergyLossMeanQOverP(slab, absPdg, mass,
                                                   static_cast<double>(qop[0]));
        } else {
          // TODO path length dependence; see above
          dgdqopValue = deriveEnergyLossModeQOverP(slab, absPdg, mass,
                                                   static_cast<double>(qop[0]));
        }
      }
