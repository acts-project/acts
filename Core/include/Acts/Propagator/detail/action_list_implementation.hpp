...
struct action_caller {
  ...
  static void action(const actor& act, result_t& result,
                     propagator_state_t& state, const stepper_t& stepper,
                     const navigator_t& navigator, Args&&... args) {
    act(result.template get<detail::result_type_t<actor>>(), state, stepper,
...
struct action_list_impl<first, others...> {
  template <typename T, typename result_t, typename propagator_state_t,
            typename stepper_t, typename navigator_t, typename... Args>
  static void action(const T& obs_tuple, result_t& result,
                     propagator_state_t& state, const stepper_t& stepper,
                     const navigator_t& navigator, Args&&... args) {
    constexpr bool has_result = has_result_type_v<first>;
    const auto& this_action = std::get<first>(obs_tuple);
    action_caller<has_result>::action(this_action, result, state, stepper,
                                      navigator, args...);
    action_list_impl<others...>::action(obs_tuple, result, state, stepper,
                                        navigator, args...);
  }
};
