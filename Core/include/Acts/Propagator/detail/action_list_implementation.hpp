...
struct action_caller {
  template <PropagatorStage propagator_stage, typename actor_t,
            typename result_t, typename propagator_state_t, typename stepper_t,
  ...
  static void action(const actor_t& act, propagator_state_t& state,
    ...
    act.template operator()<propagator_stage>(
...
struct action_caller<false> {
  ...
  static void action(const actor_t& act, propagator_state_t& state,
                     const stepper_t& stepper, const navigator_t& navigator,
                     result_t& /*result*/, Args&&... args) {
    act.template operator()<propagator_stage>(state, stepper, navigator,
                                              std::forward<Args>(args)...);
  }
};
...
struct action_list_impl<first, others...> {
  ...
  static void action(const T& actors_tuple, propagator_state_t& state,
    ...
    action_caller<has_result>::template action<propagator_stage>(
        this_actor, state, stepper, navigator, result, args...);
    action_list_impl<others...>::template action<propagator_stage>(
        actors_tuple, state, stepper, navigator, result, args...);
  }
};
...
struct action_list_impl<last> {
  ...
  static void action(const T& actors_tuple, propagator_state_t& state,
    ...
    action_caller<has_result>::template action<propagator_stage>(
        this_actor, state, stepper, navigator, result,
        std::forward<Args>(args)...);
  }
};
