...
struct actor_caller<false> {
  ...
  static bool check(const actor_t& actor, propagator_state_t& state,
                    const stepper_t& stepper, const navigator_t& navigator,
                    Args&&... args) {
    return actor.check(state, stepper, navigator, std::forward<Args>(args)...);
  }
};
...
struct actor_list_impl<first, others...> {
  ...
  static void act(const T& actors_tuple, propagator_state_t& state,
    ...
    actor_caller<has_result>::act(this_actor, state, stepper, navigator,
                                  args...);
    actor_list_impl<others...>::act(actors_tuple, state, stepper, navigator,
                                    args...);
  }
  ...
  static bool check(const T& actors_tuple, propagator_state_t& state,
    ...
    constexpr bool has_result = has_result_type_v<first>;
    const auto& this_actor = std::get<first>(actors_tuple);
    return actor_caller<has_result>::check(this_actor, state, stepper,
                                           navigator, args...) ||
           actor_list_impl<others...>::check(actors_tuple, state, stepper,
                                             navigator, args...);
  }
};
...
struct actor_list_impl<last> {
  ...
  static bool check(const T& actors_tuple, propagator_state_t& state,
    ...
    constexpr bool has_result = has_result_type_v<last>;
    const auto& this_actor = std::get<last>(actors_tuple);
    return actor_caller<has_result>::check(
        this_actor, state, stepper, navigator, std::forward<Args>(args)...);
  }
};
