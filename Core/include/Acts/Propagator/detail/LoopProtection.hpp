...
namespace detail {
  ...
  struct LoopProtection
  {
    ...
    void operator()(state_t& state,
                    const steppter_t& stepper) const
    {
      // Estimate the loop protection limit
      if (state.options.loopProtection){
        // Get the field at the start position 
        Vector3D field =  stepper.getField(state.stepping,state.stepping.position());
        // Momentum in SI units and B field
        const double p = units::Nat2SI<units::MOMENTUM>(state.stepping.momentum().mag());
        const double B = field.mag();
        const double helixPath = 10*p/B;
        // now set the new loop limit  
        auto& pathAborter = state.targetAborters.template get<path_arborter_t>(); 
