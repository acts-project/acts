...
inline bool calibrateOuterStripSpacePoint(
    std::span<const float, 3> direction, std::span<const float, 3> ihvCrossOhv,
    std::span<const float, 3> iosvCrossOhv,
    std::span<const float, 3> iosvCrossIhv, std::span<const float, 3> oc,
    std::span<const float, 3> ohv, std::span<float, 3> calibrated,
    float tolerance) {
