...
struct Cell {
  /// @brief construct cell with its coordinates and index of the origin
  /// @param the_coordinates coordinates of the cell
  /// @param src_index index which identifies the source of the cell data
...
struct ConnectionHelper {
  /// @brief dimension of the cell.
  static constexpr std::size_t NDim = traits::getCellDimension<cell_t>();
  /// @brief the type of an individual coordinate.
  ...
  /// @param coordinates_diff the difference of the coordinates of each cell in all dimensions.
  /// @return true if the two cells used to compute the coordinate differences are connected.
  ...
  /// @param coordinates_diff the difference of the coordinates of each cell in all dimensions.
  /// @param sort_axis_i index of the axis that was used for sorting the cells.
  /// @return returns surprisingly true if the search can be aborted.
  ///
...
void labelSortedCells(cell_collection_t &cells,
  ...
  for (index_t idx_a = 0; idx_a < cells.size(); ++idx_a) {
    ...
    for (index_t idx_b = idx_a; idx_b-- > 0;) {
      // Bogus initialization to satisfy clang-tidy. The actual
      // initialization happens in the loop below.
      ...
      if (connection_helper.isConnected(diff)) {
        if (traits::getLabel(cells[idx_a]) < idx_a) {
          // Bogus initialization to satisfy clang-tidy. The actual
          // initialization happens in the two complementary branches below.
          label_t min_label{};
          label_t max_label{};
...
/// It is assumed that clusterize was called previously for the given
