...
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::find(
  ...
  while (((m_cfg.addSingleTrackVertices && seedTracks.size() > 0) ||
    ...
    auto seedResult = doSeeding(seedTracks,
                                currentConstraint,
                                vertexingOptions,
                                seedFinderState,
                                removedSeedTracks);
    ...
    auto prepResult = canPrepareVertexForFit(searchTracks,
                                             seedTracks,
                                             vtxCandidate,
                                             currentConstraint,
                                             fitterState,
    ...
    if (not keepVertex) {
      auto deleteVertexResult = deleteLastVertex(vtxCandidate,
                                                 allVertices,
                                                 allVerticesPtr,
                                                 fitterState,
                                                 vertexingOptions);
...
template <typename vfitter_t, typename sfinder_t>
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::doSeeding(
...
template <typename vfitter_t, typename sfinder_t>
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::getIPSignificance(
...
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::
  ...
  if (fitterState.vtxInfoMap[&vtx].trackLinks.empty()) {
    ...
    if (nearTrackFound) {
      ...
      auto res = addCompatibleTracksToVertex(
          allTracks, vtx, fitterState, vertexingOptions);
      if (!res.ok()) {
...
template <typename vfitter_t, typename sfinder_t>
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::canPrepareVertexForFit(
    const std::vector<const InputTrack_t*>& allTracks,
    const std::vector<const InputTrack_t*>& seedTracks,
    Vertex<InputTrack_t>& vtx,
    const Vertex<InputTrack_t>& currentConstraint,
  ...
  auto resComp = addCompatibleTracksToVertex(
      allTracks, vtx, fitterState, vertexingOptions);
  if (!resComp.ok()) {
  ...
  auto resRec = canRecoverFromNoCompatibleTracks(allTracks,
                                                 seedTracks,
                                                 vtx,
                                                 currentConstraint,
                                                 fitterState,
  ...
  for (const auto& trk : fitterState.vtxInfoMap[&vtx].trackLinks) {
    ...
    if ((trkAtVtx.vertexCompatibility < m_cfg.maxVertexChi2 &&
      ...
      auto foundIter = std::find_if(
          seedTracks.begin(), seedTracks.end(), [&trk](auto seedTrk) {
            return trk == seedTrk;
          });
...
template <typename vfitter_t, typename sfinder_t>
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::keepNewVertex(
    Vertex<InputTrack_t>& vtx,
...
template <typename vfitter_t, typename sfinder_t>
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::isMergedVertex(
    const Vertex<InputTrack_t>& vtx,
...
template <typename vfitter_t, typename sfinder_t>
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::deleteLastVertex(
    Vertex<InputTrack_t>& vtx,
    std::vector<std::unique_ptr<Vertex<InputTrack_t>>>& allVertices,
    std::vector<Vertex<InputTrack_t>*>& allVerticesPtr,
    FitterState_t& fitterState,
...
template <typename vfitter_t, typename sfinder_t>
auto
Acts::AdaptiveMultiVertexFinder<vfitter_t, sfinder_t>::getVertexOutputList(
