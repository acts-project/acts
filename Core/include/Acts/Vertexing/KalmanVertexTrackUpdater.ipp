...
void Acts::KalmanVertexTrackUpdater::update(TrackAtVertex<input_track_t>& track,
  ...
  const ActsSymMatrix<3> vtxCov =
      vtx.fullCovariance().template block<3, 3>(0, 0);
  const ActsSymMatrix<3> vtxWeight = vtxCov.inverse();

  // New track covariance matrix
  ActsSymMatrix<3> newTrkCov =
  ...
  const ActsSymMatrix<3>& reducedVtxWeight = matrixCache.newVertexWeight;
...
Acts::KalmanVertexTrackUpdater::detail::createFullTrackCovariance(
  ...
  trkJac.block<4, 4>(1, 2) = ActsSymMatrix<4>::Identity();
