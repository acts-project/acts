...
Acts::NumericalTrackLinearizer<propagator_t, propagator_options_t>::
  ...
  // Length scale at which we consider to be sufficiently close to the Perigee surface to skip the propagation.
  ...
  auto intersection = perigeeSurface->intersect(gctx, params.position(gctx), params.unitDirection(), false);
  ...
  pOptions.direction = Direction::fromScalarZeroAsPositive(intersection.intersection.pathLength);
  ...
  // These quantities will be used in the computation of the constant term in the Taylor expansion
  Vector4 pca;
  ...
  // Fill "curvilinearParamVec", "pca", and "momentumAtPCA"
 {
  ...
  // Wiggling each of the curvilinear parameters at the PCA and computing the Perigee parametrization of the resulting new track.
  // This allows us to approximate the numerical derivatives.
  for (unsigned int i = 0; i < nCurvilinearParams; i++) {
    // Wiggle
    curvilinearParamVec(i) += delta;
    newPos = curvilinearParamVec.head(4);
    newDir = Vector3(std::sin(curvilinearParamVec(5)) * std::cos(curvilinearParamVec(4)),
    ...
    intersection = perigeeSurface->intersect(gctx, newPos.head(3), newDir, false);
    pOptions.direction = Direction::fromScalarZeroAsPositive(intersection.intersection.pathLength);

    // Propagate to the new PCA and extract Perigee parameters
    auto newResult = m_cfg.propagator->propagate(
        newCurvilinearParams, *perigeeSurface, pOptions);
  ...
  ActsMatrix<eBoundSize, 4> positionJacobian{Jacobian.block<eBoundSize, 4>(0, 0)};
  ActsMatrix<eBoundSize, 3> momentumJacobian{Jacobian.block<eBoundSize, 3>(0, 4)};
  ...
  return LinearizedTrack(perigeeParams, parCovarianceAtPCA, weightAtPCA, linPoint,
