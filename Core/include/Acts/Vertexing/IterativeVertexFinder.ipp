...
Acts::IterativeVertexFinder<vfitter_t, sfinder_t>::find(
  ...
  while (seedTracks.size() > 1 && nInterations < m_cfg.maxVertices) {
    ...
    auto res = fillPerigeesToFit(seedTracks,
                                 seedVertex,
                                 perigeesToFit,
    ...
    if (!isGoodVertex) {
      ...
      if (m_cfg.reassignTracksAfterFirstFit && (!m_cfg.createSplitVertices)) {
        ...
        auto result = reassignTracksToNewVertex(vertexCollection,
                                                currentVertex,
                                                perigeesToFit,
                                                seedTracks,
                                                origTracks,
      ...
      if (isGoodVertex) {
        removeUsedCompatibleTracks(
            currentVertex, perigeesToFit, seedTracks, vertexingOptions, state);
    ...
    if (m_cfg.createSplitVertices) {
      ...
      if (!isGoodSplitVertex) {
        ...
        removeUsedCompatibleTracks(currentSplitVertex,
                                   perigeesToFitSplitVertex,
                                   seedTracks,
                                   vertexingOptions,
                                   state);
...
Acts::IterativeVertexFinder<vfitter_t, sfinder_t>::getCompatibility(
  ...
  // Linearize track
  auto result =
      m_cfg.linearizer.linearizeTrack(params,
                                      vertex.fullPosition(),
...
Acts::IterativeVertexFinder<vfitter_t, sfinder_t>::removeUsedCompatibleTracks(
  ...
  for (const auto& trackAtVtx : tracksAtVertex) {
    ...
    auto foundFitIter =
        std::find_if(perigeesToFit.begin(),
                     perigeesToFit.end(),
  ...
  for (const auto& myPerigeeToFit : perigeesToFit) {
    ...
    if (chi2 < m_cfg.maximumChi2cutForSeeding) {
      auto foundIter = std::find_if(seedTracks.begin(),
                                    seedTracks.end(),
      ...
      auto foundIter =
          std::find_if(tracksAtVertex.begin(),
                       tracksAtVertex.end(),
...
Acts::IterativeVertexFinder<vfitter_t, sfinder_t>::fillPerigeesToFit(
  ...
  for (const auto& sTrack : perigeeList) {
    ...
    // still large amount of tracks available, check compatibility
      ...
      auto distanceRes =
          m_cfg.ipEst.calculate3dDistance(vertexingOptions.geoContext,
                                          sTrackParams,
                                          seedVertex.position(),
                                          state.ipState);
      if (!distanceRes.ok()) {
...
Acts::IterativeVertexFinder<vfitter_t, sfinder_t>::reassignTracksToNewVertex(
  ...
  for (auto& vertexIt : vertexCollection) {
    ...
    for (auto tracksIter = tracksBegin; tracksIter != tracksEnd;) {
      ...
      auto resultNew = getCompatibility(
          trackPerigee, currentVertex, vertexingOptions, state);
      if (!resultNew.ok()) {
...
Acts::IterativeVertexFinder<vfitter_t, sfinder_t>::countSignificantTracks(
  ...
  return std::count_if(vtx.tracks().begin(),
                       vtx.tracks().end(),
