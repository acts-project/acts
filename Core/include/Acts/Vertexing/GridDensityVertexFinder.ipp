...
auto Acts::GridDensityVertexFinder<mainGridSize, trkGridSize, vfitter_t>::find(
  ...
  if (m_cfg.cacheGridStateForTrackRemoval && state.isInitialized &&
    ...
    state.mainGrid = Vector<float, mainGridSize>::Zero();
  ...
  double z = 0;
  double width = 0;
  if (state.mainGrid != Vector<float, mainGridSize>::Zero()) {
  ...
  ActsSymMatrix<4> seedCov = vertexingOptions.vertexConstraint.fullCovariance();
