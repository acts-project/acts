...
struct GsfActor {
  ...
  void convoluteComponents(propagator_state_t& state, const stepper_t& stepper,
    ...
    for (auto [idx, cmp] : zip(tmpStates.tips, cmps)) {
      ...
      Acts::BoundTrackParameters bound(
          proxy.referenceSurface().getSharedPtr(), proxy.filtered(),
  ...
  void applyBetheHeitler(const propagator_state_t& state,
                         const navigator_t& navigator,
                         const Acts::BoundTrackParameters& old_bound,
...
struct FinalStateCollector {
  ...
  void operator()(propagator_state_t& state, const stepper_t& stepper,
    ...
    for (auto cmp : stepper.componentIterable(state.stepping)) {
      ...
      if (bs.ok()) {
        const auto& btp = std::get<Acts::BoundTrackParameters>(*bs);
