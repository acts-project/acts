...
/// @return a vector
std::vector<AlgebraScalar> phiSegments(
    AlgebraScalar phiMin = -M_PI, AlgebraScalar phiMax = M_PI,
    const std::vector<AlgebraScalar>& phiRefs = {},
    AlgebraScalar phiTolerance = 1e-6);
...
void createSegment(std::vector<vertex_t>& vertices,
                   std::pair<AlgebraScalar, AlgebraScalar> rxy,
                   AlgebraScalar phi1, AlgebraScalar phi2, unsigned int lseg,
                   int addon = 0, const vertex_t& offset = vertex_t::Zero(),
  ...
  segs = segs > 0 ? segs : 1;
  AlgebraScalar phistep = (phi2 - phi1) / segs;
  // Create the segments
  for (unsigned int iphi = 0; iphi < segs + addon; ++iphi) {
    AlgebraScalar phi = phi1 + iphi * phistep;
...
std::vector<Vector2D> ellipsoidVertices(
    AlgebraScalar innerRx, AlgebraScalar innerRy, AlgebraScalar outerRx,
    AlgebraScalar outerRy, AlgebraScalar avgPhi = 0.,
    AlgebraScalar halfPhi = M_PI, unsigned int lseg = 1);
...
/// @param lseg The number of segments for for a full 2*pi segment
/// @return a vector of 2d-vectors
std::vector<Vector2D> circularVertices(AlgebraScalar innerR,
                                       AlgebraScalar outerR,
                                       AlgebraScalar avgPhi = 0.,
                                       AlgebraScalar halfPhi = M_PI,
                                       unsigned int lseg = 1);
...
bool onHyperPlane(const std::vector<Vector3D>& vertices,
                  AlgebraScalar tolerance = s_onSurfaceTolerance);
