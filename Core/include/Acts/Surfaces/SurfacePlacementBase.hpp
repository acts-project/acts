...
namespace Acts{
    ...
    ///        sensor planes.
    ///
    ///        If the user wants to let the sensor surface float, he has to implement
    ///        an experiment-specifc class inherting from SurfacePlacementBase and
    ///        to instantiate the surface with such an instance.
    ///
    ///        In every call for the Surface's current position the GeometryContext which
    ///        is also contextualized by the experiment is piped through the entire call
    ///        stack until its piped back to the experiment specific implementation via
    ///        the call of the `SurfacePlacementBase::localToGlobalTransform`. There the
    ///        user needs to unpack the GeometryContext, look-up the appropiate
    ///        cached transform and return it back to the Acts library
