...
class SurfaceArray {
 public:
  ...
  struct ISurfaceGridLookup {
    ...
    virtual void
    fill(const GeometryContext& gctx, const SurfaceVector& surfaces) = 0;
    ...
    lookup(const Vector3D& position) = 0;
    ...
    lookup(const Vector3D& position) const = 0;
    ...
    virtual const SurfaceVector&
    neighbors(const Vector3D& position) const = 0;
    ...
    virtual std::vector<BinningValue>
    binningValues() const {
      return {};
    };
  ...
  struct SurfaceGridLookup : ISurfaceGridLookup {
   ...
   public:
    ...
    /// @return @c SurfaceVector at given bin
    SurfaceVector&
    lookup(const Vector3D& position) override {
    ...
    /// @return @c SurfaceVector at given bin
    const SurfaceVector&
    lookup(const Vector3D& position) const override {
      return m_grid.atPosition(m_globalToLocal(position));
    }
    ...
    /// @return @c SurfaceVector at given bin. Copy of all bins selected
    const SurfaceVector&
    neighbors(const Vector3D& position) const override {
  ...
  /// position
  const SurfaceVector&
  at(const Vector3D& position) const {
    return p_gridLookup->lookup(position);
  }
  ...
  SurfaceVector
  neighbors(const Vector3D& position) const {
