...
concept SurfaceConcept =
    requires(S s, const S cs, S s2, const S cs2, GeometryContext gctx,
      ...
      {
        cs.globalToLocal(gctx, Vector3{}, Vector3{}, long double{5})
      ...
      {
        cs.intersect(gctx, Vector3{}, Vector3{}, tolerance,
                     std::declval<long double>())
      } -> std::same_as<SurfaceMultiIntersection>;

      {
        cs.toStream(gctx)
      } -> std::same_as<GeometryContextOstreamWrapper<Surface>>;
...
concept RegularSurfaceConcept =
    SurfaceConcept<S> && requires(S s, const S cs, GeometryContext gctx,
      ...
      {
        cs.globalToLocal(gctx, Vector3{}, Vector3{},
                         std::declval<long double>())
      } -> std::same_as<Result<Vector2>>;
