...
concept SurfaceConcept = requires(S s, const S cs, S s2, const S cs2,
  ...
  { cs.type() }
  ->std::same_as<Surface::SurfaceType>;
  { cs.transform(gctx) }
  ->std::same_as<const Transform3&>;
  { cs.center(gctx) }
  ->std::same_as<Vector3>;
  { cs.normal(gctx, Vector3{}, Vector3{}) }
  ->std::same_as<Vector3>;
  { cs.bounds() }
  ->std::convertible_to<const SurfaceBounds&>;
  { cs.associatedDetectorElement() }
  ->std::same_as<const DetectorElementBase*>;

  { cs.associatedLayer() }
  ->std::same_as<const Layer*>;
  { s.associateLayer(std::declval<const Layer&>()) }
  ->std::same_as<void>;

  { cs.surfaceMaterial() }
  ->std::same_as<const ISurfaceMaterial*>;
  ...
  { cs.localToGlobal(gctx, Vector2{}, Vector3{}) }
  ->std::same_as<Vector3>;
  ...
  { cs.pathCorrection(gctx, Vector3{}, Vector3{}) }
  ->std::same_as<double>;
...
concept RegularSurfaceConcept =
  ...
  { cs.normal(gctx, Vector2{}) }
  ->std::same_as<Vector3>;

  { cs.normal(gctx, Vector3{}) }
  ->std::same_as<Vector3>;

  { cs.globalToLocal(gctx, Vector3{}, Vector3{}, std::declval<double>()) }
  ->std::same_as<Result<Vector2>>;

  { cs.localToGlobal(gctx, Vector2{}) }
  ->std::same_as<Vector3>;
};
}  // namespace Acts
