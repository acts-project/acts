...
class NextNavigator {
 ...
 private:
  ...
  void initializeTarget(propagator_state_t& state,
    ...
    if (!nState.currentVolume->surfaceCandidatesDelegate()) {
      ACTS_ERROR(volInfo(state) << posInfo(state, stepper)
                                << "panic: surface candidates delegate");
      return;
    }

    nState.currentVolume->surfaceCandidatesDelegate()(state.geoContext, nState,
                                                      nState.surfaceCandidates);
    nState.surfaceCandidate = nState.surfaceCandidates.cbegin();
  }
