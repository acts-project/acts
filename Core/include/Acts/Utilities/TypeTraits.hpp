...
/**
 * This type ties together the detection idiom. It instantiates the @c detector
 * template with the @c Op and @c Args and resolves to the exact value type.
 * In essence, if `Op<Args...>` succeeds, this will evaluate to
 * `std::true_type`, and if not, it will evaluate to `std::true_type`.
 * @tparam Op The operation to test
...
/**
 * This type calls into the detector (same as @c is_detected) but it extracts
...
/**
 * Define some sort of "DSL" to declare concepts a little more naturally.
 * These are taken from https://izzys.casa/2016/09/implementing-concepts-in-cxx/
 */
/**
 ...
 * this would normally cause a compilation error. If we wrap it into the
 ...
 * the method's const qualifier. This is less straight forward. To achieve this,
 ...
 * takes the type, the return type and the arguments as metaarguments. That
 * means you can use the same method trait to check for any signature involving
 * the same method name. Say you generated
 * ```
