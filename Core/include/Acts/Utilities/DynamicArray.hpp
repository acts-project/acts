...
namespace Acts{
    ...
    template <class T, size_t D> class DynamicArray {
    public:
         ...
         *               constructs an array with maximum 4, 7 and 9 elements along the first, second and third direction
         */
        template <typename... args> DynamicArray(args... jks);
        ...
        /// Returns the array containing the offsets in memory between two neigbouring elements in the same dimension
         ...
         */
        template <typename... args> value_type& get(args... jks);
        template <typename... args> const value_type& get(args... jks) const;
        /**
        ...
        template <typename... args> size_t index(args... jks) const;
        /**
         ...
         *          will assign memory to store 2x4x5 elements. If one of the values is zero, then the memory will be cleared
         *
         */
        template <typename... args> void changeStripes(args... jks);
        void changeStripes(const std::array<size_t, D>& stripes);

        /// Standard logical operators. The operator() returns true if the array is not empty.
        operator bool() const;
        bool operator!() const;
        bool empty() const;
         ...
         * operator. For every D dimensional array, there exist D-1 ArrayNavigators that create other ArrayNavigators having one dimension
         * less until N = 1 is reached. This navigator grants the direct access to the data. Each Navigator stores the index in the
         * corresponding braket and is directly connected to its parent which is the DynamicArray for D-1.
         */
        template <size_t N> class ArrayNavigator {
        public:
            using parent_type = typename std::conditional<N == D - 1, DynamicArray, ArrayNavigator<N + 1>>::type;
            using value_type = typename std::conditional<N == 1, T&, ArrayNavigator<N - 1>>::type;

            ArrayNavigator(parent_type* parent, size_t idx);
            /// Access to the data
            value_type operator[](size_t i);
            /// Global index calculator upstream
            template <typename... args> size_t index(args... indices) const;
        ...
        template <size_t N> class ConstArrayNavigator {
        public:
            using parent_type = typename std::conditional<N == D - 1, DynamicArray, ConstArrayNavigator<N + 1>>::type;
            using value_type = typename std::conditional<N == 1, const T&, ConstArrayNavigator<N - 1>>::type;
         ...
         * stripSize() or the array does not store any data, an exception is thrown. To access the specific element one needs to specify all
         * indices MultiDimensionalArray<double,5> arr{4,6,8,9,10};         * arr[0][1][2][3][4] = 24.;
        ...
        /// Depending on the dimension of the array the [] operator needs to return either the leightweight ArrayNavigators or the value
        /// directly. The latter is only true if the dimension is one which make this class essentially to a std::vector<T>
        using array_type = typename std::conditional<D != 1, ArrayNavigator<D - 1>, T&>::type;
        using const_array_type = typename std::conditional<D != 1, ConstArrayNavigator<D - 1>, const T&>::type;

        inline array_type operator[](size_t i);
        inline const_array_type operator[](size_t i) const;

    private:
        template <size_t pos, typename... args> size_t index(size_t arg_val, args... jks) const;
        template <size_t pos, typename... args> void assign_stripes(size_t val, args... jks);

        template <size_t pos> size_t index(size_t arg_val) const;
        template <size_t pos> void assign_stripes(size_t val);
        void allocate();
