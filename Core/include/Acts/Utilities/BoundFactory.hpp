 ...
 * requested at a later stage the factory automatically returns the cached
 * bounds. This provides a simple sharing mechanism of the same bounds across
 * multiple surfaces / volumes */
namespace detail {
...
class BoundFactory {
 ...
 private:
   ...
   *         For a pair of two bound pointers, the struct defines the < operator
   * which is then exploited by a std::set to deduplicate equivalent bounds. */
