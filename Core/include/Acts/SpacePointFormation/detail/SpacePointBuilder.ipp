...
Acts::Vector2
Acts::SpacePointBuilder<spacepoint_t>::localCoords(
    const Measurement& meas) const {
  // Local position information
  const auto par = std::visit([](const auto& x) { return x.paremeters(); }, meas);
...
Acts::SpacePointBuilder<spacepoint_t>::
    SpacePointBuilder(Acts::SpacePointBuilderConfig cfg)
    : m_config(cfg) {}
...
Acts::SpacePointBuilder<spacepoint_t>::globalCoords(
    ...
    const auto& slink = std::visit([](const auto& x) { return &x.sourceLink(); }, meas);
...
template <typename spacepoint_t>
void Acts::SpacePointBuilder<spacepoint_t>::
  ...
  //  auto measurement = clus.measurement();
    ...
    const auto& slink = std::visit([](const auto& x) { return &x.sourceLink(); }, meas);
    //auto sp = spacepoint_t(gPos, gCov[0], gCov[1], {static_cast<const ActsExamples::InDexSourceLink&>(slink).index()});
    //std::vector<std::reference_wrapper<const SourceLink>> slinks;
