...
template <typename T>
void ObjVisualization3D<T>::vertex(const VertexType& vtx, ColorRGB color) {
  m_vertexColors[m_vertices.size()] = color;
  m_vertices.push_back(vtx.template cast<ValueType>());
}

template <typename T>
void ObjVisualization3D<T>::line(const VertexType& a, const VertexType& b,
                                 ColorRGB color) {
...
template <typename T>
void ObjVisualization3D<T>::face(const std::vector<VertexType>& vtxs,
                                 ColorRGB color) {
  if (color != ColorRGB{0, 0, 0}) {
...
template <typename T>
void ObjVisualization3D<T>::faces(const std::vector<VertexType>& vtxs,
