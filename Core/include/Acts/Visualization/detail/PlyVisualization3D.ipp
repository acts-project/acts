...
template <typename T>
void PlyVisualization3D<T>::vertex(const VertexType& vtx, ColorRGB color) {
  m_vertices.emplace_back(vtx.template cast<ValueType>(), color);
}

template <typename T>
void PlyVisualization3D<T>::face(const std::vector<VertexType>& vtxs,
                                 ColorRGB color) {
  FaceType idxs;
...
template <typename T>
void PlyVisualization3D<T>::faces(const std::vector<VertexType>& vtxs,
...
template <typename T>
void PlyVisualization3D<T>::line(const VertexType& a, const VertexType& b,
                                 ColorRGB color) {
  vertex(a, color);
