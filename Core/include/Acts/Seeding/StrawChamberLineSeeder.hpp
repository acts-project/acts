...
namespace Acts{
    ...
    template <typename Calibrator_t,
        ...
        requires(const Calibrator_t calibrator,
                 const UnCalibCont_t& uncalibCont,
                CalibCont_t& calibCont,
                 const Vector3& trackPos,
                 const Vector3& trackDir,
                 const double trackT0,
                 const CalibrationContext& ctx,
                 detail::pointer_const_dref_t<typename UnCalibCont_t::value_type> singleUnCalibSp,
                 detail::pointer_const_dref_t<typename CalibCont_t::value_type> singleCalibSp) {
            ...
            {calibrator.calibrate(ctx, trackPos, trackDir, trackT0, uncalibCont)} -> std::same_as<CalibCont_t>;
            ...
            {calibrator.calibrate(ctx, trackPos, trackDir, trackT0, singleUnCalibSp, calibCont)};            
             ...
             *  @param ctx: Calibration context to access the calibration constants (Experiment specific)
             *  @param singleCalibSp: Reference to the calibrated space point */
            {calibrator.driftAcceleration(ctx, singleCalibSp)} -> std::same_as<double>;
    };
    ...
    class StrawChamberLineSeeder{
        public:
             ...
             *                         e.g. a Hough transform. The parameters may include both directions
             *                         and intercept. The components, in the non-precision plane are later
             *                         used to derive the calibrations of the space points using the calibrator
             * @param seedHits: List of straw & strip hits to run the seeding on. Seeds are always generated from 
             *                  the straw hits and compatible strip hits are added later once the straw seed passes
             *                  the configured selection criteria
        ...
        private:
            ...
            template <typename Trait_t> struct UncalibSpBook_t {
                using type = Trait_t;
            };            
            ...
            using SpInSeedBookCont_t = std::vector<typename UncalibSpBook_t<UncalibSp_t>::type>;
            
            SeedParam_t m_grainPars{SeedParam_t::Zero()};
             ...
             *         seed parameters to the drift circle measurements ignoring the time component
             ...
             *  @param ctx: Calibration context to access the calibration constants (Experiment specific)
             *  @param candidateSeed: Reference to the drift circle candidate seed */
            SeedFitAuxWithT0 estimateAuxillaries(const CalibrationContext& ctx,
