...
/// Generic Implementation to fit a straight line to set of composite space
/// point measurements. The line is parameterized by x_{0}, y_{0}, theta, phi,
/// where the first two parameters are the line's intercept at z=0 and the
/// latter two are the polar and azimuthal angle in the local frame common to
/// all space points. Optionally, the fitter may also estimate the offset in the
/// time of arrival at the line's reference plane.
class CompositeSpacePointLineFitter {
 public:
  ...
  /// During the repetitive recalibration, single hits may be invalidated under
  /// the track parameters. Define a Delegate to sort out the invalid hits
  ...
  /// Fitter configuration object
  struct Config {
    /// If the parameter change or the gradient's magnitude is below the cutOff
    /// the fit is converged
    ...
    /// Gradient decent step size if the Hessian is singular
    double gradientStep{1.e-4};
    /// Number of iterations
    std::size_t maxIter{1000};
    /// Fit the time offset if possible
    bool fitT0{false};
    /// Recalibrate the hits between two iterations
    bool recalibrate{false};
    /// Switch to use the fast fitter if only straw measurements are passed
    bool useFastFitter{false};
    /// Threshold on the fast straw chi2 above which the fit is reattempted but
    /// with swapped straw signs.
    double badFastChi2SignSwap{5.};
    /// Switch to use the fast fitter as pre-fitter. The flag useFastFitter
    /// needs to be enabled
    bool fastPreFitter{true};
    /// Switch to try the full fit when the fast pre-fitter fails. The flags
    /// useFastFitter and fastPreFitter have to be enabled
    bool ignoreFailedPreFit{false};
    /// Use the second derivative in the residual calculation
    ...
    /// Allowed parameter ranges. If the lower interval edge is higher than the
    /// upper edge, the parameters are unbound
    using RangeArray = std::array<std::array<double, 2>, s_nPars>;
    /// Allowed parameter ranges
    RangeArray ranges{
        filledArray<std::array<double, 2>, s_nPars>(std::array{1., -1.})};
    /// Overwrite the set of parameters to use, if it's absolutely necessary
    std::vector<FitParIndex> parsToUse{};
  };
  /// Auxiliary object to store the fitted parameters, covariance, the chi2 /
  /// nDoF & the number of required iterations
  struct FitParameters {
    ...
    /// Local straight line parameters
    ParamVec_t parameters{filledArray<double, s_nPars>(0)};
    /// Covariance on the local line parameters
  ...
  ///  Fit parameters together with the calibrated measurements.
  ...
  /// Configuration object parsed per each fit. It contains the measurement
  /// container, a pointer to the measurement calibrator + calibration context,
  /// a delegate used to sort out badly calibrated measurements, and also
  /// initial start parameters stemming from an external seeder.
  template <CompositeSpacePointContainer Cont_t,
            CompositeSpacePointCalibrator<Cont_t, Cont_t> Calibrator_t>
  struct FitOptions {
    /// List of measurements to fit
    Cont_t measurements{};
    /// Abrivation of the SpacePoint type
    using Sp_t = SpacePoint_t<Cont_t>;
    /// Good hit selector
    Selector_t<Sp_t> selector{};
    /// Calibrator
    const Calibrator_t* calibrator{nullptr};
    /// Experiment specific calibration context
  ...
  /// Struct counting the different types of degrees of freedom.
  struct DoFcounts {
    /// Measurement in the non-bending coordinate
    std::size_t nonBending{0u};
    /// Measurement in the bending coordinate
  ...
  /// Classify measurements according to whether they measure loc0, loc1, time
  /// or are straw measurements
  ...
  /// Helper function to extract which parameters shall be extracted from the
  /// hit counts.
 ...
 private:
  /// Enumeration to classify the parameter update
  ...
  /// Checks whether the parameters from the iteration or the final result
  /// parameters
  ...
  /// Checks whether a parameter value is within the range intreval defined by
  /// the user
  ...
  /// Executes a fast (pre)fit using the FastStrawLineFitter. First the
  /// parameters
  ...
  /// Executes the fast fit in the bending direction.
  /// @param measurements: List of measurements to fit
  /// @param initialGuess: Line representing the start parameters parsed by the user. Needed to determine
  ///                      the L<->R ambiguity of the straws
  ...
  /// Executes the fast fit in the non-bending direction. In case of success,
  /// the
  ...
  /// Update the straight line parameters based on the current chi2 and its
  ...
  /// Configuration object of the fitter
  Config m_cfg{};
  /// Logger instance
  std::unique_ptr<const Logger> m_logger{};
  /// Helper function to create the fast fitter configuration object
  detail::FastStrawLineFitter::Config fastFitterCfg() const;
  /// Instance of the fast line fitter
