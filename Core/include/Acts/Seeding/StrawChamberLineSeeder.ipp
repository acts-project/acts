 ...
 *  @param strawMeas: Reference to the straw tube measurement. */
 ...
 *  @param linePos: Arbitrary point on the segment line
 *  @param lineDir: Direction of the segment line
 *  @param stripMeas: Reference to the strip measurement. */
template <StationSpacePoint UncalibSp_t>
...
void StrawChamberLineSeeder<UnCalibCont_t, Splitter_t, CalibSp_t,
  ...
  while (!converged && inSeed.nIter++ <= m_cfg.nMaxIter) {
    ...
    const double thetaPrime = 0.5 * auxVars.T_zzyy * sin2Theta -
                              auxVars.T_yz * cos2Theta -
                              auxVars.T_rz * cosTheta - auxVars.T_ry * sinTheta;
    ...
    const double thetaTwoPrime =
        auxVars.T_zzyy * cos2Theta + 2. * auxVars.T_yz * sin2Theta +
        auxVars.T_rz * sinTheta - auxVars.T_ry * cosTheta;
  ...
  const double fitY0 = (auxVars.centerOfGrav.y() * cosTheta -
                        auxVars.centerOfGrav.z() * sinTheta + auxVars.fitY0) /
                       cosTheta;
  ACTS_VERBOSE(
...
void StrawChamberLineSeeder<
  ...
  while (!converged && seed.nIter++ <= m_cfg.nMaxIter) {
    ...
    /// d^{2}chi^{2} / d^{2}theta
    cov(0, 0) = auxVars.T_zzyy * cos2Theta + 2. * auxVars.T_yz * sin2Theta +
                auxVars.T_rz * sinTheta - auxVars.T_ry * cosTheta;

    cov(1, 0) = cov(0, 1) = auxVars.T_vz * cosTheta + auxVars.T_vy * sinTheta;
    cov(1, 1) = -auxVars.fitY0Prime * auxVars.fitY0Prime -
                auxVars.fitY0Prime * auxVars.fitY0TwoPrime -
                auxVars.T_az * sinTheta - auxVars.T_ay * cosTheta +
                auxVars.R_vv + auxVars.R_va;

    grad[0] = 0.5 * auxVars.T_zzyy * sin2Theta - auxVars.T_yz * cos2Theta -
              auxVars.T_rz * cosTheta - auxVars.T_ry * sinTheta;
    grad[1] = auxVars.fitY0 * auxVars.fitY0Prime - auxVars.R_vr +
              auxVars.T_vz * sinTheta - auxVars.T_vy * cosTheta;
  ...
  const double cosTheta = std::cos(pars[0]);
  const double fitY0 =
      (auxVars.centerOfGrav.y() * cosTheta -
       auxVars.centerOfGrav.z() * std::sin(pars[0]) + auxVars.fitY0) /
      cosTheta;
  ACTS_VERBOSE(__func__ << "() " << __LINE__
