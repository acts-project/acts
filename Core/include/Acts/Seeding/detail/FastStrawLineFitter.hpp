...
/// The FastStrawLineFitter fits a straight line to a set of straw measurements
/// The space points  passed to the fitter need to be all straw space points and
/// their straw wires need to be approximately parallel to each other. Under
/// this assumption, each straw measurements can be projected onto the plane
/// which is perpendicular to its straw wire and the residual to the k-th straw
/// is written as
///
///                 R_{k} = T_{z,k} * sin \theta - (T_{y,k} -  y_{0})* cos \theta - sign_{k} * r_{k}
///
/// where, \theta and y_{0} are the two line parameters to fit, T_{z,k} and T_{y,k} are the coordinates of the straw-tube in the plane, sign_{k} fixes the left-right ambiguity, and r_{k} is the drift radius of the measurement. The z & y coordinates are given by the dot product of the straw's local position with its planeNormal & toNextSensor vector, respectively. Assuming that the drift radius does not need to be calibrated during the fit, the entire problem reduces to the minimization of a polynomial having the form
///
///                   A * cos(\theta)  + B * sin(\theta)  + C * cos(2\theta) + D * sin(2\theta)
/// where A, B, C, D are fit constants depending on the configuration of the
/// tubes to fit. They are documented in
/// https://gitlab.cern.ch/atlas-nextgen/work-package-2.5/analyticalsegment
///
/// In general, the primary source of the straw drift radius is a record of the
/// time-of-arrival of an electronics signal which is then translated using a
/// r-t relation which is assumed to be twice differentiable. Just because a
/// particle might arrive a bit later, the drift radius then becomes a function
/// of a generic time offset, t_{0}:
///
///                       r(t) = r(t_{time of record} - t_{0})
///
///The \chi^{2} then becomes a function of the two parameters (\theta, t_{0}) which need to be minimized by varying these two parameters.

class FastStrawLineFitter {
 public:
  /// Abrivation of the residual indices to fetch the proper covariance
  ...
  struct Config {
    ...
    /// Cutoff to define the fit to be converged if the parameter change is
    /// below threshold
    double precCutOff{1.e-9};
    /// If the parameter change normalized to their uncertainties is below the
    /// cutOff the fit is converged. For negative cut-offs this convergence
    /// schema is effectively deactivated
  ...
  struct FitResult {
    ...
    /// Fitted inclination angle
    double theta{0.};
    /// Uncertainty on the fitted angle
    double dTheta{0.};
    /// Fitted line intercept
    double y0{0.};
    /// Uncertainty on the intercept
    double dY0{0.};
    /// Evaluated chi2 postfit
    double chi2{0.};
  ...
  /// Expansion of the FitResult to also return information
  ///        on the fitted time offset t0
  struct FitResultT0 : public FitResult {
    void print(std::ostream& ostr) const override;
    /// Fitted time offset t0
    double t0{0.};
    /// Uncertainty on the time offset
    double dT0{0.};
  };
  /// Fit a straight line to a set of straw measurements and return the theta
  /// angle & the intercept as well as the associated uncertainties.
  ...
  /// Fit a straight line to a set of strip measurements and return the
  /// associated angle & intercept. As strips are assumed to measure bending &
  ...
  /// Fit a straight line to a set of straw measurements taking the time offset
  /// into account. The floating t0 parameter shrinks / blows-up the drift radii
  /// of each straw measurement
 ...
 private:
  /// Index of the drift circle covariance inside the straw space-point's
  /// covariance array
  static constexpr auto s_covIdx = toUnderlying(ResidualIdx::bending);

  /// Auxiliary struct to calculate the fast-fit constants
  struct FitAuxiliaries {
    ...
    /// Tube position center - y weighted with inverse covariances
    double centerY{0.};
    /// Tube position center - z weighted with inverse covariances
    double centerZ{0.};
    /// Inverse covariances per straw measurement
    std::vector<double> invCovs{};
    /// One over the sum of the inverse straw measurement covariances
    double covNorm{0.};
    ...
    double T_ry{0.};
    /// Predicted y0 given as the expectation value of the radii divided by the
    /// inverse covariance sum.
    double fitY0{0.};
    /// Number of degrees of freedom
    std::size_t nDoF{0};
  };
  /// Extension of the auxiliary fit constants needed for the seed refinement
  /// when T0 is floating
  ...
  /// Small Helper struct to calculate sin & cos of theta & 2 theta
  struct TrigonomHelper {
    /// Constructor passing the theta angle
  ...
  /// Calculate the pure angular derivatives of the chi2 w.r.t theta
  ...
  /// Calculate the fit constants for a set of straw circles
  ...
  FitAuxiliaries fillAuxiliaries(const StrawCont_t& measurements,
                                 const std::vector<int>& signs) const;
  /// Evaluate the chi2 after the fit has converged.
  ...
  /// Evaluate the chi2 after the fit with t0 has converged
  /// @param measurements: List of straw measurements
  ...
  /// Calculate the chi2 term from a straw measurement given the known theta, y0
  ...
  /// Fit the track inclination angle and calculate the intercept afterwards
  ...
  /// Calculate the starting parameters on theta from the fit constants
  static double startTheta(const FitAuxiliaries& fitPars);
  /// Calculate the time gradient of the chi2 w.r.t theta
  /// @param angles: Wrapper object to pass sin & cos of theta
  ...
  /// Fill the y0 and its uncertainty in the result
  ...
  /// Enumeration to describe the outcome of the fit iteration
  ...
  /// Update the straw circle parameters for a fit with ts0 & theta
  /// @param fitPars: Fit constants from the straw measurements
  ...
  /// Calculate the extension of the fit constants needed for the simultaneous
  /// theta - t0 fit
  /// @param ctx: Reference to the experiment specific calibration context to
  ///             calculate the updated straw drift radius, velocity &
  ///             acceleration
  ...
  /// ConFiguration object of the instantiated fitter
  const Config m_cfg{};
  /// Pointer to the logger object
  std::unique_ptr<const Acts::Logger> m_logger{};
  /// Dereference the logger object
