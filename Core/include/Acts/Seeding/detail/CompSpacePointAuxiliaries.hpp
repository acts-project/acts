...
/// Print the position, the drift radius & the sensor directions of a space
/// point. If the space point is shipped with an ostream operator, this one is
/// used
...
/// Helper class to calculate the residual between a straight line and a
/// CompositeSpacePoint measurement as well as the partial derivatives. The
/// residual is expressed as a 3D vector, where first two components describe
/// the spatial residual w.r.t. the precision & non-precision direction, and the
/// last one expresses the residual between the parametrized time of arrival of
/// the track and the measurement's recorded time.
///
/// For straw type measurements, the precision residual is calculated as the
/// difference between the signed line distance between straw wire & line, and
/// the signed drift radius where the sign simply encodes the left/right
/// amibiguity. If the measurement additionally carries information about the
/// passage along the wire, the non-precision residual is then defined to be the
/// distance along the wire.
///
/// For strip type measurements, the residual is the distance in the
/// strip-readout plane between the point along the line that intersects the
/// plane spanned by the measurement.
///
/// If the strip measurements provide also the time of their record, the
/// residual calculation can be extended to the time of arrival parameter The
/// residual is then defined as the strip's recorded time minus the time of
/// flight of a particle on a straight line minus the time offset.
///
...
/// chip records are composed timestamp that includes the drift time & the time
/// of flight of the ionizing particle. An update of the point of closest
/// approach leads to an indirect update of the drift radius and hence
/// additional terms need to be considered when calculating the residual's
/// derivatives.
...
class CompSpacePointAuxiliaries {
 public:
  ...
  /// Assignment of the residual components.
  enum class ResidualIdx : std::uint8_t {
    nonBending = 0,
    bending = 1,
    time = 2
  };
  /// Prints a fit parameter as string
  static std::string parName(const FitParIndex idx);
  /// Define the ostream operator
  friend std::ostream& operator<<(std::ostream& ostr, const FitParIndex idx) {
    return ostr << parName(idx);
  }
  /// Print a residual index as string
  static std::string residualName(const ResidualIdx resIdx);
  /// Define the ostream operator
  friend std::ostream& operator<<(std::ostream& ostr, const ResidualIdx idx) {
    return ostr << residualName(idx);
  }
  /// Configuration object of the residual calculator
  struct Config {
    /// Transform to place the composite station frame inside the global
    /// experiment's frame. Needed for the time residual calculation with time
    /// of flight
    Acts::Transform3 localToGlobal{Acts::Transform3::Identity()};
    /// Flag toggling whether the hessian of the residual shall be calculated
    bool useHessian{false};
    /// Flag toggling whether the along the wire component of straws shall be
    /// calculated if provided by the straw measurement.
    bool calcAlongStraw{true};
    /// Flag toggling whether the residual along the strip direction shall be
    /// calculated if the space point does not measure both spatial coordinates
    /// on the plane
    bool calcAlongStrip{true};
    ///  Include the time of flight assuming that the particle travels with the
    ///  speed of light in the time residual calculations
    bool includeToF{true};
    /// List of fit parameters to which the partial derivative of the residual
    /// shall be calculated
  ...
  /// Updates the spatial residual components between the line and the passed
  /// measurement. The result is cached internally and can be later fetched by
  /// the residual(), gradient() and hessian() methods. If the residual
  /// calculation fails due to parallel line & measurement, all components are
  /// set to zero.
  ...
  /// Updates all residual components between the line and the passed
  /// measurement First the spatial components are calculated and then if the
  ...
  /// Helper struct to calculate the overall chi2 from the composite space
  /// points
  struct ChiSqWithDerivatives {
    /// Chi2 squared term
    double chi2{0.};
    /// First derivative of the chi2 w.r.t. the fit parameters
    Acts::Vector<s_nPars> gradient{Acts::Vector<s_nPars>::Zero()};
    /// Second derivative of the chi2 w.r.t. the fit parameters
    Acts::SquareMatrix<s_nPars> hessian{Acts::SquareMatrix<s_nPars>::Zero()};
    /// Set the chi2, the gradient and hessian back to zero
    void reset();
  };
  /// Updates the passed chi2 object by adding up the residual contributions
  /// from the previous composite space point used to update the Auxiliary
  /// class. For the Hessian term only the lower triangle is updated. The other
  /// triangle needs to be copied later from the lower one.
  ...
  /// Fills the upper triangle of the Hessian matrix with the values from the
  /// lower triangle
  /// @param chiSqObj: Reference to the chiSqObj carrying the Hessian
  void symmetrizeHessian(ChiSqWithDerivatives& chiSqObj) const;
  /// Returns the previously calculated residual.
  const Vector& residual() const;
  /// Returns the gradient of the previously calculated residual
  ...
  /// Returns whether the passed parameter describes a direction angle
  static constexpr bool isDirectionParam(const FitParIndex param) {
    return param == FitParIndex::theta || param == FitParIndex::phi;
  }
  /// Returns whether the passed parameter describes the displacementin the
  /// reference plane
  static constexpr bool isPositionParam(const FitParIndex param) {
    return param == FitParIndex::x0 || param == FitParIndex::y0;
  }
  /// Extrapolates the straight line onto the space point's strip-plane defined
  /// by the `localPosition()` and the `planeNormal()`
  ...
  /// Extrapolates the straight line parametrized by a point and a direction
  /// onto the space point's strip-plane defined by the `localPosition()` and
  /// the `planeNormal()`
  ...
  /// Calculates the spaztial chiSq term of a composite space point to a line
  /// @param line: Reference to the line of interest
  /// @param hit: Reference to the hit of interest
  template <CompositeSpacePoint SpacePoint_t>
  static double chi2Term(const Line_t& line, const SpacePoint_t& hit);
  /// Calculates the spatial chiSq term of a composite space point to a line
  /// parameterized by a position & direction vector
  /// @param pos: Point on the line
  /// @param dir: Direction of the line
  /// @param hit: Reference to the hit of interest
  template <CompositeSpacePoint SpacePoint_t>
  static double chi2Term(const Vector& pos, const Vector& dir,
                         const SpacePoint_t& hit);
  /// Calculates the chiSq term of a composite space point to a line taking into
  /// account the time offset t0
  /// @param line: Reference to the line of interest
  ...
  static double chi2Term(const Vector& pos, const Vector& dir, const double t0,
                         const SpacePoint_t& hit);
  /// Calculates the chiSq term of a composite space point taking into account
  /// the time offset t0 & the time of arrival of the particle assuming the
  /// speed of light
  /// @param line: Reference to the line of interest
  /// @param t0: Time off set evaluated at the measurement's plane
  /// @param hit: Reference to the hit of interest
  ...
  static double chi2Term(const Vector& pos, const Vector& dir,
                         const Acts::Transform3& localToGlobal, const double t0,
                         const SpacePoint_t& hit);
  /// Calculate whether the track passed on the left (-1) or the right (1) side
  ...
  /// Calculate whether a generic line is on the left (-1) or right (1) side
  ...
  /// Calculate the straw signs for a set of measurements
  /// @param line: Reference to the line to which the residual is calculated
  ...
  static std::vector<int> strawSigns(const Vector& pos, const Vector& dir,
                                     const StrawCont_t& measurements);
  /// Resets the residual and all partial derivatives to zero.
  void reset();

 private:
  /// Reference to the logging object
  const Logger& logger() const { return *m_logger; }
  ///  Update the auxiliary variables needed to calculate the residuals of a
  ///  straw measurement to the current line. They are the projection of the
  ///  line direction vector into the straw's wire plane and its derivatives.
  ///  Returns false if line and wire are parallel to each other
  ...
  /// Updates the residuals of a straw measurement to a line ignoring the
  /// distance along the straw wire. Returns false if the residual cannot be
  /// updated due to parallelism between straw wire and line
  /// @param line: Reference to the line to which the residual is calculated
  ...
  /// Calculates the along-the wire component of the straw measurement's
  /// residual to the line
  /// @param line: Reference to the line to which the residual is calculated
  ...
  /// Calculates the residual of a strip measurement w.r.t. the line
  /// @param line: Reference to the line to which the residual is calculated
  ...
  /// Calculates the residual of a strip measurement w.r.t. the time offset
  /// parameter
  /// @param sensorN: Reference to the first basis vector inside the strip measruement plane,
  ///            which is given by the sensor normal
  /// @param sensorD: Reference to the second basis vector inside the strip measruement plane,
  ...
  /// Calculates the residual derivatives of a straw tube measurement when the
  /// drift radius is an implicit function of the point of closest approach and
  ...
  /// Resets the time residual and the partial derivatives
  void resetTime();
  Config m_cfg{};
  std::unique_ptr<const Logger> m_logger{};

  /// Cached residual vector calculated from the measurement & the parametrized
  /// line
  Vector m_residual{Vector::Zero()};
  /// Partial derivatives of the residual w.r.t. the fit parameters parameters
  std::array<Vector3, s_nPars> m_gradient{
      filledArray<Vector3, s_nPars>(Vector3::Zero())};
  ///  Second partial derivatives of the residual w.r.t. the fit parameters
  ///  parameters
  ...
  /// Gradient vector of the point of closest approach
  std::array<Vector, s_nLinePars> m_gradCloseApproach{
      filledArray<Vector, s_nLinePars>(Vector::Zero())};
  /// Partial derivative of the actual distance of the closest approach
...
ACTS_OSTREAM_FORMATTER(
    Acts::Experimental::detail::CompSpacePointAuxiliaries::FitParIndex);
ACTS_OSTREAM_FORMATTER(
