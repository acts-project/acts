...
void StrawLineFitAuxiliaries::updateSpatialResidual(const Line_t& line,
  ...
  if (spacePoint.isStraw()) {
    ...
    if (spacePoint.measNonPrecCoord()) {
    ...
    const Vector3& b1 = spacePoint.measPrecCoord()
                            ? spacePoint.sensorNormal()
                            : spacePoint.sensorDirection();
    const Vector3& b2 = spacePoint.measPrecCoord()
                            ? spacePoint.sensorDirection()
                            : spacePoint.sensorNormal();

    updateStripResidual(line, spacePoint.planeNormal(), b1, b2,
