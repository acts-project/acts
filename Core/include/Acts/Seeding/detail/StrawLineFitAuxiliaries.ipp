...
#ifdef STONJEK
  if (hit.dimension() == 2) {
    m_residual[nonBending] = (hitMinSeg.dot(line.direction()) * m_wireProject -
        ...
        if (hit.dimension() == 2) {
          const double partialSqAlongWire =
              ...
              hitMinSeg.dot(line.hessian[lineIdx]) * m_wireProject *
                  m_invProjLenSq +
              hitMinSeg.dot(line.dir) * partSqLineProject * m_invProjLenSq +
        ...
        if (hit.dimension() == 2) {
          const double partialSqAlongWire =
              -(line.gradient[param1].dot(line.gradient[param]) *
                    m_wireProject +
                line.gradient[param1].dot(line.dir) * m_gradProjDir[param]) *
                  m_invProjLenSq +
