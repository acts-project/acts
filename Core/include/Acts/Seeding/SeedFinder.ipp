...
template <typename external_spacepoint_t, typename platform_t>
inline void SeedFinder<external_spacepoint_t, platform_t>::
    filterCandidatesDetailedMeasurements(
        Acts::SpacePointData& spacePointData,
        const InternalSpacePoint<external_spacepoint_t>& spM,
        const Acts::SeedFinderOptions& options,
  ...
  for (const std::size_t& b : sorted_bottoms) {
    ...
    for (size_t index_t = t0; index_t < numTopSP; index_t++) {
      ...
      if (S2 < B2 * options.minHelixDiameter2) {
        // std::cout << "S2 " << S2 << " " << B2 << " " <<
        // 1/options.minHelixDiameter2 << std::endl;
        continue;
      }
      ...
      float p2scatterSigma = iHelixDiameter2 * sigmaSquaredSPtDependent;
      // if deltaTheta larger than allowed scattering for calculated pT, skip
      // std::cout << "deltaCotTheta2 " << deltaCotTheta2 << " > " << error2 <<
      // " + " << p2scatterSigma << std::endl; std::cout << " cotThetaB -
      // cotThetaT " << cotThetaB << " " << cotThetaT << std::endl;
      if (deltaCotTheta2 > (error2 + p2scatterSigma)) {
        continue;
      }
      // A and B allow calculation of impact params in U/V plane with linear
