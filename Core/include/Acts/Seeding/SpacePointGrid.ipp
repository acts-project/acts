...
Acts::SpacePointGridCreator::createGrid(
  ...
  if (config.bFieldInZ == 0) {
    ...
    // Divide by config.numPhiNeighbors since we combine config.numPhiNeighbors number
    // of consecutive phi bins in the seed making step. So each individual
    // bin should cover 1/config.numPhiNeighbors of the maximum expected azimutal deflection
    float deltaPhi =
        (outerAngle - innerAngle + deltaAngleWithMaxD0) / config.numPhiNeighbors;
