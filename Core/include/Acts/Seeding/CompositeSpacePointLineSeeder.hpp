...
namespace detail {
/// Concept for a utility class to fill the space points from
...
concept CompositeSpacePointSeedFiller =
    ...
    requires(const SeedFiller_t& filler, const CalibrationContext& cctx,
      ...
      /// Utility function to choose the good straw space points for seeding
      /// @param testSp: Reference to the straw-type measurement to test
      { filler.goodCandidate(testSp) } -> std::same_as<bool>;
      /// Calculates the pull of the space point w.r.t. to the
      ...
      /// Returns the radius of the straw tube in which the space point is
      /// recorded
      /// @param testSp: Reference to the straw space point
      { filler.strawRadius(testSp) } -> std::same_as<double>;
      /// Creates a new empty container to construct a new segment seed
      ...
      /// Appends the candidate candidate space point to the segment
      ...
      /// Helper method to send a stop signal to the line seeder, if for
      /// instance, the two layers are too close to each other. The method is
...
/// Define the concept of the space point measurement sorter. The sorter shall
/// take a collection of station space points and split them into straw &&
/// strip hits. Hits from each category are then subdivided further into the
/// particular detector layers.
///
...
concept CompositeSpacePointSorter =
    ...
    requires(const Splitter_t& sorter) {
      /// Return the straw-hit space point sorted by straw layer
      {
        sorter.strawHits()
      } -> std::same_as<const std::vector<SpacePointCont_t>&>;
      /// Return the strip-hit  space points sorted by detector layer
      {
        sorter.stripHits()
      } -> std::same_as<const std::vector<SpacePointCont_t>&>;
    };
...
/// Initial line parameters from a pattern recognition like
...
class CompositeSpacePointLineSeeder {
 public:
  /// Use the assignment of the parameter indices from the
  /// CompSpacePointAuxiliaries
  ...
  /// Abrivation of the straight line. The first element is the
  /// reference position and the second element is the direction
  using Line_t = std::pair<Vector, Vector>;

  /// Enumeration to pick one of the four tangent lines to the straw circle
  /// pair.
  ...
  /// Helper struct describing the line parameters that are
  ...
  struct TwoCircleTangentPars {
    ...
    double dY0{0.};
    /// Flag indicating which solution is constructed
   ...
   protected:
    /// Actual implementation of the printing
    /// @param ostr: Mutable reference to the stream to print to
    virtual void print(std::ostream& ostr) const;
  };

  /// Configuration of the cuts to sort out generated seeds with poor quality.
  struct Config {
    /// Cut on the theta angle
    std::array<double, 2> thetaRange{0., 0.};
    /// Cut on the intercept range
    std::array<double, 2> interceptRange{0., 0.};
    /// Upper cut on the hit chi2 w.r.t. seed in order to be associated to the
    /// seed
    double hitPullCut{5.};
    /// How many drift circles may be on a layer to be used for seeding
    std::size_t busyLayerLimit{2};
    /// Layers may contain measurements with bad hits and hence the
    bool busyLimitCountGood{true};
    /// Try at the first time the external seed parameters as candidate
    bool startWithPattern{false};
    /// Use explicitly the line distance and the driftRadius to calculate
    /// the pull from the seed line to the space point.
    bool useSimpleStrawPull{true};
    /// How many drift circle hits needs the seed to contain in order to be
    /// valid
    ...
    /// Once a seed with even more than initially required hits is found,
    /// reject all following seeds with less hits
    bool tightenHitCut{true};
    /// Check whether a new seed candidate shares the same left-right solution
    /// with already accepted ones. Reject the seed if it has the same amount of
    /// hits
  ...
  friend std::ostream& operator<<(std::ostream& ostr, const TangentAmbi ambi) {
    return ostr << toString(ambi);
  }

  /// Translate the combination of two drift signs into the proper
  ...
  /// Construct the line that is tangential to a pair of two straw circle
  /// measurements
  ...
  /// Creates the direction vector from the reference hit used to
 ...
 private:
  /// Cache object of a constructed & valid seed solution.
  ...
  struct SeedSolution : public TwoCircleTangentPars {
    /// Constructor taking the constructed tangential parameters &
    ...
    /// Abrivation of the underlying space point reference
    using SpacePoint_t = ConstDeRef_t<typename UnCalibCont_t::value_type>;
    /// Helper function to calculate the straw signs of the seed hits
    ...
    /// Returns the number of hits cached by the seed
    std::size_t size() const { return m_seedHits.size(); }
    /// Returns the i-th seed hit
    /// @param idx: Index of the hit to to return
    SpacePoint_t getHit(const std::size_t idx) const;
    /// Appends a new seed hit to the solution
    void append(const std::size_t layIdx, const std::size_t hitIdx);
    /// Vector of the associate left-rignt ambiguities
    std::vector<int> solutionSigns{};
    /// Number of good straw measurements
    std::size_t nStrawHits{0ul};

   private:
    /// Pointer to the space point per layer splitter to gain access to the
    /// input space point container
    const Splitter_t& m_splitter;
    /// Set of hits collected onto the seed. For each element
    ...
    /// Prints the seed solution to the screen
    /// @param ostr: Mutable reference to the stream to print to
    void print(std::ostream& ostr) const final;
  };

 public:
  /// Helper struct to pack the parameters and the associated
  ...
  struct SegmentSeed {
    /// Constructor taking the seed parameters && a new hit container
    ...
    /// Seed line parameters
    SeedParam_t parameters;
    /// Collection of hits
    contType_t hits;
  };

  /// Central auxiliary struct to steer the seeding process.
  ...
  struct SeedingState : public Delegate_t {
    /// Declare the public constructor by explicitly forwarding the
    ...
    /// (Needed to combine the precision parameters with a non-precision
    /// estimate)
    ...
    /// Stringstream output operator
    friend std::ostream& operator<<(std::ostream& ostr,
                                    const SeedingState& opts) {
      opts.print(ostr);
      return ostr;
    }
    /// Return the number of generated seeds
   ...
   private:
    /// List of straw measurement already constructed straw measurement seeds
    std::vector<SeedSolution<UncalibCont_t, Delegate_t>> m_seenSolutions{};
    /// Index of the upper layer under consideration for the seeding
    std::optional<std::size_t> m_upperLayer{std::nullopt};
    /// Index of the lower layer under consideration for the seeding
    std::optional<std::size_t> m_lowerLayer{std::nullopt};
    /// Index of the hit in the lower layer under consideration for the
    /// seeding
    std::size_t m_lowerHitIndex{0ul};
    /// Index of the hit in the upper layer under consideration for the
    /// seeding
    std::size_t m_upperHitIndex{0ul};
    /// Index of the sign combination under consideration for the seeding
    std::size_t m_signComboIndex{0ul};
    /// Number of minimum straw hits a seed must have
    std::size_t m_nStrawCut{0ul};
    /// Flag toggling whether the upper of the lower layer shall be moved
    bool m_moveUpLayer{true};
    /// Flag toggling whether the pattern parameters shall be returned as
    /// first seed
    bool m_patternSeedProduced{false};
    /// Prints the seed solution to the screen
    void print(std::ostream& ostr) const;
    /// Estimated parameters from pattern
    SeedParam_t m_initialPars{};
  };
  /// Main interface method provided by the SeederClass. The user instantiates
 ...
 private:
  ...
  /// Abrivation of the selector delegate to skip invalid straw hits in the seed
  template <CompositeSpacePointContainer Cont_t>
  using Selector_t = Delegate<bool(ConstDeRef_t<typename Cont_t::value_type>)>;
  /// Abrivation of the split hit containers
  template <CompositeSpacePointContainer Cont_t>
  using StrawLayers_t = std::vector<Cont_t>;
  /// Counts the number of hits inside the container. Depending on whether
  ...
  /// Moves to the hit index to the next good hit inside the layer.
  ...
  /// Sets the parsed index to the first good hit inside the straw layer.
  /// @param hitVec: Reference to  the straw hits inside the layer
  /// @param selector: Delegate method to skip bad bad hits
  /// @param hitIdx: Mutable reference to the index that incremented
  template <CompositeSpacePointContainer UnCalibCont_t>
  bool firstGoodHit(const UnCalibCont_t& hitVec,
                    const Selector_t<UnCalibCont_t>& selector,
                    std::size_t& hitIdx) const;
  /// Move the layer index towards the possible value or,if the
  /// layer index is not yet initializes the lyaer index to the next possible
  /// value.
  ...
  /// @param moveForward: Flag toggling whether the layer index shall be incremented or decremented.
  template <CompositeSpacePointContainer UnCalibCont_t>
  ...
  /// Move the layer and hit indices inside the state towards the next
  /// candidate
  ...
  void moveToNextCandidate(
      const Selector_t<UncalibCont_t>& selector,
      SeedingState<UncalibCont_t, CalibCont_t, Delegate_t>& state) const;
  /// Attempts to construct the next seed from the given configuration of
  ...
  std::optional<SegmentSeed<CalibCont_t>> buildSeed(
      ...
      SeedingState<UncalibCont_t, CalibCont_t, Delegate_t>& state) const;
  /// Checks whether the new seed candidate passes the quality cuts on
  ...
  bool passSeedCuts(
      ...
      SeedingState<UncalibCont_t, CalibCont_t, Delegate_t>& state) const;
  /// Converts the accepted seed solution to the segment seed returned by
  ...
  /// Construct the final seed parameters by combining the initial
  ...
  /// Constructs a line from the parsed seed parameters. The first element is
  /// the reference point && the second one is the direction
  /// @param pars: Reference to the line parameters from which the line is created
  Line_t makeLine(const SeedParam_t& pars) const;
  /// Check whether the generated seed parameters are within the ranges defined
  /// by the used
  ...
  /// Array encoding the four possible left right solutions.
