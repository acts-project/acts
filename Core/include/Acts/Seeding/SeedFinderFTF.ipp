...
template <typename external_spacepoint_t>
void SeedFinderFTF<external_spacepoint_t>::runGNN_TrackFinder(std::vector<GNN_TrigTracklet<external_spacepoint_t>>& vTracks, const Acts::RoiDescriptor& roi, const Acts::TrigFTF_GNN_Geometry<external_spacepoint_t>& gnngeo){
  ...
  for(std::map<int, std::vector<FasTrackConnector::LayerGroup> >::const_iterator it = fastrack.m_layerGroups.begin();it!=fastrack.m_layerGroups.end();++it, currentStage++) {
    ...
    for(const auto& layerGroup : (*it).second) {
      ...
      const TrigFTF_GNN_Layer<external_spacepoint_t>* pL1 = gnngeo.getTrigFTF_GNN_LayerByKey(dst);
      ...
      for(const auto& conn : layerGroup.m_sources) {//loop over L2(L1) for the current stage
        ...
        const TrigFTF_GNN_Layer<external_spacepoint_t>* pL2 = gnngeo.getTrigFTF_GNN_LayerByKey(src);
        ...
        for(int b1=0;b1<nDstBins;b1++) {//loop over bins in Layer 1
          ...
          const TrigFTF_GNN_EtaBin<external_spacepoint_t>& B1 = m_storage->getEtaBin(pL1->m_bins.at(b1));
          ...
          for(int b2=0;b2<nSrcBins;b2++) {//loop over bins in Layer 2
            ...
            const TrigFTF_GNN_EtaBin<external_spacepoint_t>& B2 = m_storage->getEtaBin(pL2->m_bins.at(b2));
            ...
            for(typename std::vector<TrigFTF_GNN_Node<external_spacepoint_t>*>::const_iterator n1It = B1.m_vn.begin();n1It!=B1.m_vn.end();++n1It) {//loop over nodes in Layer 1
              ...
              for(unsigned int n2PhiIdx = first_it; n2PhiIdx<B2.m_vPhiNodes.size();n2PhiIdx++) {//sliding window over nodes in Layer 2
                ...
                TrigFTF_GNN_Node<external_spacepoint_t>* n2 = B2.m_vn.at(B2.m_vPhiNodes.at(n2PhiIdx).second);
  ...
  for(int nodeIdx=0;nodeIdx<nNodes;nodeIdx++) {
    ...
    for(int inIdx = 0;inIdx<static_cast<int>(pN->m_in.size());inIdx++) {
      ...
      Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pS = &(edgeStorage.at(inEdgeIdx));
    ...
    for(int outIdx = 0;outIdx<static_cast<int>(pN->m_out.size());outIdx++) {
      ...
      Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pS = &(edgeStorage.at(outEdgeIdx));
    ...
    for(unsigned int in_idx=0;in_idx<in_sort.size();in_idx++) {//loop over incoming edges
      ...
      for(unsigned int out_idx = last_out;out_idx<out_sort.size();out_idx++) {
        ...
        Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pNS = &(edgeStorage.at(outEdgeIdx));
  ...
  for(int edgeIndex=0;edgeIndex<nEdges;edgeIndex++) {
    ...
    Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pS = &(edgeStorage.at(edgeIndex));
  ...
  for(;iter<maxIter;iter++) {
    ...
    for(auto pS : v_old) {
      ...
      for(int nIdx=0;nIdx<pS->m_nNei;nIdx++) {
        ...
        Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pN = &(edgeStorage.at(nextEdgeIdx));
  ...
  std::sort(vSeeds.begin(), vSeeds.end(), typename Acts::TrigFTF_GNN_Edge<external_spacepoint_t>::CompareLevel());
  ...
  for(auto pS : vSeeds) {
    ...
    for(typename std::vector<Acts::TrigFTF_GNN_Edge<external_spacepoint_t>*>::reverse_iterator sIt=rs.m_vs.rbegin();sIt!=rs.m_vs.rend();++sIt) {
      ...
      for(unsigned int idx_o = idx_m+1; idx_o < vSP.size(); idx_o++) {
        ...
        for(unsigned int idx_i = 0; idx_i < idx_m; idx_i++) {
          ...
          nTriplets++;
	        ...
	        if(nTriplets >= m_config.m_maxTripletBufferLength) break;
	      }
	      if(nTriplets >= m_config.m_maxTripletBufferLength) break;
      }
      if(nTriplets >= m_config.m_maxTripletBufferLength) break;
...
template <typename external_spacepoint_t>
void SeedFinderFTF<external_spacepoint_t>::createSeeds(const Acts::RoiDescriptor& roi, const Acts::TrigFTF_GNN_Geometry<external_spacepoint_t>& gnngeo){
