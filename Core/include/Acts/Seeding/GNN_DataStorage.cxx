...
//need to find/make the equivelant of these files 
//#include "TrigInDetEvent/TrigSiSpacePointBase.h" //dont think i need this, doing template version 
// #include "InDetPrepRawData/PixelCluster.h" //need this for cluster width 
// #include "TrigInDetPattRecoEvent/TrigInDetSiLayer.h" //loks like just use for if it is a barral or not 
...
TrigFTF_GNN_Node::TrigFTF_GNN_Node(const space_point_t& p, float minT = -100.0, float maxT = 100.0)  : m_sp(p), m_minCutOnTau(minT), m_maxCutOnTau(maxT) {
  m_in.clear();
  m_out.clear();
}
...
template <typename space_point_t>  
int TrigFTF_GNN_DataStorage::addSpacePoint(const space_point_t& sp, bool useML = false) {
