...
namespace Acts {
struct SeedFilterConfigNA60 {
  // the allowed delta between two inverted seed radii for them to be considered
  // compatible.
  float deltaInvHelixDiameter = 0.01 * 1. / Acts::UnitConstants::mm;  //MODIFIED
  ...
  float impactWeightFactor = 100.;
  // the logitudinal impact parameters (z0) is multiplied by this factor and
  // subtracted from weight
  float zOriginWeightFactor = 100.;
  // seed weight increased by this value if a compatible seed has been found.
  float compatSeedWeight = 100.;
  // minimum distance between compatible seeds to be considered for weight boost
  float deltaYMin = 50. * Acts::UnitConstants::mm;   //MODIFIED --> to be changed if I use more layers
  ...
  SeedFilterConfigNA60 toInternalUnits() const {
    if (isInInternalUnits) {
      throw std::runtime_error(
          "Repeated conversion to internal units for SeedFilterConfigNA60");
    }
    using namespace Acts::UnitLiterals;
    SeedFilterConfigNA60 config = *this;
