...
class ITripletSeedCuts {
 public:
  ...
  virtual void cutPerMiddleSp(
      std::span<TripletCandidate>& seedCandidates) const = 0;
};
...
class BroadTripletSeedFilter final : public ITripletSeedFilter {
 public:
  ...
  struct Config {
    ...
    /// Maximum number (minus one) of seeds kept per middle space-point
    ...
    /// Maximum number of normal-quality seeds kept in seed confirmation
  ...
  struct Cache {
    ...
    std::vector<TripletCandidate> sortedCandidates;
  };
