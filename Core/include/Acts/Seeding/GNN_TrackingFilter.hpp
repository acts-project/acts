...
class TrigFTF_GNN_TrackingFilter {
 public:
  TrigFTF_GNN_TrackingFilter(const std::vector<Acts::TrigInDetSiLayer>& g, std::vector<Acts::TrigFTF_GNN_Edge<external_spacepoint_t>>& sb): m_geo(g), m_segStore(sb) {
  ...
  void followTrack(Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pS, TrigFTF_GNN_EdgeState<external_spacepoint_t>& output){
    ...
    TrigFTF_GNN_EdgeState<external_spacepoint_t>* pInitState = &m_stateStore[m_globalStateCounter++];
    ...
    std::sort(m_stateVec.begin(), m_stateVec.end(), typename TrigFTF_GNN_EdgeState<external_spacepoint_t>::Compare());
 ...
 protected:
  ...
  void propagate(Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pS, TrigFTF_GNN_EdgeState<external_spacepoint_t>& ts){
    ...
    TrigFTF_GNN_EdgeState<external_spacepoint_t>* p_new_ts = &m_stateStore[m_globalStateCounter++];
    ...
    for(int nIdx=0;nIdx<pS->m_nNei;nIdx++) {//loop over the neighbours of this segment
      ...
      Acts::TrigFTF_GNN_Edge<external_spacepoint_t>* pN = &(m_segStore.at(nextSegmentIdx));
    ...
    if(lCont.empty()) {//the end of chain
      ...
      if(m_globalStateCounter < MAX_EDGE_STATE) {
        ...
        if(m_stateVec.empty()) {//add the first segment state
          TrigFTF_GNN_EdgeState<external_spacepoint_t>* p = &m_stateStore[m_globalStateCounter++];
          p->clone(new_ts);
      ...
      int nBranches = 0;
      for(typename std::list<Acts::TrigFTF_GNN_Edge<external_spacepoint_t>*>::iterator sIt = lCont.begin();sIt!=lCont.end();++sIt, nBranches++) {
