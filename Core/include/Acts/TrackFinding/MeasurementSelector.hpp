  ...
  select(std::vector<typename traj_t::TrackStateProxy>& candidates,
    ...
    auto trackStateIterEnd = candidates.end();
    {
       ...
       for (;;) {
          ...
          if (chi2 >= maxChi2Cut || chi2 >= VariableCut<traj_t>(*trackStateIter, cuts, chi2CutOff, logger) ) {
             --trackStateIterEnd;
             // still check whether the element has the smallest chi2 in case an outlier is returned.
             if (chi2<minChi2) {
                minChi2 = chi2;
                // the current element will be swapped with the last unchecked element if they are
                // different
             ...
             ++trackStateIter;
             if (trackStateIter == trackStateIterEnd) break;
          }
       }
    }


    // If there are no measurements below the chi2 cut off, return the measurement with the
    ...
    if (candidates.begin() == trackStateIterEnd) {
      ...
      ACTS_VERBOSE("No measurement candidate. Return an outlier measurement chi2=" << bestIt->chi2());
      isOutlier = true;
    ...
    if ( static_cast<std::size_t>(std::distance(candidates.begin(),trackStateIterEnd)) > numMeasurementsCut && numMeasurementsCut > 0) {
