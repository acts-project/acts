...
bool Acts::PassiveLayerBuilder::constructLayers() const
{
    ...
    if (numcLayers){
        ...
        for (size_t icl = 0; icl < numcLayers; ++icl){
            ...
            std::shared_ptr<const CylinderBounds> cBounds(new CylinderBounds(m_config.centralLayerRadii.at(icl),m_config.centralLayerHalflengthZ.at(icl)));
            // create the layer
            ...
            if (m_config.centralLayerMaterialX0.size()){
                ...
                material = std::shared_ptr<const SurfaceMaterial>(new HomogeneousSurfaceMaterial(MaterialProperties(m_config.centralLayerThickness.at(icl),
    ...
    if (numpnLayers){
        ...
        for (size_t ipnl = 0; ipnl < numpnLayers; ++ipnl){
            ...
            std::shared_ptr<const DiscBounds> dBounds(new RadialBounds(m_config.posnegLayerRmin.at(ipnl), m_config.posnegLayerRmax.at(ipnl)));
            ...
            if (m_config.posnegLayerMaterialX0.size()){
                // create homogeneous material
                material = std::shared_ptr<const SurfaceMaterial>(new HomogeneousSurfaceMaterial(MaterialProperties(m_config.posnegLayerThickness.at(ipnl),
