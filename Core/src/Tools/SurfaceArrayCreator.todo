...
{
  MSG_DEBUG("Creating a SurfaceArray on a cylinder with bins in phi x z = "
            << binsPhi
            << " x "
            << binsZ);

  // create the (plain) binUtility - with the transform
  BinUtility* arrayUtility
      = new BinUtility(binsPhi, minPhi, maxPhi, closed, binPhi, transform);
...
/** SurfaceArrayCreator interface method - create an array on a disc, binned in
 * r, phi */
std::unique_ptr<Acts::SurfaceArray>
Acts::SurfaceArrayCreator::surfaceArrayOnDisc(
    ...
    std::shared_ptr<Acts::Transform3D>       transform) const
{
  MSG_DEBUG("Creating a SurfaceArray on a disc with bins in r x phi = "
            << binsR
            << " x "
            << binsPhi);
  ...
  if (binsR == 1) {
    MSG_DEBUG("Only one single ring is R is present - 1D surface array to be "
              "created.");
    ...
    auto sArray = std::make_unique<BinnedArray1D<const Surface*>>(sVector,
                                                                  arrayUtility);
    // register the neighbours
  ...
  // the bin direction / @TODO should actually be +1, -1, gets important for
  // intersection sorting method
...
{
  MSG_DEBUG("Complete binning by filling closest neighbour surfaces in "
  ...
  for (auto& surface : surfaces) {
    ...
    MSG_VERBOSE("- estimated bin [ " << bin0 << " x " << bin1
                                     << " ] for surface "
                                     << surface);
    // fill it
  ...
  MSG_VERBOSE("Number of empty bins that were filled with neighbours: "
              << completedBins);
  ...
  for (auto& outer : binSystem)
    for (auto& inner : outer) {
      MSG_VERBOSE("- bin [ " << binUtility.bin(inner.first.second, 0) << " x "
                             << binUtility.bin(inner.first.second, 1)
                             << " ] holds surface "
                             << inner.first.first);
...
void
Acts::SurfaceArrayCreator::registerNeighboursGrid(
    ...
    bool                                            open1) const
{
  ...
  MSG_DEBUG("Registering the neigbours for " << bins0 << " x " << bins1
                                             << " bins.");
  ...
  for (size_t i1 = 0; i1 < bins1; ++i1) {
    for (size_t i0 = 0; i0 < bins0; ++i0) {
      ...
      MSG_VERBOSE(
          "Processing neighbour registration for bin [" << i0 << "][" << i1
                                                        << "] - surface is "
                                                        << surface);
      ...
      MSG_VERBOSE("    element neighbour search for grid [" << p0 << " | " << i0
                                                            << " | "
                                                            << n0
                                                            << "]  x [ "
                                                            << p1
                                                            ...
                                                            << " ]");
      // bools for breaking
      bool processNext1 = true;
      bool zero1passed  = false;
      ...
      for (size_t ipn1 = p1; processNext1; increment(ipn1, bins1, open1)) {
        ...
        bool processNext0 = true;
        bool zero0passed  = false;
        for (size_t ipn0 = p0; processNext0; increment(ipn0, bins0, open0)) {
          ...
          // skip the main bin
          if (ipn0 == i0 && ipn1 == i1) {
            MSG_VERBOSE("                       - skipping the bin [" << ipn0
                                                                      << "]["
                                                                      << ipn1
                                                                      << "]");
            continue;
          }
          const DetectorElementBase* pnElement
              = surfaceArrayObjects.at(ipn1).at(ipn0)
              ? surfaceArrayObjects.at(ipn1)
                    .at(ipn0)
                    ->associatedDetectorElement()
              : nullptr;
          MSG_VERBOSE(
              "                       - neighbour at bin [" << ipn0 << "]["
                                                            << ipn1
                                                            << "] - element is "
                                                            << pnElement);
          if (element != pnElement && pnElement) {
            // we can prevent double filling already here
            if (std::find(neighbourElements.begin(),
                          neighbourElements.end(),
                          pnElement)
                == neighbourElements.end())
      ...
      // standard is 8 cell connectivity, but on open borders it only knows 5
      // cells
      ...
      if (bins0 > 1 && bins1 > 1 && neighbours != 8) {
        ...
        correctlyConnected = (((open0 && (p0 == i0 || n0 == i0))
                               || (open1 && (p1 == i1 || n1 == i1)))
                              && neighbours == 5);
        expectedNeighbours = ((open0 && (p0 == i0 || n0 == i0))
                              || (open1 && (p1 == i1 || n1 == i1)))
            ? 5
            : 8;
