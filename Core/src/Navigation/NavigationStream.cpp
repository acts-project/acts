...
const GeometryContext& gctx,
  ...
  for (auto& [sIntersection, portal, bTolerance ] :
       m_candidates) {
  ...
  m_candidates.resize(
      std::distance(m_candidates.begin(), firstInvalid));
...
void Acts::NavigationStream::addSurfaceCandidates(const std::vector<const Surface*>& surfaces,
                           BoundaryTolerance bTolerance) {
    std::for_each(surfaces.begin(), surfaces.end(), [&](const auto* surface) {
      m_candidates.push_back(Candidate{ObjectIntersection<Surface>::invalid(surface),
                                     nullptr, bTolerance});
    });
  } 
