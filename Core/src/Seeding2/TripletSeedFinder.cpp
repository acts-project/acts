...
inline StripData deriveOuterStripSpacePointCalibrationDetails(
    const std::array<float, 3> bsv, const std::array<float, 3> tsv,
    const std::array<float, 3> scd, const std::array<float, 3> tcv) {
  return StripData{
...
class Impl final : public TripletSeedFinder {
 public:
  ...
  void createStripTripletTopCandidates(
    ...
    const StripData calM = deriveOuterStripSpacePointCalibrationDetails(
    ...
    const StripData calB = deriveOuterStripSpacePointCalibrationDetails(
    ...
    for (auto [topDoublet, topDoubletIndex] :
      ...
      if (!stripCoordinateCheck(m_cfg.toleranceParam, calM, directionMiddle,
                                rMTransf)) {
        continue;
      }
      ...
      if (!stripCoordinateCheck(m_cfg.toleranceParam, calB, directionBottom,
                                rBTransf)) {
        continue;
      }
      ...
      if (!stripCoordinateCheck(m_cfg.toleranceParam, spT, directionTop,
                                rTTransf)) {
        continue;
      }
