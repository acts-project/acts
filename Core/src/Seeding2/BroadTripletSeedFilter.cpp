...
void BroadTripletSeedFilter::filterTripletTopCandidates(
  ...
  for (const std::size_t topSpIndex : cache().topSpIndexVec) {
    ...
    // Delta-eta weight: penalise seeds where the seed direction (eta) is
    // inconsistent with the vector from beamspot to the point of closest
    // approach. Real displaced-decay seeds have small delta-eta.
