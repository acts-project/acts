  ...
  void createDoubletsImpl(const ConstSpacePointProxy& middleSp,
    ...
    for (auto [indexO, xyO, zrO, varianceZO, varianceRO] : candidateSps.zip(
      ...
      if constexpr (!interactionPointCut) {
        ...
        // discard doublets based on experiment specific cuts (also applied here,
        // in the interactionPointCut == false branch, not only in the branch
        // below -- otherwise the cut would silently never run when the IP cut is
        // disabled)
