...
Acts::FreeVector Acts::estimateTrackParamsFromSeed(const Vector3& sp0,
  ...
  long double deltaX21 = local2(0) - local1(0);
  long double sumX21 = local2(0) + local1(0);
  ...
  long double ia = deltaX21 / local2(1);
  ...
  long double b = 0.5 * (local2(1) + ia * sumX21);
  ...
  int sign = ia > 0 ? -1 : 1;
  const long double R = circleCenter.norm();
  long double invTanTheta =
      local2.z() / (2 * R * std::asin(local2.head<2>().norm() / (2 * R)));
  // The momentum direction in the new frame (the center of the circle has the
  // coordinate (-1.*A/(2*B), 1./(2*B)))
  long double A = -circleCenter(0) / circleCenter(1);
  ...
  long double qOverPt = sign / (bField.norm() * R);
