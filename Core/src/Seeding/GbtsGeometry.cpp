...
std::int32_t GbtsLayer::getEtaBin(float zh, float rh) const {
  if (m_bins.size() == 1) {
    return m_bins.at(0);
  }
  ...
  if (idx >= static_cast<std::int32_t>(m_bins.size())) {
...
const GbtsLayer* GbtsGeometry::getGbtsLayerByKey(std::uint32_t key) const {
  std::map<std::uint32_t, GbtsLayer*>::const_iterator it = m_layMap.find(key);
  if (it == m_layMap.end()) {
    return nullptr;
  }
  return (*it).second;
...
const GbtsLayer* GbtsGeometry::addNewLayer(const TrigInDetSiLayer& l,
  ...
  m_layMap.insert(std::pair<std::uint32_t, GbtsLayer*>(layerKey, ref));
