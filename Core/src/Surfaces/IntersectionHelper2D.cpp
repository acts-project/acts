...
Intersection2D IntersectionHelper2D::intersectSegment(const Vector2& s0,
                                                      ...
                                                      const Vector2& dir,
                                                      bool boundCheck) {
  using Line = Eigen::ParametrizedLine<long double, 2>;
  using Plane = Eigen::Hyperplane<long double, 2>;

  Vector2 edge(s1 - s0);
  long double det = edge.x() * dir.y() - edge.y() * dir.x();
...
std::array<Intersection2D, 2> IntersectionHelper2D::intersectEllipse(
    long double Rx, long double Ry, const Vector2& origin, const Vector2& dir) {
  auto createSolution =
      [&](const Vector2& sol,
    ...
    long double solD = std::copysign(toSolD.norm(), toSolD.dot(dir));
    long double altD = std::copysign(toAltD.norm(), toAltD.dot(dir));
  ...
  if (std::abs(dir.x()) < s_epsilon) {
    long double solx = origin.x();
    long double D = 1. - solx * solx / (Rx * Rx);
    if (D > 0.) {
      long double sqrtD = std::sqrt(D);
    ...
    long double soly = origin.y();
    long double D = 1. - soly * soly / (Ry * Ry);
    if (D > 0.) {
      long double sqrtD = std::sqrt(D);
  ...
  // General solution
  long double k = dir.y() / dir.x();
  long double d = origin.y() - k * origin.x();
  long double Ry2 = Ry * Ry;
  long double alpha = 1. / (Rx * Rx) + k * k / Ry2;
  long double beta = 2. * k * d / Ry2;
  long double gamma = d * d / Ry2 - 1;
  RealQuadraticEquation solver(alpha, beta, gamma);
  if (solver.solutions == 1) {
    long double x = solver.first;
    Vector2 sol(x, k * x + d);
    Vector2 toSolD(sol - origin);
    long double solD = std::copysign(toSolD.norm(), toSolD.dot(dir));
    return {Intersection2D(sol, solD, IntersectionStatus::reachable),
    ...
    long double x0 = solver.first;
    long double x1 = solver.second;
...
Intersection2D IntersectionHelper2D::intersectCircleSegment(
    long double R, long double phiMin, long double phiMax,
    const Vector2& origin, const Vector2& dir) {
  auto intersections = intersectCircle(R, origin, dir);
  for (const auto& candidate : intersections) {
    if (candidate.pathLength() > 0.) {
      long double phi = VectorHelpers::phi(candidate.position());
