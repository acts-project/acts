...
///////////////////////////////////////////////////////////////////
// SlidingDiscSurface.cpp, ACTS project
///////////////////////////////////////////////////////////////////

// Geometry module
#include "ACTS/Surfaces/SlidingDiscSurface.hpp"
#include "ACTS/Surfaces/DiscBounds.hpp"
#include "ACTS/Utilities/Definitions.hpp"

// default constructor
Acts::SlidingDiscSurface::SlidingDiscSurface()
  : Acts::DiscSurface(), m_depth(), m_etaBin(), m_align()
{
}
...
Acts::SlidingDiscSurface::SlidingDiscSurface(const Acts::DiscSurface&  dsf,
  ...
  : Acts::DiscSurface(dsf), m_depth(offset), m_etaBin(bu), m_align(align)
{
}
...
Acts::SlidingDiscSurface&
Acts::SlidingDiscSurface::operator=(const SlidingDiscSurface& dsf)
{
  if (this != &dsf) {
...
bool
Acts::SlidingDiscSurface::operator==(const Acts::Surface& sf) const
{
  // first check the type not to compare apples with oranges
  const Acts::SlidingDiscSurface* dsf
      = dynamic_cast<const Acts::SlidingDiscSurface*>(&sf);
  if (!dsf) return false;
...
void
Acts::SlidingDiscSurface::localToGlobal(const Acts::Vector2D& locpos,
                                        const Acts::Vector3D&,
...
bool
Acts::SlidingDiscSurface::globalToLocal(const Acts::Vector3D& glopos,
                                        const Acts::Vector3D&,
                                        Acts::Vector2D& locpos) const
{
  Acts::Vector3D loc3D0 = m_align ? m_align->inverse() * glopos
                                  : glopos;  // used to retrieve localEta bin
  Acts::Vector3D loc3Dframe(Acts::Surface::transform().inverse() * glopos);
  locpos = Acts::Vector2D(loc3Dframe.perp(), loc3Dframe.phi());
  return ((fabs(loc3Dframe.z() - (*m_depth)[m_etaBin->bin(loc3D0)])
...
bool
Acts::SlidingDiscSurface::isOnSurface(const Acts::Vector3D&      glopo,
                                      const Acts::BoundaryCheck& bchk) const
{
  Acts::Vector3D loc3D0 = m_align ? m_align->inverse() * glopo
                                  : glopo;  // used to retrieve localEta bin
