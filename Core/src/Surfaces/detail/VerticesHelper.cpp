...
std::vector<long double> detail::VerticesHelper::phiSegments(
    long double phiMin, long double phiMax,
    const std::vector<long double>& phiRefs, unsigned int quarterSegments) {
  ...
  for (long double phiRef : phiRefs) {
  ...
  std::vector<long double> phiSegments = {phiMin, phiMax};
  ...
  for (unsigned int i = 0; i < 4 * quarterSegments + 1; ++i) {
    ...
    if (phiExt > phiMin && phiExt < phiMax &&
        std::ranges::none_of(phiSegments, [&phiExt](long double phi) {
  ...
  for (const auto& phiRef : phiRefs) {
    if (phiRef > phiMin && phiRef < phiMax) {
      if (std::ranges::none_of(phiSegments, [&phiRef](long double phi) {
...
std::vector<Vector2> detail::VerticesHelper::ellipsoidVertices(
    long double innerRx, long double innerRy, long double outerRx,
    long double outerRy, long double avgPhi, long double halfPhi,
...
std::vector<Vector2> detail::VerticesHelper::circularVertices(
    long double innerR, long double outerR, long double avgPhi,
    long double halfPhi, unsigned int quarterSegments) {
...
bool detail::VerticesHelper::onHyperPlane(const std::vector<Vector3>& vertices,
  ...
  auto hyperPlane = Eigen::Hyperplane<long double, 3>::Through(
