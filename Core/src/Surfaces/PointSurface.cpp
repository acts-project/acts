...
Acts::PointSurface::PointSurface(std::shared_ptr<const PointBounds> pbounds,
    ...
    : GeometryObject(), Surface(detelement), m_bounds(std::move(pbounds)) {
  throw_assert(m_bounds, "PointBounds must not be nullptr");
}
...
Acts::Vector3 Acts::PointSurface::localToGlobal(
...
Acts::Result<Acts::Vector2> Acts::PointSurface::globalToLocal(
...
Acts::RotationMatrix3 Acts::PointSurface::referenceFrame(
    const GeometryContext& gctx, const Vector3& /*position*/,
    const Vector3& direction) const {
...
Acts::Vector3 Acts::PointSurface::binningPosition(
    const GeometryContext& gctx, BinningValue /*bValue*/) const {
  return center(gctx);
}

Acts::Vector3 Acts::PointSurface::normal(const GeometryContext& gctx,
                                         const Vector2& /*lpos*/) const {
  const auto& tMatrix = transform(gctx).matrix();
  return Vector3(tMatrix(0, 2), tMatrix(1, 2), tMatrix(2, 2));
}

const Acts::SurfaceBounds& Acts::PointSurface::bounds() const {
  if (m_bounds) {
    return (*m_bounds.get());
  }
  return s_noBounds;
...
Acts::BoundToFreeMatrix Acts::PointSurface::boundToFreeJacobian(
...
Acts::FreeToPathMatrix Acts::PointSurface::freeToPathDerivative(
...
Acts::AlignmentToPathMatrix Acts::PointSurface::alignmentToPathDerivative(
    const GeometryContext& gctx, const FreeVector& parameters) const {
  // The global posiiton
  const auto position = parameters.segment<3>(eFreePos0);
  // The direction
...
Acts::ActsMatrix<2, 3> Acts::PointSurface::localCartesianToBoundLocalDerivative(
