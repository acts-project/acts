...
Acts::PerigeeSurface::PerigeeSurface(const Acts::Vector3D& gp)
  : Surface(), m_lineDirection(nullptr)
{
...
Acts::PerigeeSurface::PerigeeSurface(
    std::shared_ptr<Acts::Transform3D> tTransform)
  : Surface(tTransform), m_lineDirection(nullptr)
{
}

Acts::PerigeeSurface::PerigeeSurface(
    std::unique_ptr<Acts::Transform3D> tTransform)
...
Acts::PerigeeSurface::PerigeeSurface(const PerigeeSurface& pesf)
  : Surface(pesf), m_lineDirection(nullptr)
{
}
...
{
  if (pesf.m_center)
    Surface::m_center = new Acts::Vector3D(shift * (*pesf.m_center));
  if (pesf.m_transform)
    Surface::m_transform = std::shared_ptr<Acts::Transform3D>(
        new Acts::Transform3D(shift * (*pesf.m_transform)));
}
...
// assignment operator
Acts::PerigeeSurface&
Acts::PerigeeSurface::operator=(const Acts::PerigeeSurface& pesf)
{
  if (this != &pesf) {
...
bool
Acts::PerigeeSurface::operator==(const Acts::Surface& sf) const
{
  // first check the type not to compare apples with oranges
  const Acts::PerigeeSurface* persf
      = dynamic_cast<const Acts::PerigeeSurface*>(&sf);
...
void
Acts::PerigeeSurface::localToGlobal(const Acts::Vector2D& locpos,
...
bool
Acts::PerigeeSurface::globalToLocal(const Acts::Vector3D& glopos,
                                    const Acts::Vector3D& glomom,
...
// return the measurement frame - this is the frame where the covariance is
// defined
const Acts::RotationMatrix3D
Acts::PerigeeSurface::measurementFrame(const Acts::Vector3D&,
