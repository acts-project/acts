...
Acts::EllipseBounds::EllipseBounds()
  : Acts::PlanarBounds(), m_boundValues(EllipseBounds::bv_length, 0.)
{
}
  ...
  : Acts::PlanarBounds(), m_boundValues(EllipseBounds::bv_length, 0.)
{
  ...
  if (m_boundValues.at(EllipseBounds::bv_rMinX)
      > m_boundValues.at(EllipseBounds::bv_rMaxX))
    swap(m_boundValues.at(EllipseBounds::bv_rMinX),
         m_boundValues.at(EllipseBounds::bv_rMaxX));
  if (m_boundValues.at(EllipseBounds::bv_rMinY)
      > m_boundValues.at(EllipseBounds::bv_rMaxY))
    swap(m_boundValues.at(EllipseBounds::bv_rMinY),
         m_boundValues.at(EllipseBounds::bv_rMaxY));
}
...
Acts::EllipseBounds::EllipseBounds(const EllipseBounds& discbo)
  : Acts::PlanarBounds(), m_boundValues(discbo.m_boundValues)
{
}
...
Acts::EllipseBounds&
Acts::EllipseBounds::operator=(const EllipseBounds& discbo)
{
...
Acts::EllipseBounds&
Acts::EllipseBounds::operator=(EllipseBounds&& discbo)
{
  if (this != &discbo) m_boundValues = std::move(discbo.m_boundValues);
  return *this;
}

bool
Acts::EllipseBounds::operator==(const Acts::SurfaceBounds& sbo) const
{
  // check the type first not to compare apples with oranges
  const Acts::EllipseBounds* discbo
      = dynamic_cast<const Acts::EllipseBounds*>(&sbo);
...
//
double
Acts::EllipseBounds::minDistance(const Acts::Vector2D& pos) const
{
  ...
  fb = (dF > 0.)
      ? m_boundValues.at(EllipseBounds::bv_averagePhi)
          + m_boundValues.at(EllipseBounds::bv_halfPhiSector)
      : m_boundValues.at(EllipseBounds::bv_averagePhi)
          - m_boundValues.at(EllipseBounds::bv_halfPhiSector);
  sn         = sin(fb);
  cs         = cos(fb);
...
Acts::EllipseBounds::dump(std::ostream& sl) const
{
  ...
  sl << "(" << this->rMinX() << ", " << this->rMinY() << ", " << this->rMaxX()
     << ", " << this->rMaxY() << ", " << this->averagePhi() << ", "
     << this->halfPhiSector() << ")";
