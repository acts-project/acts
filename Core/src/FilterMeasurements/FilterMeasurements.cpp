...
#include "Acts/FilterMeasurements/FilterMeasurements.hpp"
...
static void removeTrack(FilterMeasurements::State& state,
                        std::size_t iTrack) {
...
void FilterMeasurements::resolve(State& state) const {
  ...
  for (std::size_t i = 0; i < m_cfg.maximumIterations; ++i) {
    ...
    //std::cout<<"maximum shared measurements " << state.sharedMeasurementsPerTrack[maximumSharedMeasurements]<<std::endl;
