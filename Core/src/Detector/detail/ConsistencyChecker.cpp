...
void Acts::Experimental::detail::ConsistencyChecker::checkRotationAlignment(
...
Acts::Experimental::detail::ConsistencyChecker::checkCenterAlignment(
    const GeometryContext& gctx,
    const std::vector<std::shared_ptr<Experimental::DetectorVolume>>& volumes,
  ...
  auto refAxis = volumes[0u]->transform(gctx).rotation().col(axisValue);
