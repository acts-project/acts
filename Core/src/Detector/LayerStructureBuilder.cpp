...
void adaptBinningRange(std::vector<Acts::Experimental::ProtoBinning>& pBinning,
  ...
  for (auto& pb : pBinning) {
    ...
    if (extent.constrains(pb.axisDirection)) {
      const auto& range = extent.range(pb.axisDirection);
...
Acts::Experimental::InternalNavigationDelegate createUpdater(
  ...
  Acts::Experimental::detail::IndexedSurfacesGenerator<
      ...
      isg{lSurfaces,
          assignToAll,
          {aBinning.axisDirection, bBinning.axisDirection},
...
Acts::Experimental::LayerStructureBuilder::construct(
  ...
  if (internalSurfaces.size() >= m_cfg.nMinimalSurfaces) {
    ...
    if (binnings.empty()) {
      ...
      if (binning.axisBoundaryType == AxisBoundaryType::Closed) {
        ACTS_VERBOSE("-- closed binning option.");
        internalCandidatesUpdater = createUpdater<AxisBoundaryType::Closed>(
            gctx, internalSurfaces, assignToAll, binning);
      } else {
        ACTS_VERBOSE("-- bound binning option.");
        internalCandidatesUpdater = createUpdater<AxisBoundaryType::Bound>(
            gctx, internalSurfaces, assignToAll, binning);
      }
      ...
      std::ranges::sort(binnings, {},
                        [](const auto& b) { return b.axisDirection; });
      ...
      if (binning0.axisBoundaryType == AxisBoundaryType::Closed) {
        ACTS_VERBOSE("-- closed/bound binning option.");
        internalCandidatesUpdater =
            createUpdater<AxisBoundaryType::Closed, AxisBoundaryType::Bound>(
                gctx, internalSurfaces, assignToAll, binning0, binning1);
      } else if (binning1.axisBoundaryType == AxisBoundaryType::Closed) {
        ACTS_VERBOSE("-- bound/closed binning option.");
        internalCandidatesUpdater =
            createUpdater<AxisBoundaryType::Bound, AxisBoundaryType::Closed>(
                gctx, internalSurfaces, assignToAll, binning0, binning1);
      } else {
        ACTS_VERBOSE("-- bound/bound binning option.");
        internalCandidatesUpdater =
            createUpdater<AxisBoundaryType::Bound, AxisBoundaryType::Bound>(
                gctx, internalSurfaces, assignToAll, binning0, binning1);
      }
    }
  } else {
