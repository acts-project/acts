  ...
  if (container.has_value() && !container.value().constituentVolumes.empty()) {
    ...
    if (!layersPresent) {
      ...
      for (unsigned int iv = 1; iv < cts.constituentVolumes.size(); ++iv) {
        ...
        long double zero = lv.extent.min(binValue);
        long double low = lv.extent.max(binValue);

        auto& hv = cts.constituentVolumes[iv];
        long double high = hv.extent.min(binValue);
        long double mid = 0.5 * (low + high);
        long double max = hv.extent.max(binValue);
      ...
      long double containerMin = extent.min(binValue);
      ...
      long double constituentsMax = lVolume.extent.max(binValue);
      ...
      long double containerMax = extent.max(binValue);
