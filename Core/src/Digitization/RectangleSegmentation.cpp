...
Acts::RectangularSegmentation::RectangularSegmentation(std::shared_ptr<const Acts::RectangleBounds> mBounds,
   ...
   m_binsY(numCellsY)
{
    ...
    if (numCellsY > 1){
        BinUtility yBinUtility(numCellsY, -mBounds->halflengthY(), mBounds->halflengthY(), Acts::open, Acts::binY);
        if (m_binUtility)
...
void 
Acts::RectangularSegmentation::createSegmenationSurfaces(SurfacePtrVector& boundarySurfaces,
                                                         ...
                                                         double lorentzAngle) const
{
    ...
    std::shared_ptr<const PlanarBounds> moduleBounds(new RectangleBounds(m_activeBounds->halflengthX(),
    ...
    if (lorentzAngle == 0.){
        ...
        std::shared_ptr<const PlanarBounds> lorentzReducedBounds(new RectangleBounds(lorentzReducedHalfX,
                                                                                     m_activeBounds->halflengthY()));
        ...
        (*counterPlaneTransform).translation() = Vector3D(counterPlaneShift,0.,-readoutDirection*halfThickness); 
    }
    ...
    // now, let's create the shared bounds of all surfaces marking x bins - choice fixes orientation of the matrix
    std::shared_ptr<const PlanarBounds> xBinBounds(new RectangleBounds(m_activeBounds->halflengthY(),
                                                                       halfThickness));
    ...
    std::shared_ptr<const PlanarBounds> lorentzPlaneBounds = (lorentzAngle==0.) ? xBinBounds :
             std::shared_ptr<const PlanarBounds>(new RectangleBounds(m_activeBounds->halflengthY(),
                                                                     lorentzPlaneHalfX));
    ...
    RotationMatrix3D lorentzPlaneRotationMatrix =  (lorentzAngle !=0.) ? 
        xBinRotationMatrix * AngleAxis3D(lorentzAngle, Vector3D::UnitX()) : xBinRotationMatrix; 
    ...
    for (size_t ibinx = 0; ibinx <= m_binsX; ++ibinx){
           ...
           if (!ibinx || ibinx == m_binsX){
               // check if it a straight boundary or not: always straight for no lorentz angle, 
               // and either the first boundary or the last dependening on lorentz & readout
               bool boundaryStraight = (lorentzAngle == 0. || (!ibinx && readoutDirection*lorentzAngle > 0.) 
                                        || (ibinx==m_binsX && readoutDirection*lorentzAngle < 0));
               // set the low boundary parameters : position & rotation 
               Vector3D boundaryXPosition = boundaryStraight  ? Vector3D(cPosX, 0.,0.) : 
                                            Vector3D(cPosX-readoutDirection*lorentzPlaneShiftX, 0., 0.);
               // rotation of the boundary: striaght or lorentz
               const RotationMatrix3D& boundaryXRotation = boundaryStraight ?
                                                           xBinRotationMatrix : lorentzPlaneRotationMatrix;
               // build the rotation from it
               auto boundaryXTransform = std::make_shared<Transform3D>(getTransformFromRotTransl(boundaryXRotation,
                                                                                                 boundaryXPosition));
               // the correct bounds for this
               std::shared_ptr<const PlanarBounds> boundaryXBounds = boundaryStraight ? xBinBounds : lorentzPlaneBounds;
               // boundary surfaces 
               boundarySurfaces.push_back(std::shared_ptr<const PlaneSurface>(new PlaneSurface(boundaryXTransform, boundaryXBounds)));
           // (ii) this is the in between bins  --- ( 1 <= ibin < m_mbnsX ) 
           } else {
               // shift by the lorentz angle 
               Vector3D lorentzPlanePosition(cPosX-readoutDirection*lorentzPlaneShiftX, 0., 0.);
               auto lorentzPlaneTransform = std::make_shared<Transform3D>(getTransformFromRotTransl(lorentzPlaneRotationMatrix,
                                                                                                    lorentzPlanePosition));
               // lorentz plane surfaces 
               segmentationSurfacesX.push_back(std::shared_ptr<const PlaneSurface>(new PlaneSurface(lorentzPlaneTransform,
                                                                                                    lorentzPlaneBounds)));
           }
    }
    ...
    std::shared_ptr<const PlanarBounds> yBinBounds(new RectangleBounds(m_activeBounds->halflengthX(),
                                                                       halfThickness));
    // reserve, it's always (number of bins-1) as the boundaries are within the boundarySurfaces
...
const Acts::DigitizationStep Acts::RectangularSegmentation::digitizationStep(const Vector3D& startStep,
