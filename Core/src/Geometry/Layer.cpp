...
Acts::Layer::compatibleSurfaces(
  ...
  if (m_approachDescriptor &&
    ...
    auto approachSurface = m_approachDescriptor->approachSurface(
        gctx, position, direction, options.boundaryCheck, overstepLimit);
    if (approachSurface) {
...
Acts::SurfaceIntersection Acts::Layer::surfaceOnApproach(
  ...
  auto findValidIntersection =
    ...
    for (const auto& sfi : sfmi.split()) {
      if (sfi && detail::checkIntersection(sfi.intersection(), oLimit)) {
        return sfi;
      }
    }
  ...
  if (m_approachDescriptor && (resolvePS || resolveMS)) {
    SurfaceIntersection aSurface = m_approachDescriptor->approachSurface(
        gctx, position, direction, options.boundaryCheck, oLimit);
    return aSurface;
  }
