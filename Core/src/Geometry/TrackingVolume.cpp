...
void TrackingVolume::glueTrackingVolume(const GeometryContext& gctx,
  ...
  // Find the connection of the two tracking volumes: BinningValue::binR returns the center
  // except for cylindrical volumes
  Vector3 bPosition(binningPosition(gctx, BinningValue::binR));
  Vector3 distance = Vector3(neighbor->binningPosition(gctx, BinningValue::binR) - bPosition);
  // glue to the face
...
void TrackingVolume::glueTrackingVolumes(
  ...
  Vector3 bPosition(binningPosition(gctx, BinningValue::binR));
  Vector3 distance =
      Vector3(nRefVolume->binningPosition(gctx, BinningValue::binR) - bPosition);
