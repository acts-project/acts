...
void CylinderVolumeStack::initializeOuterVolume(AxisDirection direction,
  ...
  ACTS_DEBUG("Creating CylinderVolumeStack from "
             << m_volumes.size() << " volumes in direction " << direction);
  if (m_volumes.empty()) {
    throw std::invalid_argument(
        "CylinderVolumeStack requires at least one volume");
  }
  ...
  if (direction != AxisDirection::AxisZ && direction != AxisDirection::AxisR) {
    throw std::invalid_argument(axisDirectionToString(direction) +
                                " is not supported ");
  }
  ...
  if (direction == AxisDirection::AxisZ) {
  ...
  } else if (direction == AxisDirection::AxisR) {
    ...
    ACTS_ERROR("Binning in " << direction << " is not supported");
    throw std::invalid_argument(axisDirectionToString(direction) +
                                " is not supported ");
  }
}
