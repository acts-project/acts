...
Acts::OrientedSurfaces Acts::CutoutCylinderVolumeBounds::orientedSurfaces(
  ...
  if (m_innerCylinderBounds != nullptr) {
    auto posChokeTrf = transform * Translation3D(Vector3(0, 0, zChoke));
    ...
    auto negChokeTrf = transform * Translation3D(Vector3(0, 0, -zChoke));
  ...
  // Two Outer disks
  auto posOutDiscTrf =
      transform * Translation3D(Vector3(0, 0, get(eHalfLengthZ)));
  auto posOutDisc =
  ...
  // Two Inner disks
  auto posInDiscTrf =
      transform * Translation3D(Vector3(0, 0, get(eHalfLengthZcutout)));
  auto posInDisc =
