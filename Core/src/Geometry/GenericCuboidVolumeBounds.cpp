...
Acts::GenericCuboidVolumeBounds::GenericCuboidVolumeBounds(
    const std::array<Acts::ActsScalar,
...
bool Acts::GenericCuboidVolumeBounds::inside(const Acts::Vector3& gpos,
  ...
  for (std::size_t i = 1; i < 6; i++) {
    Acts::ActsScalar dot = (gpos - m_vertices[vtxs[i]]).dot(m_normals[i]);
...
std::vector<Acts::ActsScalar> Acts::GenericCuboidVolumeBounds::values() const {
  std::vector<Acts::ActsScalar> rvalues;
