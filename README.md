...
ACTS is a collaborative open source software project, and hence does not base its organizational structure on (intellectual) *ownership* of certain pieces of code by individuals.
The usage github's `CODEOWNERS` system is currently under evaluation primarily to identify opt-in contact persons for certain parts of the code base in an attempt to simplify worklows and identify information contacts and reviewers. A detailed description of our
test usage is part of the `CODEOWNERS` file.
