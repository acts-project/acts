...
using ChannelStep = ActsFatras::Segmentizer::ChannelStep;
using ChannelSegment = ActsFatras::Segmentizer::ChannelSegment;
using Bin2D = ActsFatras::Segmentizer::Bin2D;
using BinDelta2D = ActsFatras::Segmentizer::BinDelta2D;

auto stepsToSegments(std::vector<ChannelStep>& cSteps,
                     const Acts::Vector2& start, const Bin2D& bstart) {
...
std::vector<ChannelSegment> segmentPlaneSurface(
    const Acts::BinUtility& segmentation, const Acts::Vector2& start,
    const Acts::Vector2& end) {
  ...
  return stepsToSegments(cSteps, start, bstart);
}

std::vector<ChannelSegment> segmentDiscSurface(
    ...
    for (const auto& r : rbbounds) {
      auto radIntersection =
          Acts::detail::IntersectionHelper2D::intersectCircleSegment(
              r, std::min(phistart, phiend), std::max(phistart, phiend), start,
