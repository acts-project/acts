...
SurfaceMaterial compress(const SampledProfile& profile, double tolerance) {
  ...
  // material starts where the surface does, so everything in front of it is a
  // band of nothing
  if (profile.min > 0.) {
    bounds.push_back(static_cast<float>(profile.min));
    radiationLengths.push_back(0.f);
    nuclearLengths.push_back(0.f);
  }
...
DetectorLayout makeLayoutFromTrackingGeometry(
  ...
  trackingGeometry.visitSurfaces(
      [&](const Acts::Surface* surface) {
        ...
        SurfaceGroup& group = groups[groupKeyOf(*surface)];
  ...
  if (options.materialFromGeometry || options.includeMaterialSurfaces) {
    ...
    trackingGeometry.visitSurfaces(
        [&](const Acts::Surface* surface) {
          ...
          const GroupKey key = groupKeyOf(*surface);
  ...
  // The material goes on in the order the surfaces are added, which is the only
  // thing that names them: a reference coordinate does not, an endcap keeping
  // one disc per ring at the same z, and matching those by position hands all
  // of them whichever came last.
  DetectorLayoutBuilder builder;
  std::vector<const SurfaceMaterial*> materials;
  if (options.passiveBeamPipeRadius.has_value()) {
    builder.addPassiveCylinder(*options.passiveBeamPipeRadius);
  ...
  for (const ReducedSurface& cylinder : cylinders) {
    ...
    if (!cylinder.sensitive) {
      ...
      builder.addPassiveCylinder(radius, halfLengthZ);
    } else {
      builder.addCylinder(radius, halfLengthZ, options.modulesPerSurface);
    }
    materials.push_back(&cylinder.material);
  }
  for (const SurfaceSide side :
       {SurfaceSide::Positive, SurfaceSide::Negative}) {
    const bool positive = side == SurfaceSide::Positive;
    for (const ReducedSurface& disc : discs) {
      // a disc centred on z = 0 belongs to no side, which a real endcap does
      // not produce
      if (disc.refCoord == 0. || (disc.refCoord > 0.) != positive) {
        continue;
      }
      ...
      if (!disc.sensitive || disc.rings.empty()) {
        ...
        if (disc.rings.empty()) {
          builder.addPassiveDisc(side, absZ, static_cast<float>(disc.minBound),
                                 static_cast<float>(disc.maxBound));
        } else {
          builder.addPassiveDisc(side, absZ, disc.rings.front().rMin,
                                 disc.rings.back().rMax);
        }
      } else {
        builder.addDisc(side, absZ, disc.rings);
      }
      materials.push_back(&disc.material);
    }
  }

  DetectorLayout layout = builder.build();

  if (options.materialFromGeometry) {
    for (std::size_t s = 0; s < layout.surfaces.size(); ++s) {
      if (materials[s] != nullptr) {
        layout.surfaces[s].material = *materials[s];
      }
    }
  }
