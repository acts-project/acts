...
Acts::Result<std::pair<Acts::Vector2D, Acts::Vector2D>>
ActsFatras::detail::PlanarSurfaceMask::apply(const Acts::Surface& surface,
  ...
  if (surfaceType == Acts::Surface::Plane or
    ...
    Acts::Vector2D localStart =
        (surfaceType == Acts::Surface::Plane)
            ? segment.first
            : Acts::Vector2D(Acts::VectorHelpers::perp(segment.first),
                             Acts::VectorHelpers::phi(segment.first));

    Acts::Vector2D localEnd =
        (surfaceType == Acts::Surface::Plane)
            ? segment.second
            : Acts::Vector2D(Acts::VectorHelpers::perp(segment.second),
                             Acts::VectorHelpers::phi(segment.second));
    ...
    if (not startInside) {
      auto maskedR = polygonMask(segment.first, segment.second, vertices);
      if (maskedR.ok()) {
        clipped.first = maskedR.value();
      } else {
        return DigitizationError::MaskingError;
      }
    }
    if (not endInside) {
      auto maskedR = polygonMask(segment.second, segment.first, vertices);
      if (maskedR.ok()) {
        clipped.second = maskedR.value();
      } else {
        return DigitizationError::MaskingError;
      }
    }
    ...
    // Polar coordinates
    Acts::Vector2D sPolar(Acts::VectorHelpers::perp(segment.first),
                          Acts::VectorHelpers::phi(segment.first));
    Acts::Vector2D ePolar(Acts::VectorHelpers::perp(segment.second),
                          Acts::VectorHelpers::phi(segment.second));
    ...
    std::array<Segment, 2> sectorEdges;
    if (surface.bounds().type() == Acts::SurfaceBounds::eDisc) {
      ...
      Segment minEdge = {Acts::Vector2D(rMin * cphiMin, rMin * sphiMin),
                         Acts::Vector2D(rMax * cphiMin, rMax * sphiMin)};
      Segment maxEdge = {Acts::Vector2D(rMin * cphiMax, rMin * sphiMax),
      ...
      Segment minEdge = {cartesianEdges[0], cartesianEdges[3]};
      Segment maxEdge = {cartesianEdges[1], cartesianEdges[2]};
      sectorEdges = {minEdge, maxEdge};
    ...
    std::array<PolarSegment, 2> segEdges = {
        PolarSegment(segment.first, segment.first, segment.second, startInside),
        PolarSegment(segment.second, segment.second, segment.first, endInside)};
    ...
    for (auto& se : segEdges) {
      ...
      if (not se.inside) {
        ...
        for (const auto& sector : sectorEdges) {
          auto tSolution = intersector.intersectSegment(
              sector.first, sector.second, se.start, sedir, true);
...
Acts::Result<Acts::Vector2D> ActsFatras::detail::PlanarSurfaceMask::polygonMask(
  ...
  for (size_t iv = 0; iv < vertices.size(); ++iv) {
    ...
    Acts::Vector2D lineSegment = (inside - outside).normalized();
    auto intersection =
        intersector.intersectSegment(s0, s1, outside, lineSegment, true);
