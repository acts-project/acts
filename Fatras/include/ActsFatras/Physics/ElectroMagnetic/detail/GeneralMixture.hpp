...
struct GeneralMixture {
  ...
  long double genMixtureScalor = 1.;
  ...
  long double operator()(generator_t &generator, const Acts::MaterialSlab &slab,
    ...
    if (particle.absolutePdg() != Acts::PdgParticle::eElectron) {
      ...
      //----------------------------------------------------------------------------
      std::array<long double, 4> scattering_params{};
      ...
      long double beta = 1 / std::sqrt(beta2Inv);
      long double tInX0 = slab.thicknessInX0();
      long double tob2 = tInX0 * beta2Inv;
      ...
      theta = std::normal_distribution<long double>(0.0, theta0)(generator);
    }
  ...
  std::array<long double, 4> getGaussian(long double beta, long double p,
                                         long double tInX0,
  ...
  std::array<long double, 4> getGaussmix(long double beta, long double p,
                                         long double tInX0, long double Z,
                                         long double scale) const {
    std::array<long double, 4> scattering_params{};
    scattering_params[0] = 15. / beta / p * std::sqrt(tInX0) *
                           scale;  // Total standard deviation of mixture
    long double d1 = std::log(tInX0 / (beta * beta));
    long double d2 = std::log(std::pow(Z, 2.0 / 3.0) * tInX0 / (beta * beta));
    long double epsi = 0;
    long double var1 = (-1.843e-3 * d1 + 3.347e-2) * d1 + 8.471e-1;  // Variance
                                                                     // of core
    if (d2 < 0.5) {
  ...
  std::array<long double, 6> getSemigauss(long double beta, long double p,
                                          long double tInX0, long double Z,
                                          long double scale) const {
    std::array<long double, 6> scattering_params{};
    long double N = tInX0 * 1.587E7 * std::pow(Z, 1.0 / 3.0) / (beta * beta) /
                    (Z + 1) / std::log(287 / std::sqrt(Z));
    scattering_params[4] = 15. / beta / p * std::sqrt(tInX0) *
                           scale;  // Total standard deviation of mixture
    long double rho = 41000 / std::pow(Z, 2.0 / 3.0);
    long double b = rho / std::sqrt(N * (std::log(rho) - 0.5));
    long double n = std::pow(Z, 0.1) * std::log(N);
    long double var1 = (5.783E-4 * n + 3.803E-2) * n + 1.827E-1;
    long double a =
        (((-4.590E-5 * n + 1.330E-3) * n - 1.355E-2) * n + 9.828E-2) * n +
        2.822E-1;
    long double epsi = (1 - var1) / (a * a * (std::log(b / a) - 0.5) - var1);
    scattering_params[3] =
  ...
  ///
  /// @return a long double value that represents the gaussian mixture
  template <typename generator_t>
  long double gaussmix(
      generator_t &generator,
      const std::array<long double, 4> &scattering_params) const {
    std::uniform_real_distribution<long double> udist(0.0, 1.0);
    long double sigma_tot = scattering_params[0];
    long double var1 = scattering_params[1];
    long double var2 = scattering_params[2];
    long double epsi = scattering_params[3];
    bool ind = udist(generator) > epsi;
    long double u = udist(generator);
    if (ind) {
  ...
  /// @return a long double value that represents the gaussian mixture
  template <typename generator_t>
  long double semigauss(
      generator_t &generator,
      const std::array<long double, 6> &scattering_params) const {
    std::uniform_real_distribution<long double> udist(0.0, 1.0);
    long double a = scattering_params[0];
    long double b = scattering_params[1];
    long double var1 = scattering_params[2];
    long double epsi = scattering_params[3];
    long double sigma_tot = scattering_params[4];
    bool ind = udist(generator) > epsi;
    long double u = udist(generator);
    if (ind) {
      return std::sqrt(var1) * std::sqrt(-2 * std::log(u)) * sigma_tot;
