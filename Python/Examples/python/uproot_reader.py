...
class UprootReader(acts.examples.IReader):
    ...
    All data is loaded upfront for thread-safe concurrent access from the
    sequencer.
    """

    def __init__(
        self,
        particleFilePath: Union[Path, str],
        simHitFilePath: Union[Path, str],
        outputParticles: str = "particles",
        outputSimHits: str = "simhits",
        particleTreeName: str = "particles",
        simHitTreeName: str = "hits",
        ...
        # Load particle data: one entry per event, vector-valued branches
        with uproot.open(str(particleFilePath)) as f:
            tree = f[particleTreeName]
            self._particle_data = tree.arrays(library="np")
            self._particle_event_ids = self._particle_data["event_id"]
            self._particle_entry_map = {
                int(eid): i for i, eid in enumerate(self._particle_event_ids)
            }

        # Load hit data: one row per hit, grouped by event_id
        with uproot.open(str(simHitFilePath)) as f:
            tree = f[simHitTreeName]
            ...
            # Check if the file has a packed barcode branch
            self._has_barcode_branch = "barcode" in tree.keys()

        # Compute available event range as intersection of both files
        particle_ids = set(self._particle_event_ids.tolist())
        hit_ids = set(self._hit_event_range_map.keys())
        all_ids = particle_ids | hit_ids
    ...
    def read(self, context):
        ...
        if event_number in self._particle_entry_map:
            ...
            for i in range(n):
                ...
                p.setProcess(
                    acts.examples.ProcessType(int(d["process"][idx][i]))
                )
        ...
        if event_number in self._hit_event_range_map:
            ...
            for i in range(start, end):
                ...
                hit = acts.examples.SimHit(geoid, barcode, pos4, before4, after4, index)
                hits.insert(hit)
