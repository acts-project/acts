...
void bind_vecmem_cuda(py::module_& m) {
    ...
    py::class_<traccc::memory_resource,
               std::shared_ptr<traccc::memory_resource>>(m, "TracccMemoryResource")
...
void bind_vecmem_host(py::module_& m) {
    // HostMemory composite to be passed to algorithms expecting both copy and memory resource
    py::class_<vecmem::memory_resource,
               std::shared_ptr<vecmem::memory_resource>>(m, "MemoryResource");
    py::class_<vecmem::copy,
               std::shared_ptr<vecmem::copy>>(m, "Copy");
    py::class_<vecmem::host_memory_resource, vecmem::memory_resource,
               std::shared_ptr<vecmem::host_memory_resource>>(m, "HostMemoryResource")
        .def(py::init<>());
    py::class_<vecmem::copy,
               std::shared_ptr<vecmem::copy>>(m, "HostCopy")
        .def(py::init<>());
...
// registers device memoory composites corresponding to the backends enabled during build
#if defined(ACTS_ENABLE_CUDA)
    bind_vecmem_cuda(m); 
#endif
