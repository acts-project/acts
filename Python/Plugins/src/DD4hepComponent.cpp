...
PYBIND11_MODULE(ActsPluginsPythonBindingsDD4hep, m) {
...

  {
    py::class_<DD4hepFieldAdapter, MagneticFieldProvider,
               std::shared_ptr<DD4hepFieldAdapter>>(m,
                                                          "DD4hepFieldAdapter");
  }
...

  {
    ...
    m.def(
        ...
        [](Experimental::DD4hepDetectorStructure::Options& options,
          ...
          // The Geo mapper
          auto geoIdMapper =
              std::make_shared<const DD4hepIdentifierMapper>(
          ...
          auto chainedGeoIdGenerator = std::make_shared<
              const Experimental::ChainedGeometryIdGenerator<
                  std::shared_ptr<
                      const Experimental::GeometryIdGenerator>,
                  std::shared_ptr<const DD4hepIdentifierMapper>>>(
