...
void addGeometryGen2(Context& ctx) {
...

  {
    ...
    py::class_<IInternalStructureBuilder,
               std::shared_ptr<IInternalStructureBuilder>>(
    ...
    // The internal layer structure builder
    py::class_<ISurfacesProvider, std::shared_ptr<ISurfacesProvider>>(
        m, "ISurfacesProvider");

    py::class_<LayerStructureBuilder::SurfacesHolder, ISurfacesProvider,
...

  {
    ...
    using KdtSurfacesDim1Bin100 = KdtSurfaces<1u, 100u>;
    using KdtSurfacesProviderDim1Bin100 = KdtSurfacesProvider<1u, 100u>;
    ...
    py::class_<KdtSurfacesProviderDim1Bin100, ISurfacesProvider,
...

  {
    ...
    using KdtSurfacesDim2Bin100 = KdtSurfaces<2u, 100u>;
    using KdtSurfacesProviderDim2Bin100 = KdtSurfacesProvider<2u, 100u>;
    ...
    py::class_<KdtSurfacesProviderDim2Bin100, ISurfacesProvider,
...

  {
    ...
    py::class_<IExternalStructureBuilder,
               std::shared_ptr<IExternalStructureBuilder>>(
  ...
  {
    py::class_<IGeometryIdGenerator, std::shared_ptr<IGeometryIdGenerator>>(
        m, "IGeometryIdGenerator");

    auto geoIdGen =
        py::class_<GeometryIdGenerator, IGeometryIdGenerator,
                   std::shared_ptr<GeometryIdGenerator>>(m,
                                                         "GeometryIdGenerator")
            .def(py::init([](GeometryIdGenerator::Config& config,
                             const std::string& name, Logging::Level level) {
              return std::make_shared<GeometryIdGenerator>(
                  config, getDefaultLogger(name, level));
            }));

    auto geoIdGenConfig =
        py::class_<GeometryIdGenerator::Config>(geoIdGen, "Config")
            .def(py::init<>());
...

  {
    ...
    py::class_<IDetectorComponentBuilder,
               std::shared_ptr<IDetectorComponentBuilder>>(
  ...
  {
    // The external volume structure builder
    py::class_<IRootVolumeFinderBuilder,
               std::shared_ptr<IRootVolumeFinderBuilder>>(
        m, "IRootVolumeFinderBuilder");

    auto irvBuilder =
        py::class_<IndexedRootVolumeFinderBuilder, IRootVolumeFinderBuilder,
                   std::shared_ptr<IndexedRootVolumeFinderBuilder>>(
...

  {
    ...
    auto ccBuilder =
        py::class_<CuboidalContainerBuilder, IDetectorComponentBuilder,
