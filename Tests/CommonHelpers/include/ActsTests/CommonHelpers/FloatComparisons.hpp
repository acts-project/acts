...
#define CHECK_CLOSE_REL(val, ref, reltol) \
  BOOST_CHECK(ActsTests::checkCloseRel((val), (ref), (reltol)))
...
#define CHECK_CLOSE_ABS(val, ref, abstol) \
  BOOST_CHECK(ActsTests::checkCloseAbs((val), (ref), (abstol)))
...
#define CHECK_SMALL(val, small) \
  BOOST_CHECK(ActsTests::checkSmall((val), (small)))
...
#define CHECK_CLOSE_OR_SMALL(val, ref, reltol, small) \
  BOOST_CHECK(ActsTests::checkCloseOrSmall((val), (ref), (reltol), (small)))
...
#define CHECK_CLOSE_COVARIANCE(val, ref, tol) \
  BOOST_CHECK(ActsTests::checkCloseCovariance((val), (ref), (tol)))
...
predicate_result compare(const Acts::Transform3& val,
