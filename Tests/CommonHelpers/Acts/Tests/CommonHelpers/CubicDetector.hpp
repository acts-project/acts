...
struct CubicDetector {
  ...
  CubicDetector(const GeometryContext& gctx) : geoContext(gctx) {
    ...
    rBounds =
        std::make_shared<const RectangleBounds>(RectangleBounds(0.4_m, 0.4_m));
  ...
  std::shared_ptr<const Acts::Experimental::Detector> operator()() {
    ...
    translations.push_back({0., 0., -2_m});
    translations.push_back({0., 0., -1_m});
    translations.push_back({0., 0., 1_m - eps});
    translations.push_back({0., 0., 1_m + eps});
    translations.push_back({0., 0., 2_m - eps});
    translations.push_back({0., 0., 2_m + eps});
    ...
    for (unsigned int i = 0; i < translations.size(); i++) {
      ...
      surfaces[i]->assignGeometryId(geoId);
    }

    // Build volume for surfaces with negative x-values
    Transform3 trafoVol1(Transform3::Identity());
    trafoVol1.translation() = Vector3(0., 0., -1.5_m);

    auto boundsVol1 = std::make_shared<CuboidVolumeBounds>(0.5_m, 0.5_m, 1.5_m);
    ...
    trafoVol2.translation() = Vector3(0., 0., 1.5_m);
    ...
    auto boundsVol2 = std::make_shared<CuboidVolumeBounds>(0.5_m, 0.5_m, 1.5_m);
    ...
    auto portalContainer =
        Acts::Experimental::detail::CuboidalDetectorHelper::connect(
            geoContext, detectorVolumes, Acts::BinningValue::binZ, {},
            Acts::Logging::VERBOSE);
