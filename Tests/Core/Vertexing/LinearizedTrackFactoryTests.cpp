...
struct InputTrack
  ...
  for(const InputTrack& track : tracks){
    ...
    LinearizedTrack linTrack = linFactory.linearizeTrack(&trackParams, Vector3D(0.,0.,0.));
  ...
  BOOST_AUTO_TEST_CASE(linearized_track_factory_empty_test)
  {
    ...
    LinearizedTrack linTrack = linFactory.linearizeTrack(nullptr, Vector3D(1.,2.,3.));
