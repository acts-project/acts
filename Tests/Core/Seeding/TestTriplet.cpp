...
std::vector<std::vector<const SpacePoint*>>
...
Acts::SeedmakerConfig<SpacePoint>
createConfig(std::string configFile)
{
  ...
  config.seedFilter = std::make_unique<Acts::SeedFilter<SpacePoint>>(
      Acts::SeedFilter<SpacePoint>(sfconf));
  return config;
}
...
main(int argc, char** argv)
{
  if (argc != 4) {
    std::cout
        << "call program as: ./testTriplets config.txt triplet.cvs outfile.cvs"
        << std::endl;
    return -1;
  }
  std::vector<std::vector<const SpacePoint*>> vectorOfTriplets
      = readFile(argv[2]);
  ...
  std::function<Acts::Vector2D(const SpacePoint*, float, float, float)> ct
      = [](const SpacePoint*, float, float, float) -> Acts::Vector2D {
    return {0, 0};
  };
  ...
  for (auto& triplet : vectorOfTriplets) {
    std::shared_ptr<Acts::SeedmakerState<SpacePoint>> state = sm.initState(
        triplet.begin(), triplet.end(), ct, bottomBinFinder, topBinFinder);
    ...
    outFile << triplet[2]->x() << "," << triplet[2]->y() << ","
            << triplet[2]->z() << "," << numSeeds << "\n";
  }
}
