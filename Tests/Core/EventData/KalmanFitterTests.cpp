...
namespace Test {
  ...
  BOOST_DATA_TEST_CASE(
      kalman_fitter_rnddata,
      bdata::random((bdata::seed = 20,
                     bdata::distribution
                     = std::uniform_real_distribution<>(-0.5 * units::_m,
                                                        0.5 * units::_m)))
      ...
      index)
  {
    ...
    Surface const* sur = detector->lowestTrackingVolume(pos)
                             ...
                             ->at(pos)[0];
    measurements.push_back(
        Measurement<id, eLOC_0, eLOC_1>(*sur, index + 0, cov2D, x1, y1));
    ...
    sur = detector->lowestTrackingVolume(pos)
              ...
              ->at(pos)[0];
    measurements.push_back(
        Measurement<id, eLOC_0, eLOC_1>(*sur, index + 1, cov2D, x2, y2));
