...
namespace Layers {
  ...
  { 
    ...
    //minimally need a Transform3D and a PlanarBounds object (e.g. RectangleBounds) to construct
    ...
    auto pPlaneLayerFromSurfaces = PlaneLayer::create(pTransform, pRectangle, std::move(pSurfaceArray));
    BOOST_TEST(pPlaneLayerFromSurfaces->layerType() == LayerType::active);
    //construct with thickness:
    auto pPlaneLayerWithThickness = PlaneLayer::create(pTransform, pRectangle, std::move(pSurfaceArray), thickness);
    ...
    auto adPtr=ad.get();
    auto pPlaneLayerWithApproachDescriptor = PlaneLayer::create(pTransform, pRectangle, std::move(pSurfaceArray), thickness, std::move(ad));
    BOOST_TEST(pPlaneLayerWithApproachDescriptor->approachDescriptor() == adPtr);
    //with the layerType specified...
    auto pPlaneLayerWithLayerType = PlaneLayer::create(pTransform, pRectangle, std::move(pSurfaceArray), thickness, std::move(ad), LayerType::passive);
  ...
  BOOST_AUTO_TEST_CASE(LayerProperties/*, *utf::expected_failures(1)*/)
  {
    ...
    BOOST_TEST(pPlaneLayer->surfaceRepresentation().name() == std::string("Acts::PlaneSurface"));
  }
