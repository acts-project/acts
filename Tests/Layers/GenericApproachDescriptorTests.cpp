...
namespace Test {
  namespace Layers {
    ...
    BOOST_AUTO_TEST_CASE(GenericApproachDescriptorConstruction)
    {
      ...
      BOOST_CHECK_NO_THROW(GenericApproachDescriptor<Surface> minimallyConstructedApproachDescriptor(someSurfaces));
      //
      std::vector<std::shared_ptr<const Layer>> sharedLayers{std::make_shared<LayerStub>(nullptr), std::make_shared<LayerStub>(nullptr)};
      BOOST_CHECK_NO_THROW(GenericApproachDescriptor<Layer> sharedLayerApproachDescriptor(sharedLayers));
    }
    ...
    BOOST_AUTO_TEST_CASE(GenericApproachDescriptorProperties,  *utf::expected_failures(1))
    {
      ...
      //approachSurface
      SurfaceIntersection surfIntersection = approachDescriptor.approachSurface(origin,zDir, bcheck);
      double expectedIntersection = 20.0;//property of SurfaceStub
      BOOST_CHECK(surfIntersection.intersection.pathLength == expectedIntersection);
      //containedSurfaces()
