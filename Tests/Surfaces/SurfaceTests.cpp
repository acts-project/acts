...
namespace Test {
  ...
  BOOST_AUTO_TEST_CASE(SurfaceConstruction)
  {
    ...
    std::shared_ptr<const Acts::PlanarBounds> p=std::make_shared<const RectangleBounds>(5.,10.);
  ...
  BOOST_AUTO_TEST_CASE(SurfaceProperties, * utf::expected_failures(2))
  {
    ...
    std::shared_ptr<const Acts::PlanarBounds> pPlanarBound=std::make_shared<const RectangleBounds>(5.,10.);
    ...
    auto pMaterial= std::make_shared<const HomogeneousSurfaceMaterial>(properties);
    DetectorElementStub detElement{identifier,pTransform, pPlanarBound,0.2,pMaterial};
    ...
    //bounds() method is pure virtual, is overridden in derived classes, test there
    //center()
    ...
    BOOST_CHECK(!surface.insideBounds(outside)); //fails: m_bounds only in derived classes
    //intersectionEstimate (should delegate to derived class method of same name)
    ...
    BOOST_CHECK(surface.isOnSurface(reference, true));//need to improve bounds()
    //measurementFrame()
    RotationMatrix3D unitary;
    unitary<< 1,0,0,  0,1,0,  0,0,1;
    auto measurementFrame= surface.measurementFrame(reference, mom); //need more complex case to test
    bool ok = (measurementFrame == unitary);
    BOOST_TEST(ok,"measurementFrame() returns sensible answer");
    //normal()
    auto normal = surface.Surface::normal(reference); //needs more complex test
    Vector3D zero{0.,0.,0.};
    BOOST_TEST(zero == normal);
    //pathCorrection is pure virtual
    //associatedMaterial()
    MaterialProperties newProperties{0.5,0.5,0.5,20.,10.,5.};
    auto pNewMaterial= std::make_shared<const HomogeneousSurfaceMaterial>(newProperties);
    ...
    bool constructedTransformEqualsRetrievedTransform = returnedTransform.isApprox(*pTransform);
  ...
  BOOST_AUTO_TEST_CASE(EqualityOperators){
    ...
    auto pMaterial= std::make_shared<const HomogeneousSurfaceMaterial>(properties);
    DetectorElementStub detElement1{identifier,pTransform1, pPlanarBound,0.2,pMaterial};
    DetectorElementStub detElement2{identifier,pTransform1, pPlanarBound,0.3,pMaterial};
    DetectorElementStub detElement3{identifier2,pTransform2, pPlanarBound,0.3,pMaterial}; 
    //
    ...
    SurfaceStub surface4(detElement3, identifier2);//4 has a different transform and id
    //
    bool equalSurface=(surface1 == surface2);
    BOOST_TEST(equalSurface,"Equality between similar surfaces");
    //
    bool unequalSurface=(surface1 != surface3); //only thickness is different here;
    BOOST_TEST(unequalSurface,"Different thickness surfaces should be unequal"); //will fail
    //
    unequalSurface=(surface1 != surface4); //bounds or transform must be different;
    BOOST_TEST(unequalSurface,"Different transform surfaces should be unequal");
  }
