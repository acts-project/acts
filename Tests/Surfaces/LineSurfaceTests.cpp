...
namespace Test {
  ...
  BOOST_AUTO_TEST_CASE(LineSurface_Constructors_test)
  {
    ...
    DetectorElementStub detElement{identifier,pTransform, pLineBounds,0.2,pMaterial};
    BOOST_CHECK(LineSurfaceStub(pLineBounds,detElement, identifier).constructedOk());
    ...
    BOOST_TEST_MESSAGE("All LineSurface constructors are callable without problem");
  }
  ...
  { 
    ...
    const LineBounds & bounds=dynamic_cast<const LineBounds&>(boundedLine.bounds());
    BOOST_TEST(bounds == LineBounds(10.0), "bounds() equals construction bounds");
    //
    //globalToLocal()
    Vector3D gpos{0.,1.,0,};
    ...
    auto intersection=line.intersectionEstimate({0.,0.,0.}, direction, forceDir, bcheck);
    BOOST_CHECK(intersection.valid);
    Vector3D expectedIntersection(0, -1./3., -2./3.);
    BOOST_TEST(intersection.position == expectedIntersection); //need more tests..
    //
    //isOnSurface
    const Vector3D insidePosition{0.,2.5,0.};
    BOOST_TEST(line.isOnSurface(insidePosition, false)); //need better test here
    ...
    auto returnedRotationMatrix = line.measurementFrame(globalPosition, momentum);
  ...
  BOOST_AUTO_TEST_CASE(LineSurface_assignment_test)
  {
    ...
    BOOST_TEST(assignedLine != originalLine,"LineSurfaces are different before assignment"); // operator != from base
    assignedLine = originalLine;
    BOOST_TEST(assignedLine == originalLine,"LineSurfaces are equal value after assignment"); // operator == from base
  }
