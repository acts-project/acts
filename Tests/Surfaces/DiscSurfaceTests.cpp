...
namespace Test {
  ...
  BOOST_AUTO_TEST_CASE(DiscSurface_properties_test, * utf::expected_failures(2))
  {
    ...
    BOOST_CHECK(discSurfaceObject.binningPosition(BinningValue::binRPhi ) == origin3D);
    //
    ///Test bounds 
    ...
    BOOST_TEST(discSurfaceObject.isOnSurface(point3DNotInSector, true) == false); //passes
    BOOST_TEST(discSurfaceObject.isOnSurface(point3DOnSurface, true) == true);//passes
    //
    ///Test localToGlobal
    ...
    discSurfaceObject.localToGlobal(rPhiOnDisc,ignoredMomentum, returnedPosition);
    BOOST_TEST(returnedPosition.isApprox(expectedPosition), "LocalToGlobal for rPhiOnDisc");
    //
    discSurfaceObject.localToGlobal(rPhiNotInSector,ignoredMomentum, returnedPosition);
    ...
    BOOST_TEST(discSurfaceObject.globalToLocal(point3DOnSurface, ignoredMomentum, returnedLocalPosition));//pass
    BOOST_TEST(returnedLocalPosition.isApprox(expectedLocalPosition));
    //
    BOOST_TEST(discSurfaceObject.globalToLocal(point3DNotInSector, ignoredMomentum, returnedLocalPosition) == false);// test fails
    //
    Vector3D pointOutsideRadius{0.0,100.,0};
    BOOST_TEST(discSurfaceObject.globalToLocal(pointOutsideRadius, ignoredMomentum, returnedLocalPosition) == false); // fails
    //
    ///Test localPolarToCartesian
    Vector2D rPhi1_1{std::sqrt(2.),M_PI/4.};
    Vector2D cartesian1_1{1.,1.};
    BOOST_TEST(discSurfaceObject.localPolarToCartesian(rPhi1_1).isApprox(cartesian1_1));
    //
    ///Test localCartesianToPolar
    BOOST_TEST(discSurfaceObject.localCartesianToPolar(cartesian1_1).isApprox(rPhi1_1));
    //
    ///Test localPolarToLocalCartesian
    BOOST_TEST(discSurfaceObject.localPolarToLocalCartesian(rPhi1_1).isApprox(cartesian1_1));
    //
    ///Test localCartesianToGlobal 
    Vector3D cartesian3D1_1{1.,1.,0.};
    BOOST_TEST(discSurfaceObject.localCartesianToGlobal(cartesian1_1).isApprox(cartesian3D1_1));
    //
    ///Test globalToLocalCartesian
    BOOST_TEST(discSurfaceObject.globalToLocalCartesian(cartesian3D1_1).isApprox(cartesian1_1));
    //
    ///Test pathCorrection
    double projected3DMomentum=std::sqrt(3.)*1.e6;
    Vector3D momentum{projected3DMomentum,projected3DMomentum,projected3DMomentum};
    Vector3D ignoredPosition{NaN, NaN, NaN};
    BOOST_TEST(discSurfaceObject.pathCorrection(ignoredPosition, momentum) == std::sqrt(3), tt::tolerance(0.01));
    //
    ///intersectionEstimate
    Vector3D globalPosition{1.2, 0.0, -10.};
    Vector3D direction{0.,0.,1.}; //must be normalised
    Vector3D expected{1.2,0.0,0.0};
    //intersect is a struct of (Vector3D) position, pathLength, distance and (bool) valid
    auto intersect = discSurfaceObject.intersectionEstimate(globalPosition, direction);
