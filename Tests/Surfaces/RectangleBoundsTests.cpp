...
namespace Test {
  ...
  BOOST_AUTO_TEST_CASE(RectangleBoundsConstruction)
  {
    ...
    BOOST_TEST(zeroDimensionsRectangle.type() ==  Acts::SurfaceBounds::Rectangle);
    //
    ...
    BOOST_TEST(negativeDimensionedRectangle.type() ==  Acts::SurfaceBounds::Rectangle);
  }
  ...
  BOOST_AUTO_TEST_CASE(RectangleBoundsProperties)
  {
    ...
    const std::vector<Vector2D> coords={ {10.,-5.}, {10.,5.}, {-10.,5.}, {-10.,-5.} };
    //equality, ensure ordering is ok
    BOOST_TEST( std::equal(coords.begin(), coords.end(), rect.vertices().begin()) );
    const Vector2D pointA{1.0,1.0}, pointB{9.0,1.0}, outside{10.1,5.1};
    //distance is signed, from boundary to point. (doesn't seem right, given the name of the method)
