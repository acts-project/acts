...
namespace Acts {
  ...
  class SurfaceBoundsStub:public SurfaceBounds{
    ...
    bool inside(const Vector2D &lpos, const BoundaryCheck &bcheck) const final {return true;}
    bool insideLoc0(const Vector2D &lpos, double tol0=0.) const final { return true;}
    bool insideLoc1(const Vector2D &lpos, double tol1=0.) const final { return true;}
    double distanceToBoundary(const Vector2D &lpos) const final { return 10.;}
    std::ostream & dump (std::ostream &sl) const final {sl<<"SurfaceBoundsStub"; return sl;}
