...
namespace Test {
  ...
  BOOST_FIXTURE_TEST_CASE(SurfaceArray_toVariantData, SurfaceArrayFixture)
  {
    ...
    BOOST_CHECK_CLOSE_FRACTION(phiAxis_pl.get<double>("min"), -M_PI, 1e-6);
    BOOST_CHECK_CLOSE_FRACTION(phiAxis_pl.get<double>("max"), M_PI, 1e-6);
    ...
    for (size_t i = 0; i < zAxis_bin_edges.size(); i++) {
      BOOST_CHECK_CLOSE_FRACTION(
          zAxis_bin_edges.get<double>(i), zAxis_bin_edges_exp.at(i), 1e-6);
    }
  ...
  BOOST_FIXTURE_TEST_CASE(SurfaceArray_toVariantData_1D, SurfaceArrayFixture)
  {
    ...
    BOOST_CHECK_SMALL(zAxis_pl.get<double>("min"), 1e-6);
    BOOST_CHECK_CLOSE_FRACTION(zAxis_pl.get<double>("max"), 30, 1e-6);
