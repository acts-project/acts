...
namespace Test {
  ...
  BOOST_AUTO_TEST_CASE(CylinderSurfaceConstruction)
  {
    ...
    BOOST_TEST(CylinderSurface(pNullTransform, radius, halfZ).type() == Surface::Cylinder);
    BOOST_TEST(CylinderSurface(pTransform, radius, halfZ).type() == Surface::Cylinder);
    //
    ///Constructor with transform pointer, radius, halfZ and halfPhiSector
    BOOST_TEST(CylinderSurface(pTransform, radius, halfPhiSector, halfZ).type() == Surface::Cylinder);
    //
    
    ///Constructor with transform and CylinderBounds pointer
    auto pCylinderBounds = std::make_shared<CylinderBounds>(radius, halfZ);
    BOOST_TEST(CylinderSurface(pTransform, pCylinderBounds).type() == Surface::Cylinder);
    //
    //
    ///Copy constructor
    ...
    CylinderSurface copiedTransformedCylinderSurface(cylinderSurfaceObject, *pTransform);
  ...
  BOOST_AUTO_TEST_CASE(CylinderSurfaceProperties)
  {
    ...
    BOOST_TEST(cylinderSurfaceObject.binningPosition(BinningValue::binPhi) == binningPosition);
    //
    ...
    BOOST_TEST(cylinderSurfaceObject.measurementFrame(globalPosition, momentum).isApprox(expectedFrame));
    //
    ...
    //
    ///Test bounds
    BOOST_TEST(cylinderSurfaceObject.bounds().type() == SurfaceBounds::Cylinder);
    //
    ///Test localToGlobal
    Vector2D localPosition{0.,0.};
    cylinderSurfaceObject.localToGlobal(localPosition, momentum, globalPosition);
    Vector3D expectedPosition{1,1,2};
    BOOST_TEST(globalPosition == expectedPosition,"Testing localToGlobal");
    //
    ///Testing globalToLocal
    cylinderSurfaceObject.globalToLocal(globalPosition, momentum,localPosition);
    ...
    auto intersect =cylinderSurfaceObject.intersectionEstimate(offSurface, direction);
    ...
    //
    ///Test pathCorrection
    BOOST_TEST(cylinderSurfaceObject.pathCorrection(offSurface, momentum) == std::sqrt(3.), tt::tolerance(0.01));
    //
    ///Test name
    BOOST_TEST(cylinderSurfaceObject.name() == std::string("Acts::CylinderSurface"));
    //
    ///Test dump
  ...
  BOOST_AUTO_TEST_CASE(EqualityOperators, *utf::expected_failures(1))
  {
    ...
    //
    BOOST_TEST_CHECKPOINT("Create and then assign a CylinderSurface object to the existing one");
