...
namespace Test {
  ...
  BOOST_AUTO_TEST_CASE(ConstrainedStepTest)
  {
    ...
    BOOST_CHECK(stepSize_p.values[cstep::accuracy] == 0.25);
    BOOST_CHECK(stepSize_p.values[cstep::actor] == 0.25);
    BOOST_CHECK(stepSize_p.values[cstep::aborter] == 0.25);
    BOOST_CHECK(stepSize_p.values[cstep::user] == 0.25);

    // Check the cast operation to double
    BOOST_CHECK(stepSize_p == 0.25);
    ...
    // All of the types should be 0.25 now
    BOOST_CHECK(stepSize_n.values[cstep::accuracy] == -0.25);
    BOOST_CHECK(stepSize_n.values[cstep::actor] == -0.25);
    BOOST_CHECK(stepSize_n.values[cstep::aborter] == -0.25);
    BOOST_CHECK(stepSize_n.values[cstep::user] == -0.25);

    // Check the cast operation to double
    BOOST_CHECK(stepSize_n == -0.25);
