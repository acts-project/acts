...
namespace Test {
  ...
  testGeometry(std::vector<std::unique_ptr<const Surface> >& surface_cache){
    ...
    auto cylinderVolumeHelper
        = std::make_shared<const CylinderVolumeHelper>(
            cvhConfig,
            getDefaultLogger("CylinderVolumeHelper", volumeLLevel));
    ...
    auto beamPipeVolume = beamPipeVolumeBuilder->trackingVolume(nullptr, beamPipeBounds);
    
    //-------------------------------------------------------------------------------------
    // some prep work for the material
    // Layer material properties - thickness, X0, L0, A, Z, Rho
    MaterialProperties lProperties(95.7, 465.2, 28.03, 14., 2.32e-3, 1.5*units::_mm);
    ...
    for (size_t ilp = 0; ilp < pLayerRadii.size(); ++ilp){  
        ...
        auto mBounds = std::make_shared<RectangleBounds>(pModuleHalfX[ilp],pModuleHalfY[ilp]);
        ...
        for (auto& mCenter : moduleCenters) {
          ...
          Vector3D moduleLocalZ(
              cos(modulePhi + pModuleTiltPhi[ilp]), sin(modulePhi + pModuleTiltPhi[ilp]), 0.);
        ...
        pLayers.push_back(layerCreator->cylinderLayer(layerModules,
      ...
      // layer array 
      auto pLayerArray   = layerArrayCreator->layerArray(pLayers, 25., 200., arbitrary, binR);
      auto pVolumeBounds = std::make_shared<const CylinderVolumeBounds>(25., 200., 1100.);        
      ...
      // the combined volume
      auto detectorVolume = cylinderVolumeHelper->createContainerTrackingVolume({beamPipeVolume, pVolume});
