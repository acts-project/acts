...
BOOST_AUTO_TEST_CASE(straight_line_stepper_state_test) {
  ...
  AtlasStepper<ConstantBField>::State slsState(tgContext, mfContext, cp, ndir, stepSize,
                                      tolerance);

  // Test the result & compare with the input/test for reasonable members
  BOOST_TEST(!slsState.covTransport);
  BOOST_TEST(slsState.covariance == nullptr);
  BOOST_TEST(slsState.pVector[0] == pos.x());
  BOOST_TEST(slsState.pVector[1] == pos.y());
  BOOST_TEST(slsState.pVector[2] == pos.z());
  BOOST_TEST(slsState.pVector[3] == time);
  CHECK_CLOSE_ABS(slsState.pVector[4], dir.x(), 1e-6);
  CHECK_CLOSE_ABS(slsState.pVector[5], dir.y(), 1e-6);
  CHECK_CLOSE_ABS(slsState.pVector[6], dir.z(), 1e-6);
  BOOST_TEST(slsState.pVector[7] == charge / mom.norm());
  ...
  slsState = AtlasStepper<ConstantBField>::State(tgContext, mfContext, cp, ndir,
                                        stepSize, tolerance);
  BOOST_TEST(slsState.covTransport);
  BOOST_TEST(*slsState.covariance == cov);
}
...
BOOST_AUTO_TEST_CASE(straight_line_stepper_test) {
  ...
  AtlasStepper<ConstantBField>::State slsState(tgContext, mfContext, cp, ndir, stepSize,
                                      tolerance);
  AtlasStepper<ConstantBField> sls;
