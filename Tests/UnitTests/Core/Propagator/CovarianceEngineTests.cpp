...
BOOST_AUTO_TEST_CASE(covariance_engine_test) {
	...
	parameters << position[0], position[1], position[2], time, direction[0], direction[1], direction[2], qop;
	...
	BOOST_TEST(direction == Vector3D(sqrt(5. / 22.), 3. * sqrt(2. / 55.), 7. / sqrt(110.)));
	...
	detail::covarianceTransport(tgContext, covariance, jacobian, transportJacobian, derivatives, jacobianLocalToGlobal, parameters, true, *surface);
	...
	auto curvResult = detail::curvilinearState(covariance, jacobian, transportJacobian, derivatives, jacobianLocalToGlobal, parameters, false, 1337., true);
	...
	curvResult = detail::curvilinearState(covariance, jacobian, transportJacobian, derivatives, jacobianLocalToGlobal, parameters, true, 1337., true);
	BOOST_TEST(std::get<0>(curvResult).covariance().has_value());
	...
	auto boundResult = detail::boundState(tgContext, covariance, jacobian, transportJacobian, derivatives, jacobianLocalToGlobal, parameters, false, 1337., *surface, true);
	...
	boundResult = detail::boundState(tgContext, covariance, jacobian, transportJacobian, derivatives, jacobianLocalToGlobal, parameters, true, 1337., *surface, true);
	BOOST_TEST(std::get<0>(boundResult).covariance().has_value());
	BOOST_TEST(*(std::get<0>(boundResult).covariance()) != Covariance::Identity());
