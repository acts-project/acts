...
/// Simplified stepper
struct Stepper {
	void covarianceTransport(StepperState& state) const { state.jacTransport = FreeMatrix::Identity(); }
...
BOOST_AUTO_TEST_CASE(test_material_collector) {
	...
	std::shared_ptr<const VolumeBounds> volumeBounds = std::make_shared<const CuboidVolumeBounds>(1_m, 1_m, 1_m);
	TrackingVolume const* trackingVolume = TrackingVolume::create(transform, volumeBounds).get();
	...
	std::shared_ptr<const IVolumeMaterial> volumeMaterial = std::make_shared<const HomogeneousVolumeMaterial>(material);
	TrackingVolume const* materialTrackingVolume = TrackingVolume::create(transform, volumeBounds, volumeMaterial).get();
	...
	BOOST_CHECK_EQUAL(result.currentVolume, state.navigation.currentVolume);
	BOOST_CHECK_EQUAL(result.sAccumulated, state.stepping.pathAccumulated);
	CHECK_CLOSE_ABS(result.sInX0, 0.5 * state.stepping.pathAccumulated / material.X0(), eps);
	BOOST_CHECK_EQUAL(result.previousPos, state.stepping.position);
	BOOST_CHECK_EQUAL(state.stepping.cov, BoundSymMatrix::Identity());
	...
	BOOST_CHECK_EQUAL(result.currentVolume, state.navigation.currentVolume);
	BOOST_CHECK_EQUAL(result.sAccumulated, state.stepping.pathAccumulated);
	CHECK_CLOSE_ABS(result.sInX0, 0.75 * state.stepping.pathAccumulated / material.X0(), eps);
	BOOST_CHECK_EQUAL(result.previousPos, state.stepping.position);
	BOOST_CHECK_EQUAL(state.stepping.cov, BoundSymMatrix::Identity());
	...
	// Check the resuts
	BOOST_CHECK_EQUAL(result.s, 0.);
	CHECK_CLOSE_ABS(result.qop0, state.stepping.charge / state.stepping.momentum, eps);
