...
BOOST_DATA_TEST_CASE(energy_loss_consistency, thickness* particle* momentum, x,
  ...
  auto dEBethe = computeEnergyLossBethe(slab, i, m, qOverP, absQ);
  auto dELandau = computeEnergyLossLandau(slab, i, m, qOverP, absQ);
  auto dELandauSigma = computeEnergyLossLandauSigma(slab, i, m, qOverP, absQ);
  auto dELandauSigmaQOverP =
      computeEnergyLossLandauSigmaQOverP(slab, i, m, qOverP, absQ);
...
BOOST_DATA_TEST_CASE(vacuum, thickness* particle* momentum, x, i, m, q, p) {
  ...
  BOOST_CHECK_EQUAL(computeEnergyLossBethe(vacuum, i, m, qOverP, absQ), 0);
  BOOST_CHECK_EQUAL(computeEnergyLossLandau(vacuum, i, m, qOverP, absQ), 0);
  BOOST_CHECK_EQUAL(computeEnergyLossLandauSigma(vacuum, i, m, qOverP, absQ),
                    0);
  BOOST_CHECK_EQUAL(
      computeEnergyLossLandauSigmaQOverP(vacuum, i, m, qOverP, absQ), 0);
...
BOOST_DATA_TEST_CASE(silicon_energy_loss,
  ...
  BOOST_CHECK_CLOSE(computeEnergyLossBethe(slab, absPdg, m, qOverP, absQ) /
...
BOOST_AUTO_TEST_CASE(silicon_landau) {
  ...
  const float absQ = 1_e;

  // Difference is within 5% from PDG value
  const auto dE = computeEnergyLossLandau(slab, pdg, m, qOverP, absQ) / 1_MeV;
