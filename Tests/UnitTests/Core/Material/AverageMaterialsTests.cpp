...
BOOST_AUTO_TEST_CASE(CombineSlabsVacuum) {
  ...
  // vacuum with unit thickness
  {
    ...
    BOOST_CHECK(!slab.isValid());
    BOOST_CHECK(!slab.material().isValid());
    BOOST_CHECK_EQUAL(slab.thickness(), 2.);
    BOOST_CHECK_EQUAL(slab.thicknessInX0(), 0.);
    BOOST_CHECK_EQUAL(slab.thicknessInL0(), 0.);
  }
}
...
BOOST_AUTO_TEST_CASE(CombineSlabsPercent) {
  ...
  // thickness-like properties must doubled
...
BOOST_AUTO_TEST_CASE(CombineSlabsEqualThicknessVacuum) {
  ...
  const auto slabVac = Acts::MaterialSlab(Acts::Material(), 1.);
  {
    ...
    BOOST_CHECK_EQUAL(slab.material().Z(), 0.5f * mat.Z());
...
BOOST_AUTO_TEST_CASE(CombineSlabs) {
  ...
  const auto slabMat1 = Acts::MaterialSlab(mat1, 1.);
  ...
  // check combined slabs
  {
    ...
    BOOST_CHECK_EQUAL(slab.material().Z(),
                      static_cast<float>(std::exp((0.5 / 1.5) * std::log(12.) +
                                                  (1. / 1.5) * std::log(6.))));
