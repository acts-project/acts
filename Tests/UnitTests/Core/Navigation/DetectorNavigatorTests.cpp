...
BOOST_AUTO_TEST_SUITE(DetectorNavigatorTests)
...
BOOST_AUTO_TEST_CASE(DetectorNavigatorTestsInitialization) {
    ...
    auto options = Acts::PropagatorOptions<ActionList, AbortList>(
        geoContext, mfContext);

    auto stepper = Acts::StraightLineStepper();

    Acts::Vector4 pos(-2, 0, 0, 0);
    auto start = Acts::CurvilinearTrackParameters(pos, 0_degree, 90_degree,
        1_e / 1_GeV, std::nullopt, Acts::ParticleHypothesis::electron());
    ...
    /// Run from endOfWorld
    {
        ...
        auto startEoW = Acts::CurvilinearTrackParameters(posEoW, 0_degree, 90_degree,
            1_e / 1_GeV, std::nullopt, Acts::ParticleHypothesis::electron());
    ...
    /// Initialize properly
    {
        ...
        BOOST_CHECK_EQUAL(initState.currentVolume, detector->findDetectorVolume(geoContext, start.position()));
        BOOST_CHECK_EQUAL(initState.currentSurface, nullptr);
        BOOST_CHECK_EQUAL(initState.currentPortal, nullptr);
...
BOOST_AUTO_TEST_CASE(DetectorNavigatorTestsForwardBackward) {
    ...
    auto surface3 = Acts::Surface::makeShared<Acts::PlaneSurface>(
        transform3 * Acts::Transform3(rotation), std::make_shared<Acts::RectangleBounds>(2, 2));
    auto volume3 = Acts::Experimental::DetectorVolumeFactory::construct(
        Acts::Experimental::defaultPortalAndSubPortalGenerator(), geoContext,
        "volume3", transform3, std::move(bounds3),
        {surface3}, {}, Acts::Experimental::tryNoVolumes(),
        Acts::Experimental::tryAllPortalsAndSurfaces());

    std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>> 
        detectorVolumes = {volume1, volume2, volume3};
    
    auto portalContainer = Acts::Experimental::detail::CuboidalDetectorHelper::connect(
    ...
    auto startFwd = Acts::CurvilinearTrackParameters(posFwd, 0_degree, 90_degree,
        1_e / 1_GeV, std::nullopt, Acts::ParticleHypothesis::electron());
    ...
    auto startBwd = Acts::CurvilinearTrackParameters(posBwd, 0_degree, 90_degree,
        1_e / 1_GeV, std::nullopt, Acts::ParticleHypothesis::electron());
...
BOOST_AUTO_TEST_CASE(DetectorNavigatorTestsAmbiguity) {
    ...
    auto surface = Acts::Surface::makeShared<Acts::CylinderSurface>(
        Acts::Transform3::Identity(), std::make_shared<Acts::CylinderBounds>(4, 9));
    auto volume = Acts::Experimental::DetectorVolumeFactory::construct(
        Acts::Experimental::defaultPortalAndSubPortalGenerator(), geoContext,
        "volume", Acts::Transform3::Identity(), std::move(bounds),
