...
BOOST_AUTO_TEST_CASE(FactoryTest) {
  ...
  stateManager2.pushState<MultiNavigationPolicy::State>();
  policy2.createState(gctx, args, stateManager2, *logger);
  auto policyState2 = stateManager2.currentState();
  delegate2(gctx, args, policyState2, stream, *logger);
...
BOOST_AUTO_TEST_CASE(FrustumNavigationPolicyTest){
  ...
  auto childCub = std::make_unique<TrackingVolume>(Transform3::Identity(), cubBounds, "child");
  ...
  auto cubNode = std::make_shared<Acts::StaticBlueprintNode>(std::move(childCub));
  int chamberId=1;
  ...
  for(int x=-3;x<4;x+=2){
    for(int y=-3;y<4;y+=2){
      for(int z=-3;z<4;z+=2){
	auto childVol=std::make_unique<TrackingVolume>(Transform3::Identity()*Translation3{Vector3{x*5_mm,y*5_mm,z*5_mm}},childBounds,"child_"+std::to_string(x)+"_"+std::to_string(y)+"_"+std::to_string(z));
	Acts::GeometryIdentifier chId = id.withLayer(chamberId++);
	childVol->assignGeometryId(chId);
	auto staticNode = std::make_shared<Acts::StaticBlueprintNode>(std::move(childVol));
	nodes.push_back(std::move(staticNode));
	cubNode->addChild(nodes.back());
      }
    }
  }
  std::shared_ptr<Acts::NavigationPolicyFactory> factory = std::make_shared<Acts::NavigationPolicyFactory>(Acts::NavigationPolicyFactory{}.add<Acts::Experimental::FrustumNavigationPolicy>(frustumConfig));
  ...
  NavigationPolicyStateManager stateManager;
  std::unique_ptr<Acts::Experimental::FrustumNavigationPolicy> frustumPolicy=std::make_unique<Acts::Experimental::FrustumNavigationPolicy>(gctx,*vol,*logger,frustumConfig);
  ...
  main.initialize(gctx, {Vector3{1,1,1}, Vector3{1,1,1}}, BoundaryTolerance::None());
