...
BOOST_AUTO_TEST_CASE(RootInputAMVF) {
  ...
  double m_time = 0;
  int m_nTracksEvent = 0;

  outtree->Branch("time", &m_time);
  outtree->Branch("nTracksEvent", &m_nTracksEvent);
  ...
  if (useRootFile) {
    ...
    std::cout << "loaded n events: " << events.size() << std::endl;
    ...
    for (auto evt : events) {
      ...
      // std::cout << "ACTS n vertices: " << allVertices.size() << std::endl;
      int counter = 0;
      for (auto vtx : allVertices) {
        ...
        // std::cout << "\t" << counter << ". ACTS vertex at pos: " << pos[0] <<
        // ", " << pos[1] << ", " << pos[2] << std::endl; std::cout << "\t with
        // cov: " << cov(0,0) << ", " << cov(1,1) << ", " << cov(2,2) <<
        // std::endl; std::cout << "\t with n tracks: " << nTracks << std::endl;
        // std::cout << "\t with chi2: " << vtx.fitQuality().first << std::endl;
        // std::cout << "\t with ndf: " << vtx.fitQuality().second << std::endl;
        counter++;
  ...
  //     Vector3D pos = Vector3D(-0.5, -0.5, 0);
  ...
  //     auto now12 = std::chrono::system_clock::now();
  //     auto ms1 =
  //         std::chrono::duration_cast<std::chrono::milliseconds>(now12 -
  //         now11).count();

  // std::cout << "... on AMVFTestData: time needed: " << ms1 << std::endl;
  ...
  // std::cout << "... on AMVFTestData: reco vertices: " << allVertices.size()
  // << std::endl;
...
readRootFile() {
  ...
  if (runLocally) {
    inputString =
        "/Users/bschlag/worklxplus_mount/private/vertexing/myAthena/"
        "actsAMVVTool/run/AMVF_athena_output_nEvent300.root";
  } else {
    inputString =
  ...
  // athena vertices
  std::vector<double>* m_athvx = 0;
  std::vector<double>* m_athvy = 0;
  std::vector<double>* m_athvz = 0;

  std::vector<double>* m_athcovx = 0;
  std::vector<double>* m_athcovy = 0;
  std::vector<double>* m_athcovz = 0;

  std::vector<double>* m_vchi2 = 0;
  std::vector<double>* m_vndf = 0;

  // std::vector<int>* m_athntrk = 0;
  std::vector<double>* m_athtrkwght = 0;

  theTree->SetBranchAddress("vx", &m_athvx);
  theTree->SetBranchAddress("vy", &m_athvy);
  theTree->SetBranchAddress("vz", &m_athvz);

  theTree->SetBranchAddress("vCovx", &m_athcovx);
  theTree->SetBranchAddress("vCovy", &m_athcovy);
  theTree->SetBranchAddress("vCovz", &m_athcovz);

  theTree->SetBranchAddress("vchi2", &m_vchi2);
  theTree->SetBranchAddress("vndf", &m_vndf);

  // theTree->SetBranchAddress("nTracks", &m_athntrk);
  theTree->SetBranchAddress("trkWeight", &m_athtrkwght);
  ...
  for (int iEvent = 0; iEvent < nEvents; iEvent++) {
    ...
    // std::cout << iEvent << ". event, n athena vertices: " << m_nVtx <<
    // std::endl;
    for (int i = 0; i < m_nVtx; i++) {
      // std::cout << "\t" << i << ". athena vertex at pos: " << (*m_athvx)[i]
      // << ", " << (*m_athvy)[i] << ", " << (*m_athvz)[i] << std::endl;
      // std::cout << "\t with cov: " << (*m_athcovx)[i] << ", " <<
      // (*m_athcovy)[i] << ", " << (*m_athcovz)[i] << std::endl; std::cout <<
      // "\t with n tracks: " << (*m_athntrk)[i] << std::endl; std::cout << "\t
      // with chi2: " << (*m_vchi2)[i] << std::endl; std::cout << "\t with ndf:
      // " << (*m_vndf)[i] << std::endl;
    }
    ...
    for (int i = 0; i < nTracks; i++) {
      ...
      covMat << (*m_cov11)[i], (*m_cov12)[i], (*m_cov13)[i], (*m_cov14)[i],
          ...
          (*m_cov55)[i] * 1. / (1_MeV * 1_MeV), 0, 0, 0, 0, 0, 0, 1;
