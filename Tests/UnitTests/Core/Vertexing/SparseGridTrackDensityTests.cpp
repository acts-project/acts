...
BOOST_AUTO_TEST_CASE(compare_to_analytical_solution_for_single_track) {
  ...
  SparseGridTrackDensity::Config cfg;
  ...
  SparseGridTrackDensity grid(cfg);

  // Empty map
  SparseGridTrackDensity::DensityMap mainDensityMap;
...
BOOST_AUTO_TEST_CASE(max_z_t_and_width) {
  ...
  cfg2D.useTime = true;
  SparseGridTrackDensity grid2D(cfg2D);
  ...
  // Empty maps
  SparseGridTrackDensity::DensityMap mainDensityMap1D;
  SparseGridTrackDensity::DensityMap mainDensityMap2D;
