...
BOOST_DATA_TEST_CASE(Baseline,
  ...
  if (shift <= 1.0) {
    ...
    if (strategy == VolumeAttachmentStrategy::Gap) {
      ...
      long double gapHlZ = (shift - 1.0) * hlZ;
      ...
      long double gap1Z = (-2 * hlZ * shift) + hlZ + gapHlZ;
      long double gap2Z = (2 * hlZ * shift) - hlZ - gapHlZ;
      ...
      long double wGap = (shift - 1.0) * hlZ * 2;
      ...
      BOOST_CHECK_EQUAL(newBounds1->get(CylinderVolumeBounds::eHalfLengthZ),
                        hlZ + wGap / 2.0);
      long double pZ1 = -2 * hlZ * shift + wGap / 2.0;
      ...
      BOOST_CHECK_EQUAL(newBounds3->get(CylinderVolumeBounds::eHalfLengthZ),
                        hlZ + wGap / 2.0);
      long double pZ3 = 2 * hlZ * shift - wGap / 2.0;
      Transform3 expectedTransform3 = base * Translation3{0_mm, 0_mm, pZ3};
      CHECK_CLOSE_OR_SMALL(vol3->transform().matrix(),
                           expectedTransform3.matrix(), 1e-10, 1e-14);
      ...
      BOOST_CHECK_EQUAL(newBounds1->get(CylinderVolumeBounds::eHalfLengthZ),
                        hlZ + wGap / 4.0);
      long double pZ1 = -2 * hlZ * shift + wGap / 4.0;
      Transform3 expectedTransform1 = base * Translation3{0_mm, 0_mm, pZ1};
      CHECK_CLOSE_OR_SMALL(vol1->transform().matrix(),
                           expectedTransform1.matrix(), 1e-10, 1e-14);
      ...
      BOOST_CHECK_EQUAL(newBounds3->get(CylinderVolumeBounds::eHalfLengthZ),
                        hlZ + wGap / 4.0);
      long double pZ3 = 2 * hlZ * shift - wGap / 4.0;
      Transform3 expectedTransform3 = base * Translation3{0_mm, 0_mm, pZ3};
      CHECK_CLOSE_OR_SMALL(vol3->transform().matrix(),
                           expectedTransform3.matrix(), 1e-10, 1e-14);
    }
...
BOOST_AUTO_TEST_CASE(Asymmetric) {
  long double hlZ1 = 200_mm;
  long double pZ1 = -1100_mm;
  long double hlZ2 = 600_mm;
  long double pZ2 = -200_mm;
  long double hlZ3 = 400_mm;
  long double pZ3 = 850_mm;
  ...
  long double midZ = (pZ1 - hlZ1 + pZ3 + hlZ3) / 2.0;
...
BOOST_DATA_TEST_CASE(RotationInZ, boost::unit_test::data::make(strategies),
                     strategy) {
  long double hlZ = 400_mm;
  long double gap = 100_mm;
  long double shift = 300_mm;
...
BOOST_DATA_TEST_CASE(Baseline,
  ...
  long double fInner = 1.0 + f;
  long double fOuter = 1.0 - f;
  ...
  BOOST_CHECK_EQUAL(stackBounds->get(CylinderVolumeBounds::eMaxR),
                    fOuter * 900_mm);
  long double expectedHalfLengthZ = (40_mm + 30_mm + 2 * hlZ) / 2.0;
  BOOST_CHECK_EQUAL(stackBounds->get(CylinderVolumeBounds::eHalfLengthZ),
                    expectedHalfLengthZ);
  ...
  for (std::size_t i = 0; i < volumes.size() - 1; ++i) {
    ...
    long double aMidR = (aBounds->get(CylinderVolumeBounds::eMinR) +
                         aBounds->get(CylinderVolumeBounds::eMaxR)) /
                        2.0;

    long double bMidR = (bBounds->get(CylinderVolumeBounds::eMinR) +
