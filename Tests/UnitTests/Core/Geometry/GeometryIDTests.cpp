...
BOOST_AUTO_TEST_CASE(GeometryID_max_values) {
  ...
  BOOST_CHECK_EQUAL(GeometryID(ref).setVolume(volumeMax + 1),
                    GeometryID(ref).setVolume(0u));
  BOOST_CHECK_EQUAL(GeometryID(ref).setBoundary(boundaryMax + 1),
                    GeometryID(ref).setBoundary(0u));
  BOOST_CHECK_EQUAL(GeometryID(ref).setLayer(layerMax + 1),
                    GeometryID(ref).setLayer(0u));
  BOOST_CHECK_EQUAL(GeometryID(ref).setApproach(approachMax + 1),
                    GeometryID(ref).setApproach(0u));
  BOOST_CHECK_EQUAL(GeometryID(ref).setSensitive(sensitiveMax + 1),
...
BOOST_AUTO_TEST_CASE(GeometryID_order) {
  ...
  BOOST_CHECK_LT(GeometryID(vol1).setBoundary(64u), vol2);
  BOOST_CHECK_LT(GeometryID(vol1).setLayer(64u), vol2);
  BOOST_CHECK_LT(GeometryID(vol1).setApproach(64u), vol2);
  BOOST_CHECK_LT(GeometryID(vol1).setSensitive(64u), vol2);
  BOOST_CHECK_LT(vol2, GeometryID(vol1).setVolume(3u));
  // other components are hierachical
  BOOST_CHECK_LT(GeometryID(vol1).setVolume(1u).setBoundary(2u),
                 GeometryID(vol1).setVolume(2u).setBoundary(1u));
  BOOST_CHECK_LT(GeometryID(vol1).setBoundary(1u).setLayer(2u),
                 GeometryID(vol1).setBoundary(2u).setLayer(1u));
  BOOST_CHECK_LT(GeometryID(vol1).setLayer(1u).setApproach(2u),
                 GeometryID(vol1).setLayer(2u).setApproach(1u));
  BOOST_CHECK_LT(GeometryID(vol1).setApproach(1u).setSensitive(2u),
