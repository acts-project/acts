...
BOOST_AUTO_TEST_CASE(test_constructors) {
    ...
    auto surface = Acts::Surface::makeShared<Acts::PlaneSurface>(Vector3::Ones(), Vector3::Ones().normalized());
    ...
    const auto aps = MultiComponentBoundTrackParameters<SinglyCharged>(surface, std::get<1>(a.front()), std::get<2>(a.front()));
    const auto bps = MultiComponentBoundTrackParameters<SinglyCharged>(surface, std::get<1>(b.front()), std::get<2>(b.front()));
