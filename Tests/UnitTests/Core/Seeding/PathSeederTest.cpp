                ...
                for (auto& surface : m_surfaces) {
                    ...
                    if (closestForward.status() == 
                        ...
                        closestForward.pathLength() > 0.0) {
                            sIntersections.push_back({closestForward.object()->geometryId(),closestForward.position()});
                            continue;
                    }
                }
...
class SourceLinkGrid : public ISourceLinkGrid<Axis> {
    public:
        ...
        void initialize(
            ...
            std::vector<SourceLink> sourceLinks) override {
                ...
                for (auto& sl : sourceLinks) {
                    ...
                    Acts::Vector3 globalPos = m_surfaceAccessor(
                        sl)->localToGlobal(
                            geoCtx, 
                            ssl.parameters, 
                            Acts::Vector3{0, 1, 0});

                    auto bin = lookupTable.at(id.sensitive()).localBinsFromPosition(
        ...
        eGrid getSourceLinkTable(const Acts::GeometryIdentifier& geoId) const override {
...
class TrackEstimator {
    public:
        ...
        std::tuple<ActsScalar, Vector3, Vector3, Vector3>
        operator()(
            const GeometryContext& /*geoCtx*/, 
...
std::shared_ptr<Experimental::Detector> 
constructTelescopeDetector() {
    ...
    auto surf0bounds =
        std::make_unique<RectangleBounds>(halfY, halfZ);
    ...
    auto transform1 =
        Transform3::Identity() * 
        Translation3(Vector3(2*deltaX, 0, 0));
    ...
    auto transform2 = 
        Transform3::Identity() *
        Translation3(Vector3(4*deltaX, 0, 0));
    ...
    auto transform3 = 
        Transform3::Identity() *
        Translation3(Vector3(6*deltaX, 0, 0));
...
std::vector<SourceLink> createSourceLinks(
    ...
    const Experimental::Detector& detector) {
        ...
        for (ActsScalar phi : phis) {
            ...
            auto intersections = intersectionFinder(
                geoCtx, vertex, direction);
            ...
            for (auto& [id,refPoint] : intersections) {
                ...
                Acts::detail::Test::TestSourceLink sourceLink(
...
BOOST_AUTO_TEST_CASE(PathSeederZeroField) {
    ...
    pathSeederCfg.intersectionFinder.connect<&NoFieldIntersectionFinder::operator()>(
        &intersectionFinder);
    ...
    firstLayerExtent.set(
        Acts::binX, -0.1, 0.1);
    firstLayerExtent.set(
        Acts::binY, -halfY - deltaYZ, halfY + deltaYZ);
    firstLayerExtent.set(
        Acts::binZ, -halfZ - deltaYZ, halfZ + deltaYZ);
