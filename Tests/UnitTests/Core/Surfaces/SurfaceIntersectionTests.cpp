...
BOOST_AUTO_TEST_CASE(CylinderIntersectionTests) {
  ...
  auto testCylinderIntersection = [&](const Transform3& transform) -> void {
    ...
    BOOST_CHECK_EQUAL(aIntersection[0].status(), Intersection3D::Status::reachable);
    ...
    BOOST_CHECK_EQUAL(aIntersection[1].status(), Intersection3D::Status::onSurface);
    ...
    BOOST_CHECK_EQUAL(cIntersection[0].status(), Intersection3D::Status::reachable);
    // There MUST be a second solution
    BOOST_CHECK(cIntersection[1]);
    // The other intersection MUST be reachable
    BOOST_CHECK_EQUAL(cIntersection[1].status(), Intersection3D::Status::reachable);
    // There MUST be one forward one backwards solution
    BOOST_CHECK_LT(cIntersection[1].pathLength() * cIntersection[0].pathLength(),
                0);
    ...
    // The status of this one MUST be reachable
    BOOST_CHECK_EQUAL(oIntersection[0].status(), Intersection3D::Status::reachable);
    // There MUST be a second solution
    BOOST_CHECK(oIntersection[1]);
    // The other intersection MUST be reachable
    BOOST_CHECK_EQUAL(oIntersection[1].status(), Intersection3D::Status::reachable);
    // There MUST be one forward one backwards solution
    BOOST_CHECK_GT(oIntersection[1].pathLength() * oIntersection[0].pathLength(),
                0);
    ...
    BOOST_CHECK_EQUAL(eIntersection[0].status(), Intersection3D::Status::reachable);
    // There MUST be a second solution
    BOOST_CHECK(eIntersection[1]);
    // The other intersection MUST be reachable
    BOOST_CHECK_EQUAL(eIntersection[1].status(), Intersection3D::Status::reachable);
    ...
    BOOST_CHECK_EQUAL(eIntersection[1].status(), Intersection3D::Status::missed);
...
BOOST_AUTO_TEST_CASE(PlanarIntersectionTest) {
  ...
  auto testPlanarIntersection = [&](const Transform3& transform) -> void {
    ...
    BOOST_CHECK_EQUAL(fIntersection[0].status(), Intersection3D::Status::reachable);
    ...
    BOOST_CHECK(oIntersection[0]);
    // The intersection MUST be reachable
    BOOST_CHECK_EQUAL(oIntersection[0].status(), Intersection3D::Status::onSurface);
    // The path length MUST be positive
    BOOST_CHECK_LT(std::abs(oIntersection[0].pathLength()), s_onSurfaceTolerance);
    // The intersection MUST be unique
    BOOST_CHECK(!oIntersection[1]);
    ...
    // The intersection MUST be reachable
    BOOST_CHECK_EQUAL(bIntersection[0].status(), Intersection3D::Status::reachable);
    ...
    // The intersection MUST be reachable
    BOOST_CHECK_EQUAL(mIntersection[0].status(), Intersection3D::Status::missed);
    // The path length MUST be negative
