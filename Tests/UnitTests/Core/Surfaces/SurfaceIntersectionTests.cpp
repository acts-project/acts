...
BOOST_AUTO_TEST_CASE(CylinderIntersectionTests) {
  ...
  auto testCylinderIntersection = [&](const Transform3& transform) -> void {
    ...
    // Check the validity of the intersection
    BOOST_CHECK(cIntersection.get<0>());
    // The status of this one MUST be reachable
    BOOST_CHECK(cIntersection.get<0>().status() ==
                Intersection3D::Status::reachable);
    // There MUST be a second solution
    BOOST_CHECK(cIntersection.get<1>());
    // The other intersection MUST be reachable
    BOOST_CHECK(cIntersection.get<1>().status() ==
                Intersection3D::Status::reachable);
    // There MUST be one forward one backwards solution
    BOOST_CHECK(cIntersection.get<1>().pathLength() *
                    cIntersection.get<0>().pathLength() <
                0);
    ...
    // Check the validity of the intersection
    BOOST_CHECK(oIntersection.get<0>());
    // The status of this one MUST be reachable
    BOOST_CHECK(oIntersection.get<0>().status() ==
                Intersection3D::Status::reachable);
    // There MUST be a second solution
    BOOST_CHECK(oIntersection.get<1>());
    // The other intersection MUST be reachable
    BOOST_CHECK(oIntersection.get<1>().status() ==
                Intersection3D::Status::reachable);
    // There MUST be one forward one backwards solution
    BOOST_CHECK(oIntersection.get<1>().pathLength() *
                    oIntersection.get<0>().pathLength() >
                0);
    ...
    // Check the validity of the intersection
    BOOST_CHECK(!iIntersection.get<0>());
    ...
    BOOST_CHECK(eIntersection.get<0>().status() ==
                Intersection3D::Status::reachable);
    // There MUST be a second solution
    BOOST_CHECK(eIntersection.get<1>());
    // The other intersection MUST be reachable
...
BOOST_AUTO_TEST_CASE(PlanarIntersectionTest) {
  ...
  auto testPlanarIntersection = [&](const Transform3& transform) -> void {
    ...
    BOOST_CHECK(fIntersection.get<0>());
    // The intersection MUST be reachable
    BOOST_CHECK(fIntersection.get<0>().status() ==
                Intersection3D::Status::reachable);
    // The path length MUST be positive
    BOOST_CHECK(fIntersection.get<0>().pathLength() > 0.);
    // The intersection MUST be unique
    BOOST_CHECK(!fIntersection.get<1>());
    ...
    BOOST_CHECK(oIntersection.get<0>().status() ==
                Intersection3D::Status::onSurface);
    // The path length MUST be positive
    BOOST_CHECK(std::abs(oIntersection.get<0>().pathLength()) <
                s_onSurfaceTolerance);
    // The intersection MUST be unique
    BOOST_CHECK(!oIntersection.get<1>());
    ...
    // The intersection MUST be valid
    BOOST_CHECK(bIntersection.get<0>());
    // The intersection MUST be reachable
    BOOST_CHECK(bIntersection.get<0>().status() ==
                Intersection3D::Status::reachable);
    // The path length MUST be negative
    BOOST_CHECK(bIntersection.get<0>().pathLength() < 0.);
    // The intersection MUST be unique
    BOOST_CHECK(!bIntersection.get<1>());
    ...
    BOOST_CHECK(!mIntersection.get<0>());
    // The intersection MUST be reachable
    BOOST_CHECK(mIntersection.get<0>().status() ==
                Intersection3D::Status::missed);
    // The path length MUST be negative
    BOOST_CHECK(mIntersection.get<0>().pathLength() > 0.);
    // The intersection MUST be unique
    BOOST_CHECK(!mIntersection.get<1>());
    ...
    BOOST_CHECK(!iIntersection.get<0>());
    // The intersection MUST be reachable
    BOOST_CHECK(iIntersection.get<0>().status() ==
                Intersection3D::Status::unreachable);
    // The intersection MUST be unique
    BOOST_CHECK(!iIntersection.get<1>());
  };
