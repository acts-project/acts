...
/// Unit test for creating compliant/non-compliant PointSurface object
BOOST_AUTO_TEST_CASE(PointSurface_Constructors_test) {
  ...
  BOOST_CHECK(PointSurfaceStub(translation, radius).constructedOk());
  // ctor with nullptr for LineBounds
  BOOST_CHECK(PointSurfaceStub(translation).constructedOk());
  // ctor with LineBounds
  auto pPointBounds = std::make_shared<const PointBounds>(2.);
  BOOST_CHECK(PointSurfaceStub(translation, pPointBounds).constructedOk());
  // ctor with LineBounds, detector element, Identifier
  auto pMaterial =
      std::make_shared<const HomogeneousSurfaceMaterial>(makePercentSlab());
  DetectorElementStub detElement{translation, pPointBounds, 0.2, pMaterial};
  BOOST_CHECK(PointSurfaceStub(pPointBounds, detElement).constructedOk());
  PointSurfaceStub lineToCopy(translation, 2.0);
  // Copy ctor
  BOOST_CHECK(PointSurfaceStub(lineToCopy).constructedOk());
  // Copied and transformed ctor
  BOOST_CHECK(
      PointSurfaceStub(tgContext, lineToCopy, translation).constructedOk());

  /// Construct with nullptr bounds
  DetectorElementStub detElem;
  BOOST_CHECK_THROW(PointSurfaceStub nullBounds(nullptr, detElem),
                    AssertionFailureException);

  BOOST_TEST_MESSAGE(
      "All PointSurface constructors are callable without problem");
}
/// Unit tests of all named methods
BOOST_AUTO_TEST_CASE(PointSurface_allNamedMethods_test) {
  // binningPosition()
  Translation3 translation{0., 1., 2.};
  PointSurfaceStub line(translation, 2.0);
  Vector3 referencePosition{0., 1., 2.};
  CHECK_CLOSE_ABS(referencePosition, line.binningPosition(tgContext, binX),
                  1e-6);
  //
  // bounds()
  auto pPointBounds = std::make_shared<const PointBounds>(2., 10.0);
  PointSurfaceStub boundedLine(translation, pPointBounds);
  const PointBounds& bounds =
      dynamic_cast<const PointBounds&>(boundedLine.bounds());
  BOOST_CHECK_EQUAL(bounds, PointBounds(2.));
  //
  // globalToLocal()
  Vector3 gpos{0., 1., 0.};
  ...
  output << line.name();
  BOOST_CHECK(output.is_equal("Acts::PointSurface"));
  //
  // normal
...
/// Unit test for testing PointSurface assignment
BOOST_AUTO_TEST_CASE(PointSurface_assignment_test) {
  Translation3 translation{0., 1., 2.};
  PointSurfaceStub originalLine(translation, 2.0);
  PointSurfaceStub assignedLine(translation, 1.0);
  BOOST_CHECK(assignedLine != originalLine);  // operator != from base
  assignedLine = originalLine;
  BOOST_CHECK(assignedLine == originalLine);  // operator == from base
}

/// Unit test for testing PointSurface alignment derivatives
BOOST_AUTO_TEST_CASE(PointSurfaceAlignment) {
  Translation3 translation{0., 1., 2.};
  Transform3 transform(translation);
  PointSurfaceStub line(translation, 2.0);
...
BOOST_AUTO_TEST_CASE(PointSurfaceTransformRoundTrip) {
  PointSurfaceStub surface(Translation3::Identity());
...
BOOST_AUTO_TEST_CASE(PointSurfaceTransformRoundTripEtaStability) {
  PointSurfaceStub surface(Translation3::Identity());
