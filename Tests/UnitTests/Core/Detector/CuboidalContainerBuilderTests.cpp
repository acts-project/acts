    ...
    public:
        ...
        DetectorComponent construct(
            ...
            auto surface = Acts::Surface::makeShared<Acts::PlaneSurface>(
                (m_transform), std::make_shared<Acts::RectangleBounds>(m_surfaceBounds));
...
BOOST_AUTO_TEST_CASE(CuboidalContainerBuildingXYZVolumes) {
    std::array<Acts::BinningValue, 3> binningValues = 
    ...
    for (auto bVal : binningValues) {
        ...
        auto builderA =
            std::make_shared<CuboidalVolumeBuilder>(
                Acts::Transform3::Identity(), 
                box, Acts::RectangleBounds(2,2), "VolumeA");

        auto builderB =
            std::make_shared<CuboidalVolumeBuilder>(
                transformB, 
                box, Acts::RectangleBounds(2,2), "VolumeB");
        ...
        BOOST_CHECK_EQUAL(roots.volumes.at(0)->transform(tContext).translation()[bVal], 0);
        BOOST_CHECK_EQUAL(roots.volumes.at(1)->transform(tContext).translation()[bVal], 20);

        for (auto& portal : portals) {
            if (portal.second->attachedDetectorVolumes().at(0).size() == 0) {
                BOOST_CHECK_EQUAL(portal.second->attachedDetectorVolumes().at(1).size(), 2u);
            }
            else {
                BOOST_CHECK_EQUAL(portal.second->attachedDetectorVolumes().at(0).size(), 2u);
            }
        }
    }
}
