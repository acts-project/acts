...
class CompBuilder final : public Acts::Experimental::IDetectorComponentBuilder {
 public:
  explicit CompBuilder(
      const std::vector<Acts::MaybeSharedPtr<Acts::Surface>>& sensitives = {})
 ...
 private:
  std::vector<Acts::MaybeSharedPtr<Acts::Surface>> m_sensitives;
};
...
BOOST_AUTO_TEST_CASE(DetectorBuilder_Misconfigured) {
  ...
  std::vector<Acts::MaybeSharedPtr<Acts::Surface>> sensitives;
