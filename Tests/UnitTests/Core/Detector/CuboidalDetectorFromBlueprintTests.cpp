...
BOOST_AUTO_TEST_CASE(CuboidalDetectorFromBlueprintTest) {
    ...
    std::vector<Acts::ActsScalar> detectorBounds = 
    ...
    // Left arm
    std::vector<Acts::ActsScalar> leftArmBounds = 
        {detectorX*0.5, detectorY, detectorZ};
    ...
    auto leftArm = std::make_unique<Acts::Experimental::Blueprint::Node>(
        "leftArm", leftArmTransform, Acts::VolumeBounds::eCuboid,
        leftArmBounds, leftArmBins);

    // Pixel layer L1
    std::vector<Acts::ActsScalar> pixelL1Boundaries = 
    ...
    auto pixelL1Structure =
        std::make_shared<SurfaceBuilder>(
            pixelL1Transform, 
            Acts::RectangleBounds(pixelX*0.8, pixelY*0.8));
    ...
    // Pixel layer L2
    std::vector<Acts::ActsScalar> pixelL2Boundaries = 
        {pixelX, pixelY, pixelZ};
    ...
    // Right arm
    std::vector<Acts::ActsScalar> rightArmBounds = 
        {detectorX*0.5, detectorY, detectorZ};
    ...
    // Pixel layer R1
    std::vector<Acts::ActsScalar> pixelR1Boundaries = 
        {pixelX, pixelY, pixelZ};
    ...
    // Pixel layer R2
    std::vector<Acts::ActsScalar> pixelR2Boundaries = 
        {pixelX, pixelY, pixelZ};
    ...
    dCfg.auxiliary =
        "*** Test : auto generated cuboidal detector builder  ***";
    ...
    for (auto& volume : detector->volumes()) {
        portals.insert(portals.end(), 
            volume->portals().begin(), 
            volume->portals().end());
    }
    ...
    for (int i = 0; i < 4; i++) {
        samePortalY1 = samePortalY1 && 
            (detector->volumes()[i]->portals().at(4) == 
                detector->volumes()[i+1]->portals().at(4));
        samePortalY2 = samePortalY2 &&
            (detector->volumes()[5+i]->portals().at(4) == 
    ...
    for (int i = 0; i < 4; i++) {
        samePortalY1 = samePortalY1 && 
            (detector->volumes()[i]->portals().at(5) == 
                detector->volumes()[i+1]->portals().at(5));
        samePortalY2 = samePortalY2 &&
            (detector->volumes()[5+i]->portals().at(5) == 
    ...
    for (int i = 0; i < 4; i++) {
        bool samePortalZ1 =
            (detector->volumes()[i]->portals().at(1) == 
                detector->volumes()[i+1]->portals().at(0));
        bool samePortalZ2 =
            (detector->volumes()[5+i]->portals().at(1) == 
    ...
    for (int i = 0; i < 5; i++) {
        bool samePortalX =
            (detector->volumes()[i]->portals().at(3) == 
