...
#define BOOST_TEST_MODULE MultiIndex
...
BOOST_AUTO_TEST_CASE_TEMPLATE(triviality, T, Indices) {
  ...
  BOOST_TEST(std::is_standard_layout_v<T>);
  ...
  BOOST_TEST((std::is_trivially_assignable_v<T, T>));
...
BOOST_AUTO_TEST_CASE(index32_construct) {
  // construct from encoded value
  {
    ...
    BOOST_TEST(idx.value() == 0xabcd2400u);
    BOOST_TEST(idx.level(0) == 0xabcdu);
    BOOST_TEST(idx.level(1) == 0x24u);
    BOOST_TEST(idx.level(2) == 0x00u);
  }
...
BOOST_AUTO_TEST_CASE(index32_set) {
  ...
  idx.set(0, 24u);
  BOOST_TEST(idx.value() == 0x00180000u);
  BOOST_TEST(idx.level(0) == 24u);
  BOOST_TEST(idx.level(1) == 0u);
  BOOST_TEST(idx.level(2) == 0u);
  // set a specific level outside the valid range, should be truncated
  idx.set(2, 0xfff);
  BOOST_TEST(idx.value() == 0x001800ffu);
  BOOST_TEST(idx.level(0) == 24u);
  BOOST_TEST(idx.level(1) == 0u);
  BOOST_TEST(idx.level(2) == 255u);
}
...
BOOST_AUTO_TEST_CASE(index32_set_overflow) {
  ...
  for (auto maxValue : maxValues) {
    BOOST_TEST(Index32::Zeros().set(lvl, maxValue + 1) ==
...
BOOST_AUTO_TEST_CASE(index64_set_overflow) {
  ...
  std::size_t lvl = 0;
  for (auto maxValue : maxValues) {
    BOOST_TEST(Index64::Zeros().set(lvl, maxValue + 1) ==
...
BOOST_AUTO_TEST_CASE(index32_encode) {
  ...
  BOOST_TEST(idx3.level(0) == 13u);
  BOOST_TEST(idx3.level(1) == 14u);
  BOOST_TEST(idx3.level(2) == 15u);
  // first two levels set
  auto idx2 = Index32::Encode(13u, 14u);
  BOOST_TEST(idx2.level(0) == 13u);
  BOOST_TEST(idx2.level(1) == 14u);
  BOOST_TEST(idx2.level(2) == 0u);
  // only the first level set
  auto idx1 = Index32::Encode(13u);
  BOOST_TEST(idx1.level(0) == 13u);
  BOOST_TEST(idx1.level(1) == 0u);
  BOOST_TEST(idx1.level(2) == 0u);
  // nothing set
  auto idx0 = Index32::Encode();
  BOOST_TEST(idx0 == Index32::Zeros());
  BOOST_TEST(idx0.value() == 0u);
  BOOST_TEST(idx0.level(0) == 0u);
  BOOST_TEST(idx0.level(1) == 0u);
  BOOST_TEST(idx0.level(2) == 0u);
}

BOOST_AUTO_TEST_CASE(index64_encode) {
  // all four levels set
  auto idx4 = Index64::Encode(23u, 14u, 15u, 17u);
  BOOST_TEST(idx4.level(0) == 23u);
  BOOST_TEST(idx4.level(1) == 14u);
  BOOST_TEST(idx4.level(2) == 15u);
  BOOST_TEST(idx4.level(3) == 17u);
  // first three levels set
  auto idx3 = Index64::Encode(23u, 14u, 15u);
  BOOST_TEST(idx3.level(0) == 23u);
  BOOST_TEST(idx3.level(1) == 14u);
  BOOST_TEST(idx3.level(2) == 15u);
  BOOST_TEST(idx3.level(3) == 0u);
  // first two levels set
  auto idx2 = Index64::Encode(23u, 14u);
  BOOST_TEST(idx2.level(0) == 23u);
  BOOST_TEST(idx2.level(1) == 14u);
  BOOST_TEST(idx2.level(2) == 0u);
  BOOST_TEST(idx2.level(3) == 0u);
  // only the first level set
  auto idx1 = Index64::Encode(23u);
  BOOST_TEST(idx1.level(0) == 23u);
  BOOST_TEST(idx1.level(1) == 0u);
  BOOST_TEST(idx1.level(2) == 0u);
  BOOST_TEST(idx1.level(3) == 0u);
  // nothing set
  auto idx0 = Index64::Encode();
  BOOST_TEST(idx0 == Index64::Zeros());
  BOOST_TEST(idx0.value() == 0u);
  BOOST_TEST(idx0.level(0) == 0u);
  BOOST_TEST(idx0.level(1) == 0u);
  BOOST_TEST(idx0.level(2) == 0u);
  BOOST_TEST(idx0.level(3) == 0u);
}

BOOST_AUTO_TEST_CASE(uint32_sibling) {
  auto idx = Index32::Encode(1u, 12u, 123u);
  BOOST_TEST(idx.makeNextSibling(0) == Index32::Encode(2u));
  BOOST_TEST(idx.makeNextSibling(1) == Index32::Encode(1u, 13u));
  BOOST_TEST(idx.makeNextSibling(2) == Index32::Encode(1u, 12u, 124u));
}

BOOST_AUTO_TEST_CASE(uint64_sibling) {
  auto idx = Index64::Encode(1u, 12u, 123u, 1234u);
  BOOST_TEST(idx.makeNextSibling(0) == Index64::Encode(2u));
  BOOST_TEST(idx.makeNextSibling(1) == Index64::Encode(1u, 13u));
  BOOST_TEST(idx.makeNextSibling(2) == Index64::Encode(1u, 12u, 124u));
  BOOST_TEST(idx.makeNextSibling(3) == Index64::Encode(1u, 12u, 123u, 1235u));
}

BOOST_AUTO_TEST_CASE(uint32_descendant) {
  auto idx = Index32::Encode(1u, 12u, 123u);
  BOOST_TEST(idx.makeLastDescendant(0) == Index32::Encode(1u, 255u, 255u));
  BOOST_TEST(idx.makeLastDescendant(1) == Index32::Encode(1u, 12u, 255u));
  // last index has no descendant and stays the same
  BOOST_TEST(idx.makeLastDescendant(2) == Index32::Encode(1u, 12u, 123u));
}
...
BOOST_AUTO_TEST_CASE(uint64_descendant) {
  ...
  auto max2 = (1u << 21) - 1u;
  auto max3 = (1u << 13) - 1u;
  BOOST_TEST(idx.makeLastDescendant(0) ==
             Index64::Encode(1u, max1, max2, max3));
  BOOST_TEST(idx.makeLastDescendant(1) == Index64::Encode(1u, 12u, max2, max3));
  BOOST_TEST(idx.makeLastDescendant(2) == Index64::Encode(1u, 12u, 123u, max3));
  // last index has no descendant and stays the same
  BOOST_TEST(idx.makeLastDescendant(3) ==
...
BOOST_AUTO_TEST_CASE(index32_as_key) {
  ...
  BOOST_TEST(not set.count(Index32(0u)));
  BOOST_TEST(not set.count(Index32(UINT32_MAX)));
  BOOST_TEST(set.size() == 3);
  // automatically converts encoded value to MultiIndex
  BOOST_TEST(set.count(0x00010204u));
  BOOST_TEST(set.count(0x00010304u));
  BOOST_TEST(set.count(Index32::Encode(2u)));
}
...
BOOST_AUTO_TEST_CASE(index64_as_key) {
  ...
  BOOST_TEST(not set.count(Index64(0u)));
  BOOST_TEST(not set.count(Index64(UINT64_MAX)));
  BOOST_TEST(set.size() == 3);
}
