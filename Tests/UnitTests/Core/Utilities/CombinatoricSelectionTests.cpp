...
struct ArraySorter{
    template <std::size_t K> bool operator()(const std::array<std::size_t, K>&a,
...
template <std::size_t K> std::ostream& operator<<(std::ostream& ostr,const std::array<std::size_t, K>&x) {
    ostr<<"[";
...
template <std::size_t K> using CombinationSet = std::set<std::array<std::size_t, K>, ArraySorter>;
...
BOOST_AUTO_TEST_CASE(CombinationDraw4D) {
    ...
    for (std::size_t comb = 0; comb < indexGenerator.size(); ++comb) {
        ...
        std::cout<<"Iteration: "<<comb<<"-> drawn indices: "<<combination<<std::endl;
