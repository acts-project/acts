...
>;
using ScalarsAndVariableSizeSubspace = std::tuple<
  std::tuple<float, detail::VariableSizeSubspace<5u>>,
  std::tuple<float, detail::VariableSizeSubspace<6u>>,
  std::tuple<float, detail::VariableSizeSubspace<8u>>,
  std::tuple<double, detail::VariableSizeSubspace<5u>>,
  std::tuple<double, detail::VariableSizeSubspace<6u>>,
  std::tuple<double, detail::VariableSizeSubspace<8u>>,
  std::tuple<double, detail::VariableSizeSubspace<5u, 3u>>,
  std::tuple<double, detail::VariableSizeSubspace<6u, 4u>>,
  std::tuple<double, detail::VariableSizeSubspace<8u, 4u>>
>;
// clang-format on
...
BOOST_AUTO_TEST_CASE_TEMPLATE(VariableSizeSubspaceFloat, ScalarAndSubspace,
                              ScalarsAndVariableSizeSubspace) {
  // extract the test types
  using Scalar = std::tuple_element_t<0, ScalarAndSubspace>;
  using Subspace = std::tuple_element_t<1, ScalarAndSubspace>;
  ...
  // give a reasonable set of different subspace configurations.
  do {
    // create a copy of the current indices permutation so we can modify it
    // without breaking the permutation loop
    std::vector<std::size_t> indices = fullIndices;

    // test all possible smaller subspaces
    for (auto n = 1u; n < Subspace::maxSize(); ++n) {
      // sort the first n elements of the current permutation so they are a
      // valid input to the subspace constructor
      auto subspaceIndicesEnd = std::next(indices.begin(), n);
      std::sort(indices.begin(), subspaceIndicesEnd);
      Subspace subspace(indices.begin(), subspaceIndicesEnd);
      BOOST_CHECK_EQUAL(subspace.size(), n);
      ...
      BOOST_CHECK_EQUAL(subspace.template expander<Scalar>().transpose(),
                        subspace.template projector<Scalar>());
      // project into the variable subspace
      auto s0 = subspace.projectVector(x);
      auto s1 = (subspace.template projector<Scalar>() * x).eval();
      // since the subspace is variable, check that the projected vector match
      BOOST_CHECK_EQUAL(s0.size(), n);
      BOOST_CHECK_EQUAL(s1.size(), n);
      for (auto i = 0u; i < n; ++i) {
        BOOST_TEST_CHECKPOINT("Checking projected subspace component " << i);
        BOOST_CHECK_EQUAL(s0[i], x[indices[i]]);
        BOOST_CHECK_EQUAL(s1[i], x[indices[i]]);
      }
      // expand from the variable subspace back into the full space
      auto y0 = subspace.expandVector(s1);
      auto y1 = (subspace.template expander<Scalar>() * s0).eval();
      for (auto i = 0u; i < Subspace::fullSize(); ++i) {
        BOOST_TEST_CHECKPOINT("Checking expanded fullspace component " << i);
        BOOST_CHECK_EQUAL(y0[i], subspace.contains(i) ? x[i] : 0);
        BOOST_CHECK_EQUAL(y1[i], subspace.contains(i) ? x[i] : 0);
      }
    }
