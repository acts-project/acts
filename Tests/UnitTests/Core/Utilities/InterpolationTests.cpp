...
BOOST_AUTO_TEST_CASE(interpolation_2d) {
  ...
  const Point low{1., 1.};
  const Point high{2., 3.};
  const Values v{10., 30., 20., 40.};
...
BOOST_AUTO_TEST_CASE(interpolation_3d) {
  ...
  const Point low{1., 1., 1.};
  const Point high{2., 3., 4.};
  const Values v{10., 50., 30., 70., 20., 60., 40., 80.};
  ...
  CHECK_CLOSE_REL(interpolate(Point{1.5, 2., 2.5}, low, high, v), 360. / 8,
                  1e-6);
  CHECK_CLOSE_REL(interpolate(Point{1.3, 2.1, 1.6}, low, high, v), 32., 1e-6);
}
...
BOOST_AUTO_TEST_CASE(interpolation_mixed_point_values) {
  ...
  CHECK_CLOSE_REL(interpolate(Point1{1}, low, high, v), 10., 1e-6);
  CHECK_CLOSE_REL(interpolate(Point1{1.3}, low, high, v), 13., 1e-6);
  CHECK_CLOSE_REL(interpolate(Point1{1.5}, low, high, v), 15., 1e-6);
  CHECK_CLOSE_REL(interpolate(Point1{1.8}, low, high, v), 18., 1e-6);
  CHECK_CLOSE_REL(interpolate(Point1{2}, low, high, v), 20., 1e-6);
}
