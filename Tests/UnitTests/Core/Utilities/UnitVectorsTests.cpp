...
BOOST_AUTO_TEST_CASE(DirectionPhiEta) {
  ...
  // along positive z
  const auto zPos1 = makeDirectionUnitFromPhiEta<Acts::ActsScalar>(0.0, inf);
  CHECK_CLOSE_REL(zPos1.norm(), 1, eps);
  CHECK_CLOSE_REL(zPos1.dot(Vector3D(0, 0, 1)), 1, eps);
  const auto zPos2 = makeDirectionUnitFromPhiEta<Acts::ActsScalar>(M_PI_2, inf);
  CHECK_CLOSE_REL(zPos2.norm(), 1, eps);
  CHECK_CLOSE_REL(zPos2.dot(Vector3D(0, 0, 1)), 1, eps);
  // along negative z
  const auto zNeg1 = makeDirectionUnitFromPhiEta<Acts::ActsScalar>(0.0, -inf);
  CHECK_CLOSE_REL(zNeg1.norm(), 1, eps);
  CHECK_CLOSE_REL(zNeg1.dot(Vector3D(0, 0, -1)), 1, eps);
  const auto zNeg2 =
      makeDirectionUnitFromPhiEta<Acts::ActsScalar>(M_PI_2, -inf);
  CHECK_CLOSE_REL(zNeg2.norm(), 1, eps);
  CHECK_CLOSE_REL(zNeg2.dot(Vector3D(0, 0, -1)), 1, eps);

  // mixed direction
  const auto mixed1 =
      makeDirectionUnitFromPhiEta<Acts::ActsScalar>(M_PI_4, 1.0);
...
BOOST_AUTO_TEST_CASE(DirectionPhiTheta) {
  ...
  // along positive x
  const auto xPos1 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(0.0, M_PI_2);
  CHECK_CLOSE_REL(xPos1.norm(), 1, eps);
  CHECK_CLOSE_REL(xPos1.dot(Vector3D(1, 0, 0)), 1, eps);
  const auto xPos2 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(2 * M_PI, M_PI_2);
  CHECK_CLOSE_REL(xPos2.norm(), 1, eps);
  CHECK_CLOSE_REL(xPos2.dot(Vector3D(1, 0, 0)), 1, eps);
  // along negative x
  const auto xNeg1 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(M_PI, M_PI_2);
  CHECK_CLOSE_REL(xNeg1.norm(), 1, eps);
  CHECK_CLOSE_REL(xNeg1.dot(Vector3D(-1, 0, 0)), 1, eps);
  const auto xNeg2 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(-M_PI, M_PI_2);
  ...
  // along positive y
  const auto yPos1 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(M_PI_2, M_PI_2);
  CHECK_CLOSE_REL(yPos1.norm(), 1, eps);
  CHECK_CLOSE_REL(yPos1.dot(Vector3D(0, 1, 0)), 1, eps);
  const auto yPos2 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(-3 * M_PI_2, M_PI_2);
  CHECK_CLOSE_REL(yPos2.norm(), 1, eps);
  CHECK_CLOSE_REL(yPos2.dot(Vector3D(0, 1, 0)), 1, eps);
  // along negative y
  const auto yNeg1 =
  ...
  // along positive z
  const auto zPos1 = makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(0.0, 0.0);
  CHECK_CLOSE_REL(zPos1.norm(), 1, eps);
  CHECK_CLOSE_REL(zPos1.dot(Vector3D(0, 0, 1)), 1, eps);
  const auto zPos2 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(M_PI_2, 0.0);
  CHECK_CLOSE_REL(zPos2.norm(), 1, eps);
  CHECK_CLOSE_REL(zPos2.dot(Vector3D(0, 0, 1)), 1, eps);
  // along negative z
  const auto zNeg1 = makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(0.0, M_PI);
  CHECK_CLOSE_REL(zNeg1.norm(), 1, eps);
  CHECK_CLOSE_REL(zNeg1.dot(Vector3D(0, 0, -1)), 1, eps);
  const auto zNeg2 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(M_PI_2, M_PI);
  CHECK_CLOSE_REL(zNeg2.norm(), 1, eps);
  CHECK_CLOSE_REL(zNeg2.dot(Vector3D(0, 0, -1)), 1, eps);

  // mixed direction
  const auto mixed1 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(M_PI_4, M_PI_4);
  CHECK_CLOSE_REL(mixed1.norm(), 1, eps);
  CHECK_CLOSE_REL(mixed1.dot(Vector3D(1, 1, M_SQRT2).normalized()), 1, eps);
  const auto mixed2 =
      makeDirectionUnitFromPhiTheta<Acts::ActsScalar>(M_PI_4, 3 * M_PI_4);
  CHECK_CLOSE_REL(mixed2.norm(), 1, eps);
