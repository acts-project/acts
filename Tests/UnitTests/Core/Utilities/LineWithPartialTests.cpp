...
ParamVector makeTrack(RandomEngine& rndEngine){
    ...
    pars[ParIndices::x0]    = static_cast<double>(rndEngine() % 1000) - 500; // Random x0 in [-50, 50]
    pars[ParIndices::y0]    = static_cast<double>(rndEngine() % 1000) - 500; // Random y0 in [-50, 50]
    pars[ParIndices::theta] = static_cast<double>(rndEngine() % 180) * 1_degree; // Random theta in [0, 180)
    pars[ParIndices::phi]   = static_cast<double>(rndEngine() % 360) * 1_degree; // Random phi in [-180, 180)
    return pars;
}
...
BOOST_AUTO_TEST_CASE(lineParameterTest){
    ...
    for (unsigned i = 0; i < trials; ++i) {
        ...
        BOOST_CHECK_CLOSE(newLine.position()[Acts::eX], pars[ParIndices::x0], tolerance);
        BOOST_CHECK_CLOSE(newLine.position()[Acts::eY], pars[ParIndices::y0], tolerance);
        const Acts::Vector3 dir = Acts::makeDirectionFromPhiTheta(pars[ParIndices::phi], pars[ParIndices::theta]);
...
BOOST_AUTO_TEST_CASE(lineGradientTest) {
    ...
    for (unsigned trial = 0; trial < trials; ++trial){
        ...
        std::cout<<"lineGradientTest -- Generated parameters: "<<pars.transpose()<<std::endl;
        Line_t segLine{};
        ...
        for (const int param : {ParIndices::theta , ParIndices::phi} ){
            ...
            const Vector3 numDeriv{(segLineUp.direction() - segLineDn.direction())/ (2.*h)};
            ...
            for (const int param1 : {ParIndices::theta , ParIndices::phi}){
                ...
                const Vector3 numDeriv1{ (segLineUp.gradient(param) - segLineDn.gradient(param)) / (2. *h) };
