...
BOOST_AUTO_TEST_CASE(TrackParametersJsonIO) {
    ...
    Acts::FreeTrackParameters ftp(position, phi, theta, qOverP, freeCov, particle);
    ...
    Acts::CurvilinearTrackParameters ctp(position, phi, theta, qOverP, boundCov, particle);
    ...
    BOOST_CHECK(ctp.referenceSurface().transform(gctx).isApprox(ctpRead.referenceSurface().transform(gctx)));
    BOOST_CHECK_EQUAL(ctp.referenceSurface().geometryId(), ctpRead.referenceSurface().geometryId());
    BOOST_CHECK_EQUAL(ctp.referenceSurface().bounds(), ctpRead.referenceSurface().bounds());
    ...
    BOOST_CHECK(btp.referenceSurface().transform(gctx).isApprox(btpRead.referenceSurface().transform(gctx)));
    BOOST_CHECK_EQUAL(btp.referenceSurface().geometryId(), btpRead.referenceSurface().geometryId());
    BOOST_CHECK_EQUAL(btp.referenceSurface().bounds(), btpRead.referenceSurface().bounds());
}
