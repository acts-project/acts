...
BOOST_AUTO_TEST_CASE(GeoModelDetectorObjectFactory) {
  ...
  std::vector<std::vector<double>> trapVerts = {{-103,-50},{103,-50},{183,50},{-183,50}};
  std::vector<std::vector<double>> polyVerts ={{-60,-50},{60,-50},{153,0},{123,50},{-123,50},{-153,0}};
  std::vector<std::vector<double>> errVerts ={{60,-50},{153,0},{123,50},{-123,50},{-153,0}};
  double poly_z = 2;
  ...
  BOOST_CHECK_THROW(factory.convertFpv("Error", physErr, errCache, gContext), std::runtime_error);
  
  Acts::GeoModelSensitiveSurface trapSensSurface = trapCache.sensitiveSurfaces[0];
  Acts::GeoModelSensitiveSurface polySensSurface = polyCache.sensitiveSurfaces[0];
  ...
  const auto* polyBounds = dynamic_cast<const Acts::DiamondBounds*>(&polySurface->bounds());
  ...
  const auto* trapBounds = dynamic_cast<const Acts::TrapezoidBounds*>(&trapSurface->bounds());
