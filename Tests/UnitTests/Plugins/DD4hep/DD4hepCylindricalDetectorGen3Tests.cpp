  ...
  cylinder.addMaterial("Pixel_Material", [&](auto& mat) {
    ...
    for (int ecid : {-1, 1}) {
      ...
      for (const auto& [key, surfaces] : initialLayers) {
        auto& layer = protoLayers.emplace_back(GeometryContext::dangerouslyDefaultConstruct(), surfaces);
      ...
      std::vector<LayerData> mergedLayers =
          mergeLayers(GeometryContext::dangerouslyDefaultConstruct(), protoLayers);
