...
BOOST_AUTO_TEST_CASE(ConvertTrackParametersToEdm4hepWithOutPerigee) {
  ...
  std::cout << "planePar.covariance()\n"
            << planePar.covariance().value() << std::endl;
  ...
  std::cout << "converted.covariance()\n"
            << converted.covariance.value() << std::endl;

  BOOST_CHECK_EQUAL(converted.covariance.value()(0, 0), 1);

  BOOST_CHECK_GT(converted.covariance.value()(1, 1), 0);
  BOOST_CHECK_LT(converted.covariance.value()(1, 1), 1);
  ...
  std::cout << "plane out: " << roundtripPlaneParams.transpose() << std::endl;
  std::cout << "perigee out: " << roundtripPar.parameters().transpose()
            << std::endl;
  std::cout << "free out: " << roundtripFreePars.transpose() << std::endl;
  ...
  Acts::FreeToBoundMatrix freeToBound =
      planeSurface->freeToBoundJacobian(gctx, roundtripFreePars);
  ...
  std::cout << "J(perigee->plane)\n" << boundToBound << std::endl;

  BOOST_CHECK(roundtripPlaneParams.isApprox(par));

  std::cout << "roundtripPar.covariance()\n"
            << roundtripPar.covariance().value() << std::endl;

  std::cout << "roundtripParPlane.covariance()\n"
            << roundtripPlaneCov << std::endl;
