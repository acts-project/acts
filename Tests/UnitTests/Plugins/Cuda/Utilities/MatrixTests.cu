...
__global__
void matrixTransform( const int size,
...
BOOST_AUTO_TEST_CASE(MatrixCopy){
  ...
  static const int numBlocks =
    (inputDevice.size() + blockSize - 1)/blockSize;
  matrixTransform<<<numBlocks, blockSize>>>(inputDevice.size(),
                                            inputDevice.getPtr(),
                                            outputDevice.getPtr());
  ...
  for(int i = 0; i < MATRIX_SIZE; ++i) {
    for(int j = 0; j < MATRIX_SIZE; ++j) {
      maxDeviation = std::max(maxDeviation, std::abs(outputHost.get(i, j) -
                                                     EXPECTED_RESULT));
    }
  }
