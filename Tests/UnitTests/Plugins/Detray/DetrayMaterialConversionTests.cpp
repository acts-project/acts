...
auto cylinderSurface = Surface::makeShared<CylinderSurface>(
    Transform3::Identity(), std::make_shared<CylinderBounds>(5., 10.));
...
BOOST_AUTO_TEST_CASE(DetrayBinnedMaterialConversionX) {
  ...
  auto binnedMaterial = Acts::BinnedSurfaceMaterial(
      Acts::protoAxesFromBinUtility(binUtility),
      Acts::MaterialSlabMatrix{materialSlabs}, 0., Acts::MappingType::Default);

  detray::io::grid_payload<detray::io::surface_material_payload,
                           detray::io::material_id>
      payload =
          unpackGrid(*DetrayPayloadConverter::convertBinnedSurfaceMaterial(
              binnedMaterial, *planeSurface));
...
BOOST_AUTO_TEST_CASE(DetrayBinnedMaterialConversionXY) {
  ...
  auto binnedMaterial = Acts::BinnedSurfaceMaterial(
      Acts::protoAxesFromBinUtility(binUtility),
      Acts::MaterialSlabMatrix{materialSlabs0, materialSlabs1}, 0.,
...
BOOST_AUTO_TEST_CASE(DetrayBinnedMaterialConversionR) {
  ...
  detray::io::grid_payload<detray::io::surface_material_payload,
      ...
      payload =
          unpackGrid(*DetrayPayloadConverter::convertBinnedSurfaceMaterial(
              binnedMaterial, *cylinderSurface));
...
BOOST_AUTO_TEST_CASE(DetrayBinnedMaterialConversionInvalid) {
  ...
  BOOST_CHECK_THROW(DetrayPayloadConverter::convertBinnedSurfaceMaterial(
                        binnedMaterial, *cylinderSurface),
