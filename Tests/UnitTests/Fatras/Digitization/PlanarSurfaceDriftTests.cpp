...
BOOST_AUTO_TEST_CASE(PlanarSurfaceDrift) {
  ...
  auto [noDriftSegment, dScale] = psd.toReadout(
      geoCtx, *planeSurface, depletion, cPosition, cNormal, noDrift);
  ...
  auto [noDriftSegment1, dScale1] = psd.toReadout(
      geoCtx, *planeSurface, depletion, cPosition, particleDir, noDrift);
  ...
  auto [driftedSegment2, dScale2] = psd.toReadout(
      geoCtx, *planeSurface, depletion, cPosition, particleDir, holeDrift);

  BOOST_CHECK(std::abs(driftedSegment2[0].x() - driftedSegment2[1].x()) >
              Acts::s_epsilon);
  BOOST_CHECK(std::abs(driftedSegment2[0].y() - driftedSegment2[1].y()) >
              Acts::s_epsilon);
  CHECK_CLOSE_ABS(noDriftSegment[1].x(), driftedSegment2[1].x(),
                  Acts::s_epsilon);
  CHECK_CLOSE_ABS(noDriftSegment[1].y(), driftedSegment2[1].y(),
                  Acts::s_epsilon);
  BOOST_CHECK(std::abs(driftedSegment2[0].x() - noDriftSegment[0].x()) >
              Acts::s_epsilon);
  CHECK_CLOSE_ABS(driftedSegment2[0].y(), noDriftSegment[0].y(),
                  Acts::s_epsilon);
  ...
  auto [driftedSegment3, dScale3] = psd.toReadout(
      geoCtx, *planeSurface, depletion, cPosition, particleDir, chargeDrift);

  BOOST_CHECK(std::abs(driftedSegment3[0].x() - driftedSegment3[1].x()) >
              Acts::s_epsilon);
  BOOST_CHECK(std::abs(driftedSegment3[0].y() - driftedSegment3[1].y()) >
              Acts::s_epsilon);
  CHECK_CLOSE_ABS(noDriftSegment[0].x(), driftedSegment3[0].x(),
                  Acts::s_epsilon);
  CHECK_CLOSE_ABS(noDriftSegment[0].y(), driftedSegment3[0].y(),
                  Acts::s_epsilon);
  BOOST_CHECK(std::abs(driftedSegment3[1].x() - noDriftSegment[1].x()) >
              Acts::s_epsilon);
  CHECK_CLOSE_ABS(driftedSegment3[1].y(), noDriftSegment[1].y(),
                  Acts::s_epsilon);
}
