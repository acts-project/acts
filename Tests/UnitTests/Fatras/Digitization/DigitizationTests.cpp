...
struct Helper {
  ...
  ActsFatras::Segmentizer segmentizer;
  ActsFatras::PlanarSurfaceDrift surfaceDrift;
  ...
  auto makeSegments(const Acts::Vector3 hitPos,
                    const Acts::Vector3 hitDir) const {
    const auto segment = surfaceDrift.toReadout(gctx, *surface, thickness,
                                                hitPos, hitDir, driftDir);
    return segmentizer.segments(gctx, *surface, segmentation, segment,
                                thickness);
  }
};
...
BOOST_AUTO_TEST_CASE(test_upright_particle) {
  ...
  auto segments = helper.makeSegments(hitPosition, hitDirection);
