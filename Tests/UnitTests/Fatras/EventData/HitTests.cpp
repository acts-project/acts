...
BOOST_AUTO_TEST_CASE(WithoutInteraction) {
  // some hit position
  auto p4 = Vector4(1, 2, 3, 4);
  // before/after four-momenta are the same
  auto m4 = Vector4(1, 1, 1, 4);
  ...
  CHECK_CLOSE_REL(h.position(), Vector3(1, 2, 3), eps);
  CHECK_CLOSE_REL(h.time(), 4, eps);
  CHECK_CLOSE_REL(h.momentum4Before(), m4, eps);
  CHECK_CLOSE_REL(h.momentum4After(), m4, eps);
  CHECK_CLOSE_REL(h.directionBefore(), Vector3(1, 1, 1).normalized(), eps);
  CHECK_CLOSE_REL(h.directionAfter(), Vector3(1, 1, 1).normalized(), eps);
  CHECK_CLOSE_REL(h.direction(), Vector3(1, 1, 1).normalized(), eps);
...
BOOST_AUTO_TEST_CASE(WithEnergyLoss) {
  ...
  auto m40 = Vector4(2, 0, 0, 2);
  auto m41 = Vector4(1.5, 0, 0, 1.5);
  ...
  CHECK_CLOSE_OR_SMALL(h.directionBefore(), Vector3(1, 0, 0), eps, eps);
  CHECK_CLOSE_OR_SMALL(h.directionAfter(), Vector3(1, 0, 0), eps, eps);
  CHECK_CLOSE_OR_SMALL(h.direction(), Vector3(1, 0, 0), eps, eps);
...
BOOST_AUTO_TEST_CASE(WithScattering) {
  ...
  auto m40 = Vector4(2, 0, 2, 5);
  auto m41 = Vector4(0, -2, 2, 5);
  ...
  CHECK_CLOSE_OR_SMALL(h.momentum4Before(), m40, eps, eps);
  CHECK_CLOSE_OR_SMALL(h.momentum4After(), m41, eps, eps);
  CHECK_CLOSE_OR_SMALL(h.directionBefore(), Vector3(1, 0, 1).normalized(), eps,
                       eps);
  CHECK_CLOSE_OR_SMALL(h.directionAfter(), Vector3(0, -1, 1).normalized(), eps,
                       eps);
  CHECK_CLOSE_REL(h.direction(), Vector3(1, -1, 2).normalized(), eps);
  CHECK_SMALL(h.depositedEnergy(), eps);
}
...
BOOST_AUTO_TEST_CASE(WithEverything) {
  ...
  auto m40 = Vector4(3, 2, 2, 5);
  auto m41 = Vector4(2, 1, 2, 4);
  ...
  CHECK_CLOSE_OR_SMALL(h.momentum4Before(), m40, eps, eps);
  CHECK_CLOSE_OR_SMALL(h.momentum4After(), m41, eps, eps);
  CHECK_CLOSE_REL(h.directionBefore(), Vector3(3, 2, 2).normalized(), eps);
  CHECK_CLOSE_REL(h.directionAfter(), Vector3(2, 1, 2).normalized(), eps);
  CHECK_CLOSE_REL(
      h.direction(),
      Vector3(0.7023994590205035, 0.41229136135810396, 0.5802161953247991),
      eps);
