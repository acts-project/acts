...
BOOST_DATA_TEST_CASE(PhotonConversion, Dataset::parametersPhotonConversion, phi, lambda, seed) {
  ...
  ActsFatras::Particle particle = Dataset::makeParticle(Acts::PdgParticle::eElectron, phi, lambda, 1_GeV);
  ...
  particle = Dataset::makeParticle(Acts::PdgParticle::eGamma, phi, lambda, 1_keV);
  particleInit = particle;
  ...
  particle = Dataset::makeParticle(Acts::PdgParticle::eGamma, phi, lambda, 1_GeV);
  particleInit = particle;
  ...
  // Test the children
  BOOST_CHECK((generated[0].pdg() == Acts::PdgParticle::eElectron) || (generated[0].pdg() == Acts::PdgParticle::ePositron));
  BOOST_CHECK((generated[1].pdg() == Acts::PdgParticle::eElectron) || (generated[1].pdg() == Acts::PdgParticle::ePositron));
  ...
  auto sSum = sqrt(momSum[Acts::eEnergy] * momSum[Acts::eEnergy] - momVector.norm() * momVector.norm());

  auto sParticle = sqrt(particleInit.energy() * particleInit.energy() - particleInit.absoluteMomentum() * particleInit.absoluteMomentum());
