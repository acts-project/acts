...
BOOST_DATA_TEST_CASE(VariableBoundOne, bd::make(boundIndices), index) {
  ...
  auto [params, cov] = generateParametersCovariance<long double, 1u>(rng);
...
BOOST_AUTO_TEST_CASE(VariableBoundAll) {
  ...
  auto [params, cov] =
      generateParametersCovariance<long double, eBoundSize>(rng);
...
BOOST_AUTO_TEST_CASE(VariableBoundReassign) {
  ...
  auto [params1, cov1] = generateParametersCovariance<long double, 2u>(rng);
