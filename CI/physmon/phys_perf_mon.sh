...
if [[ "$mode" == "all" || "$mode" == "single" ]]; then
    $SPYRAL -l "Simulation" -o "$outdir/memory/mem_single_particles.csv" -- CI/physmon/workflows/physmon_single_particles.py $outdir 2>&1 > $outdir/run_single_particles.log
fi
echo "::endgroup::"
...
$SPYRAL_BIN plot $outdir/memory/mem_single_particles.csv --output $outdir/memory
...
if [[ "$mode" == "all" || "$mode" == "single" ]]; then
    echo "TODO generate single particle plots"
    exit 1
fi
