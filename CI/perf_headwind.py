...
for row in mdf.itertuples():
    ...
    metrics.append(Metric(name=name, value=row.max_rss, unit="GB", group="compile_max_rss"))
...
for row in tdf.itertuples():
    name = "time_"+row.file
    metrics.append(Metric(name=name, value=row.time, unit="seconds", group="compile_time"))
    if name in metric_names:
        print("Duplicate:", name)
    metric_names.append(name)
