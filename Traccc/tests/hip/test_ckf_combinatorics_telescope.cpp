...
TEST_P(CkfCombinatoricsTelescopeTests, Run) {
  ...
  typename traccc::hip::combinatorial_kalman_filter_algorithm::config_type
      cfg_no_limit;
  ...
  traccc::hip::combinatorial_kalman_filter_algorithm device_finding_limit(
  ...
  for (std::size_t i_evt = 0; i_evt < n_events; i_evt++) {
    ...
    traccc::edm::track_container<traccc::default_algebra>::buffer
        track_candidates_limit_hip_buffer = device_finding_limit(
            detector_buffer, field, measurements_buffer, seeds_buffer);

    traccc::edm::track_collection<traccc::default_algebra>::host
        track_candidates_hip{host_mr},
        track_candidates_limit_hip{host_mr};
    copy(track_candidates_hip_buffer.tracks, track_candidates_hip,
         vecmem::copy::type::device_to_host)
        ->wait();
    copy(track_candidates_limit_hip_buffer.tracks, track_candidates_limit_hip,
         vecmem::copy::type::device_to_host)
        ->wait();

    // Make sure that the number of found tracks = n_track ^ (n_planes +
    // 1)
    ASSERT_GT(track_candidates_hip.size(), track_candidates_limit_hip.size());
    ASSERT_EQ(track_candidates_hip.size(),
              std::pow(n_truth_tracks, std::get<11>(GetParam()) + 1));
    ASSERT_EQ(track_candidates_limit_hip.size(),
...
INSTANTIATE_TEST_SUITE_P(
    HIPCkfCombinatoricsTelescopeValidation, CkfCombinatoricsTelescopeTests,
    ::testing::Values(
