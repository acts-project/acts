...
TEST(grids_cuda, grid2_replace_populator) {
  ...
  auto x_interval = (xaxis.max - xaxis.min) / static_cast<scalar>(xaxis.n_bins);
  auto y_interval = (yaxis.max - yaxis.min) / static_cast<scalar>(yaxis.n_bins);
...
TEST(grids_cuda, grid2_replace_populator_ci) {
  ...
  auto x_interval = (caxis.max - caxis.min) / static_cast<scalar>(caxis.n_bins);
  ...
  for (unsigned int i_y = 0u; i_y < yaxis.bins(); i_y++) {
    for (unsigned int i_x = 0u; i_x < xaxis.bins(); i_x++) {
      ...
      for (unsigned int i_p = 0u; i_p < data.size(); i_p++) {
        ...
        point3 tp(
            {xaxis.min + gid * x_interval, yaxis.min + gid * y_interval, 0.5f});
        EXPECT_EQ(pt, tp);
      }
    }
  }
...
TEST(grids_cuda, grid2_buffer_attach_populator2) {
  ...
  grid2_buffer<host_grid2_attach> g2_buffer(xaxis, yaxis, {1, 2, 3, 4}, mng_mr);
  copy.setup(g2_buffer._buffer)->wait();
