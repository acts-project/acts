...
TEST(CUDAContainerCopy, DeviceToHost) {
  ...
  vecmem::device_vector<int> device_items_0(device_buffer.items.host_ptr()[0]);
  vecmem::device_vector<int> device_items_1(device_buffer.items.host_ptr()[1]);
  vecmem::device_vector<int> device_items_2(device_buffer.items.host_ptr()[2]);

  thrust::fill(thrust::device, device_headers.begin(), device_headers.end(), 7);
  thrust::fill(thrust::device, device_items_0.begin(), device_items_0.end(), 4);
  thrust::fill(thrust::device, device_items_1.begin(), device_items_1.end(), 1);
  thrust::fill(thrust::device, device_items_2.begin(), device_items_2.end(), 2);
...
TEST(CUDAContainerCopy, HostToDeviceToHost) {
  ...
  test_container_types::host host_copy = d2h(h2d(traccc::get_data(host_orig)));
