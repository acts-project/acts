...
TEST_P(CpuCkfCombinatoricsTelescopeTests, Run) {
  ...
  auto sim = traccc::simulator<host_detector_type, b_field_t, generator_type,
                               writer_type>(
      std::get<6>(GetParam()), n_events, detector.as<host_detector_type>(),
  ...
  traccc::host::combinatorial_kalman_filter_algorithm host_finding(cfg_no_limit,
                                                                   host_mr);
  ...
  for (std::size_t i_evt = 0; i_evt < n_events; i_evt++) {
    ...
    const traccc::edm::measurement_collection::const_view measurements_view =
