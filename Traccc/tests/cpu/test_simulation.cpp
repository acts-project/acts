...
TEST(traccc_simulation, simulation) {
  ...
  smearer.template operator()<rectangle_t>({2.f, -5.f}, bound_params, iomeas3);
...
GTEST_TEST(traccc_simulation, toy_detector_simulation) {
  ...
  for (std::size_t i_event = 0u; i_event < n_events; i_event++) {
    ...
    EXPECT_NEAR((std::sqrt(var0) - smearer.stddev[0]) / smearer.stddev[0], 0.f,
                0.1f);
    EXPECT_NEAR((std::sqrt(var1) - smearer.stddev[1]) / smearer.stddev[1], 0.f,
                0.1f);
  }
}
...
TEST_P(TelescopeDetectorSimulation, telescope_detector_simulation) {
  ...
  detray::tel_det_config<traccc::default_algebra, detray::rectangle2D> tel_cfg{
      1000.f * traccc::unit<scalar>::mm, 1000.f * traccc::unit<scalar>::mm};
