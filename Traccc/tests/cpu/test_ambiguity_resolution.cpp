...
TEST(AmbiguitySolverTests, GreedyResolverTest4) {
  ...
  for (std::size_t i = 0; i < 10000u; i++) {
    ...
    while (pattern.size() < track_length) {
      ...
      if (std::find(pattern.begin(), pattern.end(), meas_id) == pattern.end()) {
  ...
  auto duration_legacy = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  for (std::size_t i = 0; i < n_res_tracks; i++) {
    ASSERT_EQ(res_trk_cands.tracks.at(i), legacy_res_trk_cands.tracks.at(i));
  }
}
