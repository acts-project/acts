...
class ConnectedComponentAnalysisTests
 ...
 public:
  ...
  inline void test_connected_component_analysis(ParamType p) {
    ...
    traccc::io::read_cells(cells, file_hits, traccc::getDummyLogger().clone(),
                           &det_cond);
    ...
    while (truth_reader.read(io_truth)) {
      ...
      for (std::size_t i = 0; i < meas.size(); ++i) {
        if ((std::abs(meas.at(i).local_position()[0] - io_truth.channel0) <
             tol) &&
            (std::abs(meas.at(i).local_position()[1] - io_truth.channel1) <
             tol)) {
          meas_idx = i;
          break;
        }
