...
TEST_P(KalmanFittingTelescopeTests, Run) {
  ...
  auto [host_det, names] =
      detray::io::read_detector_json<host_detector_type>(shared_mr, reader_cfg);
  ...
  const traccc::detector_buffer detector_buffer =
      traccc::buffer_from_host_detector(polymorphic_detector, device_mr, copy);
  ...
  for (std::size_t i_evt = 0; i_evt < n_events; i_evt++) {
    ...
    traccc::edm::track_container<traccc::default_algebra>::buffer
        track_candidates_buffer{
            copy.to(vecmem::get_data(track_candidates.tracks), mr.main, mr.host,
                    vecmem::copy::type::host_to_device),
            {},
            measurements_buffer};
    ...
    for (std::size_t i_trk = 0; i_trk < n_truth_tracks; i_trk++) {
      ...
      ndf_tests(track_states_sycl.tracks.at(i_trk), track_states_sycl.states,
                measurements);
