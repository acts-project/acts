...
TEST(algorithm, compose_double_int_affine_1) {
  ...
  std::function<int(int &&)> f = traccc::compose(std::function<int(int &&)>(i),
                                                 std::function<int(int &&)>(i));
...
TEST(algorithm, side_effect) {
  ...
  std::function<int(const int &)> f = traccc::compose(
      std::function<int(const int &)>(i), std::function<int(const int &)>(i),
      traccc::side_effect(
          std::function<int(const int &)>(i),
          std::function<void(const int &)>([&j](const int &q) { j = q + 5; })));
