...
cca_function_t get_f_with(traccc::clustering_config cfg) {
  return [cfg](const traccc::edm::silicon_cell_collection::host& cells,
    ...
    traccc::alpaka::clusterization_algorithm cc({device_mr, &pinned_host_mr},
                                                copy, queue, cfg);
    ...
    traccc::edm::silicon_cell_collection::buffer cells_buffer{
        static_cast<traccc::edm::silicon_cell_collection::buffer::size_type>(
            cells.size()),
        device_mr};
    ...
    queue.synchronize();
    traccc::edm::measurement_collection::host measurements{pinned_host_mr};
