...
auto greedy_ambiguity_resolution_algorithm::operator()(
  ...
  for (unsigned int i = 0; i < n_tracks; i++) {
    ...
    if (n_cands < m_config.min_meas_per_track) {
      ...
      for (const auto [type, idx] : measurement_links) {
        ...
        meas_ids[i].push_back(input_tracks.measurements.at(idx).identifier());
      }
      n_meas[i] = n_cands;
  ...
  for (const unsigned int& i : accepted_ids) {
    ...
    for (const auto [type, idx] : measurement_links) {
      assert(type == edm::track_constituent_link::measurement);
      unique_meas_set.insert(input_tracks.measurements.at(idx).identifier());
    }
  }
  ...
  for (const unsigned int i : accepted_ids) {
    ...
    for (const std::size_t meas_id : deduplicated_ids) {
      const auto it =
          std::lower_bound(unique_meas.begin(), unique_meas.end(), meas_id);
  ...
  for (unsigned int iter = 0; iter < m_config.max_iterations; iter++) {
    ...
    const auto it1 =
        std::lower_bound(accepted_ids.begin(), accepted_ids.end(), worst_track);
    ...
    for (const std::size_t id : meas_ids_to_remove) {
      ...
      if (tracks.size() == 1) {
        ...
        const auto it3 = std::find(sorted_ids.begin(), sorted_ids.end(), tid);
        assert(it3 != sorted_ids.end());
        const auto it4 =
            std::lower_bound(sorted_ids.begin(), it3, tid, track_comparator);
    ...
    assert(
        std::is_sorted(sorted_ids.begin(), sorted_ids.end(), track_comparator));
  }
  ...
  for (const unsigned int i : accepted_ids) {
    ...
    output.tracks.push_back(
        {tcand.fit_outcome(),
         ...
         tcand.pval(),
         tcand.nholes(),
         {tcand.constituent_links().begin(), tcand.constituent_links().end()}});
  }
