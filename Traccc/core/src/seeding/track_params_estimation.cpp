...
track_params_estimation::output_type track_params_estimation::operator()(
  ...
  for (edm::seed_collection::const_device::size_type i = 0; i < num_seeds;
    ...
    for (std::size_t j = 0u; j < e_bound_size; ++j) {
      scalar var = m_config.initial_sigma.at(j) * m_config.initial_sigma.at(j);
      ...
      if (j == e_bound_qoverp) {
        ...
        var += math::pow(
            m_config.initial_sigma_qopt * math::sin(track_params.theta()), 2.f);
        var +=
            math::pow(m_config.initial_sigma_pt_rel * track_params.qop(), 2.f);
        var += var_theta *
