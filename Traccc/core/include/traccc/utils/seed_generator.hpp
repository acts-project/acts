...
struct seed_generator {
  ...
  bound_track_parameters<algebra_type> operator()(
    ...
    bound_track_parameters<algebra_type> bound_param{surface_link, bound_vec,
                                                     bound_cov};
  ...
  void operator()(bound_track_parameters<algebra_type>& bound_param,
    ...
    bound_param.set_bound_local(
        {static_cast<scalar>(smeared_loc0), static_cast<scalar>(smeared_loc1)});

    // Time
    bound_param.set_time(static_cast<scalar>(std::normal_distribution<double>(
        time, sigmas[e_bound_time])(m_generator)));

    // Smear direction angles phi,theta ensuring correct bounds
    const double smeared_phi =
        std::normal_distribution<double>(phi, sigmas[e_bound_phi])(m_generator);
    const double smeared_theta = std::normal_distribution<double>(
        theta, sigmas[e_bound_theta])(m_generator);
    const auto [new_phi, new_theta] = detail::wrap_phi_theta(
        static_cast<scalar>(smeared_phi), static_cast<scalar>(smeared_theta));
    bound_param.set_phi(static_cast<scalar>(new_phi));
    bound_param.set_theta(static_cast<scalar>(new_theta));

    // Compute smeared q/p
    bound_param.set_qop(static_cast<scalar>(std::normal_distribution<double>(
