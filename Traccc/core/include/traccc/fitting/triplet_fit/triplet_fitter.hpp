...
class triplet_fitter {
 public:
  ...
  TRACCC_HOST_DEVICE void linearize_triplet(
    ...
    scalar cos_phiHalf_1C = math::sqrt(1.f - sin_phiHalf_1C * sin_phiHalf_1C);
    scalar cos_phiHalf_2C = math::sqrt(1.f - sin_phiHalf_2C * sin_phiHalf_2C);
    ...
    // Index parameters
    scalar n_1C = 1.f / (Xphi_1C * cos_phiHalf_1C * sin_th_1C_2 + cos_th_1C_2);
    scalar n_2C = 1.f / (Xphi_2C * cos_phiHalf_2C * sin_th_2C_2 + cos_th_2C_2);
  ...
  TRACCC_HOST_DEVICE void calculate_pos_derivs(
    ...
    for (unsigned hit = 0; hit < 3; ++hit) {
      ...
      for (unsigned dir = 0; dir < m_max_dims; ++dir) {
        ...
        // Segment parameters
        t.m_fseg_1.emplace_back(t_shift.m_seg_1.phi - t.m_seg_1.phi,
        ...
        t.m_fseg_2.emplace_back(t_shift.m_seg_2.phi - t.m_seg_2.phi,
  ...
  do_global_fit(
    ...
    matrix_type<2u * max_ntrips, 2u * max_ntrips> K_inv = D_MS_inv + Hd * HdT;

    // Matrix inversion
    matrix_type<2u * max_ntrips, 2u * max_ntrips> K = matrix::inverse(K_inv);
    ...
    matrix_type<max_ndirs, 1u> hit_loc_pulls =
        HdT_K_psi - HdT_K_rho_norm * getter::element(rhoT_K_psi, 0u,
                                                     0u);  // to be stored ...
    ...
    scalar theta =
        triplet_first.m_seg_1.theta +
        (triplet_first.m_seg_1.tau + c_3D * triplet_first.m_seg_1.rho_theta);
    ...
    scalar p =
        mom_conv * detray::algebra::array::norm(B_vec) /
    ...
    scalar var_phi =
        sum_phph - 2.f * sum_c3D_ph * rho_phi + var_c_3D * rho_phi * rho_phi;
    ...
    for (unsigned dir_i = 0; dir_i < m_max_dims; ++dir_i) {
      ...
      var_pos_postFit[dir_i] =
          ...
          measurements.at(triplet_first.m_meas_idx[0]).local_variance()[dir_i];
    }
    ...
    getter::element(fit_cov, e_bound_qoverp, e_bound_qoverp) =
        ...
        math::pow(
            (mom_conv * detray::algebra::array::norm(B_vec)) / unit<scalar>::T,
            2.f);
