...
struct kalman_actor_state {
  ...
  /// Get the track state at a given position along the track
  TRACCC_HOST_DEVICE
  typename edm::track_state_collection<algebra_t>::device::const_proxy_type at(
  ...
  operator()(const bool backward_mode) {
    const auto state_idx{backward_mode ? static_cast<int>(size() - 1u) - m_idx
                                       : m_idx};
    assert(state_idx >= 0);
    ...
    for (unsigned int i = 0u; i < size(); ++i) {
      ...
      if (!trk_state.is_hole() && trk_state.filtered_params().is_invalid()) {
        TRACCC_DEBUG_HOST_DEVICE(
            "Missed track state %d/%d on surface %d during forward fit", i,
            size(), at(i).filtered_params().surface_link().index());
        ++n_missed;
      }
    }
  ...
  unsigned int count_missed_smoother() /*const*/ {
    ...
    for (unsigned int i = 0u; i < size(); ++i) {
      const edm::track_state trk_state = at(i);
      if (!trk_state.is_hole() && trk_state.smoothed_params().is_invalid()) {
        TRACCC_DEBUG_HOST_DEVICE(
            "Missed track state %d/%d on surface %d during smoothing", i,
            size(), at(i).smoothed_params().surface_link().index());
        ++n_missed;
      }
    }
    ...
    if constexpr (std::same_as<
      ...
      if (navigation.geometry_identifier() ==
        ...
        if (trk_state.is_hole()) {
          TRACCC_VERBOSE_HOST_DEVICE("-> Track finding flagged this as hole");
          check_if_hole(navigation);
        }

        TRACCC_VERBOSE_HOST_DEVICE(
            "-> Matched current surface to next track state: %d/%d", m_idx + 1,
            size());
      ...
      // index
      TRACCC_VERBOSE_HOST_DEVICE("--> Check other states on track for a match");
      ...
      /// state idx
      auto handle_skipped = [this, backward_mode](
      ...
      if (check_if_hole(navigation)) {
        ...
        for (unsigned int i = 0u; i < size(); ++i) {
          TRACCC_DEBUG_HOST("   - " << at(i).filtered_params().surface_link());
        }
  ...
  TRACCC_HOST_DEVICE void operator()(
    ...
    if (navigation.is_on_sensitive()) {
      ...
      if (!actor_state.match_surface_to_track_state(propagation)) {
        ...
        if (!backward_mode && navigation.current_surface().has_material()) {
          actor_state.add_to_sequence(navigation.current().surface());
        }
        return;
      } else if (actor_state.fit_result != kalman_fitter_status::SUCCESS &&
        ...
        if (actor_state.do_precise_hole_count &&
          ...
          actor_state(backward_mode)
              .filtered_params()
              .set_covariance(
                  matrix::identity<traccc::bound_matrix<algebra_t>>());
          actor_state(backward_mode)
              .filtered_params()
      ...
      if (sequencer.sequence().capacity() > 0u &&
          sequencer.sequence().size() == sequencer.sequence().capacity()) {
      ...
      bound_track_parameters<algebra_t>& bound_param = res.destination_params();
      ...
      if (!backward_mode) {
        if constexpr (direction_e == kalman_actor_direction::FORWARD_ONLY ||
                      direction_e == kalman_actor_direction::BIDIRECTIONAL) {
        ...
        if constexpr (direction_e == kalman_actor_direction::BACKWARD_ONLY ||
                      direction_e == kalman_actor_direction::BIDIRECTIONAL) {
      ...
      if (actor_state.fit_result != kalman_fitter_status::SUCCESS) {
        TRACCC_ERROR_HOST_DEVICE("Fit failure: KF status %d",
                                 static_cast<int>(actor_state.fit_result));
        TRACCC_ERROR_HOST(
            " --> reason: " << fitter_debug_msg{actor_state.fit_result}());

        if (actor_state.do_precise_hole_count || bound_param.is_invalid()) {
