  ...
  TRACCC_HOST_DEVICE void operator()(
    ...
    if (cand.chi2 <= max_chi2) {
      ...
      if (cand.chi2 <= std::numeric_limits<scalar_t>::epsilon() &&
        ...
        if (res != kalman_fitter_status::SUCCESS && bound_param.is_invalid()) {
          navigation.exit();
    ...
    if (res != kalman_fitter_status::SUCCESS && !navigation.is_edge_candidate())
        [[unlikely]] {
      ...
      if (updater_state.m_stats.n_consecutive_holes >
        ...
        TRACCC_WARNING_HOST_DEVICE("Maximum number of consecutive holes");
        navigation.exit();
        propagation.heartbeat(false);
      }
    } else {
      // Make a track state and record the track parameters
      const auto i{static_cast<int>(updater_state.m_stats.n_track_states)};
      traccc::make_track_state_candidate(updater_state.m_cand_ptr,
                                         updater_state.m_run_smoother, i, cand,
                                         bound_param);
      ...
      if (updater_state.m_stats.n_track_states >=
        ...
        TRACCC_WARNING_HOST_DEVICE("Max. number of track states reached");
        navigation.exit();
        propagation.heartbeat(false);
      } else {
        // Add the new measurement and track data to the track candidate
        TRACCC_VERBOSE_HOST_DEVICE("Assigned measurement: %d", cand.meas_idx);
      }
    }
