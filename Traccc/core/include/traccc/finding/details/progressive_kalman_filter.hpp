...
progressive_kalman_filter(
  ...
  using propagator_t = traccc::details::pkf_propagator_t<detector_t, bfield_t>;
  ...
  // Pathlimit aborter
  typename detray::actor::pathlimit_aborter<scalar_t>::state path_aborter_state;
  // Momentum aborter
  ...
  if (good_track && free_param.pT(q_new) <= static_cast<scalar_t>(cfg.min_pT)) {
    TRACCC_WARNING_HOST_DEVICE(
  ...
  if (good_track && trk_stats.n_holes > cfg.max_num_skipping_per_cand + 1) {
    TRACCC_ERROR_HOST_DEVICE("Hole count incorrect: max = %d, found = %d",
                             cfg.max_num_skipping_per_cand, trk_stats.n_holes);
  }
