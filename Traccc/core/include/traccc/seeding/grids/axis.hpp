...
struct regular {
  ...
  unsigned int bin(scalar v) const {
    auto ibin =
        static_cast<int>((v - min) / (max - min) * static_cast<scalar>(n_bins));
  ...
  std::array<unsigned int, 2> range(scalar v,
    ...
    auto pbin = static_cast<int>((v + nhood[1] - min) / (max - min) *
                                 static_cast<scalar>(n_bins));
    unsigned int min_bin = (nbin >= 0) ? static_cast<unsigned int>(nbin) : 0u;
  ...
  DETRAY_HOST_DEVICE vecmem::vector<unsigned int> zone_t(
    ...
    vecmem::vector<unsigned int> sequence(
        static_cast<vecmem::vector<unsigned int>::size_type>(nh_range[1] -
                                                             nh_range[0] + 1u),
        nh_range[0]);
    unsigned int m = 0u;
...
struct circular {
  ...
  DETRAY_HOST_DEVICE vecmem::vector<unsigned int> zone_t(
    ...
    unsigned int mo = 0;
    vecmem::vector<unsigned int> sequence(
        static_cast<vecmem::vector<unsigned int>::size_type>(vl), nh_range[0]);
  ...
  unsigned int remap(unsigned int ibin, int shood) const {
    ...
    if (opt_bin < 0) {
      return static_cast<unsigned int>(static_cast<int>(n_bins) + opt_bin);
    }
...
template <template <template <typename, std::size_t> class,
                    template <typename...> class> class axis_t,
...
inline axis_data<axis_t<array_t, vector_t>, const scalar> get_data(
  ...
  axis_data<axis_t<array_t, vector_t>, const scalar> result{axis.n_bins,
                                                            axis.min, axis.max};
  return result;
}
