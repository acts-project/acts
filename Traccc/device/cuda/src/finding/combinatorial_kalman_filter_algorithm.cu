...
void combinatorial_kalman_filter_algorithm::condense_tracks_kernel(
  ...
  condense_tracks(deviceBlocks, deviceThreads, 0, details::get_stream(stream()),
                  payload);
  TRACCC_CUDA_ERROR_CHECK(cudaGetLastError());
}
...
void combinatorial_kalman_filter_algorithm::
  ...
  kernels::fill_finding_duplicate_removal_sort_keys<<<
      deviceBlocks, deviceThreads, 0, details::get_stream(stream())>>>(payload);
  TRACCC_CUDA_ERROR_CHECK(cudaGetLastError());
}
...
void combinatorial_kalman_filter_algorithm::
  ...
  // Launch the kernel.
  kernels::gather_best_tips_per_measurement<<<deviceBlocks, deviceThreads, 0,
...
move_only_any combinatorial_kalman_filter_algorithm::create_device_detector(
  ...
  return detector_buffer_visitor<
    ...
    kernels::create_device_detector<<<1, 1, 0, details::get_stream(stream())>>>(
        det_view, device_detector_buffer.ptr());
