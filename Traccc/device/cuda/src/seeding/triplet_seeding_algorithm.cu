...
__global__ void count_doublets(
  ...
  device::count_doublets(details::global_index1(), config, spacepoints, sp_grid,
...
/// CUDA kernel for running @c traccc::device::make_mid_bot_lincircles
...
/// CUDA kernel for running @c traccc::device::make_mid_top_lincircles
...
__global__ __launch_bounds__(128) void count_triplets(
  ...
  device::count_triplets(details::global_index1(), config, spacepoints, sp_grid,
                         doublet_counter, mb_doublets, mt_doublets, spM_counter,
                         midBot_counter, midBot_circles, midTop_circles);
}
...
void triplet_seeding_algorithm::populate_grid_kernel(
  ...
  const unsigned int n_blocks =
      (payload.n_spacepoints + n_threads - 1) / n_threads;
  kernels::
      populate_grid<<<n_blocks, n_threads, 0, details::get_stream(stream())>>>(
...
void triplet_seeding_algorithm::count_doublets_kernel(
  ...
  const unsigned int n_blocks =
      (payload.n_spacepoints + n_threads - 1) / n_threads;
  kernels::
      count_doublets<<<n_blocks, n_threads, 0, details::get_stream(stream())>>>(
          payload.config, payload.spacepoints, payload.grid,
...
void triplet_seeding_algorithm::find_doublets_kernel(
  ...
  kernels::
      find_doublets<<<n_blocks, n_threads, 0, details::get_stream(stream())>>>(
          payload.config, payload.spacepoints, payload.grid,
          payload.doublet_counter, payload.mb_doublets, payload.mt_doublets);
  TRACCC_CUDA_ERROR_CHECK(cudaGetLastError());
}

void triplet_seeding_algorithm::make_mid_bot_lincircles_kernel(
    const make_mid_bot_lincircles_kernel_payload& payload) const {
  const unsigned int n_threads = 128;
  const unsigned int n_blocks = (payload.nMidBot + n_threads - 1) / n_threads;
  kernels::make_mid_bot_lincircles<<<n_blocks, n_threads, 0,
                                     details::get_stream(stream())>>>(
      payload.mb_doublets, payload.doublet_counter, payload.spacepoints,
      payload.grid, payload.mb_circles);
  TRACCC_CUDA_ERROR_CHECK(cudaGetLastError());
}

void triplet_seeding_algorithm::make_mid_top_lincircles_kernel(
    const make_mid_top_lincircles_kernel_payload& payload) const {
  const unsigned int n_threads = 128;
  const unsigned int n_blocks = (payload.nMidTop + n_threads - 1) / n_threads;
  kernels::make_mid_top_lincircles<<<n_blocks, n_threads, 0,
                                     details::get_stream(stream())>>>(
      payload.mt_doublets, payload.doublet_counter, payload.spacepoints,
      payload.grid, payload.mt_circles);
  TRACCC_CUDA_ERROR_CHECK(cudaGetLastError());
}
...
void triplet_seeding_algorithm::count_triplets_kernel(
  ...
  kernels::
      count_triplets<<<n_blocks, n_threads, 0, details::get_stream(stream())>>>(
          payload.config, payload.spacepoints, payload.grid,
          payload.doublet_counter, payload.mb_doublets, payload.mt_doublets,
          payload.spM_counter, payload.midBot_counter, payload.mb_circles,
          payload.mt_circles);
  TRACCC_CUDA_ERROR_CHECK(cudaGetLastError());
}
...
void triplet_seeding_algorithm::find_triplets_kernel(
  ...
  kernels::
      find_triplets<<<n_blocks, n_threads, 0, details::get_stream(stream())>>>(
          ...
          payload.spM_tc, payload.midBot_tc, payload.mb_circles,
          payload.mt_circles, payload.triplets);
  TRACCC_CUDA_ERROR_CHECK(cudaGetLastError());
}
