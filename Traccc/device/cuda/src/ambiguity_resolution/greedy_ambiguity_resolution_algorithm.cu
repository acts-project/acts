...
greedy_ambiguity_resolution_algorithm::operator()(
...

  {
    ...
    kernels::fill_vectors<<<nBlocks, nThreads, 0, stream>>>(
        m_config,
        device::fill_vectors_payload{.tracks_view = tracks_view,
  ...
  TRACCC_CUDA_ERROR_CHECK(cudaMemcpyAsync(n_accepted_device.get(), &n_accepted,
                                          sizeof(unsigned int),
  ...
  const unsigned int meas_count =
      static_cast<unsigned int>(thrust::unique_count(
          thrust_policy, flat_meas_ids_buffer.ptr(),
          flat_meas_ids_buffer.ptr() + n_cands_total, thrust::equal_to<int>()));
  ...
  // Unique measurement ids
  vecmem::data::vector_buffer<measurement_id_type> meas_id_to_unique_id_buffer{
      max_meas_id + 1, m_mr.main};
  ...
  thrust::fill(thrust_policy, n_accepted_tracks_per_measurement_buffer.ptr(),
               n_accepted_tracks_per_measurement_buffer.ptr() + meas_count, 0);
  ...
  while (!terminate && n_accepted > 0) {
    nBlocks_adaptive = (n_accepted + nThreads_adaptive - 1) / nThreads_adaptive;
    ...
    kernels::remove_tracks<<<1, 512, 0, stream>>>(device::remove_tracks_payload{
    ...
    */
    ...
    kernels::
        fill_inverted_ids<<<nBlocks_adaptive, nThreads_adaptive, 0, stream>>>(
    ...
    kernels::
        rearrange_tracks<<<nBlocks_rearrange, nThreads_rearrange, 0, stream>>>(
    ...
    // sorted_ids
    kernels::update_status<<<nBlocks_adaptive, nThreads_adaptive, 0, stream>>>(
  ...
  // Fill the output track candidates
  {
    if (n_accepted > 0) {
      kernels::fill_track_candidates<<<
          static_cast<unsigned int>((n_accepted + 63) / 64), 64, 0, stream>>>(
