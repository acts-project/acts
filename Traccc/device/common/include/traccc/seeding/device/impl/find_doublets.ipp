...
inline void find_doublets(
  ...
  for (unsigned int phi_bin_iterator = phi_bins[0];
    ...
    for (unsigned int z_bin = z_bins[0]; z_bin <= z_bins[1]; ++z_bin) {
      ...
      // bin.
      traccc::details::spacepoint_grid_types::const_device::serialized_storage::
          const_reference spacepoint_indices = sp_grid.bin(phi_bin, z_bin);
      ...
      for (unsigned int other_sp_idx = 0; other_sp_idx < size; ++other_sp_idx) {
        // Access the "other spacepoint".
        const edm::spacepoint_collection::const_device::const_proxy_type
            other_sp = spacepoints.at(spacepoint_indices.at(other_sp_idx));
