...
TRACCC_HOST_DEVICE inline void set_initial_covariance(
  ...
  for (std::size_t i = 0; i < e_bound_size; i++) {
    ...
    if (i == e_bound_qoverp) {
      const scalar var_theta = getter::element(track_params.covariance(),
      ...
      scalar sigma_theta = track_params.qop() / math::tan(track_params.theta());
...
TRACCC_HOST_DEVICE inline void estimate_track_params(
  ...
  details::set_initial_covariance(track_params, config);
}

template <typename detector_t, typename bfield_t>
TRACCC_HOST_DEVICE inline void estimate_track_params(
    const global_index_t globalIndex,
    const track_params_estimation_config& config, const detector_t& det,
    const edm::measurement_collection::const_view& measurements_view,
    const edm::spacepoint_collection::const_view& spacepoints_view,
    const edm::seed_collection::const_view& seeds_view, const bfield_t& bfield,
    bound_track_parameters_collection_types::view params_view) {
  ...
  bound_track_parameters<>& track_params = params.at(globalIndex);
  new (&track_params) bound_track_parameters<>();
  seed_to_bound_param_vector(track_params, det, measurements, spacepoints,
                             seeds.at(globalIndex), vector_field_at_sp);
