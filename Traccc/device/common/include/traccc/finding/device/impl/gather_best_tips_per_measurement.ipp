...
TRACCC_HOST_DEVICE inline void gather_best_tips_per_measurement(
  ...
  vecmem::device_vector<typename algebra_t::scalar> tip_pval(payload.tip_pval);
  ...
  while (barrier.blockOr(current_state < num_states || need_to_write)) {
    if (current_state < num_states || need_to_write) {
      ...
      if (need_to_write) {
        ...
        if (need_to_write && size >= payload.max_num_tracks_per_measurement &&
            pval <= worst) {
          need_to_write = false;
        }
        ...
        if (holds_lock) {
          ...
          if (size == payload.max_num_tracks_per_measurement) {
            ...
            assert(out_idx != std::numeric_limits<unsigned int>::max());
          } else {
            new_size = size + 1;
            out_idx = size;
          }
          ...
          for (unsigned int i = 0; i < new_size; ++i) {
            new_worst = std::min(new_worst,
                                 static_cast<float>(tip_pval.at(offset + i)));
          }

          [[maybe_unused]] bool cas_result = mutex.compare_exchange_strong(
              desired_set, encode_insertion_mutex(false, new_size, new_worst));
