...
TRACCC_HOST_DEVICE inline void update_tip_length_buffer(
  ...
  const float vote_fraction =
      static_cast<float>(total_votes) / static_cast<float>(total_measurements);
  ...
  if (vote_fraction < payload.min_measurement_voting_fraction) {
    tip_to_output_map.at(thread_id) = std::numeric_limits<unsigned int>::max();
  } else {
