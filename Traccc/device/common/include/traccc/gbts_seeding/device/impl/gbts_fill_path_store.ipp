...
TRACCC_HOST_DEVICE inline void gbts_fill_path_store(
  ...
  const unsigned int nTerminusPerBlock =
      (blockSize < bufferTerminusPerBlock) ? blockSize : bufferTerminusPerBlock;
  // Upper bound on path-store slots that may be claimed.
  const unsigned int nReachablePaths = payload.nPaths + payload.nTerminusEdges;
  ...
  if (threadIndex < nTerminusPerBlock && path_idx < payload.nTerminusEdges) {
    ...
    const unsigned int edge_pos = edge_size * static_cast<unsigned int>(path.x);
    ...
    for (unsigned int nei = 0; nei < nNei; ++nei) {
      ...
      const unsigned int live_idx = static_cast<unsigned int>(
          vecmem::device_atomic_ref<int, vecmem::device_address_space::local>(
              shared.n_live_paths)
              .fetch_add(1));
      if (live_idx >= static_cast<unsigned int>(
                          traccc::device::gbts_consts::live_path_buffer)) {
        break;
      }
  ...
  while (shared.n_live_paths > 0) {
    ...
    if (threadIndex == 0) {
      ...
      shared.n_live_paths =
          (shared.n_live_paths < buf_size) ? shared.n_live_paths : buf_size;
    }
    barrier.blockBarrier();
