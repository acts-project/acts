...
struct gbts_sort_nodes_payload {
  /// Number of spacepoints (sort keys)
  unsigned int nSp;
  /// Number of eta bins
  ...
  /// In/out: the node sort keys from gbts_bin_spacepoints, rejected
  /// spacepoints sorted last
  ...
  /// Output: per-eta-bin node offsets, nEtaBins + 1 entries (the last one is
  /// the number of nodes)
  vecmem::data::vector_view<unsigned int> eta_bin_offsets;
...
/// directly. The eta-bin offsets are written by the threads where the eta
/// bin of the sorted keys changes.
///
