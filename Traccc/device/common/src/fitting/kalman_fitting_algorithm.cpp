...
kalman_fitting_algorithm::output_type kalman_fitting_algorithm::operator()(
  ...
  const edm::track_collection<default_algebra>::const_view::size_type n_tracks =
      input_tracks.tracks.capacity();
  if (n_tracks == 0) {
  ...
  vecmem::data::vector_buffer<device::sort_key> track_sort_keys(n_tracks,
                                                                mr().main);
  vecmem::data::vector_buffer<unsigned int> track_indices{n_tracks, mr().main};
  prepare_track_fit_order(input_tracks.tracks, track_sort_keys, track_indices);

  // Create the buffer(s) used during the fitting.
  vecmem::data::vector_buffer<unsigned int> track_liveness(n_tracks, mr().main);
