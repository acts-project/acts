...
clusterization_algorithm::clusterization_algorithm(
  ...
  if (config.max_partition_size() >
      static_cast<unsigned int>(std::numeric_limits<details::index_t>::max())) {
  ...
  copy()
      .memset(vecmem::data::vector_view<unsigned int>{1, m_backup_mutex.get()},
              0)
      ->wait();
}
...
clusterization_algorithm::execute_impl(
  ...
  if (m_config.sort_cells) {
    ...
    sort_cells_kernel(num_cells, cells, *sorted_cells, permutation_map_buffer,
                      keep_disjoint_set);
    sorted_cells_view = edm::silicon_cell_collection::const_view(*sorted_cells);
  } else {
