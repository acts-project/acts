...
/// Kernel used in implementing @c traccc::hip::is_contiguous_on
...
bool is_contiguous_on(P&& projection, vecmem::memory_resource& mr,
  ...
  TRACCC_HIP_ERROR_CHECK(hipMemcpyAsync(out.get(), &initial_out, sizeof(bool),
                                        hipMemcpyHostToDevice, hip_stream));

  // Launch the first kernel, which will squash consecutive equal elements
  // into one element.
  hipLaunchKernelGGL(kernels::is_contiguous_on_compress_adjacent<CONTAINER>,
                     (n + block_size - 1) / block_size, block_size, 0,
                     hip_stream, projection, view, iout);
  ...
  hipLaunchKernelGGL(kernels::is_contiguous_on_all_unique, all_unique_grid_size,
                     all_unique_block_size, 0, hip_stream, iout, out.get());
  TRACCC_HIP_ERROR_CHECK(hipGetLastError());

  // Get the result from the device and return it.
  bool host_out;

  TRACCC_HIP_ERROR_CHECK(hipMemcpyAsync(&host_out, out.get(), sizeof(bool),
