...
def getThroughput(csvRow):
    ...
    return float(csvRow["processed_events"]) / (float(csvRow["processing_time"]) * 1e-9)
...
def drawPerMuSamplePlots(inputs, samples, canvas, output, maxThreads, graphicsOptions):
    ...
    for sample in samples:
        ...
        for input in inputs:
            ...
            profile.GetYaxis().SetRangeUser(0.0, min_max_throughput[1] * 1.2)
...
def drawMaxThroughputPlot(
    inputs, max_throughputs, samples, canvas, output, graphicsOptions
):
    ...
    for platform in platforms:
        ...
        for i in range(len(samples)):
            ...
            pass
        profile.GetYaxis().SetRangeUser(
            min_max_throughput[0] * 0.2, min_max_throughput[1] * 5.0
        )
        profiles.append(profile)
        counter += 1
...
def main():
    ...
    for arg in args.csv_file:
        ...
        if len(inputs[-1]) != 2:
            raise RuntimeError('Could not parse "%s" as "<filename>:<platform>"' % arg)
        pass
    ...
    drawMaxThroughputPlot(
        inputs, maxThroughputs, samples, canvas, args.output, graphicsOptions
    )
