...
struct simulator {
  ...
  simulator(const detray::pdg_particle<scalar>& ptc, std::size_t events,
    ...
    m_updater_state =
        detray::actor::parameter_updater_state<algebra_type>{m_cfg.propagation};
  }
  ...
  void run() {
    ...
    for (std::size_t event_id = 0u; event_id < m_events; event_id++) {
      typename writer_t::state writer_state(event_id, std::move(m_writer_cfg),
                                            m_directory);
      ...
      for (auto track : *m_track_generator.get()) {
        ...
        writer_state.write_particle(
            track, detail::correct_particle_hypothesis(m_cfg.ptc_type, track));

        typename propagator_type::state propagation(track, m_field, m_detector);
