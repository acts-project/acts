...
void read_particles(
  ...
  auto measurement_hit_id_reader = make_measurement_hit_id_reader(hit_map_file);
  ...
  while (measurement_hit_id_reader.read(mhid)) {
    ...
    if (hit_to_measurement.insert({mhid.hit_id, mhid.measurement_id}).second ==
        false) {
      throw std::runtime_error("Duplicate hit ID in hit->measurement map");
    }
  }
  ...
  while (hit_reader.read(h)) {
    ...
    auto particle_index = std::distance(temp_particles.begin(), particle_it);
