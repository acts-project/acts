  ...
  switch (format) {
      ...
      details::write_binary_soa(
          get_absolute_path(
              (std::filesystem::path(directory) /
               std::filesystem::path(get_event_filename(event, "-cells.dat")))
                  .native()),
    ...
    case data_format::csv:
      csv::write_cells(
          get_absolute_path(
              (std::filesystem::path(directory) /
               std::filesystem::path(get_event_filename(event, "-cells.csv")))
                  .native()),
  ...
  switch (format) {
    ...
    case data_format::obj:
      obj::write_spacepoints(
          get_absolute_path((std::filesystem::path(directory) /
                             std::filesystem::path(
                                 get_event_filename(event, "-spacepoints.obj")))
                                .native()),
          spacepoints);
      break;
    default:
  ...
  switch (format) {
    case data_format::obj:
      obj::write_seeds(
          get_absolute_path(
              (std::filesystem::path(directory) /
               std::filesystem::path(get_event_filename(event, "-seeds.obj")))
                  .native()),
          seeds, spacepoints);
      break;
    default:
...
void write(std::string_view filename, data_format format,
  ...
  switch (format) {
    case data_format::json:
      json::write_digitization_config(get_absolute_path(filename), config);
      break;
    default:
      throw std::invalid_argument("Unsupported data format");
  }
