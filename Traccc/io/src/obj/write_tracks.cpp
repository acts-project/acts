...
void write_tracks(std::string_view filename,
  ...
  const edm::track_container<default_algebra>::const_device tracks{tracks_view};
  ...
  for (size_type i = 0; i < tracks.tracks.size(); ++i) {
    ...
    for (const auto& [type, idx] : tracks.tracks.constituent_links().at(i)) {
      ...
      if (type == edm::track_constituent_link::measurement) {
        ...
        meas =
            tracks.measurements.at(tracks.states.at(idx).measurement_index());
      } else {
        // This should not happen...
        throw std::runtime_error("Unknown track constituent type found");
      }
      ...
      file << "v " << getter::element(global, 0u) << " "
           << getter::element(global, 1u) << " " << getter::element(global, 2u)
           << "\n";
    }
  }
  ...
  for (size_type i = 0; i < tracks.tracks.size(); ++i) {
    // Construct the lines.
    file << "l";
    for (size_type j = 0; j < tracks.tracks.at(i).constituent_links().size();
         ++j) {
      file << " " << vertex_counter++;
    }
    file << "\n";
