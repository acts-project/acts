...
class duplication_plot_tool {
 public:
  ...
  void book(std::string_view name, duplication_plot_cache& cache) const {
    ...
    cache.duplication_rate_vs_eta =
        plot_helpers::book_eff(TString(name) + "_duplicationRate_vs_eta",
                               "Duplication rate;#eta;Duplication rate", b_eta);
    // duplication rate vs phi
    cache.duplication_rate_vs_phi =
        plot_helpers::book_eff(TString(name) + "_duplicationRate_vs_phi",
...
#ifdef TRACCC_HAVE_ROOT
    cache.n_duplicated_vs_pT->Fill(t_pT,
                                   static_cast<double>(n_duplicated_tracks));
    cache.n_duplicated_vs_eta->Fill(t_eta,
                                    static_cast<double>(n_duplicated_tracks));
    cache.n_duplicated_vs_phi->Fill(t_phi,
                                    static_cast<double>(n_duplicated_tracks));
#endif  // TRACCC_HAVE_ROOT
  }
