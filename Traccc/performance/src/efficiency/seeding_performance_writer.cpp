...
struct seeding_performance_writer_data {
  /// Constructor
  seeding_performance_writer_data(const seeding_performance_writer::config& cfg)
...
void seeding_performance_writer::write(
  ...
  for (edm::seed_collection::const_device::size_type i = 0u; i < seeds.size();
    ...
    if (sd.bottom_index() >= spacepoints.size() ||
      ...
      TRACCC_WARNING("Seed with index "
                     << i << " (out of " << seeds.size()
                     << ") has invalid spacepoint (B: " << sd.bottom_index()
                     << ", M: " << sd.middle_index()
                     << ", T: " << sd.top_index()
                     << ", #SP: " << spacepoints.size() << ")");
      continue;
    ...
    if (spacepoints.at(sd.bottom_index()).measurement_index_1() >=
      ...
      TRACCC_WARNING(
          ...
          << ", M: " << spacepoints.at(sd.middle_index()).measurement_index_1()
          << ", T: " << spacepoints.at(sd.top_index()).measurement_index_1()
          << ", #SP: " << spacepoints.size() << ")");
      continue;
    }
    ...
    std::array<event_data::measurement_proxy, 3> seed_measurements{
        ...
        measurements.at(
            spacepoints.at(sd.middle_index()).measurement_index_1()),
        measurements.at(spacepoints.at(sd.top_index()).measurement_index_1())};
  ...
  TRACCC_INFO("Total number of seed-matched particles was "
              << total_matched_truth_particles);
  TRACCC_INFO("Total number of duplicate seeds was " << total_duplicate_seeds);
