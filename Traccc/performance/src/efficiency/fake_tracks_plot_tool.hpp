...
class fake_tracks_plot_tool {
 public:
  ...
  struct fake_tracks_plot_cache {
    ...
    std::unique_ptr<TProfile> n_fake_vs_pT;   ///< Number of fake tracks vs pT
    std::unique_ptr<TProfile> n_fake_vs_eta;  ///< Number of fake tracks vs eta
    std::unique_ptr<TProfile> n_fake_vs_phi;  ///< Number of fake tracks vs phi
  ...
  void book(std::string_view name, fake_tracks_plot_cache& cache) const {
    ...
    cache.fake_rate_vs_eta =
        plot_helpers::book_eff(TString(name) + "_fakeTracksRate_vs_eta",
                               "Fake tracks rate;#eta;Fake tracks rate", b_eta);
    // fake tracks rate vs phi
    cache.fake_rate_vs_phi =
        plot_helpers::book_eff(TString(name) + "_fakeTracksRate_vs_phi",
