...
struct finding_performance_writer_data {
  /// Constructor
  finding_performance_writer_data(const finding_performance_writer::config& cfg)
...
std::vector<std::vector<event_data::measurement_proxy>> prepare_data(
  ...
  const edm::track_container<default_algebra>::const_device tracks(track_view);
  ...
  for (unsigned int i = 0; i < n_tracks; i++) {
    ...
    for (const edm::track_constituent_link& link :
      ...
      if (link.type == edm::track_constituent_link::measurement) {
        result_measurements.push_back(tracks.measurements.at(link.index));
...
void finding_performance_writer::write_common(
  ...
  for (std::size_t i = 0; i < n_tracks; i++) {
    ...
    if (particle_hit_counts.empty()) {
      TRACCC_VERBOSE("No contributing particles found for track: " << n_tracks);
      continue;
    }
    ...
    const double completeness = static_cast<double>(n_major_hits) /
                                static_cast<double>(truth_measurements.size());

    const bool reco_matched = purity >= m_cfg.track_truth_config.matching_ratio;
  ...
  for (auto const& [pid, ptc] : evt_data.m_particle_map) {
    ...
    m_data->m_duplication_plot_tool.fill(m_data->m_duplication_plot_cache, ptc,
                                         n_matched_seeds_for_particle - 1);
    m_data->m_fake_tracks_plot_tool.fill(m_data->m_fake_tracks_plot_cache, ptc,
                                         fake_count);
  }

  TRACCC_INFO("Total number of truth particles was " << total_truth_particles);
...
void finding_performance_writer::finalize() {
  ...
  if ((!ofile) || ofile->IsZombie()) {
    throw std::runtime_error("Could not open output file \"" + m_cfg.file_path +
                             "\" in mode \"" + m_cfg.file_mode + "\"");
  }
  ofile->cd();
#else
