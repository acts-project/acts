...
std::unique_ptr<TH1> book_histo(std::string_view hist_name,
  ...
  auto result = std::make_unique<TH1F>(
      hist_name.data(),
      (std::string(hist_title) + ";" + var_binning.title + ";Entries").c_str(),
...
std::unique_ptr<TH2> book_histo(std::string_view hist_name,
  ...
  auto result = std::make_unique<TH2F>(hist_name.data(), hist_title.data(),
                                       var_x_binning.n_bins, var_x_binning.min,
                                       var_x_binning.max, var_y_binning.n_bins,
