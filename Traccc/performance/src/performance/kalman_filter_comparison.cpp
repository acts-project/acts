...
bool kalman_filter_comparison(
  ...
  for (std::size_t i = 0u; i < track_container.tracks.size(); ++i) {
    ...
    fit_actor_fw::state fit_actor_state(
        ...
        device_track_container.states, device_measurements, device_sf_sequence,
        calib_cfg);
    ...
    state_tuple.push_back(
        detray::make_tuple(updater_state, interactor_state, fit_actor_state));
  ...
  if (n_miss_truth_fw.n_total() != n_holes_fw) {
    TRACCC_ERROR("Forward filter hole counting incorrect: was "
                 << n_holes_fw << ", should be " << n_miss_truth_fw.n_total());
    test_successful = false;
  }
  if (n_trk_holes_fw < trk_stats_fw.n_tracks_w_extra) {
    TRACCC_ERROR("Forward filter number of tracks with holes incorrect: was "
  ...
  for (std::size_t i = 0u; i < tracks.size(); ++i) {
    ...
    param_smearer.generate_initial_covariance(updater_state_bw.bound_params(),
                                              cfg.particle);
  ...
  if (n_miss_truth_bw.n_total() != n_holes_bw) {
    TRACCC_ERROR("Backward filter hole counting incorrect: was "
                 << n_holes_bw << ", should be " << n_miss_truth_bw.n_total());
    test_successful = false;
  }
  if (n_trk_holes_bw != trk_stats_bw.n_tracks_w_extra) {
    TRACCC_ERROR("Backward filter number of tracks with holes incorrect: was "
