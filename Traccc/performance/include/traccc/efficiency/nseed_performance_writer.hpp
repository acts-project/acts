  ...
  void register_event(
    ...
    for (SeedIt s = sb; s != se; ++s) {
      ...
      std::transform(
          ...
          [&em, &measurements,
           &spacepoints](const edm::spacepoint_collection::host::size_type& l) {
            ...
            const edm::measurement meas = measurements.at(
                spacepoints
                    .at(static_cast<
                        edm::measurement_collection::const_device::size_type>(
                        l))
                    .measurement_index_1());
    ...
    for (const auto& [_, ptc] : em.m_particle_map) {
      ...
      write_track_row(ev, ptc.particle_id, pass, ptc.charge, eta, phi, pT);
    }
  }
