...
void soa_comparator<TYPE>::operator()(
  ...
  for (float uncertainty : m_uncertainties) {
    ...
    for (typename TYPE::const_device::size_type i = 0; i < lhs.size(); ++i) {
      if (is_matched[i]) {
        ++matched;
      } else {
        ...
        for (typename TYPE::const_device::size_type j = 0; j < rhs.size();
             ++j) {
          if (comparator(rhs[j])) {
            ++matched;
            is_matched[i] = true;
    ...
    agreements.push_back(static_cast<float>(matched) /
                         static_cast<float>(std::max(lhs.size(), rhs.size())) *
                         100.f);
  }
