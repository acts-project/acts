...
class is_same_object<edm::track<T>> {
 public:
  ...
  bool operator()(const edm::track<T>& obj) const {
    ...
    if (obj.constituent_links().size() != m_ref.constituent_links().size()) {
      return false;
    }
    ...
    const edm::track_state_collection<default_algebra>::const_device ref_states{
        m_ref_states};
    ...
    for (unsigned int i = 0; i < obj.constituent_links().size(); ++i) {
      ...
      if (obj.constituent_links()[i].type ==
        ...
        if (!is_same_object<
                ...
                ref_meas.at(m_ref.constituent_links()[i].index),
                m_unc)(test_meas.at(obj.constituent_links()[i].index))) {
          return false;
        }
        ...
        if (!is_same_object<edm::track_state_collection<
                ...
                m_unc)(test_states.at(obj.constituent_links()[i].index))) {
          return false;
        }
 ...
 private:
  ...
  const edm::track_state_collection<default_algebra>::const_view m_test_states;
