...
int seq_run(const traccc::opts::detector& detector_opts,
  ...
  for (std::size_t event = input_opts.skip;
    ...
    traccc::bound_track_parameters_collection_types::buffer params_sycl_buffer(
        0, *mr.host);
    ...
    {  // Start measuring wall time
      ...
      {
        traccc::performance::timer t("Hit reading  (cpu)", elapsedTimes);
        // Read the hits from the relevant event file
        traccc::io::read_spacepoints(
            spacepoints_per_event, measurements_per_event, event,
            input_opts.directory,
            (input_opts.use_acts_geom_source ? &host_det : nullptr), nullptr,
      ...
      copy(vecmem::get_data(measurements_per_event), measurements_sycl_buffer)
          ->wait();
      traccc::edm::spacepoint_collection::buffer spacepoints_sycl_buffer(
          static_cast<unsigned int>(spacepoints_per_event.size()), mr.main);
      copy(vecmem::get_data(spacepoints_per_event), spacepoints_sycl_buffer)
          ->wait();
...
int main(int argc, char* argv[]) {
  ...
  traccc::opts::program_options program_opts{
      ...
      {detector_opts, bfield_opts, input_opts, seeding_opts, seeding_gbts_opts,
