...
int seq_run(const traccc::opts::detector& detector_opts,
  ...
  host_spacepoint_formation_algorithm sf(host_mr,
                                         logger().clone("HostSpFormationAlg"));
  ...
  traccc::host::greedy_ambiguity_resolution_algorithm resolution_alg_cpu(
      resolution_config, host_mr, logger().clone("HostAmbiguityResolutionAlg"));
  ...
  for (std::size_t event = input_opts.skip;
...

    {
      ...
      if (accelerator_opts.compare_with_cpu) {
        ...
        track_candidates = finding_alg(host_detector, host_field,
      ...
      // CPU
      if (accelerator_opts.compare_with_cpu) {
        traccc::performance::timer timer{"Track fitting (cpu)", elapsedTimes};
