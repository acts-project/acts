...
int seq_run(const traccc::opts::track_finding& finding_opts,
  ...
  for (std::size_t event = input_opts.skip;
    ...
    traccc::event_data evt_data(
        ...
        &polymorphic_detector, input_opts.format, false);
    ...
    async_copy(track_candidates_cuda_buffer.tracks,
               track_candidates_cuda.tracks, vecmem::copy::type::device_to_host)
        ->wait();
...

    {
      ...
      // Run fitting
      track_states_cuda_buffer = device_fitting(detector_buffer, device_field,
                                                track_candidates_cuda_buffer);
    }
    traccc::edm::track_container<traccc::default_algebra>::host
...
int main(int argc, char* argv[]) {
  ...
  return seq_run(finding_opts, propagation_opts, fitting_opts, input_opts,
                 detector_opts, bfield_opts, performance_opts, accelerator_opts,
