...
///
/// @note GBTS seeding is not available on HIP. The constructor throws if it
///       is requested, so that the chain keeps the same signature as the
///       other backends.
///
class full_chain_algorithm
 ...
 public:
  ...
  using clustering_algorithm = traccc::hip::clusterization_algorithm;
  /// Track finding algorithm type
  using finding_algorithm = traccc::hip::combinatorial_kalman_filter_algorithm;
  /// Track fitting algorithm type
  using fitting_algorithm = traccc::hip::kalman_fitting_algorithm;
 ...
 private:
  ...
  vecmem::hip::host_memory_resource m_pinned_host_mr;
  /// Cached pinned host memory resource
  mutable vecmem::binary_page_memory_resource m_cached_pinned_host_mr;
  /// (vecmem) HIP stream to use
  vecmem::hip::stream_wrapper m_vecmem_stream;
  /// (traccc) HIP stream to use
  ...
  mutable vecmem::hip::async_copy m_copy;
