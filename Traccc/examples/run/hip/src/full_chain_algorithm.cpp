...
full_chain_algorithm::full_chain_algorithm(
    ...
    const seedfilter_config& filter_config, const gbts_seedfinder_config&,
  ...
  // GBTS seeding is not available on HIP.
  if (useGBTS) {
    throw std::runtime_error(
        "GBTS seeding is not implemented for the HIP backend.");
  }
  ...
  std::cout << "Using HIP device: " << props.name << " [id: " << device
...
full_chain_algorithm::output_type full_chain_algorithm::operator()(
  ...
  if (m_detector != nullptr) {
    ...
    const triplet_seeding_algorithm::output_type seeds = m_seeding(spacepoints);
