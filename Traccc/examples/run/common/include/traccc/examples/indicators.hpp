...
inline std::size_t terminal_width() {
  ...
  if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi))
    return static_cast<std::size_t>(csbi.srWindow.Right - csbi.srWindow.Left +
                                    1);
  return 80;
#else
  struct winsize size{};
 ...
 private:
    ...
    if (show_percentage_) {
      const auto pct = std::min(
          static_cast<std::size_t>(static_cast<float>(progress_) /
                                   static_cast<float>(max_progress_) * 100.0f),
          std::size_t{100});
      os << ' ' << std::setw(3) << pct << '%';
    }
      ...
      if (saved_start_time_) {
        const auto eta = std::chrono::nanoseconds(
            progress_ > 0 ? static_cast<long long>(
                                std::ceil(static_cast<float>(elapsed_.count()) *
        ...
        const auto elapsed_s = std::chrono::duration<double>(elapsed_).count();
        if (elapsed_s > 0.0) {
  ...
  void print_progress() {
    ...
    const auto pos = static_cast<std::size_t>(
        static_cast<double>(progress_) / static_cast<double>(max_progress_) *
    ...
    const std::size_t printed = prefix_text_.size() + 1 /*'['*/ + bar_width_ +
                                1 /*']'*/ + postfix.size();
