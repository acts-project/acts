...
int main(int argc, char* argv[]) {
  ...
  traccc::seed_generator<host_detector_type> sg0(host_det, seed_cfg, 0,
                                                 fit_cfg0.propagation.context);
  traccc::seed_generator<host_detector_type> sg1(host_det, seed_cfg, 0,
  ...
  for (auto event = input_opts.skip;
    ...
    if ((event - input_opts.skip) / (input_opts.events / 2) == 0) {
      ...
      evt_data.generate_truth_candidates(truth_track_candidates,
                                         truth_measurements, sg0, host_mr);
      ...
      if (performance_opts.run) {
        for (unsigned int i = 0; i < n_fitted_tracks; i++) {
          fit_performance_writer.write(track_states.tracks.at(i),
                                       track_states.states, truth_measurements,
                                       host_det, evt_data,
                                       fit_cfg0.propagation.context);
        }
      }
      ...
      traccc::edm::track_container<default_algebra>::host
          truth_track_candidates{host_mr};
      evt_data.generate_truth_candidates(truth_track_candidates,
                                         truth_measurements, sg1, host_mr);
      truth_track_candidates.measurements =
          vecmem::get_data(truth_measurements);
