...
int seq_run(const traccc::opts::input_data& input_opts,
  ...
  using finding_algorithm = traccc::host::combinatorial_kalman_filter_algorithm;
  ...
  spacepoint_formation_algorithm sf(host_mr, logger().clone("SpFormationAlg"));
  ...
  for (std::size_t event = input_opts.skip;
    ...
    traccc::host::sparse_ccl_algorithm::output_type clusters_per_event{host_mr};
    traccc::host::measurement_creation_algorithm::output_type
        measurements_per_event{host_mr};
    spacepoint_formation_algorithm::output_type spacepoints_per_event{host_mr};
    ...
    {  // Start measuring wall time.
      ...
      {
        traccc::performance::timer timer{"Clusterization", elapsedTimes};

        clusters_per_event =
            cc(vecmem::get_data(cells_per_event), vecmem::get_data(det_cond));
        measurements_per_event = mc(vecmem::get_data(cells_per_event),
                                    vecmem::get_data(clusters_per_event),
      ...
      {
        traccc::performance::timer timer{"Spacepoint formation", elapsedTimes};
        spacepoints_per_event =
            sf(polymorphic_detector, vecmem::get_data(measurements_per_event));
      }
      if (output_opts.directory != "") {
        traccc::io::write(event, output_opts.directory, output_opts.format,
...

      {
        ...
        track_candidates = finding_alg(polymorphic_detector, field,
                                       vecmem::get_data(measurements_per_event),
    ...
    if (performance_opts.run) {
      traccc::event_data evt_data(
          input_opts.directory, event, host_mr, input_opts.use_acts_geom_source,
          &polymorphic_detector, input_opts.format, true);
  ...
  std::cout << "- created  " << n_measurements << " measurements. "
            << std::endl;
  std::cout << "- created  " << n_spacepoints << " space points. " << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  traccc::opts::program_options program_opts{
      ...
      {detector_opts, bfield_opts, input_opts, output_opts, clusterization_opts,
       seeding_opts, finding_opts, resolution_opts, fitting_opts,
       propagation_opts, performance_opts, truth_finding_opts,
