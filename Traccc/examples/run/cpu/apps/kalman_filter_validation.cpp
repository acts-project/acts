...
int main(int argc, char* argv[]) {
  ...
  // Read the detector.
  auto [io_det, names] =
      detray::io::read_detector_json<host_detector_t>(host_mr, reader_cfg);
  ...
  std::optional<b_field_t::view_t> field_view{
      field
          .template as_field<traccc::const_bfield_backend_t<traccc::scalar>>()};
  ...
  traccc::kalman_filter_comparison(det, names, field, test_cfg, smearing_cfg,
                                   ilogger->clone(), tracks, truth_traces_fw_KF,
                                   device_measurements, track_container);
}
