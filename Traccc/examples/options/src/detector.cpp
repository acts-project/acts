...
detector::detector() : interface("Detector Options") {
  ...
  m_desc.add_options()("detector-file",
                       po::value(&detector_file)->default_value(detector_file),
                       "Detector file");
  m_desc.add_options()("material-file",
...
std::unique_ptr<configuration_printable> detector::as_printable() const {
  ...
  cat->add_child(
      std::make_unique<configuration_kv_pair>("Detector file", detector_file));
  cat->add_child(
      std::make_unique<configuration_kv_pair>("Material file", material_file));
  cat->add_child(
      std::make_unique<configuration_kv_pair>("Surface grid file", grid_file));
