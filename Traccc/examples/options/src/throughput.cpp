...
std::unique_ptr<configuration_printable> throughput::as_printable() const {
  ...
  cat->add_child(std::make_unique<configuration_kv_pair>("Reconstruction stage",
                                                         reco_stage_string));
  cat->add_child(std::make_unique<configuration_kv_pair>(
      "Cold run events", std::to_string(cold_run_events)));
  cat->add_child(std::make_unique<configuration_kv_pair>(
      "Processed events", std::to_string(processed_events)));
  cat->add_child(std::make_unique<configuration_kv_pair>("Log file", log_file));
  cat->add_child(std::make_unique<configuration_kv_pair>(
      "Deterministic ordering", std::format("{}", deterministic_event_order)));
  cat->add_child(std::make_unique<configuration_kv_pair>(
      "Random seed",
