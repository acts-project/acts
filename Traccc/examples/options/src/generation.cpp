...
generation::generation() : interface("Particle Generation Options") {
  m_desc.add_options()("gen-events", po::value(&events)->default_value(events),
...
std::unique_ptr<configuration_printable> generation::as_printable() const {
  ...
  cat->add_child(
      std::make_unique<configuration_kv_pair>("Phi range", phi_range_ss.str()));
  std::ostringstream eta_range_ss;
  eta_range_ss << eta_range;
  cat->add_child(
      std::make_unique<configuration_kv_pair>("Eta range", eta_range_ss.str()));
  std::ostringstream theta_range_ss;
  theta_range_ss << theta_range / traccc::unit<float>::degree << " deg";
  cat->add_child(std::make_unique<configuration_kv_pair>("Theta range",
                                                         theta_range_ss.str()));
  cat->add_child(std::make_unique<configuration_kv_pair>(
  ...
  cat->add_child(std::make_unique<configuration_kv_pair>("Include energy loss",
                                                         energy_loss_ss.str()));
