...
track_propagation::track_propagation()
  ...
  m_desc.add_options()(
      ...
      po::value(&(m_config.navigation.intersection.overstep_tolerance))
          ->default_value(m_config.navigation.intersection.overstep_tolerance /
  ...
  m_desc.add_options()(
      ...
      po::value(&(m_config.navigation.intersection.min_mask_tolerance))
          ->default_value(m_config.navigation.intersection.min_mask_tolerance /
                          traccc::unit<float>::mm),
      "The minimum mask tolerance [mm]");
  m_desc.add_options()(
      "max-mask-tolerance-mm",
      po::value(&(m_config.navigation.intersection.max_mask_tolerance))
          ->default_value(m_config.navigation.intersection.max_mask_tolerance /
                          traccc::unit<float>::mm),
...
std::unique_ptr<configuration_printable> track_propagation::as_printable()
  ...
  cat_nav->add_child(std::make_unique<configuration_kv_pair>(
      "Mask tolerance scaling",
      std::to_string(m_config.navigation.intersection.mask_tolerance_scalor)));
  cat_nav->add_child(std::make_unique<configuration_kv_pair>(
      "Path tolerance",
  ...
  cat_tsp->add_child(std::make_unique<configuration_kv_pair>(
      "Min step size",
      std::to_string(m_config.stepping.min_stepsize / traccc::unit<float>::mm) +
          " mm"));
  cat_tsp->add_child(std::make_unique<configuration_kv_pair>(
      "Runge-Kutta tolerance",
      std::to_string(m_config.stepping.rk_error_tol / traccc::unit<float>::mm) +
          " mm"));
  cat_tsp->add_child(std::make_unique<configuration_kv_pair>(
      "Max step updates", std::to_string(m_config.stepping.max_rk_updates)));
  cat_tsp->add_child(std::make_unique<configuration_kv_pair>(
      "Step size constraint", std::to_string(m_config.stepping.step_constraint /
                                             traccc::unit<float>::mm) +
                                  " mm"));
  cat_tsp->add_child(std::make_unique<configuration_kv_pair>(
      "Path limit",
