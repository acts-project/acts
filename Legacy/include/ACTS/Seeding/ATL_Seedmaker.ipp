...
// shift all spacepoints by this offset such that the beam can be assumed to be at 0,0
// z shift should not matter as beam is assumed to be parallel to central detector axis,
...
void Acts::ATL_Seedmaker<SpacePoint>::newEvent(int iteration,  
    RandIter spBegin, 
    RandIter spEnd)
{
  ...
  // convert space-points to SPForSeed and sort into radius-binned array r_Sorted
  // store number of SP per bin in r_map
  RandIter sp = spBegin;
  for(; sp != spEnd; ++sp) {

    Acts::SPForSeed<SpacePoint>* sps = newSpacePoint((*sp)); if(!sps) continue;
    int   ir = int(sps->radius()*irstep); if(ir>irmax) ir = irmax;
    r_Sorted[ir].push_back(sps);
    // if there is exactly one SP in current bin, add bin nr in r_index (for deletion later)
...
template <class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::find3Sp() 
{
  m_zminU     = m_zmin;
  m_zmaxU     = m_zmax;
...
///////////////////////////////////////////////////////////////////
template <class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::findNext () 
{
  if(m_endlist) return;
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::buildFrameWork() 
{
  ...
  //scattering factor. depends on error, forward direction and distance between SP
  m_COF       =  134*.05*9.                    ;
  ...
  for(int i=0; i!=r_size; ++i) {r_index[i]=0; r_map[i]=0;}
  ...
  if(!m_OneSeeds) m_OneSeeds  = new Acts::InternalSeed<SpacePoint> [m_maxOneSize];  
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::buildBeamFrameWork() 
{ 
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void  Acts::ATL_Seedmaker<SpacePoint>::convertToBeamFrameWork
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::fillLists() 
{
  const float pi2 = 2.*M_PI;
  ...
  for(int i=r_first; i!=r_size;  ++i) {
    ...
    for(; r!=re; ++r) {
      ...
      // assign z-bin a value between 0 and 10 identifying the z-slice of a space-point
      if(Z>0.) {
    ...
    Z>-250.?z=5:Z>-450.?z=4:Z>-925.?z=3:Z>-1400.?z=2:Z>-2500.?z=1:z= 0;
      ...
      int n = f*11+z; ++m_nsaz;
      // push back sp into rfz_Sorted, record new number of entries in bin in rfz_map, 
      // if 1st entry record non-empty bin in "rfz_index"
      rfz_Sorted[n].push_back(*r); if(!rfz_map[n]++) rfz_index[m_nrfz++] = n;
      // pixel hits between 3<=z<=7 are reassigned different z indices for vertex search
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::erase()
{
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::production3Sp()
{ 
  ...
  typename std::list<Acts::SPForSeed<SpacePoint>*>::iterator rt[9],rte[9],rb[9],rbe[9];
  int nseed = 0; 
  ...
  int NB, int NT, int& nseed) 
{
  ...
  for(; r0!=rbe[0]; ++r0) {
    ...
    float Rb =(*r)->radius();  
    float dR = R-Rb; 
    // if deltaR larger than deltaRMax, store spacepoint counter position in rb[i]
    // this is not necessary at all because r is the loop counter (rb never read again)
...
//  TODO: is check if in same surface necessary?
  ...
  breakt:
    ...
    for(int i=0; i!=Nt; ++i) {
      ...
      float dy  = sp->y()-Y   ;
      float dz  = sp->z()-Z   ;
      // calculate projection fraction of spM->spT vector pointing in same direction as
      // vector origin->spM (x) and projection fraction of spM->spT vector pointing 
    ...
    if(Im <= imax) {
      // Add penalty factor dependent on difference between cot(theta) to the quality Im (previously Impact)
      float dr;
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::newOneSeed
(Acts::SPForSeed<SpacePoint>*& p1, Acts::SPForSeed<SpacePoint>*& p2,
 Acts::SPForSeed<SpacePoint>*& p3,float z,float q)
{
  ...
  // Then, add the new seed a SECOND time to the map with the worse quality as key.
  // Then remove all entries after the newly inserted entry equal to the new seed.
  else                     {
    typename std::multimap<float,Acts::InternalSeed<SpacePoint>*>::reverse_iterator 
      l = m_mapOneSeeds.rbegin();

    if((*l).first <= q) return;
    
    Acts::InternalSeed<SpacePoint>* s = (*l).second; s->set(p1,p2,p3,z);

    typename std::multimap<float,Acts::InternalSeed<SpacePoint>*>::iterator 
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::newOneSeedWithCurvaturesComparison
    (Acts::SPForSeed<SpacePoint>*& SPb,Acts::SPForSeed<SpacePoint>*& SP0,float Zob)
{
  ...
  typename std::vector<std::pair<float,Acts::SPForSeed<SpacePoint>*>>::iterator j,jn,i = m_CmSp.begin(),ie = m_CmSp.end(); jn=i; 
  ...
  for(; i!=ie; ++i) {
    ...
    // add 200 to quality if 2 compatible seeds with high deltaR of their spT have been found
    ...
    // m_diversss < Impact parameters + penalty for cot(theta) difference < m_divermax
...
///////////////////////////////////////////////////////////////////
template<class SpacePoint>
void Acts::ATL_Seedmaker<SpacePoint>::fillSeeds ()
{
  m_fillOneSeeds = 0;
