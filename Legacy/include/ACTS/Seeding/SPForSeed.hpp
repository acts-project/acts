...
namespace Acts {
  ...
  template <typename SpacePoint>
  inline SPForSeed<SpacePoint>::SPForSeed
    (SpacePoint* const& sp, const float* r) 
    {
      set(sp,r); m_param = 0.;
    }
  ...
  template <typename SpacePoint>
  inline SPForSeed<SpacePoint>::SPForSeed
    (SpacePoint* const& sp, const float* r, const float* sc) 
    {
  ...
  template <typename SpacePoint>
  inline void SPForSeed<SpacePoint>::set
    (SpacePoint* const& sp,const float* r)
    {
      spacepoint = sp  ;
      m_x        = r[0];
...
//    //ATTENTION: v(1,1) contains error for z in barrel, error for r in endcaps
...
//    // Set error of low uncertainty dimension (i.e. r for barrel, z for EC) to "small"
