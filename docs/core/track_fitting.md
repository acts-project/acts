...
To simplify integration, the GSF also returns a {class}`Acts::KalmanFitter` result object. This allows to use the same analysis tools for both fitters. Currently, the states of the individual components are not returned by the fitter.
