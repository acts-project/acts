...
- `ON`: require zstd, failing configuration if it is unavailable.
- `OFF`: disable compression and skip zstd discovery, even if it is installed.

For reproducible builds, select `-DACTS_USE_ZSTD=ON` or `-DACTS_USE_ZSTD=OFF`
...
| ACTS_USE_ZSTD                                 | zstd support: AUTO (use if available),<br>ON (required), OFF (disabled)<br> type: `string`, default: `AUTO`                                                                                                                                                                 |
...
| ACTS_USE_SYSTEM_GBL                           | Use a system-provided General Broken<br>Lines (GBL) from Mille<br> type: `bool`, default: `OFF`                                                                                                                                                                             |
| ACTS_USE_SYSTEM_MILLE                         | Use a system-provided Mille<br> type: `bool`, default: `ACTS_USE_SYSTEM_LIBS -> OFF`                                                                                                                                                                                        |
| ACTS_USE_SYSTEM_MP2                           | Use a system-provided Millepede-2<br> type: `bool`, default: `ACTS_USE_SYSTEM_LIBS -> OFF`                                                                                                                                                                                  |
| ACTS_BUILD_PLUGIN_ACTSVG                      | Build SVG display plugin<br> type: `bool`, default: `OFF`                                                                                                                                                                                                                   |
| ACTS_BUILD_PLUGIN_ARROW                       | Build Apache Arrow/Parquet plugin<br> type: `bool`, default: `OFF`                                                                                                                                                                                                          |
...
| ACTS_ENABLE_CUDA                              | Enable CUDA for the<br>gnn/Traccc/CudaMuonSegmentFitting<br>plugins<br> type: `bool`, default: `OFF`                                                                                                                                                                        |
...
| ACTS_BUILD_PLUGIN_GBL                         | Build General Broken Lines (GBL) plugin<br>from Mille<br> type: `bool`, default: `OFF`                                                                                                                                                                                      |
...
| ACTS_GENERATE_PYTHON_STUBS                    | Generate Python stub files (.pyi) using<br>pybind11-stubgen via uv<br> type: `bool`, default: `OFF`                                                                                                                                                                         |
...
| ACTS_BUILD_TRACCC                             | Build traccc GPU tracking package<br> type: `bool`, default: `OFF`                                                                                                                                                                                                          |
...
| ACTS_BUILD_EXAMPLES_PARQUET                   | Build Arrow/Parquet-based code in the<br>examples<br> type: `bool`, default: `OFF`                                                                                                                                                                                          |
...
| ACTS_BUILD_EXAMPLES_CUDA_MUON_SEGMENT_FITTING | Build CUDA muon segment fitting example<br> type: `bool`, default: `OFF`                                                                                                                                                                                                    |
...
| ACTS_BUILD_CORE_UNITTESTS                     | Build Core unit tests<br> type: `bool`, default: `ON`                                                                                                                                                                                                                       |
| ACTS_BUILD_EXAMPLES_UNITTESTS                 | Build Examples unit tests<br> type: `bool`, default: `ACTS_BUILD_UNITTESTS AND ACTS_BUILD_EXAMPLES`                                                                                                                                                                         |
...
# Source packages with pre-generated code {#source-package}

Parts of ACTS (and of the bundled detray) are C++ headers generated at build
time with sympy. The generated files are deliberately not committed, so a normal
build downloads `uv`, a Python interpreter and the generator dependencies. This
happens even when every C++ dependency is already provided, which makes builds
depend on network access.

Everything that gets generated is listed in `codegen/manifest.json`, keyed by
where the generated file belongs. That file is the single source of truth:
`cmake/ActsCodegen.cmake` drives the build from it, and `CI/pregenerate_codegen.py`
produces the same files from it ahead of time, so the two cannot disagree about
what exists or where it goes. Adding a generator means adding it to the manifest;
`acts_code_generation` refuses a key that is not listed there.

Pre-generating the code needs `uv` and nothing else — no CMake, no compiler, and
none of the C++ dependencies a real ACTS configure requires:

```console
$ CI/pregenerate_codegen.py --output prebuilt-codegen
```

`CI/make_source_archive.sh` then packs the tracked content of a revision
together with that directory, using nothing but git, tar and gzip:

```console
$ CI/make_source_archive.sh --output acts-<version>.tar.gz \
      --prebuilt prebuilt-codegen --revision <rev> --prefix acts-<version>
```

A build configured from the unpacked archive finds `prebuilt-codegen/` next to
the top-level `CMakeLists.txt` on its own and copies the headers into place
instead of regenerating them, so it needs neither `uv`, nor Python, nor network
access for the code generation step.

Because the manifest lists every unit, the archive covers all of them regardless
of which options the machine producing it had enabled. The content comes from a
revision via `git archive` rather than from the working directory, which keeps
build directories and other untracked files out and honours `export-ignore`
attributes.

`CI/codegen_prebuilt_check` is a small CMake project that verifies a
pre-generated directory is complete and actually gets picked up. It declares no
languages and builds no code, so it runs anywhere CMake does:

```console
$ cmake -S CI/codegen_prebuilt_check -B <build> \
      -DACTS_CODEGEN_PREBUILT_DIR=<prebuilt>
$ cmake --build <build>
```

> [!important]
> A release has **two** source archives, and only one of them carries the
> generated code. The archive attached to the release as an asset does; the
> archive GitHub generates automatically for the tag
> (`.../archive/refs/tags/vX.Y.Z.tar.gz`) does not, and cannot be made to.
> Packaging recipes that want the offline build have to point at the release
> asset. Building from the auto-generated archive still works — it just falls
> back to running the generators, and with them the downloads.

Two [build options](#build-options) steer this on the consuming side.
`ACTS_CODEGEN_PREBUILT_DIR` points at a directory of pre-generated code
explicitly, and `ACTS_CODEGEN_REQUIRE_PREBUILT` decides what happens when a
generated file is not covered by it.

Generated files that have no counterpart in the pre-generated directory are
generated as usual, and CMake warns about each one. Since that warning is easy
to miss in a long configure log, a build that must not reach the network should
set `ACTS_CODEGEN_REQUIRE_PREBUILT=ON`, which turns every such fallback — and
the case of no pre-generated code at all — into a hard error. This is the
recommended setting for packaging recipes: it fails loudly when the wrong
archive was used, rather than silently taking the slow path.
