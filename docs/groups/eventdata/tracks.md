@defgroup eventdata_tracks Track Event Data Model
@brief High-level Track Event Data Model
@ingroup eventdata

# High-level Track Event Data Model {#edm_Tracks}
...
was found (*holes*), and intersections with known passive material. The
EDM allows building up a track from these track states iteratively. For
...
track states, allowing downstream consumers of the EDM to access the fully
detailed information produced during track reconstruction.

---

## Conceptual {#edm_track_conceptual}

### Architecture

The Track EDM is structured such that the memory-layout can be
SoA, while presenting an object-oriented interface for convenient
usage.

The image below shows this object-oriented access model for
...
To make the EDM implementation independent of an experiment persistency
...
@ref Acts::TrackProxy, @ref Acts::TrackContainer,
@ref Acts::TrackStateProxy and @ref Acts::MultiTrajectory. The latter
...
incoming measurement. It then instructs the EDM through the frontend
...
@image html edm/edm_diagram.svg "Diagram of the EDM architecture. The frontend layer is used by other ACTS components, and downstream clients. It is separated from the backend layer by an interface. Conversion to and from EDM4hep is possible. Examples of direct backend implementations are shown."

The picture above shows a diagram of the EDM architecture. At the center
...
## Track state iteration and forward linking {#edm_track_iteration}

By default, track states are connected as a one-directional linked list, where
each track state knows its *previous* track state. The picture below shows an
example of a track state tree, like it is constructed by the combinatorial
track finding.

In the picture states @f$S_7@f$ and @f$S_6@f$ are the two tip states of the track state tree, while @f$S_1@f$ is the single stem state.
In the case of combinatorial track finding starting from e.g. a
seed, it could be the location of the innermost space
point.

@image html edm/ckf_tree.svg "Illustration of a branching multi-trajectory that is created during combinatorial track finding." width=400px

Each track object points at a single tip state to define its track state sequence.
The @ref Acts::TrackProxy class has various methods to access the track state sequence:
...
Note that @ref Acts::TrackProxy::trackStatesReversed iterates from the tip state to
the stem state, i.e. from the outside in.

> [!important]
> By-default, it is not possible to iterate *forward* through the track states
> on a track! The track's track states need to be *forward-linked* for this to
> be possible.

The reason for this is:
As the trajectory branches at the second sensor into @f$S_2@f$/@f$S_3@f$, it is not
possible to connect the states forward, i.e. store in @f$S_1@f$ what the *next*
state is going to be: it is ambiguous!

However, when track finding has concluded, and the trajectories identified by
tip states @f$S_7@f$ and @f$S_8@f$ have been discarded or are being
...
anymore! @ref Acts::TrackProxy::copyFrom will implicitly forward link the
track states, as it is guaranteed to not branch after copying.

You can manually add forward-linking to a track by calling
@ref Acts::TrackProxy::linkForward or
@ref Acts::TrackProxy::reverseTrackStates.

> [!warning]
> Calling either @ref Acts::TrackProxy::linkForward or
> @ref Acts::TrackProxy::reverseTrackStates on a track state sequence which
...
After a copy operation of @f$S_6@f$ and @f$S_7@f$ the resulting track state
...
@f$S_1@f$/@f$S_2@f$ to @f$S_6@f$/@f$S_7@f$ and the other way around.
...
@ref Acts::TrackProxy::innermostTrackState.
...
>
> Consider again the track states in the picture above. @f$S_1@f$ is the common
> ancestor for @f$S_2@f$ and @f$S_3@f$, and has a single slot to store smoothed
> parameters. If smoothing happens for the track ending in @f$S_6@f$, then smoothing
> the track ending in @f$S_7@f$ only the values written by the final smoothing of
> @f$S_3@f$ will survive in @f$S_1@f$'s storage. This can be unexpected!

## Component sharing {#track_edm_component_sharing}

@ref Acts::MultiTrajectory is designed so that components can be shared
between track states. This can be achieved using the
@ref Acts::TrackStateProxy::shareFrom can be used to set this up.
...
To illustrate why this can be useful, consider again the track state picture, where
@f$S_2@f$ and @f$S_3@f$ branch out from a shared @f$S_1@f$. In this case, the predicted
parameter vector and covariance, as well as the jacobian from @f$S_1\to S_2@f$ and
@f$S_1 \to S_3@f$ will be identical. In this case, the combinatorial track finding
...
## Dynamic columns {#track_edm_dynamic_columns}
...
@ref Acts::MultiTrajectory and @ref Acts::TrackContainer and associated
proxies only coordinate the creation, access and copying of dynamic columns.

The following illustrates the
usage of dynamic columns for @ref Acts::TrackContainer, but usage on
@ref Acts::MultiTrajectory is identical.

Assume you create a track container using some combination of backends (see
@ref edm_track_backends for information on the backends shipped with ACTS).
...
> @ref edm_track_backends for information on the backends shipped with ACTS, and
> which types they support.

With these dynamic columns registered, it is now possible to set and get values
for these columns on tracks.
...
@ref edm_track_accessors.

## Accessors {#edm_track_accessors}
...
> [!tip]
> The same accessor also works for @ref Acts::TrackStateProxy objects, as it shares
> the same component access mechanism with @ref Acts::TrackProxy.
...
For both const and mutable proxy accessors you do not actually need a mutable
reference, as the internal accessor state is not mutated after construction.
You can safely use a static instance of these accessors to avoid constructing
...
The @ref Acts::TrackContainer implements a mechanism to optionally own the
...
- `Acts::detail::ConstRefHolder` which does not own the backends and
...
Other user-specified holders can also be used, for example, it is possible to
use `std::shared_ptr` as a holder directly, like:
...
Tracks can be created directly from the EDM interface. You start by creating or
obtaining a mutable track container:
...
## Track EDM backends {#edm_track_backends}
...
EDM. It does not implement any persistency directly. The implementation of
...
##### Helper for `Surface`s and `SourceLink`s {#podio_helper}

PODIO cannot directly store @ref Acts::Surface and @ref Acts::SourceLink instances. The
PODIO backends rely on a helper class that implements the following @ref
ActsPlugins::PodioUtil::ConversionHelper interface.

Specifically, the PODIO backends will, before persisting and after reading,
consult the helper object to convert between an in-memory @ref Acts::Surface
and an optional identifier. The identifier a 64-bit integer whose
interpretation can depend on the experiment, it could be a sensor index (hash
...
@ref Acts::SourceLink is type-erased proxy object that stands for an
...
The `default` statement deals with @ref track_edm_dynamic_columns. For support
...
vector and @f$N\times N@f$ numbers for the associated covariance matrix. To enable
...
- Mutable
  - Add a track state
  - Share track state components from another track state of the same container
    (otherwise no sharing is supported)
