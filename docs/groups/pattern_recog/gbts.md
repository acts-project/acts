...
lightweight description: a flat list of @ref Acts::Experimental::GbtsLayer
"logical layers", each subdivided into **eta bins**.
...
compatible with the allowed @f$z_0@f$ range. The result is the **bin group**
list returned by @ref Acts::Experimental::GbtsGeometry::binGroups — one inner bin
together with all outer bins it may connect to. That list is the iteration
schedule of the graph builder, ordered so that outer bins are processed before
...
@snippet{trimleft} include/Acts/Seeding/GbtsDataStorage.hpp gbts insert

This is the same pattern as
@ref Acts::CylindricalSpacePointGrid::insert: an experiment can
fill the storage straight from its own space point EDM without first copying
...
@ref Acts::Experimental::GbtsLayer::getEtaBin and buffers it.
...
@snippet{trimleft} include/Acts/Seeding/GbtsDataStorage.hpp gbts node params
...
@snippet{trimleft} include/Acts/Seeding/GbtsDataStorage.hpp gbts node edge info
...
@snippet{trimleft} include/Acts/Seeding/GbtsDataStorage.hpp gbts eta bin info
...
@snippet{trimleft} include/Acts/Seeding/GbtsDataStorage.hpp gbts edge
...
@ref Acts::Experimental::GbtsNodeEdgeInfo. Immediately after creating an edge
...
Each edge stores up to `gbtsNumSegConns` (6) such neighbours. Every inner node
also accumulates a 16-bit **@f$z_0@f$ histogram bitmask** of its confirmed edges;
...
When `useMl` is enabled, GBTS narrows the per-node @f$\tau@f$ window using a
pre-trained lookup table indexed by **pixel cluster width**. The physical basis is
that the cluster a track leaves in a pixel module grows with the track's
...
| `useMl`, `lutInputFile` | @ref gbts-ml | cluster-width based @f$\tau@f$ windows |
...
- Node storage and graph EDM: @ref Acts::Experimental::GbtsNodeStorage,
  @ref Acts::Experimental::GbtsNodeParams,
  @ref Acts::Experimental::GbtsNodeEdgeInfo,
  @ref Acts::Experimental::GbtsEtaBinInfo, @ref Acts::Experimental::GbtsEdge.
- Geometry: @ref Acts::Experimental::GbtsGeometry,
  @ref Acts::Experimental::GbtsLayer,
  @ref Acts::Experimental::GbtsLayerConnectionMap.
- Chain following: @ref Acts::Experimental::GbtsTrackingFilter,
  @ref Acts::Experimental::GbtsEdgeState.
